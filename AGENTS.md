...
- Vulkanalia is the renderer binding. The renderer requires Vulkan `VK_EXT_descriptor_heap` and
  ...
  sync-file fences to Tensor's atomic KMS owner; Vulkan must not own KMS state. Internal timeline
...
- Physical-device selection is a renderer submodule and a configuration value. Default ranking
  ...
  the Tensor tty backend; `render-device` constrains both sides and they must never select GPUs
  independently.
...
- Wayland protocol work follows **wayland-protocols tiers** (see
  ...
  `wayland-server` dispatch plus Tensor's zero-cost delegation bridge is the implementation vehicle;
