...
Fika 是面向当前 compositor 栈的 Rust 文件管理器。当前 UI 主线是默认的 `fika`
二进制：Fika 自有 retained UI 构建在可复用的原生 client runtime 之上；之前的
UI runtime 已经从源码树移除。
...
- [DEVICES_REFERENCE.zh-CN.md](../../docs/fika/DEVICES_REFERENCE.zh-CN.md) —
  设备和 Places 行为。
- [NETWORK_REFERENCE.zh-CN.md](../../docs/fika/NETWORK_REFERENCE.zh-CN.md) —
  网络位置行为。
- [PERFORMANCE_ALIGNMENT.zh-CN.md](../../docs/fika/PERFORMANCE_ALIGNMENT.zh-CN.md) —
  Dolphin-first 性能参考原则。
- [TRASH_REFERENCE.zh-CN.md](../../docs/fika/TRASH_REFERENCE.zh-CN.md) —
  回收站行为。
