...
/// the pipeline. Rasterization remains shared with Fika's retained text cache;
    ...
    pub(crate) fn new(
        ...
        let resource_heap = device
            .create_descriptor_heap(&DescriptorHeapDescriptor {
                label: Some("fika-vulkan-text-resource-heap".into()),
        ...
        let sampler_heap = device
            .create_descriptor_heap(&DescriptorHeapDescriptor {
                label: Some("fika-vulkan-text-sampler-heap".into()),
...
fn create_pipeline(
    ...
    device
        .create_graphics_pipeline(&GraphicsPipelineDescriptor {
            label: Some("fika-vulkan-text-pipeline"),
            vertex: VertexState {
...
fn compile_atlas_graph(initialized: bool, queue_family: u32) -> Result<CompiledGraph, String> {
    ...
    graph.add_pass(RenderPass {
        id: TEXT_UPLOAD,
        label: "fika-vulkan-text-atlas-upload".into(),
        depends_on: Vec::new(),
    ...
    graph.add_pass(RenderPass {
        id: TEXT_SAMPLE,
        label: "fika-vulkan-text-atlas-sample".into(),
