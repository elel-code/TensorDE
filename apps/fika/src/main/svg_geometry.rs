//! Backend-independent SVG parsing and tessellation for GPU renderers.
...
use lyon::tessellation::{
    BuffersBuilder, FillOptions, FillRule, FillTessellator, LineCap, LineJoin, StrokeOptions,
    StrokeTessellator, VertexBuffers,
};

#[path = "svg_geometry/types.rs"]
mod types;

use types::{SvgAffine, SvgGradientStop, SvgPaint, SvgPaintState, SvgVertexCtor};
pub(crate) use types::{SvgGeometry, SvgIntrinsicSize, SvgVertex};

pub(crate) fn svg_intrinsic_size(bytes: &[u8]) -> Option<SvgIntrinsicSize> {
...
pub(crate) fn tessellate_svg(bytes: &[u8], width: u32, height: u32) -> Option<SvgGeometry> {
    let text = std::str::from_utf8(bytes).ok()?;
    let document = roxmltree::Document::parse(text).ok()?;
    let root = document.root_element();
    let (vx, vy, vw, vh) = svg_view_box(root).unwrap_or((0.0, 0.0, width as f32, height as f32));
    if width == 0 || height == 0 || vw <= 0.0 || vh <= 0.0 {
        return None;
    }
    ...
    (!geometry.indices.is_empty()).then_some(SvgGeometry {
...
fn tessellate_svg_node(
    ...
    geometry: &mut VertexBuffers<SvgVertex, u32>,
    depth: usize,
) {
    if !node.is_element() || depth > 32 {
        return;
    }
    ...
    if matches!(
        tag,
        "defs" | "clipPath" | "mask" | "style" | "linearGradient" | "radialGradient" | "stop"
    ) {
        return;
    }
    if tag == "use" {
    ...
    if let Some(path) = svg_node_path(node) {
        ...
        if let Some(paint) = state.fill.clone() {
            let ctor = SvgVertexCtor {
                transform: state.transform.then(viewport),
                paint,
                bounds,
...
fn svg_node_state(
    ...
    for (key, value) in &declarations {
        match key.as_str() {
            "fill" => {
                if value != "inherit" {
                    state.fill = parse_svg_paint(value, state.color, document)
                }
            }
            "stroke" => {
                if value != "inherit" {
                    state.stroke = parse_svg_paint(value, state.color, document)
                }
            }
            "stroke-width" => {
                if let Some(v) = svg_number(value) {
                    state.stroke_width = v
                }
            }
            "stroke-linecap" => {
                state.stroke_cap = match value.as_str() {
                    "round" => LineCap::Round,
            ...
            "stroke-linejoin" => {
                state.stroke_join = match value.as_str() {
            ...
            "fill-rule" => {
                state.fill_rule = if value == "evenodd" {
                    FillRule::EvenOdd
                } else {
                    FillRule::NonZero
                }
            ...
            "fill-opacity" => {
                if let Some(v) = svg_number(value) {
                    multiply_svg_paint_alpha(&mut state.fill, v)
                }
            }
            "stroke-opacity" => {
                if let Some(v) = svg_number(value) {
                    multiply_svg_paint_alpha(&mut state.stroke, v)
                }
            }
            _ => {}
        }
...
fn parse_svg_gradient(document: &roxmltree::Document<'_>, id: &str) -> Option<SvgPaint> {
    ...
    let object_bounding_box =
        svg_gradient_attribute(node, "gradientUnits", document, 0) != Some("userSpaceOnUse");
...
fn svg_gradient_stops(
    ...
    let mut stops = node
        .children()
        .filter(|child| child.has_tag_name("stop"))
        .map(|stop| {
            let mut color = [0.0, 0.0, 0.0, 1.0];
            let mut offset = 0.0;
            let mut declarations = Vec::new();
            for key in ["offset", "stop-color", "stop-opacity"] {
                if let Some(value) = stop.attribute(key) {
                    declarations.push((key.to_string(), value.to_string()));
                }
            }
            ...
            for (key, value) in declarations {
                match key.as_str() {
                    "offset" => {
                        offset = svg_percentage_or_number(&value)
                            .unwrap_or(0.0)
                            .clamp(0.0, 1.0)
                    }
                    "stop-color" => {
                        if let Some(parsed) = parse_svg_color(&value) {
                            color = parsed
                        }
                    }
                    "stop-opacity" => {
                        if let Some(opacity) = svg_number(&value) {
    ...
    if stops.is_empty() && depth < 8 {
        ...
        if let Some(parent) = href.and_then(|href| href.strip_prefix('#')).and_then(|id| {
            document
                .descendants()
                .find(|candidate| candidate.attribute("id") == Some(id))
        }) {
    ...
    for pair in stops.windows(2) {
        if offset <= pair[1].offset {
            ...
            return std::array::from_fn(|index| {
                pair[0].color[index] + (pair[1].color[index] - pair[0].color[index]) * amount
            });
        }
    }
...
fn parse_svg_paint(
    ...
    match svgtypes::Paint::from_str(value).ok()? {
        ...
        svgtypes::Paint::FuncIRI(id, fallback) => {
            parse_svg_gradient(document, id).or_else(|| match fallback {
                Some(svgtypes::PaintFallback::CurrentColor) => Some(SvgPaint::Solid(current_color)),
                Some(svgtypes::PaintFallback::Color(color)) => {
                    Some(SvgPaint::Solid(svg_color(color)))
                }
                _ => None,
            })
        }
        ...
        svgtypes::Paint::ContextFill | svgtypes::Paint::ContextStroke => {
            Some(SvgPaint::Solid(current_color))
        }
    }
}
...
fn parse_svg_transform(value: &str) -> SvgAffine {
    ...
    for token in svgtypes::TransformListParser::from(value).flatten() {
        let next = match token {
            T::Matrix { a, b, c, d, e, f } => SvgAffine {
                a: a as f32,
                b: b as f32,
                c: c as f32,
                d: d as f32,
            ...
            T::Translate { tx, ty } => SvgAffine {
                e: tx as f32,
                f: ty as f32,
                ..SvgAffine::IDENTITY
            },
            T::Scale { sx, sy } => SvgAffine {
                a: sx as f32,
                d: sy as f32,
                ..SvgAffine::IDENTITY
            },
            T::Rotate { angle } => {
                let (s, c) = (angle as f32).to_radians().sin_cos();
                SvgAffine {
                    a: c,
                    b: s,
                    c: -s,
            ...
            T::SkewX { angle } => SvgAffine {
                c: (angle as f32).to_radians().tan(),
                ..SvgAffine::IDENTITY
            },
            T::SkewY { angle } => SvgAffine {
                b: (angle as f32).to_radians().tan(),
                ..SvgAffine::IDENTITY
            },
        };
        out = next.then(out);
...
fn svg_node_path(node: roxmltree::Node<'_, '_>) -> Option<LyonPath> {
    ...
    match node.tag_name().name() {
        ...
        "circle" | "ellipse" => {
            let cx = node.attribute("cx").and_then(svg_number).unwrap_or(0.0);
            let cy = node.attribute("cy").and_then(svg_number).unwrap_or(0.0);
            let rx = if node.tag_name().name() == "circle" {
                svg_number(node.attribute("r")?)?
            } else {
                svg_number(node.attribute("rx")?)?
            };
            let ry = if node.tag_name().name() == "circle" {
                rx
            } else {
                svg_number(node.attribute("ry")?)?
            };
            for i in 0..=48 {
                let a = i as f32 * std::f32::consts::TAU / 48.0;
                let p = point(cx + rx * a.cos(), cy + ry * a.sin());
                if i == 0 {
                    builder.move_to(p);
                } else {
                    builder.line_to(p);
        ...
        "polygon" | "polyline" => {
            ...
            for (i, pair) in nums.chunks_exact(2).enumerate() {
                if i == 0 {
                    builder.move_to(point(pair[0], pair[1]));
                } else {
        ...
        "line" => {
            builder.move_to(point(
                svg_number(node.attribute("x1")?)?,
                svg_number(node.attribute("y1")?)?,
            ));
            builder.line_to(point(
                svg_number(node.attribute("x2")?)?,
                svg_number(node.attribute("y2")?)?,
            ));
        }
        _ => return None,
    }
...
fn build_svg_path_data(data: &str, builder: &mut impl SvgPathBuilder) {
    ...
    for segment in svgtypes::PathParser::from(data).flatten() {
        match segment {
            ...
            S::SmoothCurveTo { x2, y2, x, y, .. } => builder.smooth_relative_cubic_bezier_to(
                vector(x2 as f32, y2 as f32),
                vector(x as f32, y as f32),
            ),
            S::Quadratic {
            ...
            S::Quadratic { x1, y1, x, y, .. } => builder.relative_quadratic_bezier_to(
                vector(x1 as f32, y1 as f32),
                vector(x as f32, y as f32),
            ),
            S::SmoothQuadratic { abs: true, x, y } => {
