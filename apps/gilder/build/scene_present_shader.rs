//! Cold-path native Slang shader for the live SceneColor terminal present pass.
...
pub(super) fn build_scene_present_shaders() {
    ...
    let shader_dir = out_dir.join("gilder_scene_present_shaders");
    ...
    fs::write(out_dir.join("gilder_scene_present_shaders.rs"), generated)
...
fn compile_fragment(shader_dir: &Path) -> (PathBuf, u32) {
    ...
    let native = shader_dir.join("terminal_present.frag.slang");
    let spirv = shader_dir.join("terminal_present.frag.spv");
    fs::write(&native, lowered.source).expect("write native scene-present fragment source");
    SlangCompiler::from_environment()
        .compile(&ShaderCompileRequest {
            source: native,
            entry_point: "main".to_owned(),
