//! Cold-path native Slang compilation and descriptor-heap lowering for built-in stages.
...
use vulkan_renderer_build::{
    ...
    ShaderStage, SlangCompiler, lower_generated_stage_to_native_slang,
...
pub(super) fn compile_native_slang_scene_fragment(
    shader_dir: &Path,
    key: &str,
    source: &str,
) -> NativeSceneStage {
    compile_native_slang_scene_stage(shader_dir, key, source, "frag", ShaderStage::Fragment, 0)
}

/// Cold-lowers a generated engine stage directly to native Slang before the
...
pub(super) fn compile_native_slang_scene_vertex(
    shader_dir: &Path,
    key: &str,
    source: &str,
    push_base_bytes: u32,
) -> NativeSceneStage {
    compile_native_slang_scene_stage(
        shader_dir,
        key,
        source,
        "vert",
...
pub(super) fn compile_native_slang_scene_input_attachment(
    shader_dir: &Path,
    key: &str,
    source: &str,
    push_base_bytes: u32,
    ...
    fs::write(&source_path, source).expect("write exact native Slang input-attachment source");
    let lowered =
        lower_slang_input_attachment_to_descriptor_heap_at_offset(source, "main", push_base_bytes)
            .unwrap_or_else(|error| {
                panic!("lower exact native Slang input-attachment shader {key}: {error}")
            });
    assert_eq!(
    ...
    fs::write(&native_path, &lowered.source)
        .expect("write native Slang input-attachment heap proxy");
    SlangCompiler::from_environment()
        ...
        .unwrap_or_else(|error| {
            panic!("compile exact native Slang input-attachment shader {key}: {error}")
        });
    NativeSceneStage {
        spirv: spirv_path,
        source: source_path,
...
pub(super) fn compile_native_particle_compute(
    ...
    compile_native_slang_scene_stage(shader_dir, key, source, "comp", ShaderStage::Compute, 0)
}

fn compile_native_slang_scene_stage(
    shader_dir: &Path,
    key: &str,
    source: &str,
    extension: &str,
    ...
    fs::write(&source_path, source).expect("write exact native Slang scene source");
    let (native_source, push_constant_bytes, bindings, contract) =
        if native_slang_exposes_resources(source) {
            let lowered =
                lower_slang_bindings_to_descriptor_heap_at_offset(source, "main", push_base_bytes)
                    .unwrap_or_else(|error| {
                        panic!("lower exact native Slang scene shader {key} {extension}: {error}")
                    });
            (
                lowered.source,
    ...
    fs::write(&native_path, native_source)
        .expect("write descriptor-heap native Slang scene source");
    SlangCompiler::from_environment()
        ...
        .unwrap_or_else(|error| {
            panic!("compile exact native Slang scene shader {key} {extension}: {error}")
        });
    NativeSceneStage {
        spirv: spirv_path,
        source: source_path,
...
fn compile_generated_scene_stage(
    ...
    let native = lower_generated_stage_to_native_slang(source, stage).unwrap_or_else(|error| {
        panic!("lower generated scene shader {key} {extension} to native Slang: {error}")
    });
    compile_native_slang_scene_stage(shader_dir, key, &native, extension, stage, push_base_bytes)
}

fn native_slang_exposes_resources(source: &str) -> bool {
