...
pub(super) use particle::SharedSceneParticleResources;
        ...
        let mut uploads = upload_belt
            .begin(
                ...
                &CommandEncoderDescriptor {
                    label: Some("gilder-scene-cold-resources".into()),
                },
            )
...
impl SharedSceneFrameResources {
    pub(super) fn create(
        ...
        let transform = create_upload_buffer(
            allocator,
            format!("gilder-scene-transform-slot-{frame_slot}"),
            payloads.transform,
        ...
        let video_vertex = payloads
            ...
            .map(|payload| {
                create_upload_buffer(
                    allocator,
                    format!("gilder-scene-video-vertex-slot-{frame_slot}"),
                    payload,
                    BufferUsages::VERTEX,
                )
            })
        ...
        let material = payloads
            ...
            .map(|payload| {
                create_upload_buffer(
                    allocator,
                    format!("gilder-scene-material-slot-{frame_slot}"),
                    payload,
        ...
        let skinning = payloads
            ...
            .map(|payload| {
                create_upload_buffer(
                    allocator,
                    format!("gilder-scene-skinning-slot-{frame_slot}"),
                    payload,
        ...
        let scene_owned_uniform = payloads
            ...
            .map(|payload| {
                create_upload_buffer(
                    allocator,
                    format!("gilder-scene-owned-uniform-slot-{frame_slot}"),
                    payload,
                    BufferUsages::UNIFORM | BufferUsages::SHADER_DEVICE_ADDRESS,
                )
            })
