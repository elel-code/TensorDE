...
impl SharedSceneGpuResources {
    ...
    pub(in crate::renderer::native_vulkan::vulkan::scene::runtime) fn record_particle_compute(
        &self,
        encoder: &mut CommandEncoder,
        frame_slot: usize,
        reference_phase: usize,
        ...
        let group_count = particles.emitter_count.div_ceil(64);
        if group_count == 0 {
            return Ok(false);
        }
        ...
        unsafe {
            encoder
                .update_buffer(&particles.frame_time, 0, &scene_time_seconds.to_ne_bytes())
        ...
        encoder.retain_resource(&particles.frame_time);
        {
            let mut compute = encoder.begin_compute(&ComputePassDescriptor {
                label: Some("gilder-scene-particle-update"),
            });
            compute
                .bind_machine_code_pipeline(pipeline.pipeline())
    ...
    fn record_local_read_pair(
        ...
        let descriptor = rendering_descriptor("gilder-scene-local-read", extent, &attachments);
        unsafe {
            let mut rendering = encoder
                .begin_rendering(&descriptor)
...
unsafe fn record_bound_draw(
    rendering: &mut RenderingEncoder<'_>,
    draw: &super::super::draw_recording::SceneGpuDrawCommand,
...
fn scene_scissor(scissor: Option<SceneGpuScissor>, extent: Extent2D) -> Rect2D {
    scissor.map_or_else(
