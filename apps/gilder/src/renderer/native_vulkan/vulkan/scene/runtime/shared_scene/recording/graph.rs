...
impl SharedSceneGpuResources {
    ...
    pub(in crate::renderer::native_vulkan::vulkan::scene::runtime) fn record_graphs_to_scene_color(
        &self,
        ...
        for graph in &self.frame_execution_plan.graphs {
            if !graph_is_active(graph, &self.draw_commands) {
                continue;
            }
...
fn scene_color_descriptor<'a>(
    ...
    RenderingDescriptor {
        label: Some("gilder-scene-color-pass"),
...
const fn clear_color_array(color: NativeVulkanClearColor) -> [f32; 4] {
