...
use super::super::{
    NativeVulkanSceneDeviceSnapshot, NativeVulkanScenePresentOptions,
    NativeVulkanScenePresentSnapshot, NativeVulkanSceneQueueSnapshot,
    NativeVulkanSceneSwapchainSnapshot, scene_script_effect_visibility_snapshot,
...
pub(super) fn build(
    ...
    let selected_queue = NativeVulkanSceneQueueSnapshot {
    ...
    let device = NativeVulkanSceneDeviceSnapshot {
    ...
    let swapchain = NativeVulkanSceneSwapchainSnapshot {
        created: true,
    ...
    let transform_uniform_bytes = scene.frames.iter().map(|frame| frame.transform.size()).sum();
    ...
    let scene_owned_uniform_bytes = scene
        ...
        .map(vulkan_renderer::Buffer::size)
        .sum();
    let scene_color_mesh_draw_count = super::super::draw_recording::draw_range_count(
        &scene.scene_color_draw_ranges,
    );
    ...
    let semantic_diagnostics = options
        ...
        .then(|| {
            scene_semantic_diagnostics_snapshot(
                ...
                &scene.draw_commands,
                NativeVulkanSceneDescriptorHeapSnapshot {
    ...
    Ok(NativeVulkanScenePresentSnapshot {
        ...
        terminal_present_model: "native-slang-o2-terminal-present",
