    ...
    fn create_descriptor_phase(
        ...
        let resource_heap = device
            .create_descriptor_heap(&DescriptorHeapDescriptor {
                label: Some(format!(
                    "gilder-scene-resource-heap-slot-{frame_slot}-phase-{reference_phase}"
                )),
        ...
        let sampler_heap = (self.sampler_descriptor_count != 0)
            .then(|| {
                ...
                device.create_descriptor_heap(&DescriptorHeapDescriptor {
                    label: Some(format!(
                        "gilder-scene-sampler-heap-slot-{frame_slot}-phase-{reference_phase}"
                    )),
...
fn lower_particle_descriptors<'a>(
    ...
    match (
        ...
        (Some(base), Some(particles)) => {
            for (lane, buffer) in [&particles.state, &particles.indirect, &particles.frame_time]
                .into_iter()
                .enumerate()
            {
                assign_resource(
