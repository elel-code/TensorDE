...
use vulkan_renderer::{
    CommandEncoderDescriptor, Extent2D, Features, FrameTargetPreference, PresentationBootstrap,
    FullscreenSampledSurfaceTerminalDescriptor, PresentationPathDescriptor, PresentationPathPlan,
    PresentationRequirements, PresentationTransactionDescriptor, SurfaceAcquireStrategy,
    TerminalAlphaMode, TerminalCompositeDescriptor, TerminalSampling, TextureUsages,
    OffscreenSamplerTopology,
};
#[cfg(feature = "native-vulkan-video")]
...
use crate::renderer::native_vulkan::shared_presentation::gilder_presentation_bootstrap_descriptor;
...
use super::{NativeVulkanScenePresentOptions, NativeVulkanScenePresentSnapshot};
#[cfg(feature = "native-vulkan-video")]
...
pub(super) fn run(
    mut options: NativeVulkanScenePresentOptions,
) -> Result<NativeVulkanScenePresentSnapshot, String> {
    if options.gpu_timing {
        return Err(
            "shared scene GPU timing requires renderer-owned typed timestamp queries".into(),
        );
    }
    let mut host = NativeWaylandHost::connect(options.host.clone()).map_err(|error| error.to_string())?;
    ...
    let video_requirements =
        SharedSceneVideoRuntime::requirements(&backend_plan, &options.video_sources)?;
    #[cfg(not(feature = "native-vulkan-video"))]
    let video_requirements = reject_video_without_feature(&backend_plan, &options.video_sources)?;
    ...
    let mut bootstrap = PresentationBootstrap::create(
        ...
        gilder_presentation_bootstrap_descriptor(
            "gilder-scene",
            extent,
            required_features,
            optional_features,
    ...
    .map_err(|error| format!("create Gilder scene presentation bootstrap: {error}"))?;
    #[cfg(feature = "native-vulkan-video")]
    ...
    let terminal = bootstrap
        ...
        .create_fullscreen_sampled_surface_terminal(
            ...
            &FullscreenSampledSurfaceTerminalDescriptor {
                label: Some("gilder-scene-terminal".into()),
                plan: &plan,
    ...
    let mut transaction = bootstrap
        ...
        .create_presentation_transaction(&PresentationTransactionDescriptor {
            label: Some("gilder-scene-presentation".into()),
            plan: &plan,
    ...
    if std::env::var_os("GILDER_NATIVE_VULKAN_SCENE_PIPELINE_DEBUG").is_some() {
        eprintln!("gilder-scene-startup: shared-frame-loop-ready");
    }
    ...
    let fixed_scene_time_seconds = fixed_non_negative_f32("GILDER_NATIVE_VULKAN_SCENE_FIXED_TIME");
    let fixed_frame_delta_seconds =
        fixed_non_negative_f32("GILDER_NATIVE_VULKAN_SCENE_FIXED_FRAME_DELTA");
    ...
    while deadline.is_none_or(|deadline| Instant::now() < deadline) {
                    ...
                    (|| -> Result<[_; 1], String> {
                        ...
                        let mut encoder = bootstrap
                            ...
                            .create_command_encoder(&CommandEncoderDescriptor {
                                label: Some(format!("gilder-scene-frame-{frame_slot}")),
                            })
                            .map_err(|error| {
                                format!("begin shared scene frame commands: {error}")
                            })?;
                        scene.record_particle_compute(
                            &mut encoder,
                            frame_slot,
                            reference_phase,
                            scene_time_seconds,
                        ...
                        encoder.finish().map(|commands| [commands]).map_err(|error| {
                            format!("finish shared scene frame commands: {error}")
                        })
                    })()
        ...
        accumulate_update(&mut stats, frame_update.expect("frame transaction recorded an update"));
...
fn presentation_plan(
    ...
    let physical_pass_count = backend_plan
...
fn shared_scene_frame_slot_count() -> Result<usize, String> {
    const MAX: usize = 3;
    let count = std::env::var("GILDER_NATIVE_VULKAN_SCENE_FRAME_SLOT_COUNT")
        .ok()
        .map(|value| {
            value
                .parse::<usize>()
    ...
    if !(1..=MAX).contains(&count) {
        return Err(format!("scene frame-slot count must be in 1..={MAX}, got {count}"));
    }
    Ok(count)
}

fn scene_color_4x_msaa_requested() -> Result<bool, String> {
    let Some(value) = std::env::var_os("GILDER_NATIVE_VULKAN_SCENE_MSAA") else {
        return Ok(false);
    };
    let value = value.into_string().map_err(|_| {
        "GILDER_NATIVE_VULKAN_SCENE_MSAA must contain valid UTF-8".to_owned()
    })?;
    match value.trim().to_ascii_lowercase().as_str() {
        "" | "1" | "1x" => Ok(false),
        "4" | "4x" => Ok(true),
        _ => Err(format!(
            "GILDER_NATIVE_VULKAN_SCENE_MSAA must be 1, 1x, 4, or 4x; got {value:?}"
        )),
    }
}
...
fn reject_video_without_feature(
    plan: &NativeVulkanSceneBackendPlan,
    sources: &[crate::renderer::native_vulkan::scene::NativeVulkanSceneVideoSource],
    ...
    if !requires_video && sources.is_empty() {
        ...
        Err("scene VideoFrame sources require the native-vulkan-video feature".into())
    }
}

fn accumulate_update(stats: &mut SharedFrameStats, update: super::frame_state::SceneFrameBufferUpdate) {
