...
use super::{
    BuiltinSceneShaderStage, SceneGpuDrawCommand, SceneNativeDescriptorPush,
    ScenePipelineDescriptorLayout, sampled_descriptor, sampled_slot_index,
    validate_native_push_size,
};
...
pub(in crate::renderer::native_vulkan::vulkan::scene::runtime) fn resolve_scene_shared_descriptor_pushes(
    ...
    for (draw, command) in draws.iter().zip(commands) {
        ...
        command.native_descriptor_push = match program {
            SceneResolvedGraphicsProgram::SceneOwned {
                ...
                validate_native_push_size(key, &push, max_push_data_size)?;
                Some(push)
            }
        ...
        command.disabled_native_descriptor_push = if command.disabled_pipeline_index.is_some() {
            ...
            let vertex =
                native_vulkan_scene_vertex_shader_for_primitive(passthrough, draw.primitive)
                    .ok_or_else(|| {
                        format!(
            ...
            validate_native_push_size("we/passthrough", &push, max_push_data_size)?;
            Some(push)
        } else {
            None
        };
