...
pub(super) fn flat_rounded_mask_uv_bounds(
    ...
    output_extent: [u32; 2],
    sampled_source_extent: Option<[f32; 2]>,
    ...
    if !size
        ...
        || sampled_source_extent.is_some_and(|extent| {
            extent
                .into_iter()
    ...
    let (softness_extent, filter_margin) = sampled_source_extent.map_or_else(
        || {
            (
                output_extent[0].max(output_extent[1]).max(1) as f32,
                [0.0, 0.0],
            )
        },
    ...
    let half_extent = [
        size[0] * 0.5 + edge_softness / aspect[0] + filter_margin[0],
        size[1] * 0.5 + edge_softness / aspect[1] + filter_margin[1],
    ];
    Some(FlatRoundedMaskUvBounds {
        min: [
            (0.5 - half_extent[0]).max(0.0),
            (0.5 - half_extent[1]).max(0.0),
        ],
        max: [
            (0.5 + half_extent[0]).min(1.0),
            (0.5 + half_extent[1]).min(1.0),
        ],
    })
}
...
mod tests {
    ...
    #[test]
    fn reconstructed_target_bounds_include_unorm_softness_and_linear_filter_footprint() {
        let bounds = flat_rounded_mask_uv_bounds(
            [0.9, 0.9],
            2.0,
            [550.0, 3300.0],
        ...
        .expect("sampled rounded target");
        let edge = 2.0 / 3300.0 * 2.0;
        assert!((bounds.min[0] - (0.05 - edge - 1.0 / 550.0)).abs() <= 1.0e-6);
        assert!((bounds.min[1] - (0.05 - edge / 6.0 - 1.0 / 3300.0)).abs() <= 1.0e-6);
        assert!((bounds.max[0] - (0.95 + edge + 1.0 / 550.0)).abs() <= 1.0e-6);
        assert!((bounds.max[1] - (0.95 + edge / 6.0 + 1.0 / 3300.0)).abs() <= 1.0e-6);
    }
