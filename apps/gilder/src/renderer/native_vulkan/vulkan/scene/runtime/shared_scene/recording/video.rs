...
impl SharedSceneGpuResources {
    ...
    pub(in crate::renderer::native_vulkan::vulkan::scene::runtime) fn record_graphs_to_scene_color_with_video(
        &mut self,
        ...
        for (&media_instance, frame) in video_media_instances.iter().zip(video_frames) {
            encoder
                .end_decoded_video_sampling(frame)
                .map_err(|error| {
                    format!(
    ...
    fn validate_video_frames(
        ...
        for (source_index, (&media_instance, frame)) in video_media_instances
            .iter()
            .zip(video_frames)
            .enumerate()
        {
