...
use crate::renderer::native_vulkan::scene::NativeVulkanSceneVideoSource;
...
pub(super) struct SharedSceneVideoRuntime {
    sources: Vec<NativeVulkanSharedVideoSource>,
...
impl SharedSceneVideoRuntime {
    pub(super) fn requirements(
        ...
        sources: &[NativeVulkanSceneVideoSource],
        ...
        NativeVulkanSharedVideoSource::requirements(sources.iter().map(|source| source.codec))
            .map(Some)
    }
    ...
    pub(super) fn open(
        ...
        plan: &NativeVulkanSceneBackendPlan,
        source_options: Vec<NativeVulkanSceneVideoSource>,
    ) -> Result<Self, String> {
        let required = required_media_instances(plan);
        ...
        for source in source_options {
            ...
            sources.push(NativeVulkanSharedVideoSource::open(
...
fn required_media_instances(plan: &NativeVulkanSceneBackendPlan) -> Vec<u32> {
...
mod tests {
    ...
    fn source(media_instance: u32) -> NativeVulkanSceneVideoSource {
        NativeVulkanSceneVideoSource {
            media_instance,
