...
#[path = "frame_present/frame_slots.rs"]
mod frame_slots;

pub(in crate::renderer::native_vulkan::vulkan) use frame_slots::*;
...
pub(super) fn native_vulkan_vulkanalia_record_decoded_image_present_command_buffer(
    ...
    unsafe {
        ...
        if let Some(scene_video_layer_draw) = scene_video_layer_draw {
            ...
            let push_range = vk::HostAddressRangeConstEXT::builder()
                .address(pipeline.descriptor_push.as_slice());
...
pub(super) fn native_vulkan_vulkanalia_record_decoded_image_scene_video_layer_draws_inside_rendering(
    ...
    unsafe {
        ...
        let push_range =
            vk::HostAddressRangeConstEXT::builder().address(pipeline.descriptor_push.as_slice());
        let push = vk::PushDataInfoEXT::builder().offset(0).data(push_range);
        device.cmd_push_data_ext(command_buffer, &push);
