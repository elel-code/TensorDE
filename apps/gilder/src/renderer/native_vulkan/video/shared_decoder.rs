//! Cold Gilder media frontend over renderer-owned FFmpeg Vulkan decode.
...
use super::event_source::NativeVulkanVideoEventSample;

pub(in crate::renderer::native_vulkan) struct NativeVulkanSharedVideoSource {
    media_instance: u32,
    source: PathBuf,
    ...
    frame_serial: u64,
    ended: bool,
    pacing: NativeVulkanVideoPacing,
}

impl std::fmt::Debug for NativeVulkanSharedVideoSource {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("NativeVulkanSharedVideoSource")
...
impl NativeVulkanSharedVideoSource {
    pub(in crate::renderer::native_vulkan) fn requirements(
        codecs: impl IntoIterator<Item = NativeVulkanVideoSessionCodec>,
    ...
    pub(in crate::renderer::native_vulkan) fn open(
        ...
        Ok(Self {
            ...
            frame_serial: 0,
            ended: false,
            pacing: NativeVulkanVideoPacing::default(),
        })
    }

    pub(in crate::renderer::native_vulkan) const fn media_instance(&self) -> u32 {
        self.media_instance
    }

    pub(in crate::renderer::native_vulkan) fn current_frame(
        &self,
    ...
    pub(in crate::renderer::native_vulkan) fn advance_to(
        &mut self,
        presentation_now: Instant,
        loop_on_eos: bool,
    ) -> Result<Option<NativeVulkanVideoEventSample>, String> {
        ...
        for _ in 0..MAX_CATCH_UP_DECODE_FRAMES {
            ...
            let Some(frame) = self
                ...
                return Ok(Some(NativeVulkanVideoEventSample {
            ...
            latest_sample = Some(NativeVulkanVideoEventSample {
                generation: self.generation,
                frame_serial: self.frame_serial,
                frame_identity: self.frame_serial,
                presentation_time_ns,
    ...
    pub(in crate::renderer::native_vulkan) const fn decoded_frame_count(&self) -> u64 {
        self.frame_serial
    }

    pub(in crate::renderer::native_vulkan) fn loop_index(&self) -> u64 {
...
struct NativeVulkanVideoPacing {
...
mod tests {
    ...
    fn requirement_union_keeps_every_exact_profile() {
        let requirements = NativeVulkanSharedVideoSource::requirements([
    ...
    fn pacing_holds_a_decoded_frame_until_its_pts_duration_expires() {
        ...
        let mut pacing = NativeVulkanVideoPacing::default();
