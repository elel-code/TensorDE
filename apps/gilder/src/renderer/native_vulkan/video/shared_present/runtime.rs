...
use crate::renderer::native_vulkan::video::shared_decoder::NativeVulkanSharedVideoSource;
use crate::renderer::native_vulkan::{NativeVulkanClearColor, NativeVulkanVideoSessionCodec};
use crate::renderer::native_wayland::{NativeWaylandHost, NativeWaylandHostOptions, NativeWaylandSurfaceSnapshot};
...
pub struct NativeVulkanSharedVideoPresentOptions {
...
pub struct NativeVulkanSharedVideoPresentSnapshot {
    ...
    pub surface_host: NativeWaylandSurfaceSnapshot,
...
pub fn run_native_vulkan_shared_video_present(
    options: NativeVulkanSharedVideoPresentOptions,
) -> Result<NativeVulkanSharedVideoPresentSnapshot, String> {
    ...
    let video_requirements = NativeVulkanSharedVideoSource::requirements([options.codec])?;
    ...
    .map_err(|error| format!("create Gilder video presentation bootstrap: {error}"))?;
    ...
    let mut source = NativeVulkanSharedVideoSource::open(
        video_decode,
        0,
        &options.source,
        options.codec,
    ...
    let mut resources = bootstrap
        ...
        .create_decoded_video_surface_terminal(
            ...
            &DecodedVideoSurfaceTerminalDescriptor {
                label: Some("gilder-shared-video-present".into()),
    ...
    let mut transaction = bootstrap
        ...
        .create_presentation_transaction(&PresentationTransactionDescriptor {
            label: Some("gilder-shared-video-presentation".into()),
            plan: &plan,
    ...
    Ok(NativeVulkanSharedVideoPresentSnapshot {
...
fn shared_video_frame_slot_count() -> Result<usize, String> {
    let count = std::env::var("GILDER_NATIVE_VULKAN_VIDEO_FRAME_SLOT_COUNT")
        .ok()
        .map(|value| {
            value
                .parse::<usize>()
