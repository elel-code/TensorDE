...
fn shader_catalog_resolves_we_material_names_without_runtime_files() {
    ...
    assert!(
        native_vulkan_scene_shader_for_key("effects/lut_loader__SLOTS_3__CLAMP_0__QUAD_SIZE_64")
            .is_none()
    );
...
fn generic_image_vertex_preserves_authored_mesh_input_locations() {
    let shader =
        native_vulkan_scene_shader_for_key("we/genericimage4").expect("generic image shader");

    // Native Slang O2 may omit source variable names; locations are the stable vertex ABI.
...
fn scene_color_blend_applies_the_d3d_to_vulkan_y_normalization_once() {
    ...
    let native_screen_y = vulkan_clip_y * 0.5 + 0.5;
    assert_eq!(native_screen_y.to_bits(), vm_screen_y.to_bits());
}
...
fn effect_catalog_carries_distinct_fullscreen_and_object_mesh_vertex_domains() {
    let shimmer =
        native_vulkan_scene_shader_for_key("effects/shimmer__SLOTS_9").expect("shimmer shader");
    assert_eq!(
    ...
    let iris =
        native_vulkan_scene_shader_for_key("effects/iris__SLOTS_3__MASK_1").expect("iris shader");
    assert!(
        native_vulkan_scene_vertex_spirv_for_primitive(
            iris,
            crate::engine::scene::SceneRenderingDeviceDrawPrimitive::ObjectMesh,
...
fn clipping_final_catalog_preserves_authored_non_contiguous_texture_slots() {
    let shader = native_vulkan_scene_shader_for_key("we/puppet-iris-waterripple-clipping-final")
...
fn quantized_framebuffer_water_catalog_exposes_three_typed_stages() {
    ...
    let prepass_vertex_source = include_str!(concat!(
        env!("OUT_DIR"),
        "/scene_shader_catalog/effects_caustics__SLOTS_3d__BLENDMODE_6__GILDER_FRAMEBUFFER_QUANTIZED_OVERLAY_1.vert.source.slang"
    ));
    assert!(
        prepass_vertex_source.contains("dot(u_Draw.g_ObjectUvToScreenUvRow0.xyz, vec3(uv, 1.0))")
    );
    assert!(
        prepass_vertex_source
            .contains("[[vk::location(0)]] vec2 v_FramebufferCoord : TEXCOORD0;")
    );
    ...
    let intermediate =
        native_vulkan_scene_shader_for_key("we/framebuffer-water-quantized-water-opacity")
    ...
    let final_program =
        native_vulkan_scene_shader_for_key("we/framebuffer-water-quantized-shake-final")
...
fn passthrough_catalog_exposes_only_the_explicit_exact_pixel_variant() {
    let passthrough =
        native_vulkan_scene_shader_for_key("we/passthrough").expect("passthrough shader");
    ...
    assert!(
        native_vulkan_scene_shader_for_key("we/genericimage4")
...
fn rounded_hsl_catalog_declares_exact_fragment_coordinate_snapshot_fetch() {
    ...
    assert!(
        native_vulkan_scene_shader_catalog()
            .iter()
...
fn effect_target_waterwaves_identity_is_not_emitted_for_mesh_composites() {
    let effect = native_vulkan_scene_shader_for_key("we/effect-waterwaves-direct__STAGES_6")
        .expect("effect-target waterwaves shader");
    let image = native_vulkan_scene_shader_for_key("we/image-waterwaves-direct__STAGES_6")
        .expect("image waterwaves shader");
    let puppet = native_vulkan_scene_shader_for_key("we/puppet-waterwaves-direct__STAGES_6")
    ...
    let effect_two = native_vulkan_scene_shader_for_key("we/effect-waterwaves-direct__STAGES_2")
        .expect("two-stage effect-target waterwaves shader");
    let image_two = native_vulkan_scene_shader_for_key("we/image-waterwaves-direct__STAGES_2")
