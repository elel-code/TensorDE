...
use super::{
    ...
    SceneRenderingDeviceProjectionDomain, SceneStorage, authored_texture_space_target_extent,
};
...
/// applies authored zoom and camera translation. The fixed 4,000-unit depth span is the native
/// 2D camera contract recovered from the complete D3D11 MVP stream.
...
pub(super) fn pass_projection_domain(
    ...
    if pass.role != super::SceneRenderPassKind::ObjectLocalSource {
    ...
    let [width, height] =
        authored_texture_space_target_extent(storage, graph_index, pass.target, pass.target_name)
            .expect("validated object-local source pass requires an authored texture-space target");
