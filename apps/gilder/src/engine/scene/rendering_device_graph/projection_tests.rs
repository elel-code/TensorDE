...
fn authored_texture_projection_maps_centered_quad_like_we_zero_based_quad() {
    ...
    for ((gilder_position, gilder_uv), (we_position, we_uv)) in
        gilder_vertices.into_iter().zip(we_vertices)
    {
        let gilder_clip = [
            transform[0][0] * gilder_position[0] + transform[0][3],
            transform[1][1] * gilder_position[1] + transform[1][3],
        ];
        let we_d3d_clip = [
        ...
        assert_eq!(gilder_clip, we_vulkan_clip);
        assert_eq!(gilder_uv, we_uv);
    }
}
...
#[test]
fn ordinary_effect_base_target_keeps_scene_projection() {
