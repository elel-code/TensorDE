...
//! authored macros before the generated-stage source is lowered to native
...
pub(super) fn specialize_stage(
    ...
    let mut preprocessor = NativePreprocessor::new(source, definitions)
            ...
            match command {
                ...
                "extension" if is_active(&branches) => {
                    return directive_error(
                        path,
                        line_number,
                        "active #extension is not supported by native stage lowering",
                    );
                }
                "" if is_active(&branches) => {
...
fn logical_lines(source: &str) -> Result<Vec<(usize, String)>, String> {
    ...
    for (index, raw_line) in source.trim_start_matches('\u{feff}').lines().enumerate() {
        ...
        let mut line = strip_comments(raw_line, &mut block_comment)?;
        ...
        if let Some((start, pending)) = &mut continuation {
            ...
            lines.push((*start, std::mem::take(pending)));
            continuation = None;
        } else {
...
fn strip_comments(line: &str, block_comment: &mut bool) -> Result<String, String> {
