...
use tensor_ipc::land::{
    Command, EventMessage, EventTopic, FrameDecoder, IPC_PROTOCOL_VERSION, LayoutKind, Request,
    Response, ResultBody, ServerMessage, encode,
};
use thiserror::Error;
...
#[command(
    author,
    version,
    bin_name = "tensor-msg land",
    about = "Send a message to a running Tensor WM"
)]
struct Cli {
...
pub fn run(args: Vec<String>) -> ExitCode {
    let arguments = std::iter::once("tensor-msg land".to_owned()).chain(args);
...
impl ServerReader {
    ...
    fn read_response(&mut self, stream: &mut impl Read) -> Result<Response, ClientError> {
    ...
    fn read_event(&mut self, stream: &mut impl Read) -> Result<EventMessage, ClientError> {
        match self.read_message(stream)? {
    ...
    fn read_message(&mut self, stream: &mut impl Read) -> Result<ServerMessage, ClientError> {
...
enum ClientError {
    ...
    Io(#[from] io::Error),
    #[error(transparent)]
    Codec(#[from] tensor_ipc::land::CodecError),
    #[error(transparent)]
...
mod tests {
    use super::*;
    use tensor_ipc::land::{ConfigReloadEvent, ConfigReloadEventResult, ServerEvent};

    #[test]
    fn niri_style_msg_command_parses_without_a_second_client_binary() {
        let cli = Cli::try_parse_from(["tensor-msg land", "ping"]).unwrap();
    ...
    fn reader_retains_an_event_coalesced_with_the_subscription_response() {
        ...
        let mut reader = ServerReader::new();
        assert!(matches!(
            reader.read_response(&mut input).unwrap().result,
            ResultBody::Accepted
        ));
        assert_eq!(reader.read_event(&mut input).unwrap().sequence, 1);
