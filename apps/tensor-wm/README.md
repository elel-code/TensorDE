# Tensorland Compositor

Tensorland is an early-stage Wayland compositor written in Rust. Its intended stack is:

- Tensorland-owned `wayland-server` protocol state, rootless XWayland/XWM, input/session ownership,
  and atomic DRM/KMS submission.
- Compio completion dispatch with an io_uring-only Linux product driver.
- The shared `vulkan-renderer` crate as Tensorland's only Vulkan boundary.
- A bindless descriptor heap backed by `VK_EXT_descriptor_heap`.
- Pluggable layouts: scrolling 1D, spatial 2D, and master-stack.

Design records live in `docs/tensorland/`:
[architecture](../../docs/tensorland/architecture.md),
[rendering](../../docs/tensorland/rendering.md),
[startup](../../docs/tensorland/startup.md),
[configuration](../../docs/tensorland/configuration.md),
[IPC/portal gates](../../docs/tensorland/ipc-and-portal.md),
[testing](../../docs/tensorland/testing.md), and
[contributing](../../docs/tensorland/contributing.md).

The repository currently contains Tensorland-owned protocol state (compositor, xdg-shell, SHM, output,
...
ownership, Tensorland-owned atomic KMS output submission, and the shared Vulkan renderer. The renderer
...
```sh
cargo test -p tensorland
cargo run -p tensorland -- --validate-config --config apps/tensor-wm/examples/config.kdl
cargo run -p tensorland -- --check
TENSOR_LAYOUT=spatial-2d cargo run -p tensorland -- --check
cargo run -p tensorland -- --config apps/tensor-wm/examples/config.kdl --check
```

For a direct DRM/KMS smoke test, switch to a Linux virtual terminal, log in as
the normal desktop user, and run `uv run scripts/tensorland/tty.py`. The launcher builds
the compositor, keeps automatic GPU selection intact, and starts the compiled
`tensorland --session` binary. Hardware smoke tests stop after 20
...
`--validate-config` performs only typed KDL parsing and policy validation, and prints a named,
source-labeled diagnostic without initializing logging, Wayland, or Vulkan. `--check` remains safe
to run from an ordinary terminal and additionally validates the startup and renderer gates. In a TTY launch,
Tensorland itself appends tracing records to `artifacts/tensor/logs/tensor-tty.log` through
...
For the native presentation gate, use `uv run scripts/tensorland/tty.py --dmabuf-smoke`.
After Tensorland creates its socket, the launcher runs a GBM client on the exact
render node advertised by Tensorland's linux-dmabuf feedback. It succeeds only
...
The launcher waits until Tensorland has entered its compositor event loop before
starting the supplied direct client argv. It supplies Tensorland's `WAYLAND_DISPLAY`
...
belonging to the suspended host session is removed, matching Tensorland's own
...
Tensorland before the generic client starts. For example, use
`--client /absolute/path/to/app` to exercise a different native Wayland
client. `scripts/tensorland/tty-all-clients.sh` is the no-paste TTY shortcut for Fcitx,
...
The file format is KDL 2.0 parsed through `tensor-kdl`; `TENSOR_CONFIG` and `--config` select a file,
with `$XDG_CONFIG_HOME/tensor/config.kdl` as the default. Runtime control remains the versioned IPC
protocol rather than a second configuration dialect. Tensorland watches that path and reloads KDL on a
bounded cold worker; `tensor-msg land reload-config` queues the same operation explicitly and
`tensor-msg land get-config-status` reports its generation and last bounded diagnostic.
...
Compio CQEs + direct Wayland/input dispatch
 ...
 layout systems  scene extraction
                       |
                       v
      shared Vulkan renderer
       descriptor heap -> pass lowering
                       |
                dma-buf + fence
                       |
               Tensorland DRM/KMS
...
- `src/startup.rs`: CLI, typed KDL load, capability gates, and startup sequencing.
...
1. Complete the atomic hardware cursor-plane milestone in
   [`docs/tensorland/cursor.md`](../../docs/tensorland/cursor.md) without splitting the protocol and presentation model.
2. Lower dependency-selected region effects through shared retained intermediate targets, generic
   range operations, descriptor heaps, and timeline retirement; keep dependency-free semantic
   phases fused into the direct pass.
3. Add multi-plane/YUV client import and an explicit policy for implicit dma-buf reservation fences.
4. Add direct-scanout candidate selection inside the Tensorland tty/KMS adapter.
...
`tensor-msg land` is the matching standalone CLI for state/output/workspace/config queries, reload, `watch-config`,
...
starts `tensorland.service`, waits for the compositor, drives `tensorland-shutdown.target`, and clears the
...
Display managers should install `contrib/wayland-sessions/tensorland.desktop`; no entry is provided under
`xsessions`, because Tensorland is never an X11 session.
...
The default build enables Tensorland's rootless XWayland process, direct XWM, and Compio-completed X11
socket operations.
...
backend. Tensorland has no X11 compositor backend and refuses `--session` startup when it detects an
inherited X11-only session.

## XDP portal gate

XDP here means xdg-desktop-portal integration. The dedicated `tensor-xdp`
product owns D-Bus backend objects and future PipeWire coordination. Its
completed Settings interface has no compositor dependency. Future capture or
input portals may request bounded, permission-scoped Tensorland capabilities,
but cannot receive renderer handles, Wayland resources, DRM/KMS state, or
unrestricted ECS access.
