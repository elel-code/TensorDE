...
mod tests {
    ...
    fn validate_config_is_a_config_only_mode() {
        let cli =
            Cli::try_parse_from(["tensor", "--validate-config", "--config", "/tmp/config.kdl"])
                .unwrap();
    ...
    fn validate_config_rejects_runtime_modes() {
        assert!(Cli::try_parse_from(["tensor", "--validate-config", "--check"]).is_err());
        assert!(Cli::try_parse_from(["tensor", "--validate-config", "--session"]).is_err());
    }
}
