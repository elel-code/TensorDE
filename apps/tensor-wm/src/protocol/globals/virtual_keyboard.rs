...
impl VirtualKeyboardProtocol {
    ...
    pub(crate) fn is_active(&self) -> bool {
        self.keyboards
            .iter()
            .any(|keyboard| keyboard.owner == SeatOwner::Primary)
    }

    fn register(&mut self, resource: &ZwpVirtualKeyboardV1, owner: SeatOwner) -> bool {
        self.keyboards
    ...
    fn replace_keymap(
    ...
    ) -> Option<(SeatOwner, Option<WireKeymap>, [u64; KEY_WORDS])> {
        let index = self
            .keyboards
            .iter()
        ...
        let owner = self.keyboards[index].owner;
        let releases = if owner == SeatOwner::Primary {
            self.release_counts(pressed)
        } else {
            [0; KEY_WORDS]
        };
        self.keyboards[index].keymap = Some(keymap);
        Some((owner, old_file, releases))
    }
    ...
    fn update_key(
        ...
        key: u32,
        pressed: bool,
    ) -> Result<(SeatOwner, WireKeymap, bool), KeyFailure> {
        ...
        if was_pressed == pressed {
            return Ok((owner, keymap, false));
        }
        if owner != SeatOwner::Primary {
            if pressed {
                self.keyboards[index].pressed[word] |= mask;
            } else {
                self.keyboards[index].pressed[word] &= !mask;
            }
            return Ok((owner, keymap, true));
        }
        if pressed {
            self.keyboards[index].pressed[word] |= mask;
            ...
            *count = count.saturating_add(1);
            Ok((owner, keymap, emit))
        } else {
            self.keyboards[index].pressed[word] &= !mask;
            let count = &mut self.key_counts[key];
            *count = count.saturating_sub(1);
            Ok((owner, keymap, *count == 0))
        }
    }
    ...
    fn update_modifiers(
        ...
        locked: u32,
        group: u32,
    ) -> Result<(SeatOwner, WireKeymap), KeyFailure> {
    ...
    fn remove(
        ...
        let releases = if keyboard.owner == SeatOwner::Primary {
            self.release_counts(keyboard.pressed)
        } else {
            [0; KEY_WORDS]
        };
        Some((keyboard.owner, file, releases))
    }
    ...
    fn request(
        ...
        match request {
            zwp_virtual_keyboard_manager_v1::Request::CreateVirtualKeyboard { seat, id } => {
                let Some(owner) = state.protocol_globals.seat.owner(&seat) else {
                    resource.post_error(
                ...
                if !state
                    .protocol_globals
                    .virtual_keyboard
                    .register(&keyboard, owner)
                {
                    resource.post_error(CAPACITY_ERROR, "virtual-keyboard capacity exceeded");
                    return;
                }
                match owner {
                    SeatOwner::Primary => state.reconcile_virtual_keyboard_capability(),
                    SeatOwner::Transient(id) => {
                        state.protocol_globals.transient_seat.keyboard_created(id)
                    }
                }
            }
            _ => unreachable!(),
...
impl DispatchDelegate<ZwpVirtualKeyboardV1, RuntimeState> for KeyboardData {
    fn request(
        ...
        match request {
            zwp_virtual_keyboard_v1::Request::Keymap { format, fd, size } => {
                ...
                if let Some((owner, file, releases)) = state
                    .protocol_globals
                    .virtual_keyboard
            ...
            zwp_virtual_keyboard_v1::Request::Key {
                ...
                match state.protocol_globals.virtual_keyboard.update_key(
                    ...
                    Ok((SeatOwner::Primary, keymap, true)) => {
                        state.forward_virtual_key(keymap, key, key_state != 0, time)
                    }
                    Ok((SeatOwner::Transient(id), _, _)) => {
                        state.protocol_globals.transient_seat.keyboard_event(id)
                    }
                    Ok((SeatOwner::Primary, _, false)) | Err(KeyFailure::Missing) => {}
            ...
            zwp_virtual_keyboard_v1::Request::Modifiers {
                ...
                Ok((SeatOwner::Primary, keymap)) => state.forward_virtual_modifiers(keymap),
                Ok((SeatOwner::Transient(id), _)) => {
                    state.protocol_globals.transient_seat.keyboard_event(id)
                }
    ...
    fn destroyed(
        ...
        if let Some((owner, file, releases)) =
            state.protocol_globals.virtual_keyboard.remove(resource)
        {
            match owner {
                SeatOwner::Primary => {
                    state.release_virtual_keys(file, releases);
                ...
                SeatOwner::Transient(id) => {
                    state.protocol_globals.transient_seat.keyboard_destroyed(id)
                }
            }
        }
    }
