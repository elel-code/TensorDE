...
mod region;
pub(in crate::protocol) use region::BackgroundRegion;

use super::compositor::get_region_attributes;
    ...
    fn request(
        ...
        match request {
            ext_background_effect_surface_v1::Request::SetBlurRegion { region } => {
                ...
                let region = region
                    .as_ref()
                    .map(get_region_attributes)
                    .map(BackgroundRegion::from_attributes)
                    .filter(|region| !region.is_empty());
                state
                    .protocol_globals
                    .surface_metadata
                    .set_pending_background(&surface, region);
            }
