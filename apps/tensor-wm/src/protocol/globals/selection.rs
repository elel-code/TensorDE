...
pub(in crate::protocol) struct SourceToken(u64);
...
impl SelectionProtocol {
    ...
    pub(in crate::protocol) fn toplevel_drag_source_token(
        &self,
        source: &WlDataSource,
    ) -> Option<SourceToken> {
        let token = core::source_token(source)?;
        self.sources
            .get(&token)
            .filter(|record| record.kind == SourceKind::Core)
            .map(|_| token)
    }

    pub(in crate::protocol) fn source_unused_for_toplevel_drag(&self, token: SourceToken) -> bool {
        self.sources
            .get(&token)
            .is_some_and(|source| source.kind == SourceKind::Core && source.use_.is_none())
    }
