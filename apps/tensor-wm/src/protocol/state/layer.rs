...
impl RuntimeState {
    ...
    pub(crate) fn layer_or_window_pointer_focus(
        ...
        self.layer_or_window_pointer_focus_excluding(location, None)
    }

    #[cfg(feature = "tty")]
    pub(crate) fn dnd_pointer_focus_under(
        &self,
        location: LogicalPoint<f64>,
        excluded_root: Option<&ObjectId>,
    ) -> Option<(WlSurface, LogicalPoint<f64>)> {
        self.layer_or_window_pointer_focus_excluding(location, excluded_root)
    }

    #[cfg(feature = "tty")]
    fn layer_or_window_pointer_focus_excluding(
        &self,
        location: LogicalPoint<f64>,
        excluded_root: Option<&ObjectId>,
    ) -> Option<(WlSurface, LogicalPoint<f64>)> {
        ...
        if let Some(hit) = self.window_pointer_focus(location, excluded_root) {
            return Some(hit);
        }
    ...
    pub(super) fn merge_layer_surfaces(
        ...
        for layer in map.layers() {
            ...
            let (effects, backdrop_region) =
                self.layer_surface_background_effect(layer.wl_surface(), global);
            nodes.push(
                SceneNode::new(
                    view_id,
                    stacking,
    ...
    fn window_pointer_focus(
        ...
        let mut dnd_active = None;
        let hit = self.space.element_under_excluding(
            &self.popups,
            location,
            || *dnd_active.get_or_insert_with(|| self.xwayland_dnd_pointer_grab_active()),
            excluded_root,
