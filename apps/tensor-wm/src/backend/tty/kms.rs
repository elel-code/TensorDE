...
impl TtyBackend {
    ...
    pub(crate) fn submit_output_frame(
        ...
        target
            .submit(slot, timeline_value, sync_fd, mode)
    ...
    pub(crate) fn output_allows_async_page_flip(&self, output: BackendOutputId) -> bool {
        self.session.is_active()
            && self
                ...
                .filter(|device| device.drm.is_active())
                .and_then(|device| device.native_targets.get(&output))
                .is_some_and(KmsOutput::allows_async_page_flip)
    }

    pub(crate) fn output_powered(&self, output: BackendOutputId) -> Option<bool> {
        self.devices
            .get(&output.device_id)?
            .native_targets
            .get(&output)
            .map(KmsOutput::powered)
    }

    pub(crate) fn set_output_powered(
        &mut self,
        output: BackendOutputId,
        powered: bool,
    ) -> Result<bool, BackendError> {
        if !self.session.is_active() {
            return Err(BackendError::SessionInactive);
        }
        let device =
            self.devices
        ...
        target
            .set_powered(powered)
            .map_err(|source| BackendError::KmsPower {
                output: target.name.clone(),
                message: source.to_string(),
            })
    }
...
impl KmsOutput {
    ...
    pub(super) fn ready_for(&self, slot: u8) -> bool {
        self.powered
            && self.scanout.ready_for(slot)
            && self.slots.iter().any(|candidate| candidate.slot == slot)
    }
    ...
    pub(super) fn allows_async_page_flip(&self) -> bool {
        self.powered && self.surface.allows_async_page_flip()
    }
    ...
    pub(super) fn submit(
        ...
        if !self.powered {
            return Err(KmsError::PoweredOff);
        }
        self.scanout.validate_queue(slot, timeline_value)?;
        let framebuffer = self.framebuffer(slot)?;
        if let Err(source) = self.surface.submit(framebuffer, sync_fd.as_fd(), mode) {
    ...
    /// Change DPMS state without removing the connector from output topology.
    pub(super) fn set_powered(&mut self, powered: bool) -> Result<bool, KmsError> {
        if self.powered == powered {
            return Ok(false);
        }
        if !powered {
            self.scanout.ensure_power_off_ready()?;
            self.surface.clear()?;
        ...
        let slot = self.slots.first().ok_or(KmsError::NoFramebuffers)?;
        self.surface.power_on(slot.framebuffer.handle())?;
        self.scanout.resume_from_power_off(slot.slot);
        self.powered = true;
        Ok(true)
    }
...
impl ScanoutState {
    fn ensure_power_off_ready(&self) -> Result<(), KmsError> {
    ...
    fn resume_from_power_off(&mut self, bootstrap_slot: u8) {
        *self = Self {
            quarantined: vec![bootstrap_slot],
            ..Self::default()
        };
    }
...
pub(super) enum KmsError {
    ...
    Faulted,
    #[error("output scanout is powered off")]
    PoweredOff,
...
mod tests {
    ...
    #[test]
    fn power_off_waits_for_the_pending_page_flip() {
        let mut state = ScanoutState::default();
        state.queue(1, 27);

        assert!(matches!(
            state.ensure_power_off_ready(),
            Err(KmsError::Busy(27))
        ));
        state.present().unwrap();
        state.ensure_power_off_ready().unwrap();
    }

    #[test]
    fn power_on_quarantines_the_retained_bootstrap_slot() {
        let mut state = ScanoutState::default();
        state.resume_from_power_off(0);

        assert!(!state.ready_for(0));
        assert!(state.ready_for(1));
        assert!(matches!(
            state.validate_queue(0, 1),
            Err(KmsError::QuarantinedSlot(0))
        ));
