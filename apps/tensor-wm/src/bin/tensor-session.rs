...
#[cfg(feature = "systemd")]
use tensorland::service::import_process_activation_environment;
use tensorland::service::{SystemdMode, configured_mode};

const SERVICE: &str = "tensorland.service";
const SHUTDOWN_TARGET: &str = "tensorland-shutdown.target";
...
fn main() -> ExitCode {
    ...
    #[cfg(feature = "systemd")]
    if let Err(error) = import_process_activation_environment() {
        eprintln!("failed to update D-Bus activation environment: {error}");
    }
...
fn exec_compositor(args: &[OsString]) -> ExitCode {
    let mut command = Command::new("tensor");
    ...
    #[cfg(unix)]
    {
        ...
        eprintln!("failed to exec tensor: {error}");
        ExitCode::FAILURE
    }
