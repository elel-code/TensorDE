...
fn thumbnail_uri_percent_encodes_file_path() {
    let uri = thumbnail_uri_for_path(Path::new("/tmp/Tensor Files Test/value#1.txt")).unwrap();
    assert_eq!(uri, "file:///tmp/Tensor%20Files%20Test/value%231.txt");
...
fn freedesktop_thumbnail_hash_uses_md5_uri() {
    ...
    assert_eq!(
        thumbnail_cache_key("file:///tmp/Tensor%20Files%20Test/value%231.txt"),
        "3be53e9119b809e7a49939a6411b85d1"
    );
}
...
fn cached_thumbnail_for_request_uses_request_mtime_without_restat() {
    ...
    let path = PathBuf::from("/tmp/tensor-files-thumbnail-request-missing.png");
...
fn thumbnailer_exec_expands_common_freedesktop_file_field_codes() {
    ...
    let request = ThumbnailRequest::from_entry_metadata_with_mime(
        ...
        Generation(1),
        ItemId(1),
        PathBuf::from("/tmp/Tensor Files Test/photo one.png"),
        42,
        Some("image/png".to_string()),
        ThumbnailRequestPriority::Visible,
    )
    ...
    assert_eq!(
        ...
        &[
            OsString::from("--file"),
            OsString::from("/tmp/Tensor Files Test/photo one.png"),
            OsString::from("--files"),
            OsString::from("/tmp/Tensor Files Test/photo one.png"),
            OsString::from("--url"),
            OsString::from("file:///tmp/Tensor%20Files%20Test/photo%20one.png"),
            OsString::from("--dir"),
            OsString::from("/tmp/Tensor Files Test"),
            OsString::from("--dirs"),
            OsString::from("/tmp/Tensor Files Test"),
            OsString::from("--name"),
            ...
            OsString::from("%"),
            OsString::from("--quoted=/tmp/Tensor Files Test/photo one.png"),
        ]
    );
...
fn external_thumbnailer_generation_writes_normal_cache_with_metadata() {
    ...
    fs::write(
        thumbnailer_dir.join("tensor-files.thumbnailer"),
        format!(
            "[Thumbnailer Entry]\nExec={} %i %o\nMimeType=image/png;\n",
...
fn temp_root(name: &str) -> PathBuf {
    let root = env::temp_dir().join(format!("tensor-files-thumbnail-{name}-{}", process::id()));
    let _ = fs::remove_dir_all(&root);
    root
}
