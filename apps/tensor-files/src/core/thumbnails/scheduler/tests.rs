...
fn thumbnail_scheduler_queues_once_and_clears_pane_work() {
    ...
    let mut scheduler =
        ThumbnailScheduler::new(PathBuf::from("/tmp/tensor-files-thumbnail-scheduler"));
...
fn thumbnail_scheduler_skips_incomplete_metadata_candidates() {
    let pane_id = PaneId(1);
    let generation = Generation(1);
    let mut scheduler = ThumbnailScheduler::new(PathBuf::from(
        "/tmp/tensor-files-thumbnail-incomplete-metadata",
    ));
...
fn thumbnail_scheduler_skips_plain_text_candidates() {
    let pane_id = PaneId(1);
    let generation = Generation(1);
    let mut scheduler = ThumbnailScheduler::new(PathBuf::from("/tmp/tensor-files-thumbnail-text"));
...
fn thumbnail_scheduler_prunes_deferred_work_outside_current_resolve_set() {
    ...
    let generation = Generation(1);
    let mut scheduler =
        ThumbnailScheduler::new(PathBuf::from("/tmp/tensor-files-thumbnail-active"));
...
fn thumbnail_probe_result_applies_only_to_matching_model_item_and_path() {
    ...
    let generation = Generation(1);
    let mut model =
        DirectoryModel::for_directory(PathBuf::from("/tmp/tensor-files-thumbnail-result"));
    model.replace_listing(
        PathBuf::from("/tmp/tensor-files-thumbnail-result"),
        Arc::new(vec![test_entry("image.png")]),
    );
    let item_id = model.entries()[0].id;
    let thumbnail_path = PathBuf::from("/tmp/tensor-files-thumbnail-cache/normal/image.png");
    ...
    assert!(!apply_thumbnail_probe_result_to_model(
        ...
        ThumbnailProbeResult {
            ...
            generation,
            item_id,
            path: PathBuf::from("/tmp/tensor-files-thumbnail-result/other.png"),
            modified_secs: 42,
    ...
    assert!(!apply_thumbnail_probe_result_to_model(
        ...
        ThumbnailProbeResult {
            ...
            generation,
            item_id: ItemId(999),
            path: PathBuf::from("/tmp/tensor-files-thumbnail-result/image.png"),
            modified_secs: 42,
...
fn thumbnail_probe_failure_marks_model_preview_finished() {
    ...
    let generation = Generation(1);
    let mut model =
        DirectoryModel::for_directory(PathBuf::from("/tmp/tensor-files-thumbnail-failed"));
    model.replace_listing(
        PathBuf::from("/tmp/tensor-files-thumbnail-failed"),
        Arc::new(vec![test_entry("image.png")]),
    );
    let item_id = model.entries()[0].id;
    ...
    assert!(apply_thumbnail_probe_result_to_model(
        ...
        ThumbnailProbeResult {
            ...
            generation,
            item_id,
            path: PathBuf::from("/tmp/tensor-files-thumbnail-failed/image.png"),
            modified_secs: 42,
            thumbnail_path: None,
        },
    ));
...
fn stale_thumbnail_probe_result_does_not_update_model() {
    ...
    let generation = Generation(1);
    let mut model =
        DirectoryModel::for_directory(PathBuf::from("/tmp/tensor-files-thumbnail-stale"));
    model.replace_listing(
        PathBuf::from("/tmp/tensor-files-thumbnail-stale"),
        Arc::new(vec![test_entry("image.png")]),
    );
    let item_id = model.entries()[0].id;
    ...
    assert!(!apply_thumbnail_probe_result_to_model(
        ...
        ThumbnailProbeResult {
            ...
            generation,
            item_id,
            path: PathBuf::from("/tmp/tensor-files-thumbnail-stale/image.png"),
            modified_secs: 41,
    ...
    let requests = (0..8)
        .map(|index| {
            ThumbnailRequest::from_entry_metadata_with_mime(
                ...
                generation,
                ItemId(index),
                PathBuf::from(format!("/tmp/tensor-files-thumbnail-worker-{index}.png")),
                42,
        ...
        thumbnail_probe_results_with_worker(requests, 4, None, move |request| {
            ...
            ThumbnailProbeResult {
                ...
                thumbnail_path: Some(PathBuf::from(format!(
                    "/tmp/tensor-files-thumbnail-worker-result-{}.png",
                    request.item_id().0
                ))),
            }
        })
...
fn thumbnail_probe_worker_skips_cancelled_deferred_before_start() {
    ...
    let requests = vec![
        ThumbnailRequest::from_entry_metadata_with_mime(
            ...
            generation,
            ItemId(1),
            PathBuf::from("/tmp/tensor-files-thumbnail-worker-visible.png"),
            42,
            Some("image/png".to_string()),
            ThumbnailRequestPriority::Visible,
        )
        ...
        ThumbnailRequest::from_entry_metadata_with_mime(
            ...
            generation,
            ItemId(2),
            PathBuf::from("/tmp/tensor-files-thumbnail-worker-deferred.png"),
            42,
            Some("image/png".to_string()),
