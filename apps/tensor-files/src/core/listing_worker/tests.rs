...
fn listing_worker_state_batches_same_path_requests() {
    let mut state = ListingWorkerState::default();
    let first = listing_request_at(1, 1, "/tmp/tensor-files-shared-listing");
    let different = listing_request_at(2, 1, "/tmp/tensor-files-other-listing");
    let second = listing_request_at(3, 1, "/tmp/tensor-files-shared-listing");
    ...
    assert_eq!(
        shared_batch.path,
        PathBuf::from("/tmp/tensor-files-shared-listing")
    );
...
fn retarget_listing_events_preserves_shared_listing_entries() {
    let source = listing_request_at(1, 1, "/tmp/tensor-files-shared-listing");
    let target = listing_request_at(2, 7, "/tmp/tensor-files-shared-listing");
...
fn listing_worker_state_cancels_closed_pane_work() {
    let mut state = ListingWorkerState::default();
    let first = listing_request_at(1, 1, "/tmp/tensor-files-shared-listing");
    let second = listing_request_at(2, 1, "/tmp/tensor-files-shared-listing");
    state.schedule(first.clone());
    state.schedule(second.clone());
...
fn listing_worker_cache_serves_load_with_shared_entries() {
    let mut state = ListingWorkerState::default();
    let first = listing_request_at(1, 1, "/tmp/tensor-files-cached-listing");
    let second = listing_request_at(2, 2, "/tmp/tensor-files-cached-listing");
...
fn listing_worker_cache_serves_promoted_model_snapshot() {
    let mut state = ListingWorkerState::default();
    let request = listing_request_at(7, 1, "/tmp/tensor-files-promoted-listing");
...
fn listing_worker_debug_snapshot_reports_uncached_large_directories() {
    ...
    assert!(state.record_uncached_directory(Path::new("/tmp/tensor-files-large-listing"), 10_001));
    ...
    assert_eq!(
        ...
        Path::new("/tmp/tensor-files-large-listing")
    );
    assert_eq!(
...
fn listing_worker_cache_ignores_reload_and_can_remove_directory() {
    let mut state = ListingWorkerState::default();
    let first = listing_request_at(1, 1, "/tmp/tensor-files-cached-listing");
    let mut reload = listing_request_at(2, 2, "/tmp/tensor-files-cached-listing");
    ...
    assert!(
        state
            .cache
            .get(Path::new("/tmp/tensor-files-cached-listing"))
            .is_none()
    );
    assert!(
        state
            .cached_events_for(&listing_request_at(
                3,
                3,
                "/tmp/tensor-files-cached-listing"
            ))
            .is_none()
    );

    state.remove_cached_directory(Path::new("/tmp/tensor-files-cached-listing"));
    assert!(
        state
            .cache
            .get(Path::new("/tmp/tensor-files-cached-listing"))
...
fn loading_state_tracks_current_request_and_ignores_stale_events() {
    let mut controller =
        super::super::pane::PaneController::new(PathBuf::from("/tmp/tensor-files-loading"));
...
fn loading_state_rejects_stale_started_event_for_old_generation() {
    let mut controller =
        super::super::pane::PaneController::new(PathBuf::from("/tmp/tensor-files-loading-a"));
    let pane_id = controller.focused().unwrap();
    let stale = controller.reload(pane_id).unwrap();
    controller.load(pane_id, PathBuf::from("/tmp/tensor-files-loading-b"));
    let mut loading = HashMap::new();
...
fn listing_request(pane: u64, serial: u64) -> ListingRequest {
    listing_request_at(pane, serial, &format!("/tmp/tensor-files-listing-{pane}"))
}
...
fn temp_root(name: &str) -> PathBuf {
    let root = std::env::temp_dir().join(format!("tensor-files-listing-{name}-{}", process::id()));
    let _ = fs::remove_dir_all(&root);
