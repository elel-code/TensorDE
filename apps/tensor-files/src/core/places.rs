...
const TENSOR_FILES_DATA_DIR_NAME: &str = "tensor-files";
...
fn user_places_path_for_data_home(data_home: PathBuf) -> PathBuf {
    data_home
        .join(TENSOR_FILES_DATA_DIR_NAME)
...
mod tests {
    ...
    #[test]
    fn default_user_places_path_is_tensor_files_scoped() {
        let user_places_path = user_places_path_for_data_home(PathBuf::from("/xdg/data"));
        assert_eq!(
            user_places_path,
            PathBuf::from("/xdg/data/tensor-files/places.xbel")
        );
        assert_eq!(
            place_order_path_for_user_places_path(&user_places_path),
            PathBuf::from("/xdg/data/tensor-files/places-order.xml")
        );
    }
