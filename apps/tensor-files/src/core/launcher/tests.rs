...
fn launcher_test_executable(name: &str) -> (PathBuf, PathBuf) {
    let unique = SystemTime::now()
        ...
        .unwrap_or_default();
    let dir = env::temp_dir().join(format!("tensor-files-launcher-test-{unique}"));
...
fn service_menu_roots_are_dedicated_service_directories() {
    ...
    assert_eq!(
        dirs,
        vec![
            PathBuf::from("/xdg/share/tensor-files/servicemenus"),
...
fn service_actions_deduplicate_labels_and_filter_builtin_window_actions() {
    ...
    normal.actions = vec![
        ...
        DesktopAction {
            ...
            exec: "not-tensor-files %f".to_string(),
            icon: None,
        },
        DesktopAction {
            id: "open-window".to_string(),
            name: "Open New Window".to_string(),
            exec: "not-tensor-files-open-window %f".to_string(),
            icon: None,
        },
    ];
...
fn service_menu_conditions_filter_protocol_url_count_and_executable() {
    ...
    let missing_executable = DesktopServiceMenu {
        ...
        min_url_count: None,
        max_url_count: None,
        show_if_executable: Some("/definitely/missing/tensor-files-tool".to_string()),
        actions: vec![DesktopAction {
