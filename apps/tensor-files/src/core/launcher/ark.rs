...
impl ArkCompressionMode {
    ...
    fn desktop_id(self) -> &'static str {
        match self {
            Self::Dialog => "tensor-files-ark-compress-dialog",
            Self::TarGz => "tensor-files-ark-compress-tar-gz",
            Self::Zip => "tensor-files-ark-compress-zip",
        }
    }
...
pub fn ark_extract_and_trash_launch_plan(
    ...
    ark_extract_launch_plan(
        "tensor-files-ark-extract-and-trash",
...
mod tests {
    ...
    fn ark_compress_fallback_builds_direct_suffix_launch_plans() {
        let paths = vec![
            PathBuf::from("/tmp/tensor-files-ark-compress/note.txt"),
            PathBuf::from("/tmp/tensor-files-ark-compress/todo.txt"),
        ];

        let tar_gz = ark_compress_launch_plan(&paths, ArkCompressionMode::TarGz).unwrap();
        assert_eq!(tar_gz.desktop_id, "tensor-files-ark-compress-tar-gz");
        ...
        assert_eq!(
            ...
            vec![
                ...
                "--autofilename",
                "tar.gz",
                "/tmp/tensor-files-ark-compress/note.txt",
                "/tmp/tensor-files-ark-compress/todo.txt"
            ]
        );

        let zip = ark_compress_launch_plan(&paths, ArkCompressionMode::Zip).unwrap();
        assert_eq!(zip.desktop_id, "tensor-files-ark-compress-zip");
    ...
    fn ark_compress_dialog_fallback_builds_systemd_launch_plan() {
        ...
        assert_eq!(plan.desktop_id, "tensor-files-ark-compress-dialog");
    ...
    fn ark_extract_fallback_builds_systemd_launch_plans() {
        let archives = vec![PathBuf::from("/tmp/tensor-files-ark-extract/archive.zip")];

        let here = ark_extract_here_launch_plan(&archives).unwrap();
        assert_eq!(here.desktop_id, "tensor-files-ark-extract-here");
        ...
        assert_eq!(
            ...
            vec![
                ...
                "--autosubfolder",
                "--destination",
                "/tmp/tensor-files-ark-extract",
                "/tmp/tensor-files-ark-extract/archive.zip"
            ]
        );

        let extract_to = ark_extract_to_launch_plan(&archives).unwrap();
        assert_eq!(extract_to.desktop_id, "tensor-files-ark-extract-to");
    ...
    fn ark_extract_fallback_accepts_multiple_archives() {
        let archives = vec![
            PathBuf::from("/tmp/tensor-files-ark-extract/one.zip"),
            PathBuf::from("/tmp/tensor-files-ark-extract/two.tar.gz"),
        ];
        ...
        assert_eq!(
            ...
            vec![
                ...
                "--destination",
                "/tmp/tensor-files-ark-extract",
                "/tmp/tensor-files-ark-extract/one.zip",
                "/tmp/tensor-files-ark-extract/two.tar.gz"
            ]
        );
    }
    ...
    fn ark_extract_and_trash_uses_batch_extract_plan() {
        ...
        assert_eq!(plan.desktop_id, "tensor-files-ark-extract-and-trash");
