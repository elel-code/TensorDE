...
fn terminal_launch_plan_for_commands(
    ...
    Ok(DesktopLaunchPlan {
        desktop_id: "tensor-files-terminal".to_string(),
        desktop_file: PathBuf::from("tensor-files-terminal"),
    ...
    plan.commands
        ...
        .map(|(index, command)| {
            ...
            Ok(SystemdLaunchUnit {
                ...
                description: format!("Tensor Files Open With {}", plan.app_name),
                command,
            })
        })
        .collect()
...
pub fn systemd_launch_unit_name(desktop_id: &str, index: usize, nonce: u128) -> String {
    ...
    format!("tensor-files-open-with-{component}-{index}-{nonce:x}.service")
}
