...
fn exec_embedded_multi_file_code_expands_single_path() {
    let command = exec_to_launch_commands(
        ...
        &[PathBuf::from("/tmp/tensor-files service target")],
    )
    .unwrap()
    .remove(0);
    ...
    assert_eq!(
        ...
        vec![
            "+new-window",
            "--working-directory=/tmp/tensor-files service target"
        ]
    );
}
...
fn systemd_launch_unit_name_sanitizes_desktop_id() {
    assert_eq!(
        ...
        "tensor-files-open-with-org.example.Viewer.desktop-2-2a.service"
    );
    assert_eq!(
        systemd_launch_unit_name("///", 0, 0x2a),
        "tensor-files-open-with-application-0-2a.service"
    );
}
...
fn systemd_units_for_launch_plan_resolves_executable_path() {
    ...
    assert_eq!(
        units[0].unit_name,
        "tensor-files-open-with-viewer.desktop-0-2a.service"
    );
...
fn current_executable_launch_plan_targets_running_binary() {
    let plan = current_executable_launch_plan(
        "tensor-files-new-window",
        "Tensor Files",
        vec!["/tmp/tensor-files-window".to_string()],
    )
    .unwrap();

    assert_eq!(plan.desktop_id, "tensor-files-new-window");
    assert_eq!(plan.app_name, "Tensor Files");
    assert_eq!(plan.commands.len(), 1);
    assert!(Path::new(&plan.commands[0].program).is_absolute());
    assert_eq!(plan.commands[0].args, vec!["/tmp/tensor-files-window"]);
}
...
fn terminal_launch_plan_selects_first_supported_terminal_command() {
    ...
    let plan = terminal_launch_plan_for_commands(vec![
        DesktopLaunchCommand {
            program: "/definitely/missing/tensor-files-terminal".to_string(),
            args: Vec::new(),
        },
        DesktopLaunchCommand {
            program: executable.display().to_string(),
            args: vec![
                "--workdir".to_string(),
                "/tmp/tensor-files-terminal".to_string(),
            ],
        },
    ])
    .unwrap();

    assert_eq!(plan.desktop_id, "tensor-files-terminal");
...
fn systemd_properties_include_execstart_tuple() {
    ...
    let unit = SystemdLaunchUnit {
        unit_name: "tensor-files-open-with-viewer-0.service".to_string(),
        description: "Tensor Files Open With Viewer".to_string(),
...
fn systemd_units_report_empty_plan_and_missing_program() {
    ...
    let missing = DesktopLaunchPlan {
        ...
        commands: vec![DesktopLaunchCommand {
            program: "/definitely/missing/tensor-files-viewer".to_string(),
            args: Vec::new(),
        }],
    };
    assert_eq!(
