...
mod tests {
    ...
    fn cancellable_listing_returns_none_before_touching_directory() {
        let result =
            read_entries_sync_cancellable(Path::new("/definitely/missing/tensor-files"), || true)
                .unwrap();
    ...
    fn model_entry_defers_thumbnail_path_to_visible_role_update() {
        let dir = std::env::temp_dir().join(format!(
            "tensor-files-entry-thumbnail-deferred-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn ordinary_listing_reads_basic_metadata_without_magic_resolution() {
        let dir = std::env::temp_dir().join(format!(
            "tensor-files-entry-basic-metadata-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn ordinary_listing_treats_symlink_to_directory_as_directory() {
        let dir = std::env::temp_dir().join(format!(
            "tensor-files-entry-symlink-directory-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
    ...
    fn trash_listing_reads_original_path_and_deletion_time_from_trashinfo() {
        ...
        let unique = format!(
            "tensor-files-trash-metadata-{}-{}.txt",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap_or_default()
        ...
        let original_path = PathBuf::from(format!("/tmp/tensor-files original {unique}"));
        fs::write(&trash_path, "trashed").unwrap();
        fs::write(
            &info_path,
            format!(
                "[Trash Info]\nPath=/tmp/tensor-files%20original%20{unique}\nDeletionDate=2026-06-02T10:11:12\n"
            ),
        )
        .unwrap();
