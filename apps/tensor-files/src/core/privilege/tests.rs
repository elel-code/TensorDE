...
fn external_edit_commits_scratch_back_and_cleans_token_dir() {
    ...
    let scratch_root = temp.join("tensor-files-edit");
...
#[test]
fn cleanup_refuses_paths_outside_tensor_files_edit() {
    ...
    assert!(error.contains("outside tensor-files-edit"));
    let _ = fs::remove_dir_all(temp);
}
