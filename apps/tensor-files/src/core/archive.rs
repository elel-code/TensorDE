...
mod tests {
    ...
    fn ark_dnd_extract_request_builds_session_bus_call() {
        ...
        let request =
            ark_dnd_extract_request(&payload, Path::new("/tmp/tensor-files-dnd-target")).unwrap();
        ...
        assert_eq!(request.destination, "/tmp/tensor-files-dnd-target");
    }
    ...
    fn ark_dnd_extract_request_rejects_relative_or_invalid_destination() {
        ...
        let with_nul =
            ark_dnd_extract_request(&payload, Path::new("/tmp/tensor-files\0target")).unwrap_err();
        assert!(matches!(
            with_nul,
            ArkDndExtractError::InvalidDestination { .. }
        ));
