...
/// fill, rounded corners, clipping, and outlines, so Tensor Files does not tessellate
...
/// Native frame layers use analytic instances for Tensor Files's texture-free chrome.
...
fn create_pipeline(
    ...
    let vertex_shader = device
        .create_shader_module(ShaderModuleDescriptor {
            label: Some("tensor-files-vulkan-analytic-rect-vertex".into()),
            spirv: vulkan_rect_spirv::VERTEX.to_vec(),
        })
        .map_err(|error| format!("create Vulkan analytic-rect vertex shader: {error}"))?;
    let fragment_shader = device
        .create_shader_module(ShaderModuleDescriptor {
            label: Some("tensor-files-vulkan-analytic-rect-fragment".into()),
    ...
    device
        .create_graphics_pipeline(&GraphicsPipelineDescriptor {
            label: Some("tensor-files-vulkan-analytic-rect-pipeline"),
            vertex: VertexState {
