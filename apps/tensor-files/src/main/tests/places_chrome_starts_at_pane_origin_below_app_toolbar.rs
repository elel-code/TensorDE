    ...
    fn settings_dialog_routes_actions_and_updates_background_opacity() {
        let mut scene = test_scene(vec![test_entry("alpha.txt", false)], ShellViewMode::Icons);
        let size = ui::settings::settings_dialog_window_size_scaled(scene.ui_scale());
        let hidden_row = ui::settings::settings_dialog_row_rect(
            size,
            scene.ui_scale(),
            0,
        )
        ...
        let track = ui::settings::settings_dialog_opacity_track_rect(size, scene.ui_scale());
        assert_eq!(
            settings_action_at_screen_point(
                ViewPoint {
