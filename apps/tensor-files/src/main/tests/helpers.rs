    ...
    fn test_entry_with_mime_and_modified(
        ...
        Entry::new(tensor_files_core::EntryData {
            name: Arc::from(name),
    ...
    fn test_unchecked_generic_entry(name: &str, size_bytes: u64, modified_secs: u64) -> Entry {
        Entry::new(tensor_files_core::EntryData {
            ...
            metadata_complete: true,
            mime_type: Some(Arc::from(tensor_files_core::GENERIC_BINARY_MIME)),
    ...
    fn test_dir(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("tensor-files-{name}-{unique}"))
    }
    ...
    fn write_test_thumbnail_png_with_color(
        ...
        image::RgbaImage::from_pixel(4, 4, image::Rgba(color))
            .save(path)
            .unwrap();
        tensor_files_core::write_thumbnail_metadata(path, uri, modified_secs).unwrap();
    }
    ...
    fn view_mode_setting_round_trips_and_startup_uses_storage() {
        ...
        let settings = tensor_files_core::AppSettings {
            ...
            view: tensor_files_core::ViewSettings::default(),
            appearance: tensor_files_core::AppearanceSettings::default(),
        };
    ...
    fn test_desktop_application(
        ...
        exec: &str,
        mime_types: &[&str],
    ) -> tensor_files_core::DesktopApplication {
        tensor_files_core::DesktopApplication {
            id: id.to_string(),
