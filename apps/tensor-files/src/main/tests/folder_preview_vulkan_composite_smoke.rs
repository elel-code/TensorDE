...
fn folder_preview_slot_composites_on_native_vulkan() {
    ...
    let (device, queue) = match adapter.request_device(DeviceDescriptor {
        label: Some("tensor-files-folder-preview-smoke".into()),
    ...
    let pipeline_cache = device
        .create_pipeline_cache(&PipelineCacheDescriptor {
            label: Some("tensor-files-folder-preview-smoke-cache".into()),
    ...
    let unique = format!(
        "tensor-files-folder-preview-smoke-{}-{}",
        std::process::id(),
    ...
    let mut uploads = belt
        .begin(
            &queue,
            &CommandEncoderDescriptor {
                label: Some("tensor-files-folder-preview-smoke-2".into()),
            },
        )
        .unwrap();
    renderer
