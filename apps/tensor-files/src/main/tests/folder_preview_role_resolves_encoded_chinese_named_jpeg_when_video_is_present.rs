    ...
    fn folder_preview_role_composes_cached_windows_executable_icon_thumbnail() {
        ...
        let modified_secs = app
            ...
            .unwrap()
            .as_secs();
        let uri = tensor_files_core::thumbnail_uri_for_path(&app).unwrap();
        let thumbnail =
            tensor_files_core::thumbnail_cache_path(&cache_root, tensor_files_core::ThumbnailSize::Normal, &uri);
    ...
    fn folder_preview_role_composes_multiple_child_thumbnail_cache_hits() {
        ...
        for (name, color) in children {
            ...
            let modified_secs = path
                ...
                .unwrap()
                .as_secs();
            let uri = tensor_files_core::thumbnail_uri_for_path(&path).unwrap();
    ...
    fn thumbnail_resolver_uses_freedesktop_cache_hit() {
        ...
        let uri = tensor_files_core::thumbnail_uri_for_path(&source).unwrap();
        let thumbnail =
            tensor_files_core::thumbnail_cache_path(&cache_root, tensor_files_core::ThumbnailSize::Normal, &uri);
