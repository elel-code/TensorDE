struct TensorFilesApp {
    ...
    renderer: Option<TensorFilesRenderer>,
    dialog_windows: ShellDialogWindows,
    text_input: TensorFilesTextInputRuntime,
...
impl ApplicationHandler for TensorFilesApp {
    ...
    fn can_create_surfaces(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let window = match event_loop.create_window(attrs) {
            Ok(window) => window,
            Err(error) => {
                tensor_files_log!("[tensor-files] window create failed: {error}");
        ...
        tensor_files_log!(
            "[tensor-files] clipboard-ready backend={}",
            clipboard.backend()
        );
            ...
            WindowEvent::SurfaceResized(size) => {
                if let Some(renderer) = self.renderer.as_mut() {
                    ...
                    if let Err(error) = renderer.resize(size) {
                        eprintln!("[tensor-files-vulkan] resize failed: {error}");
