...
impl SvgIconRasterizer {
    ...
    pub(super) fn rasterize(
        ...
        let vertices = create_geometry_buffer(
            allocator,
            "tensor-files-vulkan-svg-vertices",
            vertex_bytes,
            BufferUsages::VERTEX,
        )?;
        let indices = create_geometry_buffer(
            allocator,
            "tensor-files-vulkan-svg-indices",
            index_bytes,
            BufferUsages::INDEX,
        )?;
        let image = create_rgba_image(
            allocator,
            "tensor-files-vulkan-svg-icon",
            width,
            height,
        ...
        let view = image
            .create_view(&ImageViewDescriptor {
                label: Some("tensor-files-vulkan-svg-icon-view".into()),
...
fn create_pipeline(device: &Device, cache: &PipelineCache) -> Result<GraphicsPipeline, String> {
    ...
    device
        .create_graphics_pipeline(&GraphicsPipelineDescriptor {
            label: Some("tensor-files-vulkan-svg-icon-pipeline"),
            vertex: VertexState {
...
fn compile_graph(queue_family: u32) -> Result<CompiledGraph, String> {
    ...
    graph.add_pass(RenderPass {
        id: SVG_UPLOAD,
        label: "tensor-files-vulkan-svg-upload".into(),
        depends_on: Vec::new(),
        resources: vec![
            ResourceUse {
    ...
    graph.add_pass(RenderPass {
        id: SVG_RENDER,
        label: "tensor-files-vulkan-svg-render".into(),
    ...
    graph.add_pass(RenderPass {
        id: SVG_SAMPLE,
        label: "tensor-files-vulkan-svg-sample".into(),
