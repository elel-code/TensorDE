...
/// `tensor_files_preview_composite.slang` (std430 push-constant block).
///
...
impl FolderPreviewCompositor {
    ...
    pub(super) fn composite(
        ...
        let descriptor = RenderingDescriptor {
            label: Some("tensor-files-vulkan-folder-preview-render"),
...
fn create_pipeline(device: &Device, cache: &PipelineCache) -> Result<GraphicsPipeline, String> {
    let vertex_shader = device
        .create_shader_module(ShaderModuleDescriptor {
            label: Some("tensor-files-vulkan-preview-composite-vertex".into()),
    ...
    let fragment_shader = device
        .create_shader_module(ShaderModuleDescriptor {
            label: Some("tensor-files-vulkan-preview-composite-fragment".into()),
    ...
    device
        .create_graphics_pipeline(&GraphicsPipelineDescriptor {
            label: Some("tensor-files-vulkan-preview-composite-pipeline"),
            vertex: VertexState {
...
fn compile_graph(queue_family: u32) -> Result<CompiledGraph, String> {
    ...
    graph.add_pass(RenderPass {
        id: PREVIEW_RENDER,
        label: "tensor-files-vulkan-preview-render".into(),
        depends_on: Vec::new(),
    ...
    graph.add_pass(RenderPass {
        id: PREVIEW_SAMPLE,
        label: "tensor-files-vulkan-preview-sample".into(),
