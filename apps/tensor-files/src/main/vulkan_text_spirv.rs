...
pub(crate) const VERTEX: &[u32] =
    vulkan_renderer::include_spirv!("shaders/tensor_files_text.vert.spv");
pub(crate) const FRAGMENT: &[u32] =
    vulkan_renderer::include_spirv!("shaders/tensor_files_text.frag.spv");
    ...
    fn embedded_text_shaders_are_valid_spirv() {
        for (label, spirv) in [("vertex", super::VERTEX), ("fragment", super::FRAGMENT)] {
            ShaderModuleDescriptor {
                label: Some(format!("tensor-files-text-{label}")),
                spirv: spirv.to_vec(),
            }
            .validate()
            .unwrap();
