...
use ui::folder_preview::{FOLDER_PREVIEW_LAYOUT_VERSION, folder_preview_directory_seed};
#[cfg(test)]
...
use ui::open_with::geometry::{
    ...
    open_with_chooser_pointer_role_at_point, open_with_chooser_query_text_rect_scaled,
    open_with_chooser_rect_scaled, open_with_chooser_scrollbar_rects_scaled,
    open_with_chooser_visible_row_count, open_with_chooser_window_size_scaled,
    open_with_scroll_delta_rows,
};
#[cfg(test)]
use ui::open_with::geometry::{
...
use ui::selection::{NavigationAction, RubberBand, RubberBandMode, SelectionClick, ShellSelection};
use ui::service_menu::ServiceMenuLaunchRequest;
use ui::settings::{
    ShellSettingsAction, ShellSettingsDialogState, ShellSettingsSnapshot,
    background_opacity_percent, opacity_percent_at_settings_point, settings_action_at_screen_point,
    settings_dialog_row_at_screen_point, settings_dialog_window_size_scaled,
};
use ui::shortcuts::{
...
fn startup_background_opacity(settings: &AppSettings) -> f32 {
    background_opacity_percent(settings.appearance.background_opacity.unwrap_or(1.0)) as f32 / 100.0
}
fn load_startup_app_settings(settings_path: &Path) -> AppSettings {
    match load_app_settings(settings_path) {
        ...
        Err(error) => {
            tensor_files_log!(
                "[tensor-files] settings-load-error path={} error={error}",
...
fn dialog_lifecycle_autosmoke_cycles_from_env() -> usize {
    env::var_os("TENSOR_FILES_AUTOSMOKE_DIALOG_CYCLES")
...
fn dialog_lifecycle_autosmoke_kind_from_env() -> ShellDialogWindowKind {
    let Some(value) = env::var_os("TENSOR_FILES_AUTOSMOKE_DIALOG_KIND") else {
