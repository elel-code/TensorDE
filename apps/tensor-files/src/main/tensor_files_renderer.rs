...
pub(crate) struct TensorFilesRenderer {
    gpu: VulkanState,
...
impl TensorFilesRenderer {
        ...
        let exportable = match self.gpu.exportable_dmabuf_formats() {
            ...
            Err(error) => {
                if crate::tensor_files_log_enabled() {
                    eprintln!(
                        "[tensor-files] outgoing-dnd-preview capability-query-failed error={error}"
                    );
                }
    ...
    pub(crate) fn log_dmabuf_readiness(
        ...
        if crate::tensor_files_log_enabled() {
            eprintln!(
                "[tensor-files-vulkan] dmabuf-status reason={reason} vulkan={} wayland={} plan={:?}",
    ...
    fn render_inner(
        ...
        if resolve_visible_exact {
            ...
            if crate::tensor_files_log_enabled() && (stats.visible > 0 || stats.applied > 0) {
                eprintln!(
                    "[tensor-files-vulkan] visible-metadata-sync reason={} visible={} resolved={} deferred={} applied={} resolve_us={} over_budget={}",
                    icon_work_reason,
                    stats.visible,
                    stats.resolved,
                    stats.deferred,
    ...
    fn render_detached_dialog(
        ...
        match result {
            ...
            Err(error) => {
                eprintln!(
                    "[tensor-files-vulkan] {} dialog frame failed reason={}: {error}",
...
fn log_frame_stats(icons: &crate::IconFrameStats, text: &crate::TextFrameStats) {
    eprintln!(
        "[tensor-files-vulkan] icons={} quads={} fallback={} deferred={} thumbnails={}/{}/{} read_ahead={} ready={}/{}B previews={}/{}/{} read_ahead={} ready={}/{}B atlas={}/{} {}x{} {}B cache={}/{} {}/{}B hashes={:x}/{:x}/{:x}/{:x} resolve={}us text={}/{} deferred={} reused={} atlas={}/{} {}x{} {}B cache={}/{} {}/{}B swash={}/{}/{} raster={}us",
        icons.icons,
        icons.quads,
        icons.fallbacks,
        icons.deferred,
