impl TensorFilesApp {
    ...
    fn drive_directory_watchers(&mut self, _event_loop: &ActiveEventLoop) {
        ...
        for path in reload_paths {
            match self.scene.reload_panes_showing_path(&path, size) {
                ...
                Err(error) => {
                    tensor_files_log!(
                        "[tensor-files] directory-watch-reload-error path={} error={error}",
                        path.display()
                    );
                }
            }
    ...
    fn drive_dialog_lifecycle_autosmoke(&mut self, event_loop: &ActiveEventLoop) {
        ...
        match smoke_state.step {
            DialogLifecycleSmokeStep::WaitMainFrame => {
                ...
                tensor_files_log!(
                    "[tensor-files] dialog-smoke open kind={} main_frame={}",
                    kind.as_str(),
                    frame_count
                );
            ...
            DialogLifecycleSmokeStep::WaitDialogFrame => {
                ...
                let close_frame = self
                    ...
                    .unwrap_or(0);
                tensor_files_log!(
                    "[tensor-files] dialog-smoke close kind={} dialog_frame={} main_frame={}",
                    kind.as_str(),
                    frame_count,
                    close_frame
                );
    ...
    fn finish_dialog_lifecycle_autosmoke(
        ...
        tensor_files_log!(
            "[tensor-files] dialog-smoke {} main_open={} dialogs_open={}",
    ...
    fn drive_autosmoke_scroll(&mut self, size: PhysicalSize<u32>) {
        ...
        while let Some(action) = self.autosmoke_scroll_actions.pop_front() {
            ...
            tensor_files_log!(
                "[tensor-files] autosmoke-scroll action={} delta={:.1} changed={} old_scroll_x={:.1} new_scroll_x={:.1} old_scroll_y={:.1} new_scroll_y={:.1}",
                action.label,
                action.delta,
                changed as u8,
                old_x,
    ...
    ///
    /// Enable with `TENSOR_FILES_AUTOSMOKE_EXIT=1` for headless per-frame benches
    /// (e.g. `tensor-files --view icons /bin`).
    fn maybe_autosmoke_exit(&mut self, event_loop: &ActiveEventLoop) {
        use std::sync::OnceLock;
        static ENABLED: OnceLock<bool> = OnceLock::new();
        let enabled = *ENABLED.get_or_init(|| {
            std::env::var_os("TENSOR_FILES_AUTOSMOKE_EXIT").is_some_and(|value| {
        ...
        tensor_files_log!(
            "[tensor-files] autosmoke-exit frames={} icon_gpu_bytes={} thumb_ready_bytes={}",
