impl TensorFilesApp {
    fn new(
        ...
        Self {
            ...
            text_input: TensorFilesTextInputRuntime::default(),
    ...
    fn ensure_main_renderer(&mut self, event_loop: &ActiveEventLoop) -> bool {
        ...
        let mut renderer = match TensorFilesRenderer::new(window.clone()) {
            Ok(renderer) => renderer,
            Err(error) => {
                tensor_files_log!("[tensor-files] renderer init failed: {error}");
        ...
        tensor_files_log!(
            "[tensor-files] shell-ready size={}x{} scale={:.2}",
            renderer.size.width,
    ...
    fn trace_window_event(&self, window_id: WindowId, event: &WindowEvent) {
        if !tensor_files_dialog_trace_enabled() {
            return;
        }
        if window_event_trace_is_high_volume(event) && !tensor_files_dialog_trace_verbose_enabled() {
            return;
        }
        ...
        tensor_files_dialog_trace!(
            "[tensor-files] window-event event={} window={:?} role={} main={:?} dialog={} dialogs_open={}",
    ...
    fn exit_event_loop(&self, event_loop: &ActiveEventLoop, reason: &'static str) {
        tensor_files_log!(
            "[tensor-files] event-loop-exit reason={} main_open={} dialogs_open={}",
            reason,
    ...
    fn ensure_dialog_window(
        ...
        let dialog = match ShellDetachedDialogWindow::create(
            ...
            Ok(dialog) => dialog,
            Err(error) => {
                tensor_files_log!("[tensor-files] {error}");
                return false;
            }
        };
    ...
    fn close_dialog_window(&mut self, kind: ShellDialogWindowKind) -> bool {
        ...
        tensor_files_dialog_trace!(
            "[tensor-files] dialog-close-dispatch kind={} closed={}",
            kind.as_str(),
            closed as u8
        );
        closed
    ...
    fn close_dialog_state_and_window(&mut self, kind: ShellDialogWindowKind) -> bool {
        ...
        tensor_files_dialog_trace!(
            "[tensor-files] dialog-state-close kind={} changed={} closed={}",
            kind.as_str(),
            changed as u8,
            closed as u8
        );
    ...
    fn close_main_window_from_window_manager_request(
        ...
        tensor_files_dialog_trace!(
            "[tensor-files] main-close accept=1 reason={} dialogs_open={}",
            reason,
            self.dialog_windows.has_open_window() as u8,
        );
    ...
    fn handle_common_dialog_window_event(
        ...
        tensor_files_dialog_trace!(
            "[tensor-files] dialog-host-event kind={} event={:?}",
            kind.as_str(),
            event
        );
        match event {
