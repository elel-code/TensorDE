...
impl TensorFilesApp {
        ...
        match work {
            ShellClipboardWork::StoreFile { request, reply_rx } => {
                if let Err(error) = self.spawn_blocking_task_result(
                    ...
                    tensor_files_log!("[tensor-files] clipboard-reply-runtime-error error={error}");
                }
            }
    ...
    fn start_async_create(&mut self, request: CreateEntryRequest) {
        ...
        if let Err(error) = self.spawn_async_task_result(
            move || async move {
                crate::ui::create_rename::disk::create_entry_on_disk_explicit_async(
                    request_for_task,
                )
                .await
            },
            ...
            tensor_files_log!("[tensor-files] create-runtime-error {error}");
    ...
    fn start_async_rename(&mut self, request: RenameEntryRequest) {
        ...
        if let Err(error) = self.spawn_async_task_result(
            move || async move {
                crate::ui::create_rename::disk::rename_entry_on_disk_explicit_async(
                    request_for_task,
                )
                .await
            },
            ...
            tensor_files_log!("[tensor-files] rename-runtime-error {error}");
    ...
    fn start_async_device(&mut self, request: DeviceActionRequest) {
        ...
        if let Err(error) = self.spawn_async_task_result(
            ...
            tensor_files_log!("[tensor-files] device-action-runtime-error action={action_name} error={error}");
            self.scene.record_task_status(ShellTaskStatus::failed(
            ...
            match work {
                ShellLaunchWork::Systemd {
                    ...
                    ShellAsyncLaunchKind::OpenFile => {
                        match launch_with_systemd_user(plan).await {
                            Ok(result) => {
                                ...
                                tensor_files_log!(
                                    "[tensor-files] open-finished path={} app={:?} units={}",
                                    path.display(),
                                    app_name,
                                    result.units.join(",")
                                );
                            ...
                            Err(error) => {
                                ...
                                tensor_files_log!(
                                    "[tensor-files] open-finished path={} app={:?} error={error}",
                                    path.display(),
                                    app_name
                                );
                                (false, message)
                    ...
                    ShellAsyncLaunchKind::OpenWith => {
                        ...
                        .status_message();
                        tensor_files_log!("[tensor-files] open-with-finished {status}");
                        (success, status)
                    }
                    ShellAsyncLaunchKind::ServiceMenu => {
                        ...
                        .status_message();
                        tensor_files_log!("[tensor-files] service-menu-finished {status}");
                        (success, status)
                    }
    ...
    fn start_async_move_to_trash_with_options(
        ...
        if let Err(error) = self.spawn_async_task_result(
            ...
            tensor_files_log!("[tensor-files] move-to-trash-runtime-error {error}");
    ...
    fn start_async_trash_view_operation_with(
        ...
        if let Err(error) = self.spawn_async_task_result(
            ...
            tensor_files_log!(
                "[tensor-files] trash-view-runtime-error action={} {error}",
                action.as_str()
            );
