impl ShellScene {
    ...
    fn set_open_with_chooser_error(&mut self, error: String) -> bool {
        let Some(chooser) = self.open_with_chooser.as_mut() else {
            tensor_files_log!("[tensor-files] open-with-error {error}");
            return false;
        };
    ...
    fn close_open_with_chooser(&mut self) -> bool {
        ...
        self.open_with_changes += 1;
        tensor_files_log!(
            "[tensor-files] open-with open=0 changes={}",
    ...
    fn close_open_with_chooser_after_success(&mut self, request: &OpenWithLaunchRequest) -> bool {
        ...
        self.open_with_changes += 1;
        tensor_files_log!(
            "[tensor-files] open-with path={} app={:?} changes={}",
    ...
    fn log_open_with_chooser_state(&self) {
        match self.open_with_chooser.as_ref() {
            Some(chooser) => tensor_files_log!(
                "[tensor-files] open-with open=1 path={} mime={} apps={} filtered={} category={} selected={} scroll={} set_default={} query={:?} cursor={} error={:?} changes={}",
    ...
    fn apply_device_place_operation_result(
        ...
        match &result.result {
            Ok(mount_point) => {
                ...
                tensor_files_log!(
                    "[tensor-files] device-action-finished action={} id={:?} label={:?} mount={} changes={}",
            ...
            Err(error) => {
                tensor_files_log!(
                    "[tensor-files] device-action-finished action={} id={:?} label={:?} error={error} changes={}",
                    request.action.as_str(),
                    result.device_id,
                    result.label,
                    self.places_changes
    ...
    fn record_copy_location(&mut self, request: &CopyLocationRequest) {
        ...
        tensor_files_log!(
            "[tensor-files] copy-location path={} text={:?} changes={}",
            request.path.display(),
            request.text,
    ...
    fn record_task_status(&mut self, status: ShellTaskStatus) {
        ...
        if let Some(status) = self.task_statuses.front() {
            tensor_files_log!(
                "[tensor-files] task-status kind={:?} label={:?} privileged={} detail={:?} changes={}",
                status.kind,
                status.label,
    ...
    fn apply_transfer_result(
        ...
        tensor_files_log!(
            "[tensor-files] transfer source={:?} mode={} target={} success={} failure={} cancelled={} privileged={} changes={}",
            source,
            result.mode.label(),
            target_dir.display(),
    ...
    fn apply_move_to_trash_result(
        ...
        tensor_files_log!(
            "[tensor-files] trash paths={} success={} failure={} privileged={} changes={}",
            paths.len(),
            result.success_count,
            result.failure_count,
            result.privileged as u8,
