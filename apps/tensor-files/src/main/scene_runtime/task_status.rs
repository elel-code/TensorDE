impl ShellScene {
    ...
    fn add_context_target_to_places(
        ...
        if self.places.iter().any(|place| place.path == path) {
            tensor_files_log!(
                "[tensor-files] add-place label={:?} path={} added=0 duplicate=1 changes={}",
                label,
                path.display(),
                self.places_changes
            );
        ...
        tensor_files_log!(
            "[tensor-files] add-place label={:?} path={} added=1 places={} changes={}",
            label,
            path.display(),
            self.places.len(),
            self.places_changes
    ...
    fn add_network_folder_place(
        ...
        if self.places.iter().any(|place| place.path == path) {
            tensor_files_log!(
                "[tensor-files] add-network-folder label={:?} path={} added=0 duplicate=1 changes={}",
                label,
                path.display(),
                self.places_changes
            );
        ...
        self.refresh_hover(size);
        tensor_files_log!(
            "[tensor-files] add-network-folder label={:?} path={} added=1 places={} changes={}",
            label,
            path.display(),
            self.places.len(),
            self.places_changes
    ...
    fn remove_context_place(
        ...
        if !remove_user_place_at_path(user_places_path, &path)? {
            tensor_files_log!(
                "[tensor-files] remove-place label={:?} path={} removed=0 changes={}",
                label,
                path.display(),
                self.places_changes
            );
        ...
        self.refresh_hover(size);
        tensor_files_log!(
            "[tensor-files] remove-place label={:?} path={} removed=1 places={} changes={}",
            label,
            path.display(),
            self.places.len(),
            self.places_changes
    ...
    fn record_file_clipboard_export(&mut self, request: &FileClipboardExportRequest) {
        ...
        tensor_files_log!(
            "[tensor-files] clipboard-export role={} paths={} bytes={} changes={}",
    ...
    // Production paste path is async via TensorFilesApp::finish_paste_from_clipboard_text.
    #[cfg(test)]
