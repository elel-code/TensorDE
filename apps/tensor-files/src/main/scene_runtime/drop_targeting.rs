impl ShellScene {
        ...
        match &drag.source {
            ShellInternalDragSource::PaneItem { pane, index, .. } => {
                ...
                let preview_item_limit =
                    crate::ui::drag_preview_layout::multi_drag_preview_layout(
                        drag.paths.len(),
                        self.ui_scale(),
                    )
                    .item_count;
                ...
                let layout = crate::ui::drag_preview::drag_preview_layout_for_source(
                    self,
                    &drag.source,
                    &drag.label,
                    drag.start,
            ...
            ShellInternalDragSource::Place { index } => {
                ...
                Some(ShellInternalDragPreviewSource::Place {
                    ...
                    layout: crate::ui::drag_preview::drag_preview_layout_for_source(
                        self,
                        &drag.source,
                        &drag.label,
                        drag.start,
    ...
    fn clear_internal_drag(&mut self) -> bool {
        ...
        if changed {
            tensor_files_log!("[tensor-files] internal-dnd clear=1");
        }
        changed
    }
    ...
    fn clear_external_drag(&mut self) -> bool {
        ...
        if changed {
            tensor_files_log!("[tensor-files] external-dnd clear=1");
        }
        changed
    }
    ...
    fn finish_external_drag(
        ...
        if changed {
            tensor_files_log!(
                "[tensor-files] external-dnd-menu open=1 sources={} target={}",
                self.drop_menu
                    .as_ref()
    ...
    fn finish_internal_drag(&mut self, point: ViewPoint, size: PhysicalSize<u32>) -> bool {
        ...
        match self.finish_internal_drag_with_user_places_path(point, size, &user_places_path) {
            Ok(changed) => changed,
            Err(error) => {
                tensor_files_log!("[tensor-files] dnd-error {error}");
                false
            }
        }
    }
    ...
    fn finish_internal_drag_with_user_places_path(
        ...
        if changed {
            tensor_files_log!(
                "[tensor-files] dnd-menu open=1 sources={} target={}",
                self.drop_menu
                    .as_ref()
                    .map(|menu| menu.sources.len())
                    .unwrap_or(0),
    ...
    fn move_place_to_gap(
        ...
        tensor_files_log!(
            "[tensor-files] places-reorder label={:?} path={} from={} gap={} to={} changes={}",
            label,
            path.display(),
            source_index,
            gap_index,
