impl ShellScene {
    ...
    fn open_rename_dialog_from_active_selection(&mut self, privileged: bool) -> bool {
        let pane = self.active_pane();
        let Some(selection) = self.pane_selection(pane) else {
            tensor_files_log!("[tensor-files] rename-error target=none");
            return false;
        };
    ...
    // Production delete path is async via TensorFilesApp::delete_active_selection.
    ...
    // Production trash-view path is async via TensorFilesApp::perform_trash_view_context_action.
    #[cfg(test)]
    ...
    // Production conflict replace path is async via TensorFilesApp::replace_trash_restore_conflicts.
    #[cfg(test)]
    ...
    fn apply_trash_view_result_with_task(
        ...
        tensor_files_log!(
            "[tensor-files] trash-view action={} success={} failure={} conflicts={} changes={}",
            action,
        ...
        for conflict in &result.restore_conflicts {
            tensor_files_log!(
                "[tensor-files] trash-restore-conflict original={} trash={}",
        ...
        if let Some(dialog) = ShellTrashConflictDialog::new(result.restore_conflicts.clone()) {
            ...
            self.rubber_band = None;
            tensor_files_log!(
                "[tensor-files] trash-conflict open=1 conflicts={} changes={}",
                result.restore_conflicts.len(),
                self.trash_changes
            );
        }
    ...
    // Production path is async via TensorFilesApp::move_context_target_to_trash.
    #[cfg(test)]
    ...
    fn close_trash_conflict_dialog(&mut self) -> bool {
        ...
        tensor_files_log!(
            "[tensor-files] trash-conflict open=0 changes={}",
            self.trash_changes
        );
        true
    }
