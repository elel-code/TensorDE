impl ShellScene {
    ...
    fn close_context_menu(&mut self) -> bool {
        ...
        tensor_files_log!("[tensor-files] context-menu open=0");
        true
    }
    ...
    fn activate_or_close_context_menu_command(
        ...
        if let Some(action) = action {
            ...
            tensor_files_log!(
                "[tensor-files] context-menu action={} target={} actions={}",
                action.as_str(),
                self.context_target
                    .as_ref()
    ...
    fn log_context_target(&self) {
        match self.context_target.as_ref() {
            Some(ShellContextTarget::Item {
                ...
                "[tensor-files] context-target kind=item pane={} index={} dir={} selection={} path={} changes={}",
                pane.as_str(),
                index,
                *is_dir as u8,
                selection_count,
            ...
            Some(ShellContextTarget::Blank { pane, path, .. }) => tensor_files_log!(
                "[tensor-files] context-target kind=blank pane={} path={} changes={}",
                pane.as_str(),
                path.display(),
                self.context_target_changes
            ),
            Some(ShellContextTarget::Place {
                ...
                "[tensor-files] context-target kind=place index={} label={:?} device={} mounted={} ejectable={} poweroff={} network={} trash={} root={} editable={} path={} changes={}",
                index,
                label,
            ...
            None => tensor_files_log!(
                "[tensor-files] context-target kind=none changes={}",
                self.context_target_changes
            ),
        }
    }
    ...
    fn record_open_file_request(&mut self, request: &OpenFileRequest) {
        self.open_changes += 1;
        tensor_files_log!(
            "[tensor-files] open path={} uri={} changes={}",
