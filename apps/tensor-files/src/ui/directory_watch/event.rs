...
mod tests {
    ...
    fn directory_watch_event_path_matching_stays_inside_directory() {
        let event = NotifyEvent {
            ...
            paths: vec![PathBuf::from("/tmp/tensor-files-watch-root/child.txt")],
            attrs: Default::default(),
        };

        assert!(shell_directory_watch_event_touches_path(
            &event,
            Path::new("/tmp/tensor-files-watch-root")
        ));
        assert!(!shell_directory_watch_event_touches_path(
            &event,
            Path::new("/tmp/tensor-files-watch")
        ));
    }
    ...
    fn test_dir(name: &str) -> PathBuf {
        let unique = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        std::env::temp_dir().join(format!("tensor-files-watch-{name}-{unique}"))
    }
}
