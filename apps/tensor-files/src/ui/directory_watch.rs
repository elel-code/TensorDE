...
use tensor_files_core::is_network_path;
        ...
        for pane in ShellPaneId::ALL {
            ...
            match self.start_watching(&path) {
                Ok(watcher) => {
                    ...
                    tensor_files_log!(
                        "[tensor-files] directory-watch-start pane={} path={}",
                        pane.as_str(),
                        path.display()
                    );
                }
                Err(error) => {
                    ...
                    tensor_files_log!(
                        "[tensor-files] directory-watch-start-error pane={} path={} error={error}",
                        pane.as_str(),
                        path.display()
                    );
                }
    ...
    pub(crate) fn drain_events(&mut self, scene: &ShellScene) {
        ...
        while let Ok(event) = self.rx.try_recv() {
            match event {
                ...
                Ok(_) => {}
                Err(error) => {
                    tensor_files_log!("[tensor-files] directory-watch-error {error}");
