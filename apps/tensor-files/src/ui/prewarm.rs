...
pub(crate) fn icon_sync_resolve_budget(role_updates_paused: bool) -> usize {
    ...
    if let Some(budget) = env_usize("TENSOR_FILES_ICON_SYNC_RESOLVE_BUDGET")
        .or_else(|| env_usize("TENSOR_FILES_ICON_RASTER_MISS_BUDGET"))
    {
        return budget;
    }
...
pub(crate) fn default_text_raster_miss_budget() -> usize {
    env_usize("TENSOR_FILES_TEXT_RASTER_MISS_BUDGET").unwrap_or(TEXT_RASTER_MISS_BUDGET_PER_FRAME)
}
