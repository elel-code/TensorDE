...
fn autosmoke_zoom_config_from_env(
    ...
    let enabled = env_flag_enabled(get_env, "TENSOR_FILES_AUTOSMOKE_ZOOM");
    if !enabled {
    ...
    let rapid = env_flag_enabled(get_env, "TENSOR_FILES_AUTOSMOKE_ZOOM_RAPID");
    let interval = env_duration_millis(get_env, "TENSOR_FILES_AUTOSMOKE_ZOOM_INTERVAL_MS")
...
fn autosmoke_scroll_config_from_env(
    ...
    let enabled = env_flag_enabled(get_env, "TENSOR_FILES_AUTOSMOKE_SCROLL");
    if !enabled {
    ...
    let rapid = env_flag_enabled(get_env, "TENSOR_FILES_AUTOSMOKE_SCROLL_RAPID");
    let interval = env_duration_millis(get_env, "TENSOR_FILES_AUTOSMOKE_SCROLL_INTERVAL_MS")
        .unwrap_or_else(|| Duration::from_millis(if rapid { 16 } else { 120 }));
    let step = env_f32(get_env, "TENSOR_FILES_AUTOSMOKE_SCROLL_STEP").unwrap_or(default_step);
    let mut actions = VecDeque::new();
    let forward_count = env_usize(get_env, "TENSOR_FILES_AUTOSMOKE_SCROLL_FORWARD_COUNT")
        .unwrap_or(if rapid { 28 } else { 10 });
    let back_count = env_usize(get_env, "TENSOR_FILES_AUTOSMOKE_SCROLL_BACK_COUNT")
...
mod tests {
    ...
    fn autosmoke_zoom_config_uses_rapid_sequence_and_interval() {
        let env = map_env(&[
            ...
            ("TENSOR_FILES_AUTOSMOKE_ZOOM_RAPID", "true"),
        ]);
    ...
    fn autosmoke_scroll_config_uses_counts_and_step() {
        let env = map_env(&[
            ...
            ("TENSOR_FILES_AUTOSMOKE_SCROLL_FORWARD_COUNT", "2"),
            ("TENSOR_FILES_AUTOSMOKE_SCROLL_BACK_COUNT", "1"),
            ("TENSOR_FILES_AUTOSMOKE_SCROLL_INTERVAL_MS", "42"),
        ]);
