...
pub(crate) use tensor_files_core::ViewMode as ShellViewMode;
...
pub(crate) fn parse_start_options() -> Result<Option<StartupOptions>, String> {
    ...
    let program = args
        ...
        .unwrap_or_else(|| "tensor-files".to_string());
...
mod tests {
    ...
    fn default_view_mode_is_not_explicit() {
        let options =
            parse_start_options_from("tensor-files".to_string(), [OsString::from("/tmp")])
                .unwrap()
                .unwrap();
    ...
    fn rejects_duplicate_path_and_unknown_option() {
        ...
        assert_eq!(
            parse_start_options_from("tensor-files".to_string(), [OsString::from("--bad")])
                .unwrap_err(),
            "unknown option: --bad"
        );
    }
