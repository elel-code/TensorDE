...
impl ShellDetachedDialogWindow {
    ...
    pub(crate) fn sync(&mut self, spec: &ShellDialogWindowSpec) {
        ...
        {
            eprintln!(
                "[tensor-files-vulkan] {} dialog resize failed: {error}",
                self.kind.as_str()
            );
        }
        self.request_redraw();
    ...
    fn prepare_for_drop(&mut self) {
        tensor_files_dialog_trace!(
            "[tensor-files] dialog-window-renderer-idle kind={} window={:?}",
            self.kind.as_str(),
            self.window_id()
        );
        if let Err(error) = self.renderer.wait_idle("dialog-window-drop") {
            eprintln!(
                "[tensor-files-vulkan] {} dialog idle failed: {error}",
                self.kind.as_str()
            );
        }
    }

    pub(crate) fn renderer_and_window_mut(&mut self) -> (&mut TensorFilesRenderer, &Window) {
...
impl ShellDialogWindows {
    ...
    pub(crate) fn set(&mut self, kind: ShellDialogWindowKind, window: ShellDetachedDialogWindow) {
        ...
        tensor_files_dialog_trace!(
            "[tensor-files] dialog-window-set kind={} window={:?}",
            kind.as_str(),
            window.window_id()
        );
        match kind {
    ...
    pub(crate) fn close(&mut self, kind: ShellDialogWindowKind) -> bool {
        ...
        if let Some(window) = closed {
            ...
            tensor_files_dialog_trace!(
                "[tensor-files] dialog-window-close-deferred kind={} window={:?}",
                kind.as_str(),
                window_id
            );
            ...
            tensor_files_dialog_trace!(
                "[tensor-files] dialog-window-close kind={} window=<none>",
                kind.as_str()
            );
            false
        }
    ...
    pub(crate) fn drain_ready_deferred_closes(&mut self) -> bool {
        ...
        while let Some(mut close) = self.deferred_closes.pop_front() {
            ...
            tensor_files_dialog_trace!(
                "[tensor-files] dialog-window-drop-deferred kind={} window={:?}",
                close.kind.as_str(),
                close.window_id
            );
    ...
    pub(crate) fn close_all(&mut self) {
        ...
        {
            tensor_files_dialog_trace!(
                "[tensor-files] dialog-window-close-all kind={} window={:?}",
                window.kind().as_str(),
                window.window_id()
            );
        ...
        while let Some(mut close) = self.deferred_closes.pop_front() {
            tensor_files_dialog_trace!(
                "[tensor-files] dialog-window-close-all-deferred kind={} window={:?}",
                close.kind.as_str(),
                close.window_id
            );
            close.window.prepare_for_drop();
