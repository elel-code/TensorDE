...
use tensor_files_core::{
    DesktopLaunchPlan, MimeApplicationCache, MimeDatabase, path_uri_from_path,
};
...
mod tests {
    ...
    use tensor_files_core::{DesktopApplication, MimeAppsList};
    ...
    fn launch_uri_for_path_percent_encodes_local_file_without_gio() {
        assert_eq!(
            launch_uri_for_path(Path::new("/tmp/Tensor Files Test/value#1.txt")),
            "file:///tmp/Tensor%20Files%20Test/value%231.txt"
        );
    }
    ...
    fn default_open_file_launch_request_falls_back_to_path_mime() {
        let cache = MimeApplicationCache::from_applications_and_mimeapps(
            vec![desktop_application(
                ...
                "Viewer",
                "viewer %f",
                &[tensor_files_core::GENERIC_BINARY_MIME],
            )],
            &[MimeAppsList {
                default_apps: HashMap::from([(
                    tensor_files_core::GENERIC_BINARY_MIME.to_string(),
        ...
        let request =
            OpenFileRequest::from_path(PathBuf::from("/tmp/payload.unknown-tensor-files"), None);
        ...
        assert_eq!(
            ...
            vec!["/tmp/payload.unknown-tensor-files"]
        );
    }
}
