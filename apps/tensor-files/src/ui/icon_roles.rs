...
fn file_icon_kind_with_stamp(
    ...
    if !mime_magic_checked && mime_type.as_deref() == Some(tensor_files_core::GENERIC_BINARY_MIME) {
        return FileIconKind::PreliminaryFile { extension };
    }
    match mime_type {
        Some(mime) if mime.as_ref() == tensor_files_core::GENERIC_BINARY_MIME => {
...
pub(crate) fn thumbnail_display_cache_size(display_px: f32) -> u16 {
    ...
    let freestanding = tensor_files_core::ThumbnailSize::for_display_px(display)
        .max_dimension()
        .min(256);
...
pub(crate) fn file_icon_profile(
    ...
    let (icon_candidates, generic_candidates) = match kind {
        ...
        FileIconKind::File { .. } => (
            ...
            mime_generic_icon_candidates(tensor_files_core::GENERIC_BINARY_MIME, mime),
        ),
        FileIconKind::Named {
            icon_name,
            fallback,
...
fn mime_icon_candidates(mime_name: &str, mime: &tensor_files_core::MimeDatabase) -> Vec<String> {
    ...
    if mime_name == tensor_files_core::GENERIC_BINARY_MIME {
...
fn mime_generic_icon_candidates(
    mime_name: &str,
    mime: &tensor_files_core::MimeDatabase,
) -> Vec<String> {
    let mut candidates = Vec::new();
...
fn mime_theme_icon_candidates(mime_name: &str, extension: Option<&str>) -> Vec<String> {
    let mut candidates = Vec::new();
    if let Some(icon_name) = tensor_files_core::mime_icon_name(mime_name) {
        push_icon_candidate(&mut candidates, icon_name);
    }
...
mod tests {
    ...
    fn desktop_entry_icon_name_ignores_non_desktop_entry_groups() {
        let root = std::env::temp_dir().join(format!(
            "tensor-files-desktop-icon-group-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
    ...
    fn kde_directory_metadata_uses_desktop_entry_icon_before_mime_resolution() {
        let root = std::env::temp_dir().join(format!(
            "tensor-files-directory-entry-icon-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
    ...
    fn desktop_entry_role_cache_reuses_stamp_and_refreshes_changed_stamp() {
        let root = std::env::temp_dir().join(format!(
            "tensor-files-desktop-role-cache-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
