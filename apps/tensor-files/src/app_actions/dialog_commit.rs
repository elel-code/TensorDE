...
use crate::{TensorFilesApp, path_display_label, task_error_detail};
use tensor_files_core::{MimeApplicationCache, set_default_mime_application};
...
impl TensorFilesApp {
    ...
    pub(crate) fn apply_async_rename_completion(
        ...
        match reload_result {
            ...
            Err(error) => {
                tensor_files_log!("[tensor-files] rename-reload-error {error}");
                true
            }
        }
    }
    ...
    pub(crate) fn apply_async_create_completion(
        ...
        match self.scene.reload_panes_showing_path(&request.parent, size) {
            ...
            Err(error) => {
                tensor_files_log!("[tensor-files] create-new-reload-error {error}");
                true
            }
        }
    }
    ...
    pub(crate) fn commit_open_with_chooser(&mut self) {
        ...
        if let Some(default_update) = request.default_update.as_ref() {
            match set_default_mime_application(
                ...
                Ok(path) => {
                    tensor_files_log!(
                        "[tensor-files] open-with-default mime={} desktop={} path={}",
                        default_update.mime_type,
                        default_update.desktop_id,
                        path.display()
                    );
