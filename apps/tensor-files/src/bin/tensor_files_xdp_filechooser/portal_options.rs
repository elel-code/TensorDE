fn gui_executable() -> Result<PathBuf, String> {
    if let Ok(path) = env::var("TENSOR_FILES_GUI") {
        return Ok(PathBuf::from(path));
    }

    let exe = env::current_exe().map_err(|err| format!("cannot locate executable: {err}"))?;
    let Some(dir) = exe.parent() else {
        return Err(format!("cannot locate tensor-files next to {}", exe.display()));
    };
    Ok(dir.join("tensor-files"))
}
...
fn save_file_start_and_name(
    ...
    {
        let start_dir = tensor_files_core::parent_location(&current_file);
...
fn nul_terminated_bytes_to_location(bytes: Vec<u8>) -> Option<PathBuf> {
    nul_terminated_bytes_to_string(bytes).map(|value| {
        tensor_files_core::normalize_network_uri(&value)
            .map(PathBuf::from)
...
fn portal_debug_log_request(request: PortalRequestDebug<'_>) {
    ...
    eprintln!("[tensor-files portal] {}", portal_request_summary(request));
}
...
fn portal_debug_log_parent(decision: &ParentWindowDecision) {
    ...
    eprintln!(
        "[tensor-files portal] parent_window status={} handle={} parent_binding={} parent_binding_reason={} native_transient=false",
...
fn portal_debug_enabled() -> bool {
    ...
    *DEBUG_PORTAL.get_or_init(|| {
        env::var("TENSOR_FILES_DEBUG_PORTAL").is_ok_and(|value| env_flag_is_truthy(value.as_str()))
    })
}
...
fn path_to_portal_uri(path: &Path) -> String {
    tensor_files_core::path_uri_from_path(path)
}

fn print_help() {
    println!(
        "Usage: tensor-files-xdp-filechooser\n\n\
         Runs the experimental Tensor Files xdg-desktop-portal FileChooser backend.\n\
         The backend owns {BUS_NAME} and launches `tensor-files --chooser` for FileChooser requests."
    );
}
