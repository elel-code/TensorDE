impl EventLoop {
    ...
    fn process_commands(&self) {
        ...
        for command in commands {
            ...
            {
                eprintln!("[tensor-files-wayland] runtime command failed: {error}");
            }
        }
    }
            ...
            Event::Dmabuf(event) => {
                ...
                match event {
                    wayland_client_runtime::DmabufEvent::Feedback { surface, feedback } => {
                        ...
                        // Verbose format table only when TENSOR_FILES_LOG is set.
                        ...
                        if windowing_verbose_log_enabled() {
                            ...
                            eprintln!(
                                "[tensor-files] dmabuf-feedback {scope} main_device=0x{:x} formats={} tranches={} pick={pick:?}",
                    ...
                    wayland_client_runtime::DmabufEvent::BufferCreated { id } => {
                        if windowing_verbose_log_enabled() {
                            eprintln!("[tensor-files] dmabuf-buffer-created id={id:?}");
                        }
                    }
                    wayland_client_runtime::DmabufEvent::BufferFailed => {
                        if windowing_verbose_log_enabled() {
                            eprintln!("[tensor-files] dmabuf-buffer-failed");
                        }
                    }
                    wayland_client_runtime::DmabufEvent::BufferReleased { id } => {
                        if windowing_verbose_log_enabled() {
                            eprintln!("[tensor-files] dmabuf-buffer-released id={id:?}");
                        }
                    }
                }
                Ok(())
            ...
            DndEvent::Leave { offer, surface } => {
                ...
                if !dropped {
                    ...
                    if let Err(error) = self.active.runtime.borrow_mut().discard_dnd_offer(offer) {
                        eprintln!("[tensor-files-wayland] discard DnD offer failed: {error}");
                    }
                }
            }
            DndEvent::Drop {
    ...
    fn finish_dnd_if_ready(&self, id: DataTransferId) {
        ...
        {
            eprintln!("[tensor-files-wayland] finish DnD offer failed: {error}");
        }
    }
...
/// Verbose window-runtime diagnostics (`TENSOR_FILES_LOG`), using
/// the same policy as the main binary's `tensor_files_log!`.
