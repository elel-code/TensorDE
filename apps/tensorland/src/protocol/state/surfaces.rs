...
pub(super) struct SurfaceRenderSnapshot {
    ...
    pub(super) color: SurfaceColorState,
}
...
pub(crate) fn apply_surface_representation(
    surface: &WlSurface,
...
pub(crate) fn pending_surface_fourcc(surface: &WlSurface) -> Option<tensor_host::Fourcc> {
    with_states(surface, |states| {
        let mut attributes = states.cached_state.get::<SurfaceAttributes>();
        match attributes.pending().buffer.as_ref() {
            Some(BufferAssignment::NewBuffer(buffer)) => wire_buffer_fourcc(buffer),
...
pub(crate) struct TestSurfaceState {
    ...
    pub(crate) color: SurfaceColorState,
}
...
fn wire_buffer_fourcc(buffer: &WlBuffer) -> Option<tensor_host::Fourcc> {
    #[cfg(feature = "tty")]
    if let Some(dmabuf) = dmabuf_buffer(buffer) {
        return Some(dmabuf.descriptor().format.code);
    }
    if single_pixel_rgba(buffer).is_some() {
        return Some(tensor_host::Fourcc::ARGB8888);
    }
    let metadata = shm_buffer(buffer)?.metadata();
    match metadata.format {
        wayland_server::protocol::wl_shm::Format::Argb8888 => Some(tensor_host::Fourcc::ARGB8888),
        wayland_server::protocol::wl_shm::Format::Xrgb8888 => Some(tensor_host::Fourcc::XRGB8888),
        _ => None,
    }
}
