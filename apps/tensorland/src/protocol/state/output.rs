...
use presentation::decide_presentation;
...
impl RuntimeState {
    ...
    #[cfg(feature = "tty")]
    pub(super) fn queue_redraw(&mut self, output: BackendOutputId) {
        let state = self
            .redraw_states
            .entry(output)
    ...
    fn submit_output_frame(&mut self, output_id: BackendOutputId) {
        ...
        let capture_pending = self.has_pending_output_capture(&output);
        if has_presented
        ...
        let capture = self.take_output_capture(&output, target.viewport);
        let Some(result) = self.renderer.as_mut().map(|renderer| {
            renderer.submit_scene_with_capture(render_output, scene, cursors, capture)
        }) else {
        ...
        match result {
            Ok(frame) => {
                let presentation_decision = decide_presentation(
                    &frame.scene,
                    frame.target.viewport,
                    !frame.draw_plan.cursors().is_empty(),
                    capture_pending,
                    frame.pass_plan.composition_label() == "direct-single-pass",
                    self.backend
                        .as_ref()
                        .is_some_and(|backend| backend.output_allows_async_page_flip(output_id)),
                );
                ...
                .with_mode(presentation_decision.mode);
                ...
                let pass_metrics = frame.pass_plan.metrics();
                if let Some(started) = frame_started {
                    info!(
                        ...
                        serial = frame.serial,
                        timeline = frame.timeline_value,
                        composition = frame.pass_plan.composition_label(),
                        presentation_mode = ?presentation_decision.mode,
                        presentation_reason = ?presentation_decision.reason,
                        output_pixels = u64::from(frame.target.viewport.width)
                            * u64::from(frame.target.viewport.height),
                        shadow_draws = frame.draw_plan.shadow_count(),
                        shadow_pixel_upper_bound = frame.draw_plan.shadow_pixel_upper_bound(),
                        backdrop_passes = pass_metrics.backdrop_passes,
                        backdrop_sample_pixels = pass_metrics.sample_pixels,
                        backdrop_filter_pixels = pass_metrics.filter_pixels,
                        backdrop_filter_texture_samples = pass_metrics.filter_texture_samples,
                        backdrop_composite_pixel_upper_bound =
                            pass_metrics.composite_pixel_upper_bound,
                        backdrop_retained_intermediate_pixels =
                            pass_metrics.retained_intermediate_pixels,
                ...
                debug!(
                    ...
                    render_damage_regions = frame.render_damage.regions().len(),
                    composition = frame.pass_plan.composition_label(),
                    presentation_mode = ?presentation_decision.mode,
                    presentation_reason = ?presentation_decision.reason,
                    backdrop_passes = pass_metrics.backdrop_passes,
                    ...
                    backdrop_filter_pixels = pass_metrics.filter_pixels,
                    backdrop_filter_texture_samples = pass_metrics.filter_texture_samples,
                    backdrop_composite_pixel_upper_bound = pass_metrics.composite_pixel_upper_bound,
                    backdrop_retained_intermediate_pixels = pass_metrics.retained_intermediate_pixels,
