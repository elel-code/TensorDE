...
pub trait VirtualPointerHandler: 'static {
    ...
    fn on_virtual_pointer_motion(&mut self, pointer: &VirtualPointer, event: RelativeMotionEvent);
    fn on_virtual_pointer_motion_absolute(
        &mut self,
        pointer: &VirtualPointer,
        event: AbsoluteMotionEvent,
    );
    fn on_virtual_pointer_button(&mut self, pointer: &VirtualPointer, event: PointerButtonEvent);
    fn on_virtual_pointer_axis(&mut self, pointer: &VirtualPointer, event: PointerAxisEvent);
}
    ...
    fn request(
        ...
        match request {
            zwlr_virtual_pointer_v1::Request::Motion { time, dx, dy } => {
                handler.on_virtual_pointer_motion(
                    &pointer,
                    RelativeMotionEvent {
                        delta_x: dx,
                        delta_y: dy,
            ...
            zwlr_virtual_pointer_v1::Request::MotionAbsolute {
                ...
                handler.on_virtual_pointer_motion_absolute(
                    &pointer,
                    AbsoluteMotionEvent {
            ...
            zwlr_virtual_pointer_v1::Request::Button {
                ...
                handler.on_virtual_pointer_button(
                    &pointer,
                    PointerButtonEvent {
                        button,
                        pressed,
            ...
            zwlr_virtual_pointer_v1::Request::Frame => {
                if let Some(event) = self.finish_axis_frame() {
                    handler.on_virtual_pointer_axis(&pointer, event);
                }
            }
