...
impl CursorState {
    ...
    fn prepare_named_raster(
        ...
        let frames = self
            ...
            .filter_map(|image| {
                ...
                let pixels: Arc<[u8]> = image.pixels_rgba.into();
                let buffer_id = upload(size, &pixels)?;
    ...
    pub(in crate::protocol) fn pointer_capture_image(
        &self,
        scale: OutputScale,
        mut resolve: impl FnMut(&WlSurface, OutputScale) -> Option<CursorRaster>,
        ...
        match &self.image {
            ...
            CursorImage::Named(icon) => {
                let sequence = self.named_rasters.get(&(*icon, scale))?.as_ref()?;
                let frame = sequence.frames.get(sequence.current)?;
                Some(CursorCaptureImage {
                    size: frame.raster.size,
                    hotspot: frame.raster.hotspot,
                    sample_transform: frame.raster.sample_transform,
                    pixels: CursorCapturePixels::Rgba(Arc::clone(&frame.capture_pixels)),
                })
            }
            CursorImage::Surface(surface) => {
                let raster = resolve(surface, scale)?;
                Some(CursorCaptureImage {
                    size: raster.size,
                    hotspot: raster.hotspot,
                    sample_transform: raster.sample_transform,
                    pixels: CursorCapturePixels::Surface(surface.clone()),
                })
            }
        }
    }
    ...
    #[cfg(test)]
    pub(in crate::protocol) fn install_test_capture_image(
        &mut self,
        scale: OutputScale,
        size: Size,
        hotspot: Point,
        ...
        let CursorImage::Named(icon) = self.image else {
            panic!("test capture image requires a named pointer cursor");
        };
        self.named_rasters.insert(
            (icon, scale),
            Some(CursorRasterSequence {
                frames: vec![CursorRasterFrame {
                    raster: CursorRaster {
                        buffer_id: SurfaceBufferId::new(u64::MAX),
                        size,
                        hotspot,
                        sample_transform: SurfaceSampleTransform::IDENTITY,
                    },
                    capture_pixels: pixels_rgba,
                    delay_ms: 0,
                }],
                duration_ms: 0,
                current: 0,
