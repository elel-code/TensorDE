...
impl X11Wm {
    ...
    pub(crate) fn apply_property_result(
        ...
        match result.update {
            X11PropertyUpdate::Initial(properties) => {
                if window.apply_initial_properties(*properties) {
            ...
            X11PropertyUpdate::WmHints(wm_hints) => {
                if !matches!(
                    self.property_disposition(&window, PropertyQuery::WmHints)?,
                    PropertyDisposition::Apply
                ) {
                    return Ok(());
                }
                window.apply_wm_hints(wm_hints);
                self.publish_completed_map_request(window)?;
            }
            X11PropertyUpdate::Protocols(protocols) => {
                if !matches!(
                    self.property_disposition(&window, PropertyQuery::Protocols)?,
                    PropertyDisposition::Apply
                ) {
                    return Ok(());
                }
                window.apply_protocols(protocols);
                self.publish_completed_map_request(window)?;
            }
            X11PropertyUpdate::Title(title) => {
                if !matches!(
                    self.property_disposition(&window, PropertyQuery::Title)?,
                    PropertyDisposition::Apply
                ) {
                    return Ok(());
                }
                window.apply_title(title);
                self.push_event(XwmEvent::PropertyChanged(
                    window.clone(),
            ...
            X11PropertyUpdate::Class(class) => {
                if !matches!(
                    self.property_disposition(&window, PropertyQuery::Class)?,
                    PropertyDisposition::Apply
                ) {
                    return Ok(());
                }
                window.apply_class(class);
                self.push_event(XwmEvent::PropertyChanged(
                    window.clone(),
    ...
    pub(crate) fn set_active_window(
        &mut self,
        window: Option<&X11Surface>,
    ) -> Result<(), Box<dyn std::error::Error>> {
        let active = window.map(X11Surface::window_id);
    ...
    fn handle_event(&mut self, event: Event) -> Result<(), Box<dyn std::error::Error>> {
        match event {
            ...
            Event::PropertyNotify(event) => {
                if let Some(window) = self.windows.get(&event.window).cloned() {
                    let query = if event.atom == u32::from(AtomEnum::WM_TRANSIENT_FOR) {
                        ...
                        Some(PropertyQuery::WmHints)
                    } else if event.atom == self.atoms.WM_PROTOCOLS {
                        Some(PropertyQuery::Protocols)
                    } else if event.atom == self.atoms._NET_WM_NAME
                        || event.atom == u32::from(AtomEnum::WM_NAME)
                    {
                        Some(PropertyQuery::Title)
                    } else if event.atom == u32::from(AtomEnum::WM_CLASS) {
                        Some(PropertyQuery::Class)
                    } else {
                        None
                    };
            ...
            Event::ClientMessage(event) if event.type_ == self.atoms._NET_CLOSE_WINDOW => {
                if let Some(window) = self.windows.get(&event.window).cloned() {
                    self.push_event(XwmEvent::CloseRequested(window))?;
                }
            }
    ...
    fn create_window(
        ...
        let window = X11Surface::new(
            ...
            X11SurfaceAtoms {
                net_wm_state: self.atoms._NET_WM_STATE,
                net_wm_state_focused: self.atoms._NET_WM_STATE_FOCUSED,
                wm_protocols: self.atoms.WM_PROTOCOLS,
                wm_take_focus: self.atoms.WM_TAKE_FOCUS,
                wm_delete_window: self.atoms.WM_DELETE_WINDOW,
    ...
    fn property_request(&self, window: &X11Surface, query: PropertyQuery) -> X11PropertyRequest {
        ...
        match query {
            ...
            PropertyQuery::WmHints => X11PropertyRequest::WmHints { target },
            PropertyQuery::Protocols => X11PropertyRequest::Protocols {
                target,
                wm_protocols: self.atoms.WM_PROTOCOLS,
            },
            PropertyQuery::Title => X11PropertyRequest::Title {
                target,
                net_wm_name: self.atoms._NET_WM_NAME,
                utf8_string: self.atoms.UTF8_STRING,
            },
            PropertyQuery::Class => X11PropertyRequest::Class { target },
        }
    }
