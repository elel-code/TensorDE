...
impl SelectionProtocol {
    ...
    pub(crate) fn drop_dnd(&mut self) -> Option<SourceToken> {
        let active = self.active_dnd.take()?;
        let source_token = active.source_token;
    ...
    pub(crate) fn cancel_dnd(&mut self) -> Option<SourceToken> {
        let active = self.active_dnd.take()?;
        let source_token = active.source_token;
...
impl RuntimeState {
    pub(super) fn start_selection_drag(
        ...
        if let Some(token) = source {
            self.protocol_globals.xdg_toplevel_drag.drag_started(token);
            if let Some(location) = self.input_seat.pointer_location() {
                self.move_active_xdg_toplevel_drag(location);
            }
        }
        #[cfg(feature = "tty")]
    ...
    pub(in crate::protocol) fn finish_selection_dnd(&mut self) {
        if let Some(token) = self.protocol_globals.selection.drop_dnd() {
            self.protocol_globals.xdg_toplevel_drag.drag_ended(token);
        }
        #[cfg(feature = "tty")]
    ...
    fn cancel_selection_dnd(&mut self) {
        if let Some(token) = self.protocol_globals.selection.cancel_dnd() {
            self.protocol_globals.xdg_toplevel_drag.drag_ended(token);
        }
        #[cfg(feature = "tty")]
        self.retire_dnd_icon();
