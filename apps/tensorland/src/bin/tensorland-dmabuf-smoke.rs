//! A real linux-dmabuf client used to prove Tensorland's native presentation path.
//!
...
#[path = "tensorland-dmabuf-smoke/buffer_pool.rs"]
mod buffer_pool;
#[path = "tensorland-dmabuf-smoke/feedback.rs"]
mod feedback;
#[path = "tensorland-dmabuf-smoke/state.rs"]
mod state;
...
fn run(args: Args) -> Result<(), SmokeError> {
    ...
    println!(
        "tensorland-dmabuf-smoke: selected render_device={} format={:#010x} modifier={:#018x} buffers={}",
    ...
    println!("tensorland-dmabuf-smoke: PASS {}", state.success_report());
    Ok(())
}
