...
impl SmokeState {
    ...
    fn maybe_submit(&mut self, queue_handle: &QueueHandle<Self>) {
        ...
        self.surface.commit();
        self.sent_frames += 1;
        println!("tensorland-dmabuf-smoke: submitted buffer slot={frame}");
    }
    ...
    fn note_created(
        ...
        println!("tensorland-dmabuf-smoke: dma-buf import accepted slot={slot}");
    ...
    fn note_release(&mut self, buffer: &wl_buffer::WlBuffer) {
        ...
        println!("tensorland-dmabuf-smoke: compositor released buffer slot={slot}");
    }
...
impl Dispatch<xdg_surface::XdgSurface, ()> for SmokeState {
    fn event(
        ...
        if let xdg_surface::Event::Configure { serial } = event {
            ...
            println!("tensorland-dmabuf-smoke: xdg surface configured");
...
impl Dispatch<wl_callback::WlCallback, usize> for SmokeState {
    fn event(
        ...
        if matches!(event, wl_callback::Event::Done { .. }) {
            ...
            println!("tensorland-dmabuf-smoke: frame callback slot={frame}");
            state.maybe_submit(queue_handle);
        }
    }
}
    ...
    fn event(
        ...
        match event {
            wp_presentation_feedback::Event::Presented { .. } => {
                ...
                println!("tensorland-dmabuf-smoke: presentation feedback slot={frame}");
            }
