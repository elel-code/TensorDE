...
use super::{
    ...
    NativeOutputTarget, OutputCaptureRequest, OutputCaptureResult, RenderOutputId,
};
use super::{
...
impl VulkanRenderer {
    pub(crate) fn new(target: RendererTarget) -> Result<Self, RendererError> {
        ...
        info!(
            ...
            "shared Vulkan renderer device initialized"
        );
    ...
    pub(crate) fn register_output(
        ...
        let output_texture_format = texture_format_for_fourcc(target.format.format.code)
            .ok_or_else(|| RendererError::Frame("native output format is not typed".into()))?;
        self._owner
            .frame_executor
            .prepare_output_format(&self._owner.backend, output_texture_format)
            .map_err(|source| RendererError::CreateFrameResources(source.to_string()))?;
    ...
    #[cfg(feature = "tty")]
    pub(crate) fn submit_scene_with_capture(
        &mut self,
        output: RenderOutputId,
        ...
        let sync_fd = match self._owner.frame_executor.submit(
            ...
            Err(source) => {
                if let Some(capture) = capture {
                    self._owner
                        .frame_executor
                        .reject_capture(capture, source.to_string());
                }
    ...
    #[cfg(feature = "tty")]
    pub(crate) fn drain_completed_captures(
        &mut self,
    ) -> Result<Vec<OutputCaptureResult>, RendererError> {
        let completed = self.refresh_completed()?;
        Ok(self
            ._owner
            .frame_executor
            .drain_completed_captures(completed))
    }
}
