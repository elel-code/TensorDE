...
use crate::{
    ...
    render::{CursorTexture, OutputCaptureId, OutputCaptureRequest},
    scene::{
...
#[test]
fn cursor_excluding_capture_restores_retained_pixels_before_the_tap() {
    let mut scheduler = FrameScheduler::new(4096, 32, 0, 32).unwrap();
    scheduler.register_output(target()).unwrap();
    let mut completed = 0;
    for _ in 0..3 {
        let frame = scheduler
            .prepare_with_cursors(OUTPUT, scene(), cursors(&[(10, 20)]), completed)
            .unwrap();
        scheduler.commit(&frame).unwrap();
        completed = frame.timeline_value;
        scheduler.retire_completed(completed);
    }

    let capture_region = Rect::new(15, 25, 4, 5);
    let frame = scheduler
    ...
    assert_eq!(frame.render_damage.regions(), [capture_region]);
}

#[test]
fn cursor_including_capture_keeps_unchanged_slot_damage_empty() {
    let mut scheduler = FrameScheduler::new(4096, 32, 0, 32).unwrap();
    scheduler.register_output(target()).unwrap();
    let mut completed = 0;
    for _ in 0..3 {
    ...
    assert!(frame.damage.is_empty());
    assert!(frame.render_damage.is_empty());
}
