...
pub(crate) struct SurfaceDraw {
    ...
    pub(crate) color: vulkan_renderer::ColorTransformPlan,
...
impl FrameDrawPlan {
    ...
    pub(crate) fn build_with_cursors(
        ...
        let output_format = texture_format(target.format.format.code).ok_or(
            FrameError::UnsupportedOutputColorFormat(target.format.format.code),
        )?;
        ...
        for node in scene.draw_order() {
            ...
            if let Some(shadow) = shadow_draw(node, scene.viewport, target) {
                shadows.push(shadow);
                scene_draws.push(SceneDrawCommand::Shadow(shadows.len() - 1));
            }
    ...
    pub(in crate::render) fn shadows(&self) -> &[ShadowDraw] {
        &self.shadows
    }

    pub(crate) const fn shadow_count(&self) -> usize {
        self.shadows.len()
    }

    pub(crate) fn shadow_pixel_upper_bound(&self) -> u64 {
        self.shadows.iter().fold(0_u64, |pixels, shadow| {
            pixels.saturating_add(
                u64::from(shadow.clip.width).saturating_mul(u64::from(shadow.clip.height)),
            )
        })
    }
...
mod tests {
    ...
    use crate::{
        ...
        scene::{
            ContentRevision, FocusOutline, SceneNode, SurfaceContent, SurfaceLayer,
            SurfaceSampleTransform, SurfaceTransform,
        },
    };
    use tensor_protocol::{
        ColorLuminances, ColorPrimaries, ColorRepresentation, ImageDescription, ImageDescriptionId,
        RenderIntent, SurfaceColorState, TransferFunction,
    };
    ...
    #[test]
    fn committed_hdr_surface_state_reaches_the_draw_color_plan() {
        let viewport = Rect::new(0, 0, 80, 60);
        let placement = LayoutPlacement {
            geometry: viewport,
            visible: Some(viewport),
        ...
        let description = ImageDescription {
            id: ImageDescriptionId::new(4).unwrap(),
        ...
        let plan = FrameDrawPlan::build(&scene, target(viewport, OutputScale::ONE)).unwrap();
        let color = plan.draws()[0].color;

        assert_eq!(color.gamut_map, GamutMap::Perceptual);
        assert_eq!(
            color.tone_map,
            ToneMap::Bt2390 {
                source_peak: 10_000,
    ...
    #[test]
    fn shadow_lowers_to_one_clipped_descriptor_free_scene_draw() {
        let viewport = Rect::new(0, 0, 100, 80);
        let placement = LayoutPlacement {
            geometry: Rect::new(10, 10, 20, 10),
            visible: Some(Rect::new(10, 10, 20, 10)),
        };
        let color = LinearRgba16::new(100, 200, 300, 32_768);
        let scene = SceneSnapshot::new(
            WorkspaceId::new(1),
            viewport,
            vec![SceneNode::new(
        ...
        assert_eq!(
            ...
            [ShadowDraw {
                view_id: ViewId::new(1),
                destination: Rect::new(7, 8, 30, 20),
                clip: Rect::new(7, 8, 30, 20),
                box_rect: Rect::new(3, 3, 24, 14),
                color,
                opacity: crate::scene::UnitFraction::OPAQUE,
                blur_radius: 3,
                corner_radius: 6,
            }]
        );
        assert_eq!(plan.scene_draws(), [SceneDrawCommand::Shadow(0)]);
        assert!(plan.images().is_empty());
        assert_eq!(plan.shadow_count(), 1);
        assert_eq!(plan.shadow_pixel_upper_bound(), 600);
    }
