...
cargo run -p tensorland -- --validate-config --config apps/tensorland/examples/config.kdl
cargo run -p tensorland -- --check
TENSOR_LAYOUT=spatial-2d cargo run -p tensorland -- --check
cargo run -p tensorland -- --config apps/tensorland/examples/config.kdl --check
```
...
bounded cold worker; `tensorctl reload-config` queues the same operation explicitly and
`tensorctl get-config-status` reports its generation and last bounded diagnostic.
...
`tensorland-session` is a Rust equivalent of Niri's `niri-session`: it detects a working user manager,
