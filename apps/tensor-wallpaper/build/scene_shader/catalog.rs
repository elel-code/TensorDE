...
pub(crate) fn build_scene_shader_catalog() {
    ...
    for spec in &specs {
        ...
        let object_mesh_vertex = match spec.family {
            SceneShaderFamily::FlatPassthrough => Some(compile_slang_scene_vertex(
                &shader_dir,
            ...
            SceneShaderFamily::Effect => {
                ...
                if shader == "effects/iris" {
                    Some(compile_slang_scene_vertex(
                        &shader_dir,
                        &format!("{}__OBJECT_MESH", spec.key),
                        &super::iris_object_mesh_vertex_source(),
                        fragment.push_constant_bytes,
                    ))
                } else if shader == "effects/tint" && texture_slot_mask == 0x03 {
                    Some(compile_slang_scene_vertex(
                        &shader_dir,
                        &format!("{}__OBJECT_MESH", spec.key),
            ...
            SceneShaderFamily::DepthParallax => {
                let variant = super::depth_parallax::variant_from_catalog_key(&spec.key)
                    .expect("generated Depth Parallax catalog key must have a typed variant");
                Some(compile_slang_scene_vertex(
                    &shader_dir,
                    &format!("{}__OBJECT_MESH", spec.key),
                    &super::depth_parallax::object_mesh_vertex_source(variant),
                    fragment.push_constant_bytes,
                ))
            }
            SceneShaderFamily::Shake => Some(compile_slang_scene_vertex(
                &shader_dir,
                &format!("{}__OBJECT_MESH", spec.key),
                &super::shake::object_mesh_vertex_source(),
                fragment.push_constant_bytes,
            )),
            _ => None,
        };
        ...
        assert!(
            ...
            "built-in scene shader {} did not retain Slang source",
            spec.key
        );
        let local_read_shader = if matches!(spec.family, SceneShaderFamily::FlatPassthrough) {
            ...
            let stage = compile_slang_scene_input_attachment(
                &shader_dir,
    ...
    assert_eq!(
        ...
        "built-in scene shader catalog coverage regressed"
    );
    ...
    let particle_compute_source = super::particle_compute_source();
    let compute =
        compile_particle_compute(&shader_dir, "particle_compute", &particle_compute_source);
...
fn compile_scene_fragment(
    ...
    key: &str,
    source: &str,
) -> stage_compilation::CompiledSceneStage {
    if has_version_controlled_slang_source(family, key) {
        compile_slang_scene_fragment(shader_dir, key, source)
    } else {
...
fn compile_scene_vertex(
    ...
    if has_version_controlled_slang_source(family, key) {
        compile_slang_scene_vertex(shader_dir, key, source, push_base_bytes)
    } else {
...
fn scene_shader_sources(family: SceneShaderFamily, key: &str) -> (String, String) {
    match family {
        ...
        SceneShaderFamily::MeshGenericParticle => super::generic_particle_sources(),
        ...
        SceneShaderFamily::MeshImageFoliageRippleComposite => {
            super::image_foliage_ripple_composite_sources(
                key.contains("__TENSOR_WALLPAPER_FOLIAGE_POWER_TWO_1"),
            )
        }
...
fn has_version_controlled_slang_source(family: SceneShaderFamily, key: &str) -> bool {
    matches!(
        family,
