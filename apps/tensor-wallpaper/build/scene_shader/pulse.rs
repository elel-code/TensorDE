//! Slang catalog stages for Wallpaper Engine's installed Pulse effect.
//!
//! References:
//! - `reverse-engineered/tensor-wallpaper/shaders/effects/pulse.{vert,frag}`
//! - `reverse-engineered/tensor-wallpaper/shaders/common_blending.h`
//! - `reverse-engineered/tensor-wallpaper/docs/blending-modes.md`
...
pub(super) fn catalog_key(variant: PulseCatalogVariant) -> String {
    format!(
        "effects/pulse__TENSOR_WALLPAPER_CATALOG_AUDIO_{}__MASK_{}__BLENDMODE_{}",
...
pub(super) fn variant_from_catalog_key(key: &str) -> Option<PulseCatalogVariant> {
    ...
    let audio_processing = parts
        .next()?
        .strip_prefix("TENSOR_WALLPAPER_CATALOG_AUDIO_")?
        .parse::<u8>()
        .ok()?;
