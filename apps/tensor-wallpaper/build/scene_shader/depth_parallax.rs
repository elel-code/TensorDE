//! Slang catalog stages for Wallpaper Engine's installed Depth Parallax effect.
//!
//! References:
//! - `reverse-engineered/tensor-wallpaper/shaders/effects/depthparallax.{vert,frag}`
...
pub(super) fn catalog_key(variant: DepthParallaxCatalogVariant) -> String {
    format!(
        "effects/depthparallax__TENSOR_WALLPAPER_CATALOG_QUALITY_{}__MASK_{}",
        variant.quality,
...
pub(super) fn variant_from_catalog_key(key: &str) -> Option<DepthParallaxCatalogVariant> {
    ...
    let quality = parts
        .next()?
        .strip_prefix("TENSOR_WALLPAPER_CATALOG_QUALITY_")?
        .parse::<u8>()
        .ok()?;
...
pub(super) fn object_mesh_vertex_source(variant: DepthParallaxCatalogVariant) -> String {
    let mask_output = variant
        .mask_enabled
        .then_some("    [[vk::location(2)]] float2 maskTexCoord : TEXCOORD2;\n");
    let mask_statement = variant.mask_enabled.then_some(
        "    output.maskTexCoord = input.texCoord * material.texture2Resolution.zw\n        / material.texture2Resolution.xy;\n",
    );
    format!(
...
struct DepthParallaxObjectMaterialData
{{
    float4 texture1Resolution;
    float4 texture2Resolution;
    float2 parallaxPosition;
...
cbuffer DepthParallaxObjectMaterial : register(b3)
{{
    DepthParallaxObjectMaterialData material;
}}

struct DepthParallaxObjectVertexInput
{{
    [[vk::location(0)]] float2 position : POSITION;
    [[vk::location(1)]] float2 texCoord : TEXCOORD0;
}};

struct DepthParallaxObjectVertexOutput
{{
    float4 position : SV_Position;
    [[vk::location(0)]] float4 texCoord : TEXCOORD0;
    [[vk::location(1)]] float2 parallaxOffset : TEXCOORD1;
...
[[shader("vertex")]]
DepthParallaxObjectVertexOutput main(DepthParallaxObjectVertexInput input)
{{
    float2 projectedDirX = normalize(float2(
        material.effectTextureProjectionInverse[0].x,
        material.effectTextureProjectionInverse[1].x));
    float2 projectedDirY = normalize(float2(
        material.effectTextureProjectionInverse[0].y,
        material.effectTextureProjectionInverse[1].y));
    float2 parallaxInput = material.parallaxPosition * 2.0 - 1.0;

    DepthParallaxObjectVertexOutput output;
