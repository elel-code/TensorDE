//! Cold-path descriptor-heap shaders for decoded video presentation.
...
pub(super) fn build_video_present_shaders() {
    ...
    let shader_dir = out_dir.join("tensor_wallpaper_video_present_shaders");
    ...
    let fullscreen_fragment = compile_fragment(
        &shader_dir,
        "fullscreen",
    ...
    let scene_fragment = compile_fragment(
        &shader_dir,
        "scene_video_layer",
    ...
    fs::write(
        out_dir.join("tensor_wallpaper_scene_video_shaders.rs"),
        scene_generated,
    )
    ...
    fs::write(
        out_dir.join("tensor_wallpaper_shared_video_present_shaders.rs"),
...
fn compile_fragment(
    ...
    let slang = shader_dir.join(format!("{key}.frag.slang"));
    let spirv = shader_dir.join(format!("{key}.frag.spv"));
    fs::write(&slang, lowered.source).expect("write video-present descriptor-heap source");
    SlangCompiler::from_environment()
        .compile(&ShaderCompileRequest {
            source: slang,
            entry_point: "main".to_owned(),
        ...
        .unwrap_or_else(|error| panic!("compile video-present fragment {key}: {error}"));
    CompiledStage {
        spirv,
