//! Vulkan scene present runtime boundary.
//!
//! References:
...
use crate::renderer::rendering_device::{
    RenderingDeviceClearColor, RenderingDeviceError, RenderingDeviceOptions,
    RenderingDeviceScenePresentOptions, RenderingDeviceScenePresentSnapshot,
    run_rendering_device_scene_present,
};

use super::SceneExecutionPlan;

#[derive(Debug, Clone, PartialEq)]
pub struct RenderingDeviceSceneRunOptions {
    pub user_property_overrides: Map<String, Value>,
    pub pointer_events: bool,
    pub pointer_replay_normalized: Option<[f64; 2]>,
    pub clear_color_override: Option<RenderingDeviceClearColor>,
    pub surface_extent: Option<(u32, u32)>,
    pub gpu_timing: bool,
    pub semantic_diagnostics: bool,
    pub video_sources: Vec<RenderingDeviceSceneVideoSource>,
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub struct RenderingDeviceSceneVideoSource {
    pub media_instance: u32,
    pub source: PathBuf,
    pub codec: crate::renderer::rendering_device::RenderingDeviceVideoSessionCodec,
    pub loop_playback: bool,
}

impl Default for RenderingDeviceSceneRunOptions {
    fn default() -> Self {
        Self {
...
pub struct RenderingDeviceSceneRuntimeSnapshot {
    pub binding: &'static str,
    pub route: &'static str,
    pub source: PathBuf,
    pub present: RenderingDeviceScenePresentSnapshot,
...
pub fn run_scene(
    options: RenderingDeviceOptions,
    duration: Option<Duration>,
    source: PathBuf,
) -> Result<RenderingDeviceSceneRuntimeSnapshot, RenderingDeviceError> {
    run_scene_with_options(
        options,
        duration,
        source,
        RenderingDeviceSceneRunOptions::default(),
    )
}
...
pub fn run_scene_with_options(
    ...
    .map_err(|error| RenderingDeviceError::Scene(error.to_string()))?;
    ...
    let present =
        run_rendering_device_scene_present(RenderingDeviceScenePresentOptions {
            host,
        ...
        .map_err(RenderingDeviceError::Scene)?;
    ...
    Ok(RenderingDeviceSceneRuntimeSnapshot {
...
fn validate_pointer_replay_position(position: Option<[f64; 2]>) -> Result<(), RenderingDeviceError> {
    if position.is_some_and(|position| {
        position
            .iter()
            .any(|coordinate| !coordinate.is_finite() || !(0.0..=1.0).contains(coordinate))
    }) {
        return Err(RenderingDeviceError::Scene(
...
) -> Result<(), RenderingDeviceError> {
    if execution_plan.present_mode != "fifo-latest-ready" {
        return Err(RenderingDeviceError::Scene(
            "scene runtime requires FIFO latest ready present".to_owned(),
        ));
    }
    if !execution_plan.pipeline_cache.missing_shader_keys.is_empty() {
        return Err(RenderingDeviceError::Scene(format!(
            "scene runtime has unresolved shader programs: {}",
            execution_plan.pipeline_cache.missing_shader_keys.join(", ")
        )));
    }
    if execution_plan.rendering_device_graph.descriptor_heap_required
        && execution_plan
            .resource_storage
            .descriptor_heap
            .descriptor_model
...
fn scene_clear_color(storage: &SceneStorage) -> RenderingDeviceClearColor {
    let [r, g, b, a] = storage.project().clear_color;
    RenderingDeviceClearColor { r, g, b, a }
}

#[cfg(test)]
mod tests {
    use super::super::resource_storage::{
        RenderingDeviceSceneEffectTargetStoragePlan, RenderingDeviceSceneSkinningBufferPlan,
    };
    use super::*;
    use crate::engine::scene::{RendererSceneRenderPlan, SceneRenderingDeviceGraphPlan};
    use crate::renderer::rendering_device::scene::{
        RenderingDeviceSceneDescriptorHeapPlan, RenderingDeviceSceneMeshUploadPlan,
    };
    use crate::renderer::rendering_device::scene::{
        RenderingDeviceSceneHeapStoragePlan, RenderingDeviceSceneMeshBufferPlan,
        RenderingDeviceScenePipelineCachePlan, RenderingDeviceSceneRenderGraphExecutorPlan,
        RenderingDeviceSceneResourceStoragePlan,
    };

    #[test]
    fn scene_runtime_plan_rejects_missing_builtin_shader_catalog_entries() {
        let mut plan = empty_execution_plan();
    ...
    fn empty_execution_plan() -> SceneExecutionPlan {
        SceneExecutionPlan {
            ...
            render_graph_executor: RenderingDeviceSceneRenderGraphExecutorPlan {
                command_count: 0,
                draw_count: 0,
                commands: Vec::new(),
