...
fn pulse_catalog_strictly_resolves_semantic_variants_and_preserves_typed_bindings() {
    let audio = rendering_device_scene_shader_for_key(
    ...
    let masked_audio = rendering_device_scene_shader_for_key(
    ...
    let time = rendering_device_scene_shader_for_key("effects/pulse__SLOTS_3")
    ...
    for invalid in [
        ...
        assert!(
            rendering_device_scene_shader_for_key(invalid).is_none(),
...
fn depth_parallax_catalog_resolves_quality_and_mask_without_legacy_keys() {
    ...
    let masked =
        rendering_device_scene_shader_for_key("effects/depthparallax__SLOTS_7__QUALITY_0__MASK_1")
...
fn shake_catalog_resolves_direction_with_authored_default_slot_identity() {
    let shader = rendering_device_scene_shader_for_key("effects/shake__SLOTS_7__DIRECTION_1")
...
fn waterwaves_slots_7_keeps_default_identity_but_only_live_o2_accesses() {
    let shader = rendering_device_scene_shader_for_key("effects/waterwaves__SLOTS_7")
...
#[test]
fn waterwaves_default_texture_identity_keeps_sparse_and_masked_variants() {
    for (key, live_registers) in [
        ("effects/waterwaves__SLOTS_5", &[0][..]),
        ("effects/waterwaves__SLOTS_5__DUALWAVES_1", &[0][..]),
        ("effects/waterwaves__SLOTS_7__DUALWAVES_1", &[0, 1][..]),
    ] {
        let shader = rendering_device_scene_shader_for_key(key)
            .unwrap_or_else(|| panic!("WaterWaves catalog must contain {key}"));
        for kind in [
            BuiltinSceneDescriptorBindingKind::SampledImage,
            BuiltinSceneDescriptorBindingKind::Sampler,
        ] {
