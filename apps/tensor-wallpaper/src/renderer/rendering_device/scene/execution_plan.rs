//! Scene resource, pipeline, graph, and command execution planning.
//!
//! References:
...
use crate::renderer::rendering_device::present::render_item::RenderingDeviceRenderItem;

use super::pipeline_cache::{
    RenderingDeviceScenePipelineCachePlan, rendering_device_scene_pipeline_cache_plan,
};
...
use super::resource_storage::{
    RenderingDeviceSceneResourceStoragePlan, rendering_device_scene_resource_storage_plan,
};
...
pub struct SceneExecutionPlan {
    ...
    pub resource_storage: RenderingDeviceSceneResourceStoragePlan,
    pub pipeline_cache: RenderingDeviceScenePipelineCachePlan,
    pub render_graph_executor: RenderingDeviceSceneRenderGraphExecutorPlan,
    pub descriptor_heap: RenderingDeviceSceneDescriptorHeapPlan,
    pub mesh_upload: RenderingDeviceSceneMeshUploadPlan,
...
pub struct RenderingDeviceSceneDescriptorHeapPlan {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneMeshUploadPlan {
...
pub fn scene_execution_plan(storage: &SceneStorage) -> SceneExecutionPlan {
    scene_execution_plan_at(storage, 0.0)
}

pub fn scene_execution_plan_at(
...
pub fn scene_execution_plan_from_semantic_frame(
    storage: &SceneStorage,
    ...
    build_scene_execution_plan(storage, renderer_scene_render, rendering_device_graph)
}
...
fn build_scene_execution_plan(
    ...
    let resource_storage = rendering_device_scene_resource_storage_plan(
        storage,
        renderer_scene_render,
        &rendering_device_graph,
    );
    let pipeline_cache = rendering_device_scene_pipeline_cache_plan(storage, &resource_storage);
    let render_graph_executor = rendering_device_scene_render_graph_executor_plan(
        &rendering_device_graph,
        &resource_storage,
        &pipeline_cache,
    );
    ...
    SceneExecutionPlan {
        ...
        descriptor_heap: RenderingDeviceSceneDescriptorHeapPlan {
        ...
        mesh_upload: RenderingDeviceSceneMeshUploadPlan {
...
pub fn scene_execution_plan_from_render_item(
    item: &RenderingDeviceRenderItem,
) -> Result<Option<SceneExecutionPlan>, String> {
    ...
    let storage = SceneStorage::from_binary_reader(file).map_err(|err| {
        format!(
            "load scene engine binary {} into Vulkan scene storage: {err}",
            scene_source.display()
        )
    })?;
    Ok(Some(scene_execution_plan(&storage)))
}
...
mod tests {
    ...
    use crate::engine::scene::{
        ...
        SceneRenderGraphRecord, SceneRenderPassKind, SceneRenderPassRecord,
        SceneRenderSourceExtentDomain, SceneRenderTargetKind, SceneResourceId, SceneResourceKind,
        SceneResourceRecord, SceneShaderContractRecord, SceneStorage, SceneStringId, SceneVec3,
        write_scene_binary,
    };

    #[test]
    fn scene_execution_plan_requires_descriptor_heap_and_fifo_latest_ready() {
        ...
        let plan = scene_execution_plan(&storage);
    ...
    #[test]
    fn scene_execution_plan_loads_gscene_from_scene_render_item() {
        let root = std::env::temp_dir().join(format!(
            "tensor-wallpaper-scene-item-plan-{}",
            std::process::id()
        ));
        let _ = std::fs::remove_dir_all(&root);
        std::fs::create_dir_all(&root).expect("tmp");
        let scene_path = root.join("scene.gscene");
        let document = minimal_scene_execution_document();
        let mut bytes = Vec::new();
        write_scene_binary(&document, &mut bytes).expect("write scene binary");
        std::fs::write(&scene_path, bytes).expect("scene file");
        let item = RenderingDeviceRenderItem::Scene {
            scene: Box::new(crate::renderer::rendering_device::RenderingDeviceSceneRenderItem {
        ...
        let plan = scene_execution_plan_from_render_item(&item)
            .expect("scene execution plan")
    ...
    fn minimal_scene_execution_document() -> SceneBinaryDocument {
        SceneBinaryDocument {
