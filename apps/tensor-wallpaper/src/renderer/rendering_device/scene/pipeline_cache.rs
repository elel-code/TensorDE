//! Vulkan scene pipeline cache planning.
//!
//! References:
...
use super::resource_storage::{
    RenderingDeviceSceneResourceStoragePlan, RenderingDeviceSceneShaderHeapSlice,
};
use super::shader_catalog::{
    BuiltinSceneShader, rendering_device_scene_shader_for_key,
    rendering_device_scene_vertex_spirv_for_primitive,
};

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceScenePipelineCachePlan {
    pub pipeline_count: usize,
    pub entries: Vec<RenderingDeviceScenePipelineCacheEntry>,
    pub shader_program_count: usize,
    pub shader_programs: Vec<RenderingDeviceSceneShaderProgramSet>,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceScenePipelineCacheEntry {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneShaderProgramSet {
    pub shader_key: SceneStringId,
    pub shader_catalog_key: &'static str,
    pub vertex_programs: Vec<RenderingDeviceSceneVertexProgram>,
    pub base_fragment_program: RenderingDeviceSceneSpirvProgram,
    pub local_read_fragment_program: Option<RenderingDeviceSceneSpirvProgram>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneVertexProgram {
    pub primitive: SceneRenderingDeviceDrawPrimitive,
    #[serde(flatten)]
    pub program: RenderingDeviceSceneSpirvProgram,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneSpirvProgram {
    pub spirv_bytes: usize,
    pub spirv_words: &'static [u32],
}

pub fn rendering_device_scene_pipeline_cache_plan(
    ...
    RenderingDeviceScenePipelineCachePlan {
        ...
        shader_programs,
        shader_catalog_entry_count: super::shader_catalog::rendering_device_scene_shader_catalog()
            .len(),
...
fn unresolved_draw_shader_keys(
    storage: &SceneStorage,
    entries: &[RenderingDeviceScenePipelineCacheEntry],
) -> Vec<String> {
    entries
        .iter()
...
fn pipeline_entry_for_slice(
    storage: &SceneStorage,
    slice: RenderingDeviceSceneShaderHeapSlice,
) -> RenderingDeviceScenePipelineCacheEntry {
...
fn shader_programs_for_entries(
    storage: &SceneStorage,
    entries: &[RenderingDeviceScenePipelineCacheEntry],
) -> Vec<RenderingDeviceSceneShaderProgramSet> {
    ...
    for entry in entries {
        ...
        if programs.iter().any(
            |program: &RenderingDeviceSceneShaderProgramSet| {
...
fn shader_program_set(
...
) -> RenderingDeviceSceneShaderProgramSet {
    ...
    let vertex_programs = PRIMITIVES
        ...
        .filter_map(|primitive| {
            rendering_device_scene_vertex_spirv_for_primitive(shader, primitive).map(|spirv| {
                RenderingDeviceSceneVertexProgram {
                    primitive,
...
fn spirv_program(spirv_words: &'static [u32]) -> RenderingDeviceSceneSpirvProgram {
    RenderingDeviceSceneSpirvProgram {
...
#[cfg(test)]
mod tests {
    use super::super::resource_storage::rendering_device_scene_resource_storage_plan;
    use super::*;
    use crate::engine::scene::{
    ...
    fn pipeline_cache_keys_builtin_shader_contracts_and_pass_state() {
        ...
        let resource_storage =
            rendering_device_scene_resource_storage_plan(&storage, render_plan, &graph);

        let cache = rendering_device_scene_pipeline_cache_plan(&storage, &resource_storage);
    ...
    fn complete_scene_owned_program_is_available_without_a_catalog_hit() {
        ...
        let render_plan = server.renderer_scene_render_plan();
        let graph = server.rendering_device_graph_plan();
        let resources = rendering_device_scene_resource_storage_plan(&storage, render_plan, &graph);

        let cache = rendering_device_scene_pipeline_cache_plan(&storage, &resources);
