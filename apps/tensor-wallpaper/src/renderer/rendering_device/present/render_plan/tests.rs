    ...
    use crate::renderer::rendering_device::{RenderingDeviceClearColor, RenderingDeviceRenderItem};
    use crate::renderer::{
    ...
    fn scene_color_display_overrides_default_clear_color() {
        let fallback = RenderingDeviceClearColor {
            r: 0.0,
            g: 0.0,
            b: 0.0,
            a: 1.0,
        ...
        let color = rendering_device_render_item_clear_color(&item, fallback);
    ...
    fn effect_uv_transform_selection_matches_mask_slot_before_fallback() {
        ...
        let selected = rendering_device_scene_effect_uv_transform_for_render_passes(&passes, Some(1))
            .expect("mask slot transform");
        assert_eq!(selected.mask_slot, 1);
        assert_eq!(selected.offset, [0.25, 0.0]);

        let fallback = rendering_device_scene_effect_uv_transform_for_render_passes(&passes, None)
    ...
    fn scene_draw_plan_keeps_opacity_mask_duplicate_as_independent_draw() {
        ...
        carrier.image_effect_passes = vec![SceneRenderImageEffectPass {
            ...
            runtime: Some("builtin-opacity-mask".to_owned()),
            pass_index: 0,
            command: None,
            source: None,
            target: None,
        ...
        let plan = rendering_device_scene_draw_plan_from_layers(
            0,
            None,
            FitMode::Cover,
            false,
        ...
        assert_eq!(
            ...
            Some(RenderingDeviceSceneEffectUvMapping::MaterialUvTransformed {
                scale_u: 1.0,
                scale_v: 1.0,
                offset_u: 0.0,
                offset_v: 0.0
