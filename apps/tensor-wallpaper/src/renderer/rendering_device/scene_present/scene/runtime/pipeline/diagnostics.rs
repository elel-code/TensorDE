...
pub(in crate::renderer::rendering_device) fn emit_scene_pipeline_diagnostics_if_requested(
    ...
    const ENV: &str = "TENSOR_WALLPAPER_RENDERING_DEVICE_SCENE_PIPELINE_DEBUG";
    ...
    for pass in graph.pass_nodes.iter().filter(|pass| {
        ...
        let draw_objects = graph.mesh_draws[draw_start..draw_end]
            ...
            .join(",");
        eprintln!(
            "tensor-wallpaper-scene-pipeline: graph={} pass={} record={} draws={}..{} objects=[{}] target={:?}:{:?} shader={:?} pipeline_blend={:?} scene_blend={:?} resolved_blend={} cull_mode={:?} color_write_mask={:?} target_format={:?} samples={:?} pipeline_index={} material_textures=[{}] material_constants=[{}]",
            pass.graph_index,
            pass.pass_id,
