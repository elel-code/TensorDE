//! Exact Slang scene-video graphics pipeline.
...
pub(super) fn create(
    ...
    let vertex = device
        .create_shader_module(ShaderModuleDescriptor {
            label: Some("tensor-wallpaper-scene-video-vertex".into()),
            spirv: SCENE_VIDEO_LAYER_VERTEX_SPIRV.to_vec(),
        })
        .map_err(|error| format!("create scene-video vertex shader: {error}"))?;
    let fragment = device
        .create_shader_module(ShaderModuleDescriptor {
            label: Some("tensor-wallpaper-scene-video-fragment".into()),
    ...
    let pipeline = device
        .create_machine_code_graphics_pipeline(&MachineCodeGraphicsPipelineDescriptor {
            pipeline: GraphicsPipelineDescriptor {
                label: Some("tensor-wallpaper-scene-video-layer"),
                vertex: VertexState {
