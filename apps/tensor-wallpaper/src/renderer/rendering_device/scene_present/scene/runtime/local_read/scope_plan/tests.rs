...
fn planner_keeps_adjacent_exact_pixel_passes_in_one_typed_scope() {
    ...
    let scopes =
        scene_local_read_scope_plans(&storage, &graph, &plans).expect("typed local-read scope");
    ...
    assert_eq!(
        scope.color_attachment_formats,
        [TextureFormat::Rgba8Unorm; 2]
    );
    assert_eq!(scope.input_slot, 0);
    assert_eq!(scope.input_attachment_index, 0);

    let producer_access = scene_pipeline_shader_descriptor_access(&storage, SceneStringId(0))
    ...
    let consumer_access = scene_pipeline_shader_descriptor_access(&storage, SceneStringId(1))
        .expect("consumer descriptor access");
    let consumer_shader = rendering_device_scene_shader_for_key("we/passthrough")
        .expect("passthrough")
        .local_read_shader
...
fn planner_rejects_non_adjacent_or_aliased_producer_instead_of_sampling() {
    ...
    graph
        .sampled_bindings
        .push(SceneRenderingDeviceSampledBinding {
            pass_node_index: 1,
            graph_index: 0,
            mesh_draw_start: 1,
            mesh_draw_count: 1,
...
fn effect_target_plan(
    ...
    SceneEffectTargetImagePlan {
        ...
        extent: Extent2D::new(extent[0], extent[1]),
        batch_field_count: 1,
