//! Retained Tensor Wallpaper scene state owned entirely by `vulkan-renderer` objects.
...
use super::descriptor_push::resolve_scene_shared_descriptor_pushes;
use super::pipeline::{
...
use super::shared_resources::{
    ...
    SharedSceneFramePayloads, SharedSceneFrameResources, particle_frame_state,
};
...
pub(super) struct SharedSceneGpuResources {
    ...
    pub particle_frame_scratch: Vec<f32>,
...
impl SharedSceneGpuResources {
    pub(super) fn gpu_timing_graphs(
        &self,
    ) -> Result<Vec<super::gpu_timing::SceneGpuTimingGraphDescriptor>, String> {
        use super::gpu_timing::{SceneGpuTimingGraphDescriptor, SceneGpuTimingPassDescriptor};
        use execution_plan::SharedSceneFrameStep;

        let phase = self.effect_execution_plans.first().ok_or_else(|| {
            "shared scene GPU timing requires an effect reference phase".to_owned()
        })?;
        self.frame_execution_plan
            .graphs
            .iter()
            .map(|graph| {
                let passes = graph
                    ...
                    .iter()
                    .map(|step| match *step {
                        SharedSceneFrameStep::SceneColor(range) => {
                            let pass = self.graph.pass_nodes.iter().find(|pass| {
                                pass.graph_index == graph.graph_index
                                    && pass.mesh_draw_start == range.start
                                    && pass.mesh_draw_count == range.count
                            }).ok_or_else(|| format!(
                                "shared scene GPU timing cannot locate graph {} SceneColor pass {}+{}",
                                graph.graph_index, range.start, range.count,
                            ))?;
                            Ok(SceneGpuTimingPassDescriptor {
                                label: "scene-color".into(),
                                pass_record_indices: vec![pass.pass_record_index],
                            })
                        }
                        SharedSceneFrameStep::Effect { command_index } => {
                            let command = phase.commands.get(command_index).ok_or_else(|| {
                                format!("shared scene GPU timing effect command {command_index} is missing")
                            })?;
                            Ok(SceneGpuTimingPassDescriptor {
                                label: effect_timing_label(command.kind).into(),
                                pass_record_indices: vec![command.pass_record_index],
                            })
                        }
                        SharedSceneFrameStep::LocalReadPair {
                            ...
                            let producer = phase.commands.get(producer_command_index).ok_or_else(|| {
                                format!("shared scene GPU timing local-read producer {producer_command_index} is missing")
                            })?;
                            let consumer = phase.commands.get(consumer_command_index).ok_or_else(|| {
                                format!("shared scene GPU timing local-read consumer {consumer_command_index} is missing")
                            })?;
                            Ok(SceneGpuTimingPassDescriptor {
                                label: "local-read-pair".into(),
                                pass_record_indices: vec![
                                    producer.pass_record_index,
                ...
                Ok(SceneGpuTimingGraphDescriptor {
    ...
    pub(super) fn create(
        ...
        let graph = execution_plan.rendering_device_graph;
        ...
        let particle_global_descriptor_base =
            (!graph.particle_gpu_emitters.is_empty()).then(|| {
                ...
                resource_slot_kinds.extend([DescriptorSlotKind::StorageBuffer; 5]);
                base
            });
        ...
        let particle_frame_scratch = particle_frame_state(&graph, 0.0, 0.0)?;
        ...
        Ok(Self {
            ...
            scene_color_attachment_clear_enabled: std::env::var_os(
                "TENSOR_WALLPAPER_RENDERING_DEVICE_DISABLE_SCENE_COLOR_ATTACHMENT_CLEAR",
            )
            .is_none(),
...
fn effect_timing_label(kind: super::effect_target::SharedSceneEffectCommandKind) -> &'static str {
    use super::effect_target::SharedSceneEffectCommandKind;
    match kind {
        SharedSceneEffectCommandKind::Copy { .. } => "copy",
        SharedSceneEffectCommandKind::SwapReferences { .. } => "swap-references",
        SharedSceneEffectCommandKind::DynamicRender { .. } => "dynamic-render",
    }
}
