//! Typed push-data payloads for `SPV_EXT_descriptor_heap` scene shaders.
...
pub(super) enum SceneDescriptorPush {
    EngineBuiltIn(Vec<u8>),
    SceneOwned(Vec<u8>),
}

impl SceneDescriptorPush {
...
fn validate_descriptor_push_size(
    key: &str,
    push: &SceneDescriptorPush,
...
fn sampled_slot_index(
    ...
    layout
        ...
        .ok_or_else(|| {
            format!("scene shader sampled-image register {register} is not in the retained layout")
        })
}
