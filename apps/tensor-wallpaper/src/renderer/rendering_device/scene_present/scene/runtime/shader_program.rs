...
pub(super) fn resolve_scene_graphics_program(
    ...
    if key.starts_with("workshop/") {
        return Err(format!(
            "package-owned graphics program {key:?} has no embedded generated stages"
        ));
    }
    let shader = rendering_device_scene_shader_for_key(key)
        .ok_or_else(|| format!("engine-owned scene shader {key:?} is not built in"))?;
    let vertex =
        rendering_device_scene_vertex_shader_for_primitive(shader, primitive).ok_or_else(|| {
...
mod tests {
    ...
    fn package_identity_never_falls_back_to_the_static_catalog() {
        ...
        assert!(error.contains("no embedded generated stages"));
    }
