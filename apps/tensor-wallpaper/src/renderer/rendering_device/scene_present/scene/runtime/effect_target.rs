...
pub(in crate::renderer::rendering_device) use local_read_usage::{
...
pub(super) fn scene_logical_target_extent(
    storage: &SceneStorage,
    allocation: SceneRenderingDeviceTargetAllocation,
    surface_extent: Extent2D,
) -> Result<Extent2D, String> {
    logical_target_extent(storage, allocation, surface_extent)
}

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::renderer::rendering_device) struct SceneEffectTargetImagePlan {
...
pub(in crate::renderer::rendering_device) struct SceneEffectTargetCommandPlan {
...
pub(in crate::renderer::rendering_device) struct SceneEffectTargetCommand {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::rendering_device) struct SceneEffectTargetTimingCommand {
...
pub(in crate::renderer::rendering_device) fn scene_effect_target_timing_commands(
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::rendering_device) struct LogicalEffectTargetKey {
    graph_index: u32,
...
pub(in crate::renderer::rendering_device) fn scene_effect_target_image_plan(
    storage: &SceneStorage,
...
pub(in crate::renderer::rendering_device) fn scene_effect_target_command_plan(
    commands: &[SceneEffectTargetCommand],
    graph: &SceneRenderingDeviceGraphPlan,
            ...
            match command.kind {
                SceneEffectTargetCommandKind::DynamicRender => {
                    if command.batch_atlas_tile.is_none() {
                        ...
                        plan.discard_load_count += usize::from(command.fully_overwrites_target);
                    }
                }
                SceneEffectTargetCommandKind::Copy => {
                    plan.copy_command_count += usize::from(!command.direct_scene_color_snapshot);
                }
...
pub(in crate::renderer::rendering_device) fn scene_effect_target_commands(
    storage: &SceneStorage,
    graph: &SceneRenderingDeviceGraphPlan,
            ...
            match pass.role {
                ...
                _ => Some(SceneEffectTargetCommand {
                    ...
                    clear_before_draw: pass.role == SceneRenderPassKind::Clear
                        || storage.document().render_passes[pass.pass_record_index as usize]
                            .clear_target,
...
pub(in crate::renderer::rendering_device) fn effect_batch_instance_count(
    commands: &[SceneEffectTargetCommand],
) -> usize {
    commands
        .iter()
