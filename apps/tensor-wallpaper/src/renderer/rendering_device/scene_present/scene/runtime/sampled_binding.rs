...
pub(in crate::renderer::rendering_device) enum SceneVideoPlane {
    Y,
    Uv,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::rendering_device) enum SceneSampledImageSource {
    FallbackWhite,
    SceneTexture {
...
pub(in crate::renderer::rendering_device) struct SceneSampledImageBindingPlan {
...
pub(in crate::renderer::rendering_device) fn scene_sampled_image_binding_plan(
...
pub(in crate::renderer::rendering_device) fn scene_sampled_image_binding_cycle(
    graph: &SceneRenderingDeviceGraphPlan,
    sampled_slots: &[u32],
    input_attachment_slots: &[u32],
    ...
    if std::env::var_os("TENSOR_WALLPAPER_RENDERING_DEVICE_SCENE_SAMPLED_BINDING_DEBUG").is_some() {
            ...
            for draw_index in 0..graph.mesh_draws.len() {
                for (sampled_index, slot) in sampled_slots.iter().copied().enumerate() {
                    ...
                    if !matches!(source, SceneSampledImageSource::FallbackWhite) {
                        eprintln!(
                            "tensor-wallpaper-sampled-binding: phase={phase} draw={draw_index} slot={slot} source={source:?}"
                        );
                    }
                }
            }
...
fn lower_pass_sampled_bindings(lowering: PassSampledBindingLowering<'_>) -> Result<(), String> {
...
pub(in crate::renderer::rendering_device) fn target_is_direct_scene_color_snapshot(
    graph: &SceneRenderingDeviceGraphPlan,
    graph_index: u32,
