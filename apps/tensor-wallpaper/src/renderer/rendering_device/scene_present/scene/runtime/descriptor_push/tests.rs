...
fn built_in_fragment_push_uses_shared_dense_heap_indices() {
    ...
    let plan = dense_plan(&resource_kinds, 1);
    let shader = crate::renderer::rendering_device::scene::rendering_device_scene_shader_for_key(
        "tensor-wallpaper/dynamic-text",
    )
...
fn built_in_clipping_push_uses_authored_slot_identity_not_declaration_order() {
    ...
    assert_eq!(push_words(&push), [3, 4, 5, 11, 6, 0, 1, 2, 8, 3, 1, 0, 2]);
}
...
fn built_in_local_read_push_uses_the_input_attachment_heap_lane() {
    ...
    let plan = dense_plan(&resource_kinds, 1);
    let shader =
        crate::renderer::rendering_device::scene::rendering_device_scene_shader_for_key("we/passthrough")
...
fn effect_passthrough_pipeline_selects_its_independent_shared_push() {
    ...
    draw.descriptor_push = Some(SceneDescriptorPush::EngineBuiltIn(vec![1]));
    draw.disabled_descriptor_push = Some(SceneDescriptorPush::EngineBuiltIn(vec![2]));

    assert_eq!(draw.active_descriptor_push().unwrap().bytes(), &[1]);
    draw.pipeline_index = 4;
    assert_eq!(draw.active_descriptor_push().unwrap().bytes(), &[2]);
}
...
fn push_words(push: &SceneDescriptorPush) -> Vec<u32> {
    push.bytes()
        .chunks_exact(4)
