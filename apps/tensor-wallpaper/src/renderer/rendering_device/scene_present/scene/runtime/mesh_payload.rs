...
use super::fullscreen_primitive::{
    append_authored_texture_quad_vertices, append_fullscreen_triangle_indices,
    append_fullscreen_triangle_vertices,
};
...
pub(super) fn pack_scene_vertices(
    ...
    let vertex_count = storage
        ...
        .mesh_vertices
        .len()
        .saturating_add(graph.bound_utility_vertex_count(storage))
    ...
    for draw in graph.mesh_draws.iter().filter(|draw| {
        ...
        append_authored_texture_quad_vertices(&mut payload, draw.authored_source_extent);
    }
    for draw in graph
        .mesh_draws
        .iter()
