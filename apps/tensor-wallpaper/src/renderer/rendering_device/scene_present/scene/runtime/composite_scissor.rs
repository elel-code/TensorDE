...
impl LocalBounds {
    fn include(&mut self, position: [f32; 3]) {
        for (lane, value) in position.into_iter().enumerate() {
            self.min[lane] = self.min[lane].min(value);
            self.max[lane] = self.max[lane].max(value);
        }
    }
    ...
    while graph_pass_start < graph.pass_nodes.len() {
        ...
        for pass in graph_passes {
            ...
            if has_object_composite {
                ...
                {
                    let Some(draw_bounds) = object_mesh_pixel_bounds_from_coverage(
                        storage,
                        coverage,
                        graph,
                        draw,
            ...
            if flat_rounded_support_quad_shader(shader) {
                let scissors = draws
                    ...
                    .collect::<Vec<_>>();
                if std::env::var("TENSOR_WALLPAPER_RENDERING_DEVICE_SCENE_SCISSOR_DEBUG")
                    .ok()
                ...
                {
                    eprintln!(
                        "tensor-wallpaper-flat-rounded-mask-scissor: graph={graph_index} scissors={scissors:?}"
                    );
                }
                ...
                || shader.eq_ignore_ascii_case("we/objectcomposite-screen-group")
            {
                ...
                {
                    eprintln!(
                        "tensor-wallpaper-scene-composite-scissor: graph={graph_index} bounded={coverage_is_bounded} bounds={bounds:?} scissor={scissor:?}"
                    );
                }
...
fn object_mesh_pixel_bounds_from_coverage(
    ...
    for (local_bone, bounds) in &mesh.bone_bounds {
        ...
        let bone_bounds =
            projected_local_bounds(*bounds, Some(bone.matrix), transform, output_extent)?;
...
#[cfg(test)]
#[path = "composite_scissor/tests.rs"]
mod tests;
