...
pub(in crate::renderer::rendering_device::scene_present::scene::runtime) enum SceneColorCopyCoverage {
    #[default]
    FullTarget,
...
pub(super) fn scene_color_copy_coverage(
    ...
    let Some(shader) = rendering_device_scene_shader_for_key(shader_key) else {
        return SceneColorCopyCoverage::FullTarget;
    };
...
pub(in crate::renderer::rendering_device) fn graph_copies_scene_color(
...
pub(in crate::renderer::rendering_device) fn graph_requires_effect_target_execution(
    commands: &[SceneEffectTargetCommand],
    graph_index: u32,
) -> bool {
    commands.iter().any(|command| {
...
pub(in crate::renderer::rendering_device) fn graph_uses_direct_scene_color_snapshot(
    commands: &[SceneEffectTargetCommand],
    graph_index: u32,
) -> bool {
    commands.iter().any(|command| {
