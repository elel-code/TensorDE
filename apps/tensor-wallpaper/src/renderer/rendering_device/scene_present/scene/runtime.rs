//! Tensor Wallpaper scene runtime over the shared `vulkan-renderer` ownership root.
...
use crate::renderer::wayland::{WaylandHostOptions, WaylandSurfaceSnapshot};
...
pub use semantic_diagnostics::RenderingDeviceSceneSemanticDiagnosticsSnapshot;
...
const SLANG_CONSTANT_BUFFER_DESCRIPTOR_KIND: vulkan_renderer::DescriptorSlotKind =
    vulkan_renderer::DescriptorSlotKind::UniformBuffer;

#[derive(Debug, Clone, PartialEq)]
pub(in crate::renderer::rendering_device) struct RenderingDeviceScenePresentOptions {
    ...
    pub video_sources: Vec<crate::renderer::rendering_device::scene::RenderingDeviceSceneVideoSource>,
}

#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneQueueSnapshot {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneDeviceSnapshot {
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneSwapchainSnapshot {
    pub created: bool,
    pub format: String,
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct RenderingDeviceSceneScriptEffectVisibilitySnapshot {
...
pub struct RenderingDeviceScenePresentSnapshot {
    ...
    /// agree with `swapchain.extent` on Tensor Wallpaper's exact-extent path.
    pub wayland_surface: WaylandSurfaceSnapshot,
    pub selected_queue: RenderingDeviceSceneQueueSnapshot,
    pub device: RenderingDeviceSceneDeviceSnapshot,
    pub swapchain: RenderingDeviceSceneSwapchainSnapshot,
    ...
    pub script_effect_visibility: Vec<RenderingDeviceSceneScriptEffectVisibilitySnapshot>,
    #[serde(skip_serializing_if = "Option::is_none")]
    pub semantic_diagnostics: Option<RenderingDeviceSceneSemanticDiagnosticsSnapshot>,
    ...
    pub particle_gpu_simulation_bytes: u64,
...
pub(in crate::renderer::rendering_device) fn run_rendering_device_scene_present(
    options: RenderingDeviceScenePresentOptions,
...
fn scene_script_effect_visibility_snapshot(
...
) -> Result<Vec<RenderingDeviceSceneScriptEffectVisibilitySnapshot>, String> {
        ...
        .filter(|delta| delta.target == SceneScriptTarget::EffectVisible)
    {
        let binding = storage
            ...
            .ok_or_else(|| {
                format!(
                    "script effect visibility binding {} is missing",
                    delta.selector
                )
            })?;
        ...
        let object = storage
            ...
            .ok_or_else(|| {
                format!(
                    "script effect visibility object {} is missing",
                    delta.object.0
                )
            })?;
        let effect_index = delta
            ...
            .ok_or_else(|| {
                format!(
                    "effect binding {} is outside object {}",
                    delta.selector, object.id.0
                )
            })?;
        ...
        snapshots.push(RenderingDeviceSceneScriptEffectVisibilitySnapshot {
