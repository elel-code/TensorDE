...
struct RenderingDeviceAudioEventChannelSample {
    generation: u64,
    sample_time_ns: u64,
...
pub(in crate::renderer::rendering_device) struct RenderingDeviceAudioEventChannel {
    state: Arc<Mutex<RenderingDeviceAudioEventChannelState>>,
}

#[derive(Debug, Default)]
struct RenderingDeviceAudioEventChannelState {
    latest: Option<RenderingDeviceAudioEventChannelSample>,
    normalizer: SpectrumNormalizer,
    last_capture_time_ns: Option<u64>,
}

impl RenderingDeviceAudioEventChannel {
    pub(in crate::renderer::rendering_device) fn publish(
        &self,
        generation: u64,
        sample_time_ns: u64,
        spectrum: StereoSpectrum64,
        ...
        if let Ok(mut state) = self.state.lock() {
            state.latest = Some(RenderingDeviceAudioEventChannelSample {
                generation,
                sample_time_ns,
                spectrum,
            });
    ...
    pub(in crate::renderer::rendering_device) fn capture(
        &self,
        generation: u64,
...
#[derive(Debug, Default)]
pub(in crate::renderer::rendering_device) struct RenderingDeviceAudioEventSource {
    normalizer: SpectrumNormalizer,
    last_capture_time_ns: Option<u64>,
}

impl RenderingDeviceAudioEventSource {
    pub(in crate::renderer::rendering_device) fn capture(
        &mut self,
        sample_time_ns: u64,
    ) -> SceneAudioState {
    ...
    pub(in crate::renderer::rendering_device) fn status(&self) -> (&'static str, bool) {
...
pub(in crate::renderer::rendering_device) fn audio_state_summary(
...
fn diagnostic_spectrum64() -> Option<StereoSpectrum64> {
    *DIAGNOSTIC_SPECTRUM64.get_or_init(|| {
        std::env::var("TENSOR_WALLPAPER_SCENE_AUDIO_SPECTRUM64")
            .ok()
...
mod tests {
    ...
    fn media_channel_rejects_spectrum_from_another_generation() {
        let channel = RenderingDeviceAudioEventChannel::default();
        channel.publish(
            2,
            10,
            StereoSpectrum64 {
