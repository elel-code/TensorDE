//! Tensor Wallpaper presentation policy lowered into shared renderer descriptors.
...
pub(in crate::renderer::rendering_device) fn tensor_wallpaper_presentation_bootstrap_descriptor(
    label: &str,
...
fn pipeline_binary_cache_root() -> Result<PathBuf, String> {
    const CACHE_DIRECTORY_ENV: &str = "TENSOR_WALLPAPER_PIPELINE_BINARY_CACHE_DIR";
    ...
    fn cache_path_has_product_and_payload_scope() {
        assert_eq!(
            PathBuf::from("/tmp/example-home")
                ...
                .join("tensor-wallpaper")
                .join("vulkan-pipeline-binaries"),
            PathBuf::from("/tmp/example-home/.cache/tensor-wallpaper/vulkan-pipeline-binaries")
        );
    }
}
