...
pub(super) fn rendering_device_video_fit_push_constants(
    fit: FitMode,
    source_size: (u32, u32),
    surface_size: (u32, u32),
) -> [f32; 4] {
    let (offset, scale) = rendering_device_video_uv_transform(fit, source_size, surface_size);
...
fn rendering_device_video_uv_transform(
    fit: FitMode,
    source_size: (u32, u32),
    surface_size: (u32, u32),
) -> ([f32; 2], [f32; 2]) {
...
mod tests {
    ...
    fn flips_video_sampling_to_top_left_frame_origin() {
        let push =
            rendering_device_video_fit_push_constants(FitMode::Stretch, (1920, 1080), (1920, 1080));
    ...
    fn preserves_vertical_cover_crop_while_flipping() {
        let push =
            rendering_device_video_fit_push_constants(FitMode::Cover, (1440, 1080), (1920, 1080));
