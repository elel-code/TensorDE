...
pub(super) struct RenderingDeviceTimelinePtsDeltaSummary {
...
pub(super) fn rendering_device_timeline_pts_delta_summary<I>(
    target_fps: Option<u32>,
    deltas_ms: I,
) -> RenderingDeviceTimelinePtsDeltaSummary
where
    I: IntoIterator<Item = Option<u64>>,
{
    ...
    let (expected_min_ms, expected_max_ms) =
        rendering_device_timeline_expected_pts_delta_range_ms(target_fps);
...
fn rendering_device_timeline_expected_pts_delta_range_ms(
    target_fps: Option<u32>,
...
pub(super) fn rendering_device_timeline_loop_boundary_reset(
    timeline_item_index: u32,
    frame_serial: u32,
    previous_frame_serial: u32,
) -> bool {
    timeline_item_index > 0
        && rendering_device_timeline_frame_serial_stale(frame_serial, previous_frame_serial)
}

pub(super) fn rendering_device_timeline_frame_serial_stale(
    frame_serial: u32,
...
mod tests {
    ...
    fn computes_expected_pts_delta_range_from_target_fps() {
        assert_eq!(
            rendering_device_timeline_expected_pts_delta_range_ms(Some(240)),
            (Some(4), Some(5))
        );
        assert_eq!(
            rendering_device_timeline_expected_pts_delta_range_ms(Some(60)),
            (Some(16), Some(17))
        );
    }
    ...
    fn marks_pts_delta_range_valid_when_inside_frame_period_bounds() {
        let summary = rendering_device_timeline_pts_delta_summary(Some(240), [Some(4), Some(5)]);
    ...
    fn marks_pts_delta_range_invalid_when_outside_frame_period_bounds() {
        let summary = rendering_device_timeline_pts_delta_summary(Some(240), [Some(3), Some(6)]);
        assert_eq!(summary.in_expected_range, Some(false));
    }

    #[test]
    fn loop_boundary_reset_requires_non_initial_serial_change() {
        assert!(!rendering_device_timeline_loop_boundary_reset(0, 1, 0));
        assert!(!rendering_device_timeline_loop_boundary_reset(10, 1, 1));
        assert!(rendering_device_timeline_loop_boundary_reset(10, 2, 1));
    }

    #[test]
    fn frame_serial_stale_follows_ffplay_queue_serial_rule() {
        assert!(!rendering_device_timeline_frame_serial_stale(3, 3));
        assert!(rendering_device_timeline_frame_serial_stale(2, 3));
    }
}
