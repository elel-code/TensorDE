...
use crate::renderer::rendering_device::audio::event_source::{
    RenderingDeviceAudioEventChannel, audio_state_summary,
};

static NEXT_MEDIA_SESSION_ID: AtomicU64 = AtomicU64::new(1);

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::rendering_device) struct RenderingDeviceVideoEventSample {
    pub generation: u64,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::rendering_device) struct RenderingDeviceMediaEventSource {
    session: SceneMediaSessionId,
}

#[derive(Debug, Clone, PartialEq, Serialize)]
pub struct RenderingDeviceMediaEventRuntimeSnapshot {
...
pub(in crate::renderer::rendering_device) struct RenderingDeviceMediaEventRuntime {
    source: RenderingDeviceMediaEventSource,
    audio: RenderingDeviceAudioEventChannel,
...
impl RenderingDeviceMediaEventRuntime {
    pub(in crate::renderer::rendering_device) fn new(audio: RenderingDeviceAudioEventChannel) -> Self {
        let session_identity = NEXT_MEDIA_SESSION_ID.fetch_add(1, Ordering::Relaxed);
        Self {
            source: RenderingDeviceMediaEventSource::new(session_identity),
            audio,
    ...
    pub(in crate::renderer::rendering_device) fn publish_presented_frame(
        &mut self,
        sample: RenderingDeviceVideoEventSample,
    ) -> &SceneFrameEvents {
    ...
    pub(in crate::renderer::rendering_device) fn snapshot(
        &self,
    ) -> RenderingDeviceMediaEventRuntimeSnapshot {
...
impl RenderingDeviceMediaEventSource {
    pub(in crate::renderer::rendering_device) fn next() -> Self {
        Self::new(NEXT_MEDIA_SESSION_ID.fetch_add(1, Ordering::Relaxed))
    }

    pub(in crate::renderer::rendering_device) fn new(session_identity: u64) -> Self {
        Self {
            session: SceneMediaSessionId(session_identity),
        }
    }

    pub(in crate::renderer::rendering_device) fn publish_video(
        self,
    ...
    pub(in crate::renderer::rendering_device) fn publish_audio(
        self,
        queue: &mut SceneEventQueue,
        generation: u64,
        sample_time_ns: u64,
...
mod tests {
    ...
    fn audio_and_video_share_media_session_and_generation() {
        let source = RenderingDeviceMediaEventSource::new(42);
        let mut queue = SceneEventQueue::default();
        source.publish_video(
            &mut queue,
            RenderingDeviceVideoEventSample {
                generation: 3,
                frame_serial: 8,
                frame_identity: 88,
    ...
    fn retained_runtime_publishes_one_coherent_frame_snapshot() {
        let audio = RenderingDeviceAudioEventChannel::default();
        audio.publish(
            4,
            1_500_000,
            StereoSpectrum64 {
        ...
        let mut runtime = RenderingDeviceMediaEventRuntime::new(audio);
        runtime.publish_presented_frame(RenderingDeviceVideoEventSample {
            generation: 4,
            frame_serial: 9,
            frame_identity: 99,
