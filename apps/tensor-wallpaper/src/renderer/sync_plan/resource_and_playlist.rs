...
#[path = "resource_and_playlist/resource_and_output.rs"]
mod resource_and_output;

pub(super) use resource_and_output::*;
...
pub(super) fn playlist_weighted_selection_seed(
    ...
    "tensor-wallpaper-playlist-weighted-random-v1".hash(&mut hasher);
...
pub(super) fn playlist_local_time_override() -> Option<u16> {
    std::env::var("TENSOR_WALLPAPER_PLAYLIST_LOCAL_TIME")
        .ok()
        .as_deref()
...
pub(super) fn playlist_local_weekday_override() -> Option<PlaylistWeekday> {
    std::env::var("TENSOR_WALLPAPER_PLAYLIST_LOCAL_WEEKDAY")
        .ok()
        .as_deref()
