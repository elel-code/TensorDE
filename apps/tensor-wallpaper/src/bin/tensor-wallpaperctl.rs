...
#[path = "tensor-wallpaperctl/telemetry_csv.rs"]
mod telemetry_csv;
...
fn run() -> Result<(), String> {
    ...
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        println!("{}", tensor_wallpaper::ipc::help_text());
        return Ok(());
    }
    ...
    let socket = env::var_os("TENSOR_WALLPAPER_SOCKET")
        .map(PathBuf::from)
        .or_else(tensor_wallpaper::ipc::runtime_socket_path)
        .ok_or_else(|| {
    ...
    if matches!(command, tensor_wallpaper::ipc::ClientCommand::Watch) {
...
struct Invocation {
    command: tensor_wallpaper::ipc::ClientCommand,
...
fn parse_invocation(args: &[String]) -> Result<Invocation, String> {
    match args {
        ...
        _ => Ok(Invocation {
            command: tensor_wallpaper::ipc::parse_client_args(args)?,
            format: ResponseFormat::Json,
            response_file: None,
        }),
    }
...
#[cfg(test)]
#[path = "tensor-wallpaperctl/tests.rs"]
mod tests;
