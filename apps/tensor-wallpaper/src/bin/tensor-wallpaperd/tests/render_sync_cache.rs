...
fn current_render_sync_cache_invalidates_when_manifest_changes() {
    let package_dir = TestDir::new("tensor-wallpaper-render-sync-cache-package");
...
fn current_render_sync_cache_ignores_existing_output_properties() {
    let package_dir = TestDir::new("tensor-wallpaper-render-sync-property-cache-package");
    write_static_package_manifest(package_dir.path(), "#101418");
...
fn current_render_sync_cache_ignores_non_render_config() {
    let package_dir = TestDir::new("tensor-wallpaper-render-sync-config-cache-package");
    write_static_package_manifest(package_dir.path(), "#101418");
    ...
    context.config.outputs.insert(
        "eDP-1".to_owned(),
        OutputConfig {
            fit: Some(tensor_wallpaper::core::FitMode::Contain),
            ..OutputConfig::default()
        },
    );
    let updated = current_render_sync(&mut context);
    assert_ne!(updated, cached);
    assert_eq!(updated.plans[0].fit, tensor_wallpaper::core::FitMode::Contain);
}

#[test]
fn current_render_sync_cache_invalidates_when_cache_policy_changes() {
    let package_dir = TestDir::new("tensor-wallpaper-render-sync-cache-config-package");
    write_static_package_manifest(package_dir.path(), "#101418");
