fn persist_or_error(id: &serde_json::Value, context: &DaemonContext) -> Option<String> {
    tensor_wallpaper::state::save_state(&context.paths.state_file, &context.state)
        .err()
        .map(|err| tensor_wallpaper::ipc::error_response(Some(id), "internal_error", &err.to_string()))
}

fn refresh_desktop(context: &mut DaemonContext) {
    context.desktop = tensor_wallpaper::desktop::adapters::read_desktop_snapshot(&context.config.adapters);
    ...
    names
        ...
        .map(|name| {
            ...
            let performance = render_sync
                ...
                .unwrap_or_else(|| {
                    let performance = tensor_wallpaper::policy::decide_performance(
                        &performance_config,
                        &context.desktop,
                        desktop_output,
                        &state,
                    );
                    tensor_wallpaper::policy::apply_adaptive_policy(
                        performance,
                        &context.config,
                        &name,
                        desktop_output,
...
fn renderer_action_response(
    ...
    if !cfg!(any(
        feature = "rendering-device",
        feature = "wayland-renderer"
    )) {
        result["note"] = json!("renderer was built without the rendering-device feature");
    }
    tensor_wallpaper::ipc::success_response(id, result)
}
...
fn renderer_name() -> &'static str {
    match (
        ...
        (true, true) => "rendering-device+wayland-host",
        (true, false) => "rendering-device",
        (false, true) => "wayland-host",
...
fn renderer_capabilities() -> Value {
    json!({
        "gtk": null,
        "wayland": wayland_renderer_capabilities(),
        "rendering_device": rendering_device_renderer_capabilities(),
...
fn adaptive_action_report(context: &DaemonContext) -> Value {
    ...
    let actions = names
        .into_iter()
        .filter(|name| tensor_wallpaper::adaptive::output_enabled(&context.config, name))
        .map(|name| {
            let desktop_output = context.desktop.output(&name);
            match tensor_wallpaper::adaptive::output_action(&context.config, &name) {
                tensor_wallpaper::config::AdaptiveAction::PauseUnfocused
                ...
                tensor_wallpaper::config::AdaptiveAction::PauseDynamic => {
                    json!({
                        "output_name": name,
                        "type": "pause-dynamic",
                ...
                action => {
                    let max_fps = tensor_wallpaper::adaptive::output_throttle_max_fps(&context.config, &name);
                    json!({
                        "output_name": name,
                        "type": "throttle",
...
fn video_renderer_capabilities() -> Value {
    json!({
        "built": cfg!(feature = "video"),
        "headless_worker": false,
        "visible_surface_path": if cfg!(feature = "video") {
            Some("video")
        } else {
            None::<&str>
        },
...
#[cfg(feature = "wayland-renderer")]
fn wayland_renderer_capabilities() -> Value {
    json!(tensor_wallpaper::renderer::wayland::capabilities())
}

#[cfg(not(feature = "wayland-renderer"))]
fn wayland_renderer_capabilities() -> Value {
    json!({
        "built": false,
        "experimental": false,
...
#[cfg(feature = "rendering-device")]
fn rendering_device_renderer_capabilities() -> Value {
    json!({
        "capabilities": tensor_wallpaper::renderer::rendering_device::capabilities(),
        "presentation_contract":
            tensor_wallpaper::renderer::rendering_device::wallpaper_presentation_contract(),
    })
}
...
fn rendering_device_renderer_capabilities() -> Value {
    json!({
        "capabilities": {
            ...
            "reuses_wayland_host": false,
            "owns_layer_shell_surface_now": false,
            "renderer_owns_vulkan_instance": false,
            "renderer_owns_vulkan_device": false,
            "renderer_owns_wayland_vulkan_surface": false,
            "renderer_owns_swapchain": false,
            "renders_frames_now": false,
            "consumes_render_sync": false,
            "direct_video_memory_status": "not built",
            "unsafe_policy": "unsafe is not used by this build",
        },
        "presentation_contract": null,
    })
}
...
fn current_render_sync(context: &mut DaemonContext) -> StaticRenderSyncPlan {
    ...
    let render_sync = tensor_wallpaper::renderer::static_render_sync_plan_with_config_and_adaptive(
        &context.config,
        &context.desktop,
        &context.state,
...
fn render_sync_cache_key_for_plan(
    ...
    RenderSyncCacheKey {
        ...
        playlist_clock: render_sync.and_then(|render_sync| {
            tensor_wallpaper::renderer::current_playlist_clock_cache_key(
...
fn render_sync_config_key(config: &TensorWallpaperConfig) -> RenderSyncConfigKey {
    RenderSyncConfigKey {
    ...
    fn new(path: String) -> Self {
                ...
                == Some("gwpdir")
        {
            Some(PackageManifestFingerprint {
                json: metadata_fingerprint(&package_path.join(tensor_wallpaper::core::MANIFEST_FILE)),
                toml: metadata_fingerprint(&package_path.join(tensor_wallpaper::core::MANIFEST_TOML_FILE)),
            })
        } else {
            None
        };
...
fn runtime_desktop_refresh_interval(runtime: &DaemonRuntime) -> Duration {
    match runtime.lock_context() {
        ...
        Err(err) => {
            eprintln!("tensor-wallpaperd: failed to read desktop refresh interval: {err}");
            desktop_refresh_interval(&PerformanceConfig::default())
        }
    }
}

fn adaptive_refresh_interval(config: &tensor_wallpaper::config::AdaptiveConfig) -> Duration {
