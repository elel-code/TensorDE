...
mod tests {
    ...
    fn read_requests_refresh_desktop_only_after_interval() {
        ...
        context.config.adapters = tensor_wallpaper::config::AdapterConfig {
            generic_wayland: false,
            hyprland: false,
            niri: false,
        };
        context.config.performance.desktop_refresh_interval_ms = 1_000;
        context.desktop.outputs = vec![tensor_wallpaper::desktop::DesktopOutput::virtual_output("eDP-1")];
    ...
    fn status_response_reports_daemon_telemetry() {
        let mut context = test_context();
        let request = tensor_wallpaper::ipc::IpcRequest {
            id: json!(1),
    ...
    fn output_reports_apply_output_performance_override() {
        ...
        context.config.outputs.insert(
            "eDP-1".to_owned(),
            tensor_wallpaper::config::OutputConfig {
                performance: tensor_wallpaper::config::OutputPerformanceConfig {
                    interactive_max_fps: Some(42),
                    ..tensor_wallpaper::config::OutputPerformanceConfig::default()
                },
                ..tensor_wallpaper::config::OutputConfig::default()
            },
        );
    ...
    fn output_reports_apply_adaptive_throttle() {
        ...
        context.adaptive_snapshot = tensor_wallpaper::adaptive::AdaptiveSnapshot {
            monitoring_enabled: true,
            active_triggers: vec![tensor_wallpaper::adaptive::AdaptiveTrigger {
                metric: tensor_wallpaper::adaptive::AdaptiveMetric::CpuPressureSomeAvg10,
                value_x100: 9_000,
                threshold_x100: 7_500,
            }],
            ..tensor_wallpaper::adaptive::AdaptiveSnapshot::default()
        };
        let reports = output_reports(&context, None);
    ...
    fn output_reports_apply_adaptive_pause_unfocused() {
        let mut context = test_context();
        context.desktop.outputs = vec![tensor_wallpaper::desktop::DesktopOutput {
            focused: false,
            ..tensor_wallpaper::desktop::DesktopOutput::virtual_output("eDP-1")
        }];
        context.config.adaptive.enabled = true;
        context.config.adaptive.action = tensor_wallpaper::config::AdaptiveAction::PauseUnfocused;
        context.adaptive_snapshot = tensor_wallpaper::adaptive::AdaptiveSnapshot {
            monitoring_enabled: true,
            active_triggers: vec![tensor_wallpaper::adaptive::AdaptiveTrigger {
                metric: tensor_wallpaper::adaptive::AdaptiveMetric::CpuPressureSomeAvg10,
    ...
    fn output_reports_prefer_render_sync_final_performance_decision() {
        let test_dir = TestDir::new("tensor-wallpaper-output-final-performance");
        let mut context = test_context();
        ...
        context.config.performance.battery = tensor_wallpaper::config::PowerPolicy::PauseDynamic;
        context.desktop = tensor_wallpaper::desktop::DesktopSnapshot {
            power: tensor_wallpaper::desktop::PowerState::Battery,
            outputs: vec![tensor_wallpaper::desktop::DesktopOutput::virtual_output("eDP-1")],
            ..tensor_wallpaper::desktop::DesktopSnapshot::default()
        };
    ...
    fn adaptive_action_report_reports_pause_dynamic_scope() {
        ...
        context.desktop.outputs = vec![tensor_wallpaper::desktop::DesktopOutput::virtual_output("eDP-1")];
        context.config.adaptive.enabled = true;
        context.config.adaptive.action = tensor_wallpaper::config::AdaptiveAction::PauseDynamic;
        context.adaptive_snapshot = tensor_wallpaper::adaptive::AdaptiveSnapshot {
            monitoring_enabled: true,
            active_triggers: vec![tensor_wallpaper::adaptive::AdaptiveTrigger {
                metric: tensor_wallpaper::adaptive::AdaptiveMetric::CpuPressureSomeAvg10,
    ...
    fn test_context() -> DaemonContext {
        DaemonContext {
            paths: ApplicationPaths {
                config_file: PathBuf::from("/tmp/tensor-wallpaper-test/config.toml"),
                state_file: PathBuf::from("/tmp/tensor-wallpaper-test/state.json"),
                cache_dir: PathBuf::from("/tmp/tensor-wallpaper-test/cache"),
                data_dir: PathBuf::from("/tmp/tensor-wallpaper-test/data"),
            },
            config: TensorWallpaperConfig::default(),
            state: AppState::default(),
            desktop: tensor_wallpaper::desktop::DesktopSnapshot::default(),
    ...
    fn write_static_package_manifest(root: &Path, background: &str) {
        ...
        std::fs::write(
            root.join(tensor_wallpaper::core::MANIFEST_FILE),
            format!(
                r#"{{
  "format": "tensor-wallpaper.wallpaper",
  "format_version": 1,
  "id": "io.github.elelcode.tensor-wallpaper.cache-test",
  "version": "0.1.0",
  "title": "Cache Test",
  "kind": "static-image",
  "entry": {{
...
"#
    ...
    fn empty_render_sync() -> StaticRenderSyncPlan {
        StaticRenderSyncPlan {
            ...
            errors: Vec::new(),
            decisions: Vec::new(),
            playlist_clock_dependency: tensor_wallpaper::renderer::PlaylistClockDependency::None,
