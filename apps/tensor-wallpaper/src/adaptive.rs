...
use crate::config::TensorWallpaperConfig;
...
const ENV_ADAPTIVE_STATE: &str = "TENSOR_WALLPAPER_ADAPTIVE_STATE";
...
impl AdaptiveMonitor {
    ...
    pub fn refresh(&mut self, config: &TensorWallpaperConfig) -> AdaptiveSnapshot {
        let now = Instant::now();
...
pub fn monitoring_enabled(config: &TensorWallpaperConfig) -> bool {
...
pub fn output_enabled(config: &TensorWallpaperConfig, output_name: &str) -> bool {
    if config.adaptive.kill_switch {
        return false;
    }
    config
...
pub fn output_throttle_max_fps(config: &TensorWallpaperConfig, output_name: &str) -> u32 {
    config
        .outputs
        .get(output_name)
...
fn read_sample(config: &TensorWallpaperConfig) -> (AdaptiveSystemSample, Option<String>) {
