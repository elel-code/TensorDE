...
pub fn runtime_socket_path() -> Option<PathBuf> {
    runtime_socket_path_from_env(env::var_os("TENSOR_WALLPAPER_SOCKET"), env::var_os("XDG_RUNTIME_DIR"))
}
...
mod tests {
    ...
    fn resolves_default_socket_under_xdg_runtime_dir() {
        ...
        assert_eq!(
            socket,
            Some(PathBuf::from("/tmp/runtime/tensor-wallpaper/tensor-wallpaper.sock"))
        );
    }
}
