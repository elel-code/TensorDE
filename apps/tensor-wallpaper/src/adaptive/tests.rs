...
fn kill_switch_disables_global_and_output_adaptive_policy() {
    let mut config = TensorWallpaperConfig {
        adaptive: AdaptiveConfig {
            enabled: true,
            kill_switch: true,
            ..AdaptiveConfig::default()
        },
        ..TensorWallpaperConfig::default()
    };
    config.outputs.insert(
        "eDP-1".to_owned(),
        OutputConfig {
...
fn pressure_thresholds_create_triggers() {
    let config = TensorWallpaperConfig {
        adaptive: AdaptiveConfig {
...
fn validation_override_inactive_does_not_trigger() {
    let config = TensorWallpaperConfig::default();
...
fn validation_override_rejects_unknown_state() {
    let err = validation_sample_override(&TensorWallpaperConfig::default(), "busy").unwrap_err();

    assert!(err.contains("TENSOR_WALLPAPER_ADAPTIVE_STATE"));
}
