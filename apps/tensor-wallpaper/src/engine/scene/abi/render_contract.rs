...
///
/// `GraphSource` resolves through the render graph's explicit source-image
/// domain. `OwnerAuthored` resolves through the graph owner and its declared
/// source extent. `PhysicalSurface` resolves through the live physical
/// SceneColor extent. This remains explicit in the binary ABI so scene execution
/// backends never infer it from target names.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
...
impl SceneTargetExtentDomain {
    pub const fn to_u32(self) -> u32 {
        match self {
            Self::GraphSource => 1,
            Self::PhysicalSurface => 2,
            Self::OwnerAuthored => 3,
        }
    }

    pub const fn from_u32(value: u32) -> Option<Self> {
        match value {
            1 => Some(Self::GraphSource),
            2 => Some(Self::PhysicalSurface),
            3 => Some(Self::OwnerAuthored),
            _ => None,
        }
    }
}

/// The concrete source image selected for one render graph.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum SceneRenderSourceExtentDomain {
    PhysicalSurface,
    OwnerAuthored,
}

impl SceneRenderSourceExtentDomain {
    pub const fn to_u32(self) -> u32 {
        match self {
