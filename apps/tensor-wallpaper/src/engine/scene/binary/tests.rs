...
fn scene_binary_round_trip_preserves_sprite_trail_maximum() {
    let particle = SceneParticleSystemRecord {
        ...
        animation_mode: SceneParticleAnimationMode::RandomFrame,
        ...
        ..SceneParticleSystemRecord::unsupported(
            SceneObjectHandle(7),
            SceneResourceId(11),
            SceneMaterialHandle(13),
            0,
            40,
            1.0,
            0.0,
    ...
    let mut bytes = Vec::new();
    write_scene_binary(&document, &mut bytes).expect("write current particle scene");
    assert_eq!(
        u32::from_le_bytes(bytes[8..12].try_into().unwrap()),
        SCENE_BINARY_VERSION
    );

    let decoded = read_scene_binary_bytes(&bytes).expect("read current particle scene");
    assert_eq!(decoded.particles, [particle]);
}
...
fn scene_binary_round_trip_preserves_target_extent_domains() {
    let document = SceneBinaryDocument {
        strings: vec![
            "scene_snapshot".to_owned(),
            "quarter_owner".to_owned(),
            "half_source".to_owned(),
        ],
        render_graphs: vec![SceneRenderGraphRecord {
            ...
            source_extent_domain: SceneRenderSourceExtentDomain::PhysicalSurface,
            pass_start: 0,
            pass_count: 0,
            unsupported_start: 0,
            unsupported_count: 0,
    ...
    assert_eq!(decoded.render_graphs, document.render_graphs);
...
fn scene_binary_rejects_unknown_render_pass_draw_primitive() {
    ...
    // graph count + one seven-u32 graph record + pass count + pass id/role.
    let first_pass_draw_primitive = render_graph_offset + 44;
