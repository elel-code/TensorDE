...
#[test]
fn accepts_descriptor_heap_spirv_and_exposes_borrowed_program_slices() {
    let document = descriptor_heap_uniform_document();
    let spirv = document.shader_spirv.clone();

    let storage = SceneStorage::from_document(document).expect("descriptor heap SPIR-V storage");
...
fn rejects_uniform_binding_without_typed_buffer() {
    let mut document = descriptor_heap_uniform_document();
...
fn descriptor_heap_uniform_document() -> SceneBinaryDocument {
    let mut spirv = vec![0x0723_0203, 0x0001_0600, 0, 2, 0];
