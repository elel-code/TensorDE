...
use fields::*;
mod object_state;
use object_state::validate_object_state;
mod user_property;
...
pub(super) fn validate_document(document: &SceneBinaryDocument) -> Result<(), SceneStorageError> {
    ...
    validate_object_state(document)?;
    ...
    for (particle_index, particle) in document.particles.iter().enumerate() {
        validate_range(
            "particle.object",
            particle.object.0,
            1,
        ...
        let root_particle = particle.parent_particle_index == INVALID_PARTICLE_INDEX;
        if root_particle {
            if particle.child_type != SceneParticleChildType::BuiltinDefault
                || particle.child_probability != 1.0
                || particle.child_max_count != 10
            {
                return Err(SceneStorageError::InvalidRange {
                    field: "particle.root_relationship",
                    start: particle.parent_particle_index,
                    count: 1,
                    len: particle_index,
                });
            }
            ...
            let Some(parent) = document
                .particles
                .get(particle.parent_particle_index as usize)
            else {
                return Err(SceneStorageError::InvalidRange {
                    field: "particle.parent_particle_index",
                    start: particle.parent_particle_index,
                    count: 1,
                    len: particle_index,
                });
            };
            if particle.parent_particle_index as usize >= particle_index
                || parent.object != particle.object
            {
                return Err(SceneStorageError::InvalidRange {
                    field: "particle.parent_relationship",
                    start: particle.parent_particle_index,
                    count: 1,
                    len: particle_index,
                });
        ...
        {
            return Err(SceneStorageError::InvalidRange {
                field: "particle.instance_relationship",
                start: particle_index as u32,
                count: 1,
        ...
        if particle.simulation == SceneParticleSimulationKind::ModuleSprite
            && (particle.emitter_shape == SceneParticleEmitterShape::Unsupported
                || particle.renderer == SceneParticleRendererKind::Unsupported
                || particle.module_mask.0 & !SceneParticleModuleMask::SUPPORTED_BITS != 0
                || !particle
                    .module_mask
                    .contains(SceneParticleModuleMask::LIFETIME_RANDOM)
                || !particle
                    .module_mask
                    .contains(SceneParticleModuleMask::SIZE_RANDOM)
                || !particle.alpha_min.is_finite()
                || !particle.alpha_max.is_finite()
                || particle.alpha_min < 0.0
                || particle.alpha_max < particle.alpha_min)
        {
            return Err(SceneStorageError::InvalidRange {
                field: "particle.module_sprite",
                start: particle.max_count,
                count: 1,
                len: document.particles.len(),
            });
        }
        if particle.renderer == SceneParticleRendererKind::SpriteTrail
            && (!particle.trail_length.is_finite()
                || particle.trail_length < 0.0
                || !particle.trail_min_length.is_finite()
                || particle.trail_min_length < 0.0
                || !particle.trail_max_length.is_finite()
                || particle.trail_max_length < 0.0)
        {
            return Err(SceneStorageError::InvalidRange {
                field: "particle.sprite_trail",
                start: particle.max_count,
                count: 1,
                len: document.particles.len(),
            });
