...
#[test]
fn fullscreen_framebuffer_graph_resolves_local_and_effect_targets_to_physical_source() {
    let target_name = SceneStringId(0);
    let document = SceneBinaryDocument {
        strings: vec!["_rt_HalfSource".to_owned()],
        render_graphs: vec![SceneRenderGraphRecord {
    ...
    for (target, name) in [
        (SceneRenderTargetKind::ImageLocalMain, SceneStringId::NONE),
        (
        ...
        assert_eq!([compatibility.target_width, compatibility.target_height], [0, 0]);
    }
}
