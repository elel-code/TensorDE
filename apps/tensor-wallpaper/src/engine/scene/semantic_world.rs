...
//! - `reverse-engineered/tensor-wallpaper/docs/exe/model-and-animation.md`
...
pub use components::{
    ...
    PuppetBindingComponent, SemanticMeshBinding, SemanticParticleEmitterBinding,
...
pub struct SceneSemanticWorld<'a> {
    ...
    particle_bindings: Vec<SemanticParticleEmitterBinding>,
...
impl<'a> SceneSemanticWorld<'a> {
    pub fn from_storage(storage: &'a SceneStorage) -> Result<Self, SceneSemanticWorldError> {
        ...
        let mut world = Self {
            ...
            particle_bindings: Vec::with_capacity(storage.particles().len()),
        ...
        for (particle_index, particle) in storage.particles().iter().enumerate() {
            if let Some(entity) = world.index.entity_for_object(particle.object) {
                ...
                world.particle_bindings.push(binding);
                component.binding_count += 1;
            }
        }
    ...
    pub fn particle_emitters(
        &self,
        object: SceneObjectHandle,
    ) -> &[SemanticParticleEmitterBinding] {
        let Some(component) = self.component_for_object(object, &self.particle_components) else {
            return &[];
        };
        let start = component.binding_start as usize;
        let end = start + component.binding_count as usize;
        self.particle_bindings.get(start..end).unwrap_or(&[])
    }
    ...
    fn resolve_frame_with_dynamic_values_at(
        &self,
        scene_time_seconds: f32,
        camera_animation_time_seconds: f32,
    ...
    fn resolve_entity(
        ...
        let state = ResolvedObjectState {
            ...
            camera_zoom: sampled_transform.camera_zoom,
            self_visible,
...
include!("semantic_world/puppet_and_effect_resolution.rs");
