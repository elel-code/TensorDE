...
fn authored_puppet_waterwaves_and_opacity_keep_twelve_draws_before_terminal_skinning() {
    ...
    assert_eq!(graph.passes[12].role, RenderPassRole::SceneComposite);
    assert_eq!(
        graph.passes[11].effect_visibility,
        RenderPassEffectVisibility::any_visible(9, 1)
    );
    assert_eq!(
        graph.passes[12].effect_visibility,
...
fn authored_puppet_two_stage_waterwaves_keep_four_draws() {
    ...
    assert_eq!(
        ...
        RenderPassEffectVisibility::any_visible(1, 1)
    );
    assert_eq!(
        graph.passes[4].effect_visibility,
        RenderPassEffectVisibility::none_visible(1, 1)
    );
    assert_puppet_source_and_terminal_state(&graph);
}
...
fn assert_puppet_source_and_terminal_state(graph: &RenderGraph) {
    ...
    for terminal in graph
        ...
        .iter()
        .rev()
        .take_while(|pass| pass.role == RenderPassRole::SceneComposite)
    {
        assert_eq!(
