...
pub use graph_builder::we_image_graph;

/// A runtime-toggleable ordinary image effect needs two exact branches. WE draws the authored
/// base material directly when the only effect is disabled; a passthrough offscreen round-trip is
/// not pixel-equivalent. Complex target/copy/swap chains remain on their explicit authored route.
fn runtime_direct_bypass_effect_range(contract: &WeImageGraphContract) -> Option<(u32, u32)> {
    let first = contract.effect_passes.first()?;
    if contract.framebuffer_snapshot.is_some()
        ...
        || contract.final_effect_material.is_some()
        || !contract
            .base_shader
            .as_deref()
            .is_some_and(is_unskinned_generic_image_shader)
        || first.effect_binding_count != 1
        || !contract.effect_passes.iter().all(|effect| {
            effect.runtime_visibility
                && effect.effect_binding_start == first.effect_binding_start
                && effect.effect_binding_count == first.effect_binding_count
    ...
    Some((first.effect_binding_start, first.effect_binding_count))
}

fn direct_base_shader(contract: &WeImageGraphContract) -> String {
    if contract.final_scene_blend == SceneBlendMode::Multiply
        && contract
            .base_shader
            .as_deref()
            .is_some_and(is_unskinned_generic_image_shader)
    {
        "we/genericimage4-multiply-composite".to_owned()
    } else {
        contract.base_shader.clone().unwrap_or_default()
    }
}
