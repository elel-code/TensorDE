...
pub(super) fn text_layer_font_path(object: &Value) -> String {
    ...
    if font.is_empty() || font == "systemfont_arial" {
        ...
        // `fonts/arial.ttf`; substituting a project font changes WE glyph metrics.
...
fn retained_glyph_upload(raster: WeTextLayerRaster) -> Result<TexUpload, TexParseError> {
    ...
    Ok(TexUpload {
        metadata: TexMetadata {
            texv_tag: "TENSOR_WALLPAPER_TEXT".to_owned(),
...
fn rasterize_text_layer_with_font(
    ...
    // The WE layout path adds `[parameters+0x10]` directly to the 26.6 glyph advance after
