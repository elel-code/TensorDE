...
const BUILTIN_ASSET_ROOT_ENV: &str = "TENSOR_WALLPAPER_WALLPAPER_ENGINE_ASSET_ROOT";
...
fn discover_builtin_asset_roots(project_root: &Path) -> Vec<PathBuf> {
    ...
    for ancestor in project_root.ancestors() {
        // The Linux Wallpaper Engine distribution used by the scene runtime carries Windows
