mod effect_material;
mod fused_authored_source;
mod puppet_effect_stream;
mod texture_storage;
...
impl WeIrBuilder {
                ...
                match decode_tex_upload(&self.resources[resource as usize].payload).and_then(
                    ...
                    Ok((upload, alpha_coverage_rows)) => self.textures.push(WeIrTexture {
                        ...
                        sequence_tag: upload
                            .sequence
                            .as_ref()
                            .map_or_else(String::new, |sequence| sequence.tag.clone()),
                        sequence_cell_width: upload
                            .sequence
                            .as_ref()
                            .map_or(0, |sequence| sequence.cell_width),
                        sequence_cell_height: upload
                            .sequence
                            .as_ref()
                            .map_or(0, |sequence| sequence.cell_height),
                        sequence_frames: upload
                            .sequence
                            .iter()
                            .flat_map(|sequence| &sequence.frames)
                            .map(|frame| WeIrTextureSequenceFrame {
    ...
    pub(super) fn add_effect(&mut self, path: &str) -> Result<u32, WeIngestError> {
            ...
            .flatten()
        {
            ...
            self.image_targets.push(WeIrImageTarget {
                ...
                name,
                format,
                // WE scales effect FBOs relative to the source image. That
                // source is owner-authored for ordinary image layers and the
                // live framebuffer for fullscreen utility layers.
                extent_domain: WeIrImageTargetExtentDomain::GraphSource,
    ...
    pub(super) fn add_render_graph_for_object(
        ...
        if final_effect.is_some() {
            self.retain_fused_authored_source_materials(base_material_handle, &effect_passes);
        }
    ...
    fn retain_authored_package_utility_quads(&self, graph: &mut RenderGraph) {
        for pass in &mut graph.passes {
            let Some(material) = pass
                .material_index
                .and_then(|index| self.materials.get(index))
            else {
                continue;
            };
            let Some(material_pass) = self.material_passes.get(material.pass_start as usize) else {
                continue;
            };
            pass.draw_primitive = authored_package_utility_primitive(
                pass.draw_primitive,
                material_pass.shader_origin,
            );
        }
    }
    ...
    pub(super) fn shader_texture_defaults(
        &mut self,
        shader_key: &str,
    ) -> Result<Vec<ShaderTextureDefault>, WeIngestError> {
        let shader_key = shader_key.split("__").next().unwrap_or(shader_key);
        if let Some(defaults) = self.shader_texture_defaults_by_shader.get(shader_key) {
            return Ok(defaults.clone());
        }
        let mut defaults = Vec::<ShaderTextureDefault>::new();
        for extension in ["vert", "frag"] {
            let path = format!("shaders/{shader_key}.{extension}");
            let Some(asset) = self.source.read_optional_asset(&path)? else {
                continue;
            ...
            let source = String::from_utf8_lossy(&asset.bytes);
            for default in parse_shader_texture_defaults(&source)
                    ...
                    .find(|existing| existing.slot == default.slot)
                {
                    if *existing != default {
                        return Err(WeIngestError::InvalidProject(format!(
                            "shader {shader_key} declares conflicting defaults for texture slot {}",
                            default.slot
                        )));
                    }
                } else {
        ...
        self.shader_texture_defaults_by_shader
            .insert(shader_key.to_owned(), defaults.clone());
        Ok(defaults)
    }
