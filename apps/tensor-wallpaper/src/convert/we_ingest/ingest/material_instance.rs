...
mod tests {
    ...
    fn ingest_materializes_effect_instance_values_and_variant_in_ir() {
        let root = std::env::temp_dir().join(format!(
            "tensor-wallpaper-we-effect-material-instance-test-{}",
            std::process::id()
        ));
        ...
        let effect_pass = ir.render_graphs[0]
            .passes
            .iter()
            .find(|pass| pass.shader.as_deref() == Some("effects/opacity__SLOTS_1"))
    ...
    fn ingest_materializes_shader_declared_full_framebuffer_binding() {
        let root = std::env::temp_dir().join(format!(
            "tensor-wallpaper-we-shader-runtime-target-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&root);
...
uniform sampler2D g_Texture2; // {"default":"_rt_FullFrameBuffer","hidden":true,"material":"backgroundTexture"}"#,
    ...
    fn project_layer_base_draw_consumes_a_typed_scene_snapshot() {
        let root = std::env::temp_dir().join(format!(
            "tensor-wallpaper-we-project-layer-snapshot-test-{}",
            std::process::id()
        ));
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(&root).expect("test directory");
