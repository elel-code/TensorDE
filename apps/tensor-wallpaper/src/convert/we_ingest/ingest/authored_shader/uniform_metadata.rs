...
"#;
    ...
    #[test]
    fn preserves_the_selected_branch_metadata_without_merging_alternatives() {
        let default_branch = parse_shader_uniform_metadata(
            "uniform float u_resolutionModifier; // {\"material\":\"Half Resolution\",\"default\":3}",
        )
        .expect("default specialized branch");
        let custom_branch = parse_shader_uniform_metadata(
            "uniform float u_resolutionModifier; // {\"material\":\"Half / Double Resolution\",\"default\":0}",
        )
        .expect("custom specialized branch");

        assert_eq!(
            default_branch.material_parameter("u_resolutionModifier"),
            Some("Half Resolution")
        );
        assert_eq!(
            default_branch.material_default("u_resolutionModifier"),
            Some("3")
        );
        assert_eq!(
            custom_branch.material_parameter("u_resolutionModifier"),
            Some("Half / Double Resolution")
        );
        assert_eq!(
            custom_branch.material_default("u_resolutionModifier"),
            Some("0")
        );
    }
}
