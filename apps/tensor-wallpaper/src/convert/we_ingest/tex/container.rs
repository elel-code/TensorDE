...
use super::{
    TexMetadata, TexParseError, TexSequence, TexSequenceFrame, is_image_payload,
    static_file_texture_sampler_contract,
};
...
pub(super) fn parse_tex_container(data: &[u8]) -> Result<TexContainer<'_>, TexParseError> {
    ...
    let sequence = find_tag(data, b"TEXS", texb_offset)
        .map(|offset| parse_texs(data, offset, width, height))
        .transpose()?;
    Ok(TexContainer {
...
fn parse_texs(
    ...
    texture_width: u32,
    texture_height: u32,
) -> Result<TexSequence, TexParseError> {
    let mut cursor = Cursor::at(data, offset);
    let tag = cursor.tag("TEXS")?;
    let frame_count = cursor.u32("TEXS frame count")?;
    if frame_count == 0 {
        return Err(TexParseError::InvalidPayload("TEXS has no frames"));
    }
    let (cell_width, cell_height) = match tag.as_str() {
        "TEXS0002" => (texture_width, texture_height),
        "TEXS0003" => (
            cursor.u32("TEXS cell width")?,
            cursor.u32("TEXS cell height")?,
        ),
        _ => return Err(TexParseError::UnsupportedTexs(tag)),
    };
    let width = texture_width.max(1) as f32;
    let height = texture_height.max(1) as f32;
    let mut frames = Vec::with_capacity(frame_count as usize);
    for _ in 0..frame_count {
        let resource_index = cursor.u32("TEXS frame resource index")?;
        let duration = cursor.f32("TEXS frame duration")?;
        let origin = [
            cursor.f32("TEXS frame origin x")? / width,
            cursor.f32("TEXS frame origin y")? / height,
        ];
        let axis_x = [
            cursor.f32("TEXS frame axis x.x")? / width,
            cursor.f32("TEXS frame axis x.y")? / height,
        ];
        let axis_y = [
...
impl<'a> Cursor<'a> {
    ...
    fn f32(&mut self, field: &'static str) -> Result<f32, TexParseError> {
        Ok(f32::from_le_bytes(
            self.bytes(4, field)?.try_into().expect("f32 slice"),
        ))
    }
}
