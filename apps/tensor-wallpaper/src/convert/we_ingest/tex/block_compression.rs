//! Convert decoded WE texture levels into GPU block-compressed BC payloads.
//!
...
pub(in crate::convert::we_ingest) fn transcode_texture_upload(
    ...
    for mip in &upload.mips {
        ...
        // The compressor needs 4x4-padded source texels, but Vulkan's copy
        // region stays at the authored logical mip extent. `ImageUpload`
        // derives the padded block rows from that extent, so do not rewrite
        // either mip dimensions or authored storage metadata here.
        let blocks = match (upload.format, target_format) {
            (SceneTextureFormat::Rgba8Unorm, SceneTextureFormat::Bc7UnormBlock) => {
                compress_bc7(source, mip.width, mip.height)?.0
            }
            (SceneTextureFormat::R8Unorm, SceneTextureFormat::Bc4UnormBlock) => {
                compress_bc4(source, mip.width, mip.height)?.0
            }
            (SceneTextureFormat::Rg8Unorm, SceneTextureFormat::Bc5UnormBlock) => {
                compress_bc5(source, mip.width, mip.height)?.0
            }
            _ => {
...
mod tests {
    ...
    #[test]
    fn bc_transcode_keeps_authored_extents_and_encodes_padded_blocks() {
        ...
        assert_eq!(bc7.metadata.storage_width, 5);
        assert_eq!(bc7.metadata.storage_height, 3);
        assert_eq!(bc7.mips.len(), 1);
        assert_eq!(bc7.mips[0].width, 5);
        assert_eq!(bc7.mips[0].height, 3);
        ...
        assert_eq!(bc4.metadata.storage_width, 5);
        assert_eq!(bc4.metadata.storage_height, 3);
        assert_eq!(bc4.mips[0].width, 5);
        assert_eq!(bc4.mips[0].height, 3);
        assert_eq!(bc4.mips[0].payload_len, 8 * 2);
    }

    #[test]
    fn bc_transcode_keeps_each_logical_mip_extent_when_edge_blocks_are_padded() {
        let upload = TexUpload {
            ...
            mips: vec![
                ...
                TexUploadMip {
                    width: 2,
                    height: 1,
                    payload_offset: 5 * 3 * 4,
                    payload_len: 2 * 4,
                },
            ],
            sequence: None,
            payload: vec![127; 5 * 3 * 4 + 2 * 4],
        };

        let bc7 = transcode_texture_upload("materials/color.tex", upload).unwrap();
        ...
        assert_eq!(
            bc7.mips
                .iter()
                .map(|mip| (mip.width, mip.height, mip.payload_len))
                .collect::<Vec<_>>(),
            vec![(5, 3, 32), (2, 1, 16)]
        );
    }
