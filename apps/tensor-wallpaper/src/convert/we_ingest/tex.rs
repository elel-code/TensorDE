...
//! - `reverse-engineered/tensor-wallpaper/docs/exe/texture-and-format.md`
//! - `reverse-engineered/tensor-wallpaper/tools/parse_tex.py`
//! - `reverse-engineered/tensor-wallpaper/tools/audit_texb_legacy_payloads.py`
...
pub struct TexUpload {
    ...
    pub sequence: Option<TexSequence>,
    pub payload: Vec<u8>,
}

#[derive(Debug, Clone, PartialEq)]
pub struct TexSequence {
    pub tag: String,
    pub cell_width: u32,
    pub cell_height: u32,
    pub frames: Vec<TexSequenceFrame>,
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub struct TexSequenceFrame {
...
impl fmt::Display for TexParseError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedTexs(tag) => write!(f, "unsupported .tex sequence tag {tag}"),
...
mod tests {
    ...
    #[test]
    fn texs0003_retains_all_normalized_frame_affines() {
        let pixels = vec![0; 512 * 512];
        let mut bytes = texb0004(9, 0, 512, 512, 1, &[(0, pixels.as_slice())]);
        bytes.extend_from_slice(b"TEXS0003\0");
        for value in [30u32, 102, 85] {
            bytes.extend_from_slice(&value.to_le_bytes());
        }
        for frame in 0..30u32 {
            bytes.extend_from_slice(&0u32.to_le_bytes());
            bytes.extend_from_slice(&(1.0f32 / 30.0).to_le_bytes());
            let values = [
                (frame % 5) as f32 * 102.4,
                (frame / 5) as f32 * 85.334,
                102.4,
                0.0,
                0.0,
                85.334,
        ...
        let upload = decode_tex_upload(&bytes).expect("TEXS0003 upload");
        let sequence = upload.sequence.expect("texture sequence");

        assert_eq!(sequence.tag, "TEXS0003");
        assert_eq!((sequence.cell_width, sequence.cell_height), (102, 85));
        assert_eq!(sequence.frames.len(), 30);
        assert!((sequence.frames[0].axis_x[0] - 0.2).abs() < 1.0e-6);
        assert!((sequence.frames[0].axis_y[1] - 0.166_667_97).abs() < 1.0e-6);
        assert!((sequence.frames[29].origin[0] - 0.8).abs() < 1.0e-6);
        assert!((sequence.frames[29].origin[1] - 0.833_339_8).abs() < 1.0e-6);
    }
