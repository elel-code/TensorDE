//! Lower Wallpaper Engine ingest IR into the new Tensor Wallpaper scene binary ABI.
//!
//! References:
...
#[cfg(test)]
use render_graph::lower_binding;
use render_graph::lower_render_graphs;
use render_state::*;
use shader_program::{LoweredShaderPrograms, lower_shader_programs};
...
pub fn lower_ir_to_scene_binary(ir: &WeSceneIr) -> Result<SceneBinaryDocument, WeLowerError> {
    ...
    let mut texture_sequence_frames = Vec::new();
    let mut texture_payload = Vec::new();
    for texture in &ir.textures {
        let mip_start = texture_mips.len() as u32;
        let sequence_frame_start = texture_sequence_frames.len() as u32;
        ...
        texture_sequence_frames.extend(texture.sequence_frames.iter().map(|frame| {
        ...
        textures.push(SceneTextureRecord {
            ...
            sequence_tag: strings.id(&texture.sequence_tag),
            sequence_cell_width: texture.sequence_cell_width,
            sequence_cell_height: texture.sequence_cell_height,
            sequence_frame_start,
            sequence_frame_count: texture.sequence_frames.len() as u32,
            payload_offset,
...
fn lower_image_targets(
    ...
    for target in &ir.image_targets {
        ...
        let extent_domain = match target.extent_domain {
            WeIrImageTargetExtentDomain::GraphSource => SceneTargetExtentDomain::GraphSource,
        ...
        push_image_target(
            ...
            SceneImageTargetRecord {
                ...
                extent_domain: match target.extent_domain {
                    crate::engine::render_graph::RenderTargetExtentDomain::GraphSource => {
