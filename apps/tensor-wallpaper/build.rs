...
fn main() {
    ...
    println!("cargo:rerun-if-changed=build/scene_shader/builtin_slang.rs");
    ...
    println!(
        "cargo:rerun-if-changed=src/renderer/rendering_device/audio/system_monitor/pipewire_monitor.c"
    );
    ...
    if env::var_os("CARGO_FEATURE_RENDERING_DEVICE").is_some() {
...
struct TensorWallpaperPuppetBonePalette {
    vec4 row0;
    vec4 row1;
    vec4 row2;
    vec4 row3;
...
layout(std430, set = 0, binding = 4) readonly buffer ScenePuppetBones {
    TensorWallpaperPuppetBonePalette g_Bones[];
} g_Puppet;
void main() {
    v_TexCoord = a_TexCoord;
    ...
    for (uint slot = 0u; slot < 4u; slot++) {
        ...
        TensorWallpaperPuppetBonePalette bone = g_Puppet.g_Bones[bone_index];
