pub(super) fn framebuffer_lightning_vertex_source() -> String {
    r#"#version 450
layout(set = 0, binding = 2) uniform FramebufferLightningDrawUniform {
    vec4 g_ScreenUvToObjectUvRow0;
    vec4 g_ScreenUvToObjectUvRow1;
    vec4 g_ObjectUvToScreenUvRow0;
    vec4 g_ObjectUvToScreenUvRow1;
} u_Draw;
layout(set = 0, binding = 3) uniform FinalFramebufferLightningProgram {
...
pub(super) fn framebuffer_lightning_fragment_source(blend_mode: u32) -> String {
    let blended = match blend_mode {
        7 => "vec3(1.0) - (vec3(1.0) - albedo.rgb) * (vec3(1.0) - lit)",
        31 => "albedo.rgb + lit * flash",
        other => panic!("unsupported final lightning blend mode {other}"),
    };
    format!(
        r#"#version 450
...
void main() {{
    vec4 albedo = texture(g_SceneSnapshot, v_FramebufferCoord);
    ...
    vec4 color = vec4(max(vec3(0.0), albedo.rgb), albedo.a);
