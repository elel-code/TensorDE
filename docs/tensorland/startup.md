...
2. Resolve, parse, and validate the complete KDL configuration through `tensor-kdl`.
...
The configuration watcher and its bounded reload worker are part of gate 8, not best-effort
services started after readiness. The watcher resolves relative paths against the startup working
directory and watches the nearest existing ancestor, allowing a missing default configuration
directory to appear later. Failure to create the worker or register the watch fails startup
explicitly.
...
logs through tracing, and stops the compositor completion loop. After the loop returns, Tensorland
...
value-only request and later observes a value-only outcome through a bounded Tensorland bridge after a
