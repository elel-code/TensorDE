...
Smithay was removed from Tensorland's dependency graph on 2026-07-27. This is a completed
...
`LICENSES/Smithay-MIT.txt` remains because several Tensorland-owned implementations were derived from
...
| Boundary | Owner |
|----------|-------|
| Core and extension wire resources | `wayland-server`, `wayland-protocols`, and Tensorland dispatch state |
...
| Session, libinput, udev, DRM scanout, atomic KMS, GBM | Tensorland tty backend using native crates |
| Output planning and present policy | `tensor-host`, `tensor-drm`, `tensor-present` |
| Rootless XWayland process, shell association, XWM | Tensorland protocol state plus `x11rb` |
| Renderer and synchronization | Shared `vulkan-renderer`; Tensorland atomic KMS consumes exported dma-bufs and sync files |
...
`IORING_OP_POLL_ADD`; Tensorland consumes its CQE and explicitly rearms the next one-shot operation.
Tensorland owns no poll/epoll readiness registry and has no calloop dependency.
...
`WM_NORMAL_HINTS`, `_NET_WM_STATE`, `WM_HINTS`, `WM_PROTOCOLS`, `_NET_WM_NAME`, `WM_NAME`, and
`WM_CLASS`; only the returned pure-value completion can publish the map
...
X11 titles prefer bounded UTF-8 `_NET_WM_NAME` and fall back to bounded legacy `WM_NAME` decoding.
The `WM_CLASS` class (or instance when the class is empty) becomes the application id. Mapped X11
toplevels are published through the same `ext-foreign-toplevel-list-v1` handles as native Wayland
toplevels, and retained property changes update those handles atomically.

Keyboard focus follows the ICCCM input model derived from `WM_HINTS` and `WM_TAKE_FOCUS`:
passive clients receive X input focus, locally-active clients receive both input focus and the
protocol message, globally-active clients receive only `WM_TAKE_FOCUS`, and no-input clients receive
neither. Tensorland publishes the selected X11 window through root `_NET_ACTIVE_WINDOW`; focusing a
native Wayland window clears the X11 focus rather than leaving a stale X client active.
`_NET_CLOSE_WINDOW` is routed through the retained `WM_PROTOCOLS` snapshot: clients supporting
`WM_DELETE_WINDOW` receive the ICCCM message, while legacy clients use the specified X11 kill-client
path.
Managed map and unmap transitions publish ICCCM `WM_STATE` as `NormalState` and
`WithdrawnState`, so legacy toolkits do not have to infer management from the Wayland association.

X11 event ordering is authoritative. A `MapRequest` or `ConfigureRequest` without the preceding
`CreateNotify` is an invariant error; Tensorland does not issue synchronous attribute/geometry queries
...
`uv run scripts/tensorland/check_crate_boundaries.py` enforces the dependency and import bans for the root
