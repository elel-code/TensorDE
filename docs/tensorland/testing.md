...
| Niri DRM lease lifecycle | only authoritative-device `non-desktop` heads are offered; connector/CRTC/primary-plane reservations create real kernel leases and revoke on destroy, hotplug, reassignment, device loss, and session pause | `tensor_drm::lease`, `backend::tty::lease`, `protocol::globals::drm_lease`, `backend::output` |
...
| Niri/Hyprland transaction and damage sequencing | first frame is full damage, movement damages old/new bounds, prepared frames can abort, and fragmented damage remains fixed-capacity without unconditional whole-output fallback | `scene::damage`, `render::frame` |
