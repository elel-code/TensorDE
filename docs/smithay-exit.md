...
| 5 | **In progress** | `run_turn` + `EventfdWake` + submitted `EventfdCompletion` read + `CompletionDriver::IoUring` (Compio = **completion** model); calloop readiness remains for some Smithay fds — replace with completed Compio ops, not epoll-as-goal |
