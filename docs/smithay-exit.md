...
| 4 | **Done (partial)** | Submit path: policy readiness + `PresentIntent` push/pop before KMS; format negotiation and renderer dma-buf descriptions are Smithay-free. `smithay-drm-extras` and Smithay's `backend_gbm` feature are removed. Tensor imports its renderer-owned dma-buf descriptions directly through GBM and creates explicit-modifier DRM framebuffers without rebuilding Smithay dma-bufs, legacy fallback, staging, or extra fd/plane clones. DRM nodes, connector modes/subpixels, event decode, scanning, and gamma use drm-rs directly; only the transitional atomic surface/device and syncobj protocol keep `backend_drm` enabled. The tty adapter refreshes connector/CRTC tables in place without constructing unused scan-event vectors or cloning connector snapshots |
...
| 7 | **In progress** | `tensor-protocol` owns IDs, scene values, attachment lifetime, and tier policy. All wire types now come directly from `wayland-server`, `wayland-protocols`, and `wayland-protocols-wlr`; Smithay reexports are gone. Tensor `WindowSpace` and compositor-thread `ProtocolWindow` have replaced Smithay's desktop space/window types without per-refresh snapshots, query vectors, or shared locking. Tensor also owns popup topology, descendant cleanup, dynamic root-relative placement, explicit-grab policy, per-output layer maps/surfaces, allocation-free surface traversal, frame callback dispatch, presentation capture, and X11 hit testing. The Smithay `desktop` feature is removed; Dispatch1 shell state remains in the protocol adapter |
