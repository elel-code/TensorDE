...
| 5 | **In progress** | `run_turn` + `EventfdWake` + `CompletionDriver::IoUring`; the compositor thread now runs a Compio completion loop directly, with no shared relay thread/channel. IPC, signalfd, GPU sync-file, security-context, Wayland listener/display, XWayland displayfd startup, udev hotplug, libinput, and Tensor-owned libseat session waits are Compio-completed operations. The calloop aggregate fd remains only as a temporary submitted wait for Smithay XWM X11 and DRM notification adapters; replace those adapters with direct completed ops, not epoll-as-goal |
