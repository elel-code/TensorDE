...
| 5 | **In progress** | `run_turn` + `EventfdWake` + submitted `EventfdCompletion` read + Compio-completed IPC accept/read/write + `CompletionDriver::IoUring`; calloop readiness remains only for transitional Smithay/Wayland/backend sources — replace it with completed ops, not epoll-as-goal |
