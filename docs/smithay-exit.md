...
| 5 | **In progress** | `run_turn` + `EventfdWake` + `CompletionDriver::IoUring`; IPC, signalfd, GPU sync-file, security-context, Wayland listener/display, and XWayland displayfd startup are Compio-completed operations. Remaining calloop sources are the shared completion relay, Smithay XWM X11 events, libinput, session notification, and DRM notification adapters — replace them with completed ops, not epoll-as-goal |
