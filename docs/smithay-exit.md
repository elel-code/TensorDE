...
| 5 | **In progress** | `run_turn` + `EventfdWake` + `CompletionDriver::IoUring`; IPC, signalfd, GPU sync-file, security-context, Wayland listener accepts, and one-shot Wayland display dispatch waits are Compio-completed operations. Remaining calloop sources are the shared completion relay, XWayland, libinput, session notification, and DRM notification adapters — replace them with completed ops, not epoll-as-goal |
