...
| 5 | **In progress** | `run_turn` + `EventfdWake` + `CompletionDriver::IoUring`; IPC, signalfd, GPU sync-file, security-context, Wayland listener/display, XWayland displayfd startup, udev hotplug, and libinput waits are Compio-completed operations. Remaining calloop sources are the shared completion relay, Smithay XWM X11 events, session notification, and DRM notification adapters — replace them with completed ops, not epoll-as-goal |
