...
a non-empty validated client-import format list. Protocol choice prefers `ext-*` / standard `wp-*`
over `zwlr_*` when both exist (see `docs/protocol-surface.md`). Custom extensions use Dispatch2. Layer surfaces map through Smithay's per-output `LayerMap`. On commit, Tensor arranges exclusive
