...
commit-timing, xwayland-keyboard-grab, wlr-data-control, ext-data-control, virtual-pointer,
gamma-control, and linux-dmabuf when the selected Vulkan device exposes a non-empty validated
client-import format list. Custom extensions use Dispatch2 (see `docs/protocol-surface.md`). Layer surfaces map through Smithay's per-output `LayerMap`. On commit, Tensor arranges exclusive
