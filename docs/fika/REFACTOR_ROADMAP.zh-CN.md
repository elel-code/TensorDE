...
> 2026-07-30 状态：Fika 已收敛为单一 `vulkan-renderer` / Vulkanalia 路径，旧 renderer
...
- Icons layout size-hint cache：
  ...
  Fika 在 `src/ui/pane_layout.rs` 增加 icons item height cache，滚动和重复 frame 不再
...
- Status model 重写：
  ...
  边界，Fika 新增 `src/ui/status.rs` 承载 pane status summary、task status store、
