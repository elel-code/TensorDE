...
This document records the source references for Tensor Files's future network
...
- `references/fika/cosmic-files/src/tab.rs`
  ...
  - `network:///` is treated as the network root and exposes an Add Network
    Drive action.
- `references/fika/cosmic-files/src/app.rs`
...
- Places sidebar
  - Adds a Network root equivalent to Dolphin `remote:/` and cosmic-files
    ...
    Tensor Files's Places file, and is not treated as a local mounted directory until a
    network backend is attached.
  - The Network root context menu exposes Dolphin's `Add Network Folder`
    affordance, but implements it inside Tensor Files: the action opens Tensor Files's own
    ...
    through the same pane/generation loading path. Tensor Files does not depend on
