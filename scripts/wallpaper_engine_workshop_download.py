...
def main() -> int:
    ...
    matrix_report_dir = Path(args.matrix_report_dir) if args.matrix_report_dir else (
        repo_root / "artifacts/video-ffmpeg-vulkan-matrix" / f"we-{timestamp}"
    )
    matrix_command = [
        "uv",
        "run",
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    ...
    parser.add_argument(
        "--download-root",
        default="artifacts/wallpaper-engine-workshop/steamcmd-root",
    )
    parser.add_argument("--steamcmd", default=os.environ.get("STEAMCMD", ""))
    parser.add_argument("--steamcmd-dir", default="artifacts/tools/steamcmd")
