...
TENSOR_FILES_PERF_ITEM_VIEW=1 log. This script does not replace the manual DnD and
...
for log_path in "$@"; do
    ...
    printf '[tensor-files analyzer] log-boundary path=%s\n' "$log_path" >> "$combined_log"
    sed -n '1,$p' "$log_path" >> "$combined_log"
done

"$script_dir/analyze-item-view-perf.sh" \
    --require-steady \
    --require-details \
