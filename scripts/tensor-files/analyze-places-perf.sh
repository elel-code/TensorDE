...
Usage: analyze-places-perf.sh [OPTIONS] LOG
       TENSOR_FILES_PERF_PLACES_VIEW=1 TENSOR_FILES_AUTOSMOKE_PLACES=targets target/debug/tensor-files /etc 2>&1 | analyze-places-perf.sh [OPTIONS] -

Summarizes TENSOR_FILES_PERF_PLACES_VIEW logs and optionally enforces Places renderer
policy gates.

Options:
  --require-autosmoke
      Fail unless the non-destructive TENSOR_FILES_AUTOSMOKE_PLACES=targets markers are
      present and show target/insert/clear projection.

  --require-overflow-autosmoke
      Fail unless TENSOR_FILES_AUTOSMOKE_PLACES=overflow markers are present and the
      log proves sidebar overflow through [tensor-files places-scrollbar].

  --require-layout-autosmoke
      Fail unless TENSOR_FILES_AUTOSMOKE_PLACES=layout markers are present and prove
      sidebar hide/show, resize, reset, restore, and persisted settings.

  --require-hit-test-autosmoke
      Fail unless TENSOR_FILES_AUTOSMOKE_PLACES=hit-test markers are present and prove
      retained Places row edge/body and section hit testing.

  --require-retained-targeting-autosmoke
      Fail unless TENSOR_FILES_AUTOSMOKE_PLACES=targeting markers are present and prove
      retained Places activation-row, row context-menu, and section context-menu
      target classification.

  --require-retained-dnd-autosmoke
      Fail unless TENSOR_FILES_AUTOSMOKE_PLACES=dnd markers are present and prove
      retained Places DnD target decisions for path-list and place drags.

  --require-interaction-policy
      Fail unless [tensor-files places-interaction-policy] is present and proves the
      current retained target-decision / GPUI event-shell boundary.

  --require-interaction-geometry
      Fail unless [tensor-files places-interaction-geometry] is present and matches
      the retained Places row/section projection counts.

  --require-event-probe
      Fail unless [tensor-files places-event-probe] is present and its hitbox count
      matches the explicit retained-probe policy count.

  --expect-custom-row-visual-policy
      Fail unless [tensor-files places-renderer-policy] matches the opt-in
      ...
      visual_kind=text. Also requires aggregated [tensor-files places-row-visual] logs
  ...
  --expect-custom-row-full-policy
      ...
      selected event policy, section_gpui=0, scrollbar_canvas=1, and
      visual_kind=full. Also requires aggregated [tensor-files places-row-visual] logs
      whose rows count matches the policy rows and row text shape-cache logs.

  --expect-custom-row-chrome-policy
      Fail unless [tensor-files places-renderer-policy] matches the Dolphin-aligned
      ...
      visual_kind=chrome. Also requires aggregated [tensor-files places-row-visual]
  ...
  --expect-retained-event-policy
      Fail unless [tensor-files places-renderer-policy] and
      [tensor-files places-interaction-policy] match the future retained event-delivery
  ...
  --snapshot-us N
      Fail if any [tensor-files places-view] snapshot exceeds N microseconds.

  --sidebar-build-us N
      Fail if any [tensor-files places-sidebar] build exceeds N microseconds.

  --slot-project-us N
      Fail if any [tensor-files places-slots] project exceeds N microseconds.

  --row-visual-paint-us N
      Fail if any [tensor-files places-row-visual] paint exceeds N microseconds.

  --row-visual-prepaint-us N
      Fail if any [tensor-files places-row-visual] prepaint exceeds N microseconds.

  --row-visual-warm-paint-us N
      Fail if any [tensor-files places-row-visual] paint after the first two frames
      exceeds N microseconds.

  --row-visual-warm-prepaint-us N
      Fail if any [tensor-files places-row-visual] prepaint after the first two frames
      exceeds N microseconds.

  --render-total-us N
      Fail if any [tensor-files render] total exceeds N microseconds when render logs
      are present.
...
/^\[tensor-files places-icon-cache-refresh\]/ {
...
/^\[tensor-files places-renderer-policy\]/ {
    policy_frames++
    rows = field("rows") + 0
...
/^\[tensor-files places-interaction-policy\]/ {
...
/^\[tensor-files places-interaction-geometry\]/ {
...
/^\[tensor-files places-event-probe\]/ {
    event_probe_frames++
    rows = field("rows") + 0
    sections = field("sections") + 0
...
/^\[tensor-files places-row-visual\]/ {
    row_visual_frames++
    rows = field("rows") + 0
...
/^\[tensor-files places-row-shape-cache\]/ {
...
/^\[tensor-files places-row-glyph-cache\]/ {
...
/^\[tensor-files autosmoke\] places action=/ {
    action = field("action")
...
/^\[tensor-files autosmoke\] places hit-test / {
    sample = field("sample")
    kind = field("kind")
    zone = field("zone")
    ok = field("ok")
...
/^\[tensor-files autosmoke\] places hit-test-summary / {
    ok = field("ok")
    rows = field("rows") + 0
    sections = field("sections") + 0
...
/^\[tensor-files autosmoke\] places targeting / {
    sample = field("sample")
    target = field("target")
...
/^\[tensor-files autosmoke\] places targeting-summary / {
    ok = field("ok")
    rows = field("rows") + 0
    sections = field("sections") + 0
    if (ok == "true" && rows > 0 && sections > 0) {
...
/^\[tensor-files autosmoke\] places dnd / {
    sample = field("sample")
    drag = field("drag")
    target = field("target")
    cursor = field("cursor")
...
/^\[tensor-files autosmoke\] places dnd-summary / {
    ok = field("ok")
    rows = field("rows") + 0
    sections = field("sections") + 0
...
/^\[tensor-files autosmoke\] places snapshot=/ {
...
END {
    if (places_view_frames == 0) {
        fail("missing [tensor-files places-view] logs")
    }
    if (sidebar_frames == 0) {
        fail("missing [tensor-files places-sidebar] logs")
    }
    if (slot_frames == 0) {
        fail("missing [tensor-files places-slots] logs")
    }
    if (policy_frames == 0) {
        fail("missing [tensor-files places-renderer-policy] logs")
    }
    ...
    if (expect_custom_row_visual_policy == "true") {
        ...
        if (row_visual_frames == 0) {
            fail("missing [tensor-files places-row-visual] logs for custom text row visual policy")
        }
        if (row_shape_cache_frames == 0) {
            fail("missing [tensor-files places-row-shape-cache] logs for custom text row visual policy")
        }
        if (row_glyph_cache_frames == 0) {
            fail("missing [tensor-files places-row-glyph-cache] logs for custom text row visual policy")
        }
    ...
    if (expect_custom_row_full_policy == "true") {
        ...
        if (row_visual_frames == 0) {
            fail("missing [tensor-files places-row-visual] logs for full custom row visual policy")
        }
        if (row_shape_cache_frames == 0) {
            fail("missing [tensor-files places-row-shape-cache] logs for full custom row visual policy")
        }
        if (row_glyph_cache_frames == 0) {
            fail("missing [tensor-files places-row-glyph-cache] logs for full custom row visual policy")
        }
        if (max_values["row_visual_rows"] != max_values["policy_rows"]) {
    ...
    if (expect_custom_row_chrome_policy == "true") {
        ...
        if (row_visual_frames == 0) {
            fail("missing [tensor-files places-row-visual] logs for custom row chrome policy")
        }
        if (row_shape_cache_frames != 0) {
    ...
    if (expect_retained_event_policy == "true") {
        ...
        if (interaction_policy_frames == 0) {
            fail("missing [tensor-files places-interaction-policy] logs for retained event-delivery policy")
        }
        ...
        if (max_values["policy_row_visual_layer"] > 0) {
            if (row_visual_frames == 0) {
                fail("missing [tensor-files places-row-visual] logs for retained custom row visual policy")
            }
            if ((policy_kind_text_seen || policy_kind_full_seen) && row_shape_cache_frames == 0) {
                fail("missing [tensor-files places-row-shape-cache] logs for retained custom row visual policy")
            }
            if ((policy_kind_text_seen || policy_kind_full_seen) && row_glyph_cache_frames == 0) {
                fail("missing [tensor-files places-row-glyph-cache] logs for retained custom row visual policy")
            }
    ...
    if (require_interaction_policy == "true") {
        if (interaction_policy_frames == 0) {
            fail("missing [tensor-files places-interaction-policy] logs")
        }
    ...
    if (require_interaction_geometry == "true") {
        if (interaction_geometry_frames == 0) {
            fail("missing [tensor-files places-interaction-geometry] logs")
        }
    ...
    if (require_event_probe == "true") {
        if (event_probe_frames == 0) {
            fail("missing [tensor-files places-event-probe] logs")
        }
