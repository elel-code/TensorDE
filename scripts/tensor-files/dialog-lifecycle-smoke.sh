...
mkdir -p "$out_dir"
log_path="$out_dir/tensor-files-dialog-lifecycle-smoke.log"
tmpdir="$(mktemp -d /tmp/tensor-files-dialog-lifecycle-smoke.XXXXXX)"
cleanup() {
    rm -rf "$tmpdir"
}
trap cleanup EXIT
...
if [[ "$skip_build" == false ]]; then
    (cd "$workspace_root" && cargo build --bin tensor-files)
fi
...
env \
    ...
    TENSOR_FILES_LOG=1 \
    TENSOR_FILES_DIALOG_TRACE=1 \
    TENSOR_FILES_AUTOSMOKE_DIALOG_LIFECYCLE=1 \
    TENSOR_FILES_AUTOSMOKE_DIALOG_KIND="$dialog_kind" \
    TENSOR_FILES_AUTOSMOKE_DIALOG_CYCLES="$cycles" \
...
if ! rg -q "\[tensor-files\] dialog-smoke complete" "$log_path"; then
    echo "fail: missing dialog-smoke complete marker" >&2
    echo "log: $log_path" >&2
    exit 1
fi

if ! rg -q "\[tensor-files\] dialog-window-drop-deferred kind=$dialog_kind" "$log_path"; then
    echo "fail: missing deferred dialog drop marker" >&2
    echo "log: $log_path" >&2
    exit 1
fi

if rg -q "\[tensor-files\] dialog-window-(close-parked|reuse-parked) kind=$dialog_kind" "$log_path"; then
    echo "fail: dialog lifecycle used hidden parked window fallback" >&2
    echo "log: $log_path" >&2
    exit 1
fi

if rg -q "\[tensor-files\] event-loop-exit reason=main-close-requested" "$log_path"; then
