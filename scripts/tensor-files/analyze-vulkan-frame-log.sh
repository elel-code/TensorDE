...
Usage: analyze-vulkan-frame-log.sh [OPTIONS] LOG
       TENSOR_FILES_LOG=1 TENSOR_FILES_FRAME_LOG_ALL=1 target/debug/tensor-files /etc 2>&1 |
           analyze-vulkan-frame-log.sh --require-frames -

Summarizes Tensor Files's native Vulkan frame logs and optionally enforces runtime
evidence gates.
...
Options:
  ...
  --require-autosmoke-scroll
      Fail unless TENSOR_FILES_AUTOSMOKE_SCROLL produced a changed scroll action.
