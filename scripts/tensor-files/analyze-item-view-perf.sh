...
Usage: analyze-item-view-perf.sh [OPTIONS] LOG
       TENSOR_FILES_PERF_ITEM_VIEW=1 target/debug/tensor-files ~/Downloads 2>&1 | analyze-item-view-perf.sh [OPTIONS] -

Summarizes TENSOR_FILES_PERF_ITEM_VIEW item-view logs and optionally enforces perf-log
acceptance gates.

Options:
  --require-steady
      Fail if no [tensor-files item-view] phase=steady frame is present.
  ...
  --require-static-visual
      Fail if [tensor-files static-item-visual] Compact/Icons paint timing or static
  ...
  --require-interaction
      Fail if [tensor-files item-interaction] hitbox timing is missing.

  --require-renderer-policy
      Fail if [tensor-files renderer-policy] surface-count logs are missing.
  ...
  --require-paint-slots
      Fail if [tensor-files item-paint-slots] retained slot activity logs are missing.
  ...
  --file-grid-build-us N
      Fail if any [tensor-files file-grid] build exceeds N microseconds.

  --static-visual-paint-us N
      Fail if any [tensor-files static-item-visual] paint exceeds N microseconds.

  --image-paint-us N
      Fail if any [tensor-files item-image] paint exceeds N microseconds.

  --custom-paint-us N
      Fail if any custom paint channel exceeds N microseconds.

  --warm-static-visual-paint-us N
      Fail if any warmed [tensor-files static-item-visual] paint exceeds N microseconds.
  ...
  --warm-image-paint-us N
      Fail if any warmed [tensor-files item-image] paint exceeds N microseconds.
...
/^\[tensor-files analyzer\] log-boundary / {
    reset_process_warm_state()
}

/^\[tensor-files item-view\]/ {
    item_view_count++
    mode = field("mode")
    phase = field("phase")
    if (phase == "") {
    ...
    total = required_us_field("total", "[tensor-files item-view]")
    raw = required_us_field("raw", "[tensor-files item-view]")
    icon_sync = required_us_field("icon_sync", "[tensor-files item-view]")
    queue = required_us_field("queue", "[tensor-files item-view]")
    convert = required_us_field("convert", "[tensor-files item-view]")
...
/^\[tensor-files item-image-cache-refresh\]/ {
...
/^\[tensor-files static-item-visual\]/ {
    static_visual_count++
    mode = field("mode")
    note_mode(mode)
    if (mode != "") {
...
/^\[tensor-files item-glyph-budget\]/ {
...
/^\[tensor-files details-shape-cache\]/ {
    details_shape_count++
    note_mode(field("mode"))
...
/^\[tensor-files details-glyph-cache\]/ {
    details_glyph_count++
    note_mode(field("mode"))
...
/^\[tensor-files details-glyph-budget\]/ {
...
/^\[tensor-files autosmoke\] item-view start / {
...
/^\[tensor-files autosmoke\] item-view complete / {
...
/^\[tensor-files autosmoke\] item-view action=/ {
    action = field("action")
...
END {
    ...
    if (item_view_count == 0) {
        fail("missing [tensor-files item-view] lines")
    }
    if (require_steady == "true" && steady_count == 0) {
        fail("missing [tensor-files item-view] phase=steady frames")
    }
    if (require_details == "true" && details_visual_count == 0) {
        fail("missing [tensor-files details-visual] lines")
    }
    if (require_details == "true" && details_shape_count == 0) {
        fail("missing [tensor-files details-shape-cache] lines")
    }
    if (require_details == "true" && details_glyph_count == 0) {
        fail("missing [tensor-files details-glyph-cache] lines")
    }
    if (require_details == "true" && details_glyph_budget_count == 0) {
        fail("missing [tensor-files details-glyph-budget] lines")
    }
    if (require_warm_details_visual == "true" && warm_details_visual_count == 0) {
        fail("missing warmed [tensor-files details-visual] paint frames")
    }
    if (require_static_visual == "true" && static_visual_count == 0) {
        fail("missing [tensor-files static-item-visual] lines")
    }
    if (require_static_visual == "true" && item_shape_count == 0) {
        fail("missing [tensor-files item-shape-cache] lines")
    }
    if (require_static_visual == "true" && item_glyph_count == 0) {
        fail("missing [tensor-files item-glyph-cache] lines")
    }
    if (require_static_visual == "true" && item_glyph_budget_count == 0) {
        fail("missing [tensor-files item-glyph-budget] lines")
    }
    ...
    if (require_interaction == "true" && item_interaction_count == 0) {
        fail("missing [tensor-files item-interaction] lines")
    }
    if (require_renderer_policy == "true" && renderer_policy_count == 0) {
        fail("missing [tensor-files renderer-policy] lines")
    }
    if (expect_retained_item_policy == "true") {
        if (renderer_policy_count == 0) {
            fail("missing [tensor-files renderer-policy] lines for retained item policy")
        }
    ...
    if (require_paint_slots == "true") {
        if (item_paint_slot_count == 0) {
            fail("missing [tensor-files item-paint-slots] lines")
        }
