...
tmpdir="$(mktemp -d /tmp/fika-vulkan-frame-analyzer.XXXXXX)"
trap 'rm -rf "$tmpdir"' EXIT

log="$tmpdir/frames.log"
cat >"$log" <<'EOF'
[fika-vulkan] frame=1 reason=initial view=compact size=1280x720 work_pending=1 render_us=900
[fika-vulkan] icons=36 quads=0 fallback=0 deferred=1
[fika] autosmoke-scroll action=forward delta=64.0 changed=1 old_scroll_x=0.0 new_scroll_x=0.0 old_scroll_y=0.0 new_scroll_y=64.0
[fika-vulkan] frame=2 reason=autosmoke-scroll view=compact size=1280x720 work_pending=0 render_us=680
[fika-vulkan] frame=3 reason=initial view=icons size=1280x720 work_pending=0 render_us=720
EOF
