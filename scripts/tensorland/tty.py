...
def command_for(args: argparse.Namespace) -> list[str]:
    command = [str(ROOT / "target" / "debug" / "tensorland")]
...
def environment_for(args: argparse.Namespace) -> dict[str, str]:
    ...
    environment.setdefault("RUST_LOG", "tensorland=info")
...
def build_binaries(dmabuf_smoke: bool) -> int:
    command = ["cargo", "build", "-p", "tensorland", "--bin", "tensorland"]
    if dmabuf_smoke:
        command.extend(["--bin", "tensorland-dmabuf-smoke"])
