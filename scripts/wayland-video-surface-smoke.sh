...
append_video_runtime_evidence_summary() {
  ...
  for key in \
    ...
    video_memory_retention_pool_reports_max \
    video_memory_retention_system_memory_pool_reports_max \
    video_memory_retention_gpu_memory_pool_reports_max \
  ...
  awk -v prefix="$prefix" -F': ' '
    ...
    $1 ~ /^video_memory_retention_level\./ ||
    $1 ~ /^video_memory_retention_sink_frame_retention\./ {
      printf "%s_%s: %s\n", prefix, $1, $2
    }
  ' "$summary"
}
...
append_telemetry_evidence_summary() {
  ...
  for key in \
    ...
    render_sync_package_cache_retained_preview_resource_references_latest \
    render_sync_package_cache_retained_unique_preview_resources_latest \
    render_sync_package_cache_retained_preview_resource_bytes_latest \
    render_sync_package_cache_retained_unique_preview_resource_bytes_latest \
