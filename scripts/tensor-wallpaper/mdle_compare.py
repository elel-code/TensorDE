"""Parse MDLE0002 from 眼睛_puppet.mdl, compare with Tensor Wallpaper's computed inverse_bind."""
import struct, json, sys, math
sys.path.insert(0, '.')
from pathlib import Path

MDL = '/tmp/tensor-wallpaper-we-3742497499-extracted/models/眼睛_puppet.mdl'
...
# ── 4. Compute Tensor Wallpaper's bind_world and inverse_bind_world ──────
def mat_mul(a, b):
...
# Compute Tensor Wallpaper-style bind_world (accumulate through parents)
...
tensor_wallpaper_inverse_bind = []
for bi in range(bone_count):
    inv = mat_inv(bind_world[bi])
    tensor_wallpaper_inverse_bind.append(inv)

# ── 5. Compare MDLE vs Tensor Wallpaper inverse_bind ─────────────────────
print("\n=== MDLE vs Tensor Wallpaper inverse_bind comparison ===")
print(f"{'bone':>5} {'parent':>6} {'name':>18} {'tx_diff':>10} {'ty_diff':>10} {'tz_diff':>10} {'max_diff':>10} {'max_diag':>10}")
diffs = []
for bi in range(bone_count):
    g = tensor_wallpaper_inverse_bind[bi]
    m = mdle_matrices[bi]
    tx_d = abs(g[12] - m[12])
    ty_d = abs(g[13] - m[13])
    tz_d = abs(g[14] - m[14])
...
out = {
    ...
    'tensor_wallpaper_inverse_bind': tensor_wallpaper_inverse_bind,
