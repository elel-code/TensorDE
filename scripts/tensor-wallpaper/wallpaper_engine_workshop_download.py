...
def main() -> int:
    ...
    matrix_report_dir = Path(args.matrix_report_dir) if args.matrix_report_dir else (
        ARTIFACTS_ROOT / "video-decode-matrix" / f"we-{timestamp}"
    )
    matrix_command = [
        "uv",
        "run",
        "python",
        "scripts/tensor-wallpaper/video_decode_matrix.py",
        "--artifact-prefix",
        "tensor-wallpaper-video-decode-workshop",
        "--work-dir",
...
def parse_args() -> tuple[argparse.Namespace, list[str]]:
    parser = argparse.ArgumentParser(
        description="用 SteamCMD 下载 Wallpaper Engine Workshop 项，并可选运行 GPU 视频解码矩阵。"
    )
    ...
    parser.add_argument("--steam-user", default=os.environ.get("TENSOR_WALLPAPER_STEAM_USER", ""))
