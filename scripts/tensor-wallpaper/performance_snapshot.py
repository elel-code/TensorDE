...
# ///
"""Sample process memory/CPU evidence for Tensor Wallpaper runs.
...
def main() -> int:
    ...
    pid = args.pid or find_tensor_wallpaperd_pid()
    if pid <= 0:
        return missing(args, "找不到 tensor-wallpaperd 进程；传入 --pid 可直接采样指定进程")
    if not Path(f"/proc/{pid}").exists():
        return missing(args, f"进程不存在: {pid}")

    out_dir = Path(args.output_dir) if args.output_dir else Path(args.work_dir) / (
        f"tensor-wallpaper-performance-{args.label}-{int(time.time())}-{os.getpid()}"
    )
...
def find_tensor_wallpaperd_pid() -> int:
    ...
    for line in result.stdout.splitlines():
        ...
        if len(parts) == 3 and parts[1] == user and parts[2] == "tensor-wallpaperd":
            return int(parts[0])
    return 0
