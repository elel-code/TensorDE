...
def main() -> int:
    args = parse_args()
    with tempfile.TemporaryDirectory(prefix="tensor-wallpaper-scene-runtime-smoke-") as tmp:
        root = Path(tmp)
        if args.source:
        ...
        artifact_dir = (
            ...
            / f"tensor-wallpaper-scene-runtime-smoke-artifacts-{int(time.time())}-{os.getpid()}"
        )
        ...
        if args.prebuilt_release_binary:
            ...
            binary = ROOT / "target/release/tensor-wallpaper"
