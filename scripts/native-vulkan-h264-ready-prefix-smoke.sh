...
This is the first continuous direct gate: IDR + P, no B frames, one L0 ref.
...
if [[ "$width" -lt 128 || "$height" -lt 128 || "$rate" -lt 1 || "$samples" -lt 1 || "$decode_prefix" -lt 2 || "$keyint" -lt 2 ]]; then
  printf 'FAIL: width/height/rate/samples/decode-prefix/keyint must be valid\n' >&2
  exit 1
fi
...
if [[ -z "$source" ]]; then
  ...
  source="$generated_dir/h264-high-ippp-${width}x${height}-${rate}fps-${frames}frames.mp4"
  ffmpeg -hide_banner -loglevel error -y \
    ...
    -x264-params "keyint=${keyint}:min-keyint=${keyint}:scenecut=0:open-gop=0:bframes=0:ref=1:repeat-headers=1:cabac=1:8x8dct=1:weightp=0" \
    "$source"
fi
...
if [[ "$result" != "h264-ready-prefix-decode-output-readback-completed" || "$codec" != "h264-high-8" || "$profile" != "high-8" || "$session_parameters_created" != "true" || "$ready_prefix" -lt "$decode_prefix" || "$decode_requested" != "true" || "$decode_completed" != "true" || "$decoded_count" -ne "$decode_prefix" || "$frame_count" -ne "$decode_prefix" || "$reset_count" -lt 1 || "$non_idr_frames" -lt 1 || "$reference_frames" -lt 1 || "$readback_copied" != "true" || "$y_nonzero" -le 0 || "$uv_nonzero" -le 0 || "$readback_bytes" -le 0 ]]; then
  {
