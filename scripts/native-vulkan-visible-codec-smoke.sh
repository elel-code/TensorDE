...
frontend_route="$(jq -r '.video_runtime.frontend_route // "none"' "$runtime_json")"
frontend_decode_owner="$(jq -r '.video_runtime.frontend_decode_owner // "none"' "$runtime_json")"
frontend_memory_preference="$(jq -r '.video_runtime.frontend_memory_preference // "none"' "$runtime_json")"
frontend_sample_queue_policy="$(jq -r '.video_runtime.frontend_sample_queue_policy // "none"' "$runtime_json")"
...
selected_vulkan_drm_extension="$(jq -r '.video_runtime.selected_vulkan_drm_device.extension_available // "none"' "$runtime_json")"
selected_vulkan_drm_primary_node="$(jq -r '.video_runtime.selected_vulkan_drm_device.primary_node // "none"' "$runtime_json")"
selected_vulkan_drm_primary_dev_t="$(jq -r '.video_runtime.selected_vulkan_drm_device.primary_dev_t // "none"' "$runtime_json")"
selected_vulkan_drm_render_node="$(jq -r '.video_runtime.selected_vulkan_drm_device.render_node // "none"' "$runtime_json")"
selected_vulkan_drm_render_dev_t="$(jq -r '.video_runtime.selected_vulkan_drm_device.render_dev_t // "none"' "$runtime_json")"
...
if [[ "$configured" != "true" || "$frames_rendered" -lt 1 || "$frontend" != "gstreamer-appsink" || "$frontend_status" != "appsink-receiving-samples" || "$frames_received" -lt 1 || "$frames_imported" -lt 1 || "$last_sample_format" != "$expected_format" || "$last_import_memory_path" == "none" || "$last_import_error" != "none" ]]; then
  {
    ...
    printf 'frontend: %s\n' "$frontend"
    printf 'frontend_route: %s\n' "$frontend_route"
    printf 'frontend_decode_owner: %s\n' "$frontend_decode_owner"
    printf 'frontend_memory_preference: %s\n' "$frontend_memory_preference"
    printf 'frontend_sample_queue_policy: %s\n' "$frontend_sample_queue_policy"
    ...
    printf 'selected_vulkan_drm_extension: %s\n' "$selected_vulkan_drm_extension"
    printf 'selected_vulkan_drm_primary_node: %s\n' "$selected_vulkan_drm_primary_node"
    printf 'selected_vulkan_drm_primary_dev_t: %s\n' "$selected_vulkan_drm_primary_dev_t"
    printf 'selected_vulkan_drm_render_node: %s\n' "$selected_vulkan_drm_render_node"
    printf 'selected_vulkan_drm_render_dev_t: %s\n' "$selected_vulkan_drm_render_dev_t"
