...
#[test]
fn lowers_a_graphics_pair_to_slang_without_frontend_syntax() {
...
void main() { v_Uv = a_Position; gl_Position = vec4(a_Position + u_Draw.rows[0].xy, 0.0, 1.0); }"#;
    let lowered = lower_generated_stage_to_slang(source, ShaderStage::Vertex).unwrap();
...
void main() {
    ...
    let lowered = lower_generated_stage_to_slang(source, ShaderStage::Vertex)
...
fn production_spirv_reflects_fixed_array_stage_io_location_span() {
    let direct = lower_generated_stage_to_slang(
...
void main() {
    ...
    .expect("fixed array direct Slang");
...
fn rejects_non_fixed_or_nested_interface_array_declarators() {
    for declarator in ["values[]", "values[0]", "values[COUNT]", "values[2][2]"] {
        ...
        let error = lower_generated_stage_to_slang(&source, ShaderStage::Vertex)
...
void main() { o_Color = texture2D(g_Texture0, v_Uv); }"#;
    let lowered = lower_generated_stage_to_slang(source, ShaderStage::Fragment).unwrap();
    ...
    assert!(lowered.contains("int2 vulkanRendererTextureSize_g_Texture0(uint mip)"));
    ...
    assert!(heap.source.contains(
        "vulkanRendererHeap_ ## S ## _texture().Sample(vulkanRendererHeap_ ## S ## _sampler(), UV)"
    ));
    assert!(
        heap.source
            .contains("vulkanRendererHeap_g_Texture0_texture().GetDimensions")
    );
}
...
void main() {
    ...
    assert!(lowered.contains("VulkanRendererFragmentOutput output;"));
...
void main() {
    ...
    .expect("integer initializer Slang lowering");
    assert!(
...
void main() {
    ...
    .expect("authored mod function Slang lowering");
...
void main() {
    ...
    assert!(lowered.contains(
        "mul(vulkanRendererUniforms0.g_ModelViewProjectionMatrix, vec4(a_Position, 1.0))"
    ));
...
void main() {
    ...
    let direct = lower_generated_stage_to_slang(source, ShaderStage::Vertex).unwrap();
    let lowered = crate::lower_slang_bindings_to_descriptor_heap(&direct, "main").unwrap();

    assert!(direct.contains("struct VulkanRendererUniforms0Data"));
    assert!(direct.contains("VulkanRendererUniforms0Data vulkanRendererUniforms0;"));
    assert!(direct.contains(
        "vulkanRendererPosition = mul(vulkanRendererUniforms0.g_ModelViewProjectionMatrix, vec4(a_Position, 1.0));"
    ));
    assert!(direct.contains("vulkanRendererPosition.x += vulkanRendererUniforms0.g_Time;"));
    assert!(!direct.contains("#define g_Time"));
    assert!(
        lowered
            .source
            .contains("DescriptorHandle<ConstantBuffer<VulkanRendererUniforms0Data>>")
    );
    assert!(lowered.source.contains(
        "vulkanRendererPosition.x += vulkanRendererHeap_vulkanRendererUniforms0().g_Time;"
    ));
...
void main() {
    ...
    .expect("global uniform Slang lowering");
    assert!(direct.contains("vulkanRendererUniforms0.g_Color4 * vulkanRendererUniforms0.u_Lod;"));
    ...
    assert!(
        heap.source.contains(
            "vulkanRendererHeap_vulkanRendererUniforms0().g_Color4 * vulkanRendererHeap_vulkanRendererUniforms0().u_Lod;"
        )
    );
    let base = std::env::temp_dir().join(format!(
...
void main() {
    ...
    .expect("typed consumer Slang lowering");
    assert!(direct.contains("strength *= 500 / vulkanRendererUniforms0.g_Texture0Resolution.xy;"));
...
#[test]
fn prunes_unused_declared_resources_before_descriptor_heap_lowering() {
    let source = r#"layout(location = 0) out vec4 o_Color;
