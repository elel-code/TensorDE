...
//! It accepts only the small, explicit generated-stage grammar, emits Slang
...
/// Slang. The output deliberately has no `layout`, `#version`, GLSL
/// sampler declaration, or GLSL entry-point ABI.
pub fn lower_generated_stage_to_slang(source: &str, stage: ShaderStage) -> Result<String, String> {
    if stage == ShaderStage::Compute {
        return Err("generated compute stages must provide Slang directly".to_owned());
    }
...
fn collect_declarations(
    ...
    while line_index < lines.len() {
        ...
        if trimmed.starts_with("attribute ") || trimmed.starts_with("varying ") {
            return Err(format!(
                "legacy stage-I/O declaration reached Slang lowering: {trimmed}"
            ));
        }
        body.push(line);
        line_index += 1;
...
/// Gives generated global uniforms explicit cbuffer ownership before Slang
/// Slang sees the source. A macro alias such as `#define u_Lod
/// vulkanRendererUniforms0.u_Lod` recursively expands the field token in Slang's
...
fn qualify_global_uniform_uses(mut source: String, declarations: &Declarations) -> String {
    for (binding, uniforms) in &declarations.uniforms_by_binding {
        let instance = format!("vulkanRendererUniforms{binding}");
...
fn lower_entry_point(
    ...
    if stage == ShaderStage::Vertex {
        body = replace_identifier(&body, "gl_Position", "vulkanRendererPosition");
    } else {
        ...
        if has_identifier(&body, "gl_FragData") {
            return Err("gl_FragData is not supported by generated stage lowering".to_owned());
        }
    }
    ...
    if uses_vertex_index {
        setup.push_str("    gl_VertexIndex = input.vulkanRendererVertexIndex;\n");
    }
    if uses_instance_index {
        setup.push_str("    gl_InstanceIndex = input.vulkanRendererInstanceIndex;\n");
    }
    if uses_frag_coord {
        setup.push_str("    gl_FragCoord = input.vulkanRendererFragCoord;\n");
    }
    if uses_front_facing {
        setup.push_str("    gl_FrontFacing = input.vulkanRendererFrontFacing;\n");
    }
    setup.push_str(&format!("    {output_name} output;\n"));
    let mut finish = String::new();
    if stage == ShaderStage::Vertex {
        finish.push_str("    output.position = vulkanRendererPosition;\n");
    }
...
fn emit_resources(declarations: &Declarations) -> String {
    ...
    for (binding, uniforms) in &declarations.uniforms_by_binding {
        ...
        let struct_name = format!("VulkanRendererUniforms{binding}Data");
        let instance = format!("vulkanRendererUniforms{binding}");
        ...
        output.push_str(&format!(
            "}};\ncbuffer VulkanRendererUniforms{binding} : register(b{binding})\n{{\n    {struct_name} {instance};\n}}\n"
        ));
    }
    output
}
