//! Explicit native-Slang constructors for generated floating-vector initializers.
//!
...
//! declared width's leading components. Native Slang diagnoses that implicit
...
/// native Slang to reject explicitly.
...
/// expression by consuming the `.xy` components of the right-hand side; native
...
void main() {
    ...
    gl_Position = vec4(a_Position, 1.0);
        ...
        .expect("WE vector narrowing native Slang lowering");
        assert!(direct.contains(
            "vec2 scale = gilderUniforms0.g_Texture0Resolution.xy / gilderUniforms0.g_Texture1Resolution.xy;"
        ));
        assert!(direct.contains(
            "vec2 offset = gilderUniforms0.g_TexOffset / gilderUniforms0.g_Texture0Resolution.xy;"
        ));
