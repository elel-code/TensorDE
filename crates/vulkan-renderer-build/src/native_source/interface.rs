...
pub(super) fn emit_statics(
    ...
    if stage == ShaderStage::Vertex {
        output.push_str("static float4 gilderPosition;\n");
    }
    ...
    if entry.uses_vertex_index {
        output.push_str("    uint gilderVertexIndex : SV_VertexID;\n");
    }
    if entry.uses_instance_index {
        output.push_str("    uint gilderInstanceIndex : SV_InstanceID;\n");
    }
    if entry.uses_frag_coord {
        output.push_str("    float4 gilderFragCoord : SV_Position;\n");
    }
    if entry.uses_front_facing {
        output.push_str("    bool gilderFrontFacing : SV_IsFrontFace;\n");
    }
    output.push_str("};\n");
