...
fn maximum_overview_plan_fits_the_bounded_frame() {
    ...
    crate::land::encode(&response).expect("the declared overview prefix must fit one IPC frame");
}

#[test]
fn response_and_event_envelopes_can_share_one_completed_read() {
    let mut frames = crate::land::encode(&ServerMessage::Response(Response::new(
        7,
        ResultBody::Accepted,
    )))
    .unwrap();
    frames.extend(
        crate::land::encode(&ServerMessage::Event(EventMessage::new(
            1,
    ...
    let messages = crate::land::FrameDecoder::new()
