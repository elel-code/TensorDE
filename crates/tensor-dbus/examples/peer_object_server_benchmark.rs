...
use tensor_dbus::{Connection, Guid, ObjectServer, PeerListener, PendingReply};
...
async fn round_trips(
    ...
    while completed < operations {
        ...
        for _ in 0..current_batch {
            pending.push(
                connection
                    .send_call(None, PATH, Some(INTERFACE), "Echo", &payload)
                    .await?,
            );
        }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let socket = std::env::temp_dir().join(format!(
        "tensor-dbus-object-benchmark-{}.sock",
        std::process::id()
    ));
    ...
    runtime.block_on(async {
        ...
        let listener = PeerListener::bind(&socket, guid).await?;
        let server_task = compio::runtime::spawn(async move {
            let mut connection = listener.accept_authenticated().await?;
            let mut objects = ObjectServer::new();
            objects.register::<Vec<u8>, Vec<u8>, _, _>(PATH, INTERFACE, "Echo", |body| async {
                Ok(body)
            })?;
            for _ in 0..total_operations {
                let unrelated = objects.serve_next(&mut connection).await?;
        ...
        let mut client = Connection::connect_peer(stream, Some(guid)).await?;
