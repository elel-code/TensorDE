...
/// Scan for `"` only — the raw-string delimiter path has no escape semantics.
#[inline(always)]
pub fn find_quote_fast(input: &[u8], index: usize, end: usize) -> usize {
    #[cfg(all(feature = "simd", target_arch = "x86_64"))]
    {
        find_quote_sse2(input, index, end)
    }
    #[cfg(not(all(feature = "simd", target_arch = "x86_64")))]
    {
        super::swar::find_quote(input, index, end)
    }
}
...
/// x86_64 16-byte dual-SWAR raw delimiter scan.
#[cfg(all(feature = "simd", target_arch = "x86_64"))]
#[inline(always)]
fn find_quote_sse2(input: &[u8], mut index: usize, end: usize) -> usize {
    use super::swar::{find_quote, first_lane, has_byte, load_u64_unaligned};

    while index + 16 <= end {
        let c0 = load_u64_unaligned(&input[index..index + 8]);
        let c1 = load_u64_unaligned(&input[index + 8..index + 16]);
        let h0 = has_byte(c0, b'"');
        let h1 = has_byte(c1, b'"');
        if h0 != 0 {
            return index + first_lane(h0);
        }
        if h1 != 0 {
...
mod tests {
    ...
    #[test]
    fn raw_quote_scan_skips_backslashes() {
        let s = b"01234567\\abcdef\"tail";
        assert_eq!(find_quote_fast(s, 0, s.len()), 15);
    }
}
