//! Shader-declared Wallpaper Engine runtime-target texture defaults.
//!
//! Effect instance JSON contains overrides, while the authored shader is the
//! authority for hidden bindings such as `_rt_FullFrameBuffer`. Parse these on
//! the converter cold path so the render graph never has to infer descriptors.
...
pub(super) struct ShaderRuntimeTextureDefault {
    pub slot: u32,
    pub target: String,
    requirements: Vec<(String, i64)>,
}

pub(super) fn parse_shader_runtime_texture_defaults(
    source: &str,
) -> Result<Vec<ShaderRuntimeTextureDefault>, String> {
    ...
    for (line_index, line) in source.lines().enumerate() {
        ...
        defaults.push(ShaderRuntimeTextureDefault {
            slot,
...
pub(super) fn apply_shader_runtime_texture_defaults(
    defaults: &[ShaderRuntimeTextureDefault],
...
impl ShaderRuntimeTextureDefault {
    fn is_enabled(
        &self,
        combos: &BTreeMap<String, i64>,
        combo_defaults: &BTreeMap<String, i64>,
...
fn is_runtime_target(path: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn parses_hidden_full_framebuffer_sampler_and_its_slot() {
        let source = r#"
...
"#;
        ...
        assert_eq!(
            parse_shader_runtime_texture_defaults(source).expect("texture defaults"),
            vec![ShaderRuntimeTextureDefault {
                slot: 2,
    ...
    #[test]
    fn applies_runtime_target_only_when_authored_combo_requirement_is_active() {
        let defaults = parse_shader_runtime_texture_defaults(
    ...
    fn rejects_malformed_sampler_metadata_instead_of_silently_dropping_it() {
        let error = parse_shader_runtime_texture_defaults(
