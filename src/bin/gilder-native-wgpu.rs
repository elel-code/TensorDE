    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--video-backend" => {
                let value = args
                    .next()
                    .ok_or("--video-backend requires auto, cpu-upload, or gst-dmabuf")?;
...
fn parse_video_backend(
    ...
    match value {
        ...
        other => Err(format!(
            "unsupported video backend: {other}; expected auto, cpu-upload, or gst-dmabuf"
        )
        .into()),
    }
}
...
fn resolve_video_backend(
    ...
    if cfg!(feature = "native-wgpu-gst-dmabuf") {
        NativeWgpuCliVideoBackend::GstDmabuf
    } else {
...
fn print_usage() {
    println!(
        "usage: gilder-native-wgpu [--source <path>] [--video-backend auto|cpu-upload|gst-dmabuf] [--duration <seconds>] [--target-fps <fps>|--no-fps-limit] [--layer background|bottom|top|overlay] [--allow-foreground-layer] [--output-name <name>] [--color #rrggbb|r,g,b] [--fit cover|contain|stretch|center] [--decoder auto|hardware-preferred|hardware-required|software] [--loop|--no-loop] [--render-mode solid|pulse] [--animate-color] [--runtime-json <path>] [--runtime-jsonl <path>] [--runtime-interval-ms <ms>]"
    );
}
