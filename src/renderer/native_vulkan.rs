...
#[cfg(feature = "native-vulkan-gst-video")]
pub fn run_h265_first_frame_video(
    options: NativeVulkanOptions,
    duration: Duration,
    source: PathBuf,
    width: u32,
...
) -> Result<NativeVulkanDirectH265FirstFrameRuntimeSnapshot, NativeVulkanError> {
    if width == 0 || height == 0 {
        return Err(NativeVulkanError::Video(
            "direct H.265 first-frame video requires a non-zero source extent".to_owned(),
        ));
    }
    let target_max_fps = options.target_max_fps;
    ...
    probe.pump_events()?;
    let surface_snapshot = probe.snapshot();
    let present_query =
        select_native_vulkan_present_queue(&probe.instance, &probe.surface_loader, probe.surface)?;
    let present_selection = present_query.selection;
    let present_queue_family_index = present_selection.queue_family_index;
    let present_queue_families = unsafe {
        probe
            .instance
            .get_physical_device_queue_family_properties(present_selection.physical_device)
    };
    let present_queue_flags = present_queue_families
        .get(present_queue_family_index as usize)
        .map(|queue| queue.queue_flags)
        .unwrap_or_default();
    let video_queue_family = native_vulkan_video_decode_queue_family_infos(
        &probe.instance,
    ...
    .ok_or_else(|| {
        NativeVulkanError::Video(format!(
            "selected present device {} has no same-device H.265 Vulkan Video decode queue",
            present_selection.physical_device_name
        ))
    })?;
    ...
    for extension in native_vulkan_video_session_required_device_extensions(
    ...
    if !native_vulkan_video_session_extent_supported(source_extent, &capabilities) {
        return Err(NativeVulkanError::Video(format!(
            "direct H.265 source extent {width}x{height} is outside {:?}..{:?} or not aligned to {:?}",
            (
    ...
    {
        return Err(NativeVulkanError::Video(format!(
            "direct H.265 first-frame video requires NV12 decode+sampled support{}",
            format_probe
                .query_error
                .as_ref()
                .map(|err| format!(": {err}"))
    ...
    let mut device_queue_family_indices = vec![present_queue_family_index];
    ...
    enabled_extensions.extend(
        native_vulkan_video_session_required_device_extensions(
            NativeVulkanVideoSessionCodec::H265Main8,
        )
        .iter()
    ...
    let device = unsafe {
        probe
            .instance
            .create_device(present_selection.physical_device, &device_create_info, None)
    }
    .map_err(|result| NativeVulkanError::Vulkan {
        operation: "vkCreateDevice(direct h265 first-frame video)",
        result,
    })?;

    let mut swapchain_loader = None::<ash::khr::swapchain::Device>;
    let mut swapchain = vk::SwapchainKHR::null();
    let mut swapchain_image_views = Vec::<vk::ImageView>::new();
    ...
    let result =
        (|| -> Result<NativeVulkanDirectH265FirstFrameRuntimeSnapshot, NativeVulkanError> {
            ...
            let video_decode_queue_device =
                ash::khr::video_decode_queue::Device::new(&probe.instance, &device);
            let memory_properties = unsafe {
                probe
                    .instance
                    .get_physical_device_memory_properties(present_selection.physical_device)
            };

            swapchain_loader = Some(ash::khr::swapchain::Device::new(&probe.instance, &device));
            let swapchain_loader_ref = swapchain_loader
                .as_ref()
                .expect("swapchain loader was just created");
            let swapchain_plan = create_native_vulkan_swapchain_plan(
                &probe.surface_loader,
                present_selection.physical_device,
                probe.surface,
                surface_snapshot.wayland_surface_logical_size,
                surface_snapshot.wayland_surface_buffer_size,
            )?;
            swapchain =
                unsafe { swapchain_loader_ref.create_swapchain(&swapchain_plan.create_info, None) }
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkCreateSwapchainKHR(direct h265 first-frame video)",
                        result,
                    })?;
            let swapchain_images = unsafe { swapchain_loader_ref.get_swapchain_images(swapchain) }
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkGetSwapchainImagesKHR(direct h265 first-frame video)",
                    result,
                })?;
            swapchain_image_views = create_native_vulkan_swapchain_image_views(
                &device,
                &swapchain_images,
                swapchain_plan.format.format,
            )?;
            ...
            command_pool = unsafe { device.create_command_pool(&command_pool_info, None) }
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkCreateCommandPool(direct h265 first-frame present)",
                    result,
                })?;
            ...
            command_buffers = unsafe { device.allocate_command_buffers(&command_buffer_info) }
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkAllocateCommandBuffers(direct h265 first-frame present)",
                    result,
                })?;
            let semaphore_create_info = vk::SemaphoreCreateInfo::default();
            image_available = unsafe { device.create_semaphore(&semaphore_create_info, None) }
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkCreateSemaphore(direct h265 image_available)",
                    result,
                })?;
            render_finished = unsafe { device.create_semaphore(&semaphore_create_info, None) }
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkCreateSemaphore(direct h265 render_finished)",
                    result,
                })?;
            let fence_create_info =
                vk::FenceCreateInfo::default().flags(vk::FenceCreateFlags::SIGNALED);
            in_flight =
                unsafe { device.create_fence(&fence_create_info, None) }.map_err(|result| {
                    NativeVulkanError::Vulkan {
                        operation: "vkCreateFence(direct h265 first-frame)",
                        result,
                    }
                })?;
            ...
            let create_info = vk::VideoSessionCreateInfoKHR::default()
                .queue_family_index(video_queue_family.queue_family_index)
                .video_profile(&profile_info)
                .picture_format(picture_format)
                .reference_picture_format(picture_format)
                .max_coded_extent(source_extent)
            ...
            video_session = native_vulkan_create_video_session(&video_queue_device, &create_info)?;

            let memory_requirements = native_vulkan_video_session_memory_requirements(
                &video_queue_device,
                video_session,
            )?;
            let mut bind_infos = Vec::with_capacity(memory_requirements.len());
            let mut session_memory_bytes = 0u64;
            for requirement in memory_requirements.iter() {
                if requirement.memory_requirements.size == 0 {
                    return Err(NativeVulkanError::Video(format!(
                        "direct H.265 session memory bind {} reported zero size",
                ...
                .ok_or(NativeVulkanError::MissingMemoryType(
                    "direct H.265 video session memory",
                ))?;
                let allocation_info = vk::MemoryAllocateInfo::default()
                    .allocation_size(requirement.memory_requirements.size)
                    .memory_type_index(memory_type_index);
                let memory = unsafe { device.allocate_memory(&allocation_info, None) }.map_err(
                    |result| NativeVulkanError::Vulkan {
                        operation: "vkAllocateMemory(direct h265 video session)",
                        result,
                    },
                )?;
                ...
                session_memory_bytes =
                    session_memory_bytes.saturating_add(requirement.memory_requirements.size);
            }
            native_vulkan_bind_video_session_memory(
                &video_queue_device,
                video_session,
                &bind_infos,
            )?;

            resource_image = Some(native_vulkan_create_video_session_resource_image(
                &video_queue_loader,
                &device,
                &memory_properties,
                present_selection.physical_device,
            ...
            let mut extract_options = NativeVulkanVideoSessionSmokeOptions::default();
            extract_options.codec = NativeVulkanVideoSessionCodec::H265Main8;
            extract_options.width = width;
            extract_options.height = height;
            extract_options.extract_bitstream = true;
            extract_options.bitstream_source = Some(source.clone());
            extract_options.bitstream_extract_max_samples = bitstream_samples.max(1);
            ...
            let parameter_sets =
                extract
                    ...
                    .ok_or_else(|| {
                        NativeVulkanError::Video(
                            "direct H.265 first-frame video requires parsed H.265 parameter sets"
                                .to_owned(),
                        )
                    })?;
            let bitstream_payload = extract.selected_access_unit.as_slice();
            let requested_bitstream_size = native_vulkan_align_up(
                bitstream_payload.len() as u64,
                capabilities.min_bitstream_buffer_size_alignment.max(1),
            );
            bitstream_buffer = Some(native_vulkan_create_video_session_bitstream_buffer(
                &device,
                &memory_properties,
                &profile_info,
                requested_bitstream_size,
                capabilities.min_bitstream_buffer_size_alignment,
                Some(bitstream_payload),
            )?);
            let parameters = native_vulkan_create_h265_video_session_parameters(
                &video_queue_device,
                video_session,
                parameter_sets,
            )?;
            video_session_parameters = parameters.parameters;
            let image = resource_image
                .as_ref()
                .expect("video resource image was just created");
            let buffer = bitstream_buffer
                .as_ref()
                .expect("bitstream buffer was just created");
            let decode = native_vulkan_decode_h265_first_frame_to_image(
                &device,
                &video_queue_device,
                &video_decode_queue_device,
                video_queue_family.queue_family_index,
                video_session,
            ...
            renderer_ref.update_descriptors(
                &device,
                texture.as_ref().expect("decoded texture was just created"),
            );
            ...
            while Instant::now() < deadline && !probe.host.is_closed() {
                ...
                unsafe {
                    device
                        .wait_for_fences(&fences, true, u64::MAX)
                        .map_err(|result| NativeVulkanError::Vulkan {
                            operation: "vkWaitForFences(direct h265 first-frame)",
                            result,
                        })?;
                    device
                        .reset_fences(&fences)
                        .map_err(|result| NativeVulkanError::Vulkan {
                            operation: "vkResetFences(direct h265 first-frame)",
                            result,
                        })?;
                }
                let (image_index, _) = unsafe {
                ...
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkAcquireNextImageKHR(direct h265 first-frame)",
                    result,
                })?;
                let image_index = image_index as usize;
                renderer_ref.record_frame(
                    ...
                    texture
                        .as_ref()
                        .expect("decoded texture exists during render"),
                    fit,
                )?;
                swapchain_image_layouts[image_index] = renderer_ref.target_final_layout();
                let wait_semaphores = [image_available];
                let wait_stages = [vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT];
                let command_buffers_for_submit = [command_buffers[image_index]];
                ...
                unsafe {
                    device
                        ...
                        .map_err(|result| NativeVulkanError::Vulkan {
                            operation: "vkQueueSubmit(direct h265 first-frame present)",
                            result,
                        })?;
                }
                ...
                unsafe {
                    swapchain_loader_ref
                        ...
                        .map_err(|result| NativeVulkanError::Vulkan {
                            operation: "vkQueuePresentKHR(direct h265 first-frame)",
                            result,
                        })?;
                }
                frames_rendered = frames_rendered.saturating_add(1);
                if let Some(NativeVulkanVideoTexture::Decoded(texture)) = texture.as_mut() {
                    texture.source_layout = vk::ImageLayout::SHADER_READ_ONLY_OPTIMAL;
                    texture.source_access = vk::AccessFlags::SHADER_READ;
                }
            ...
            unsafe {
                device
                    ...
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkWaitForFences(direct h265 first-frame final)",
                        result,
                    })?;
            }

            let elapsed = started_at.elapsed();
            Ok(NativeVulkanDirectH265FirstFrameRuntimeSnapshot {
                runtime_elapsed_ms: elapsed.as_millis().min(u64::MAX as u128) as u64,
                frames_rendered,
                average_render_fps: if elapsed.is_zero() {
                    0.0
                } else {
                    frames_rendered as f64 / elapsed.as_secs_f64()
                },
                ...
                fit,
                target_max_fps,
                wayland_surface_logical_size: surface_snapshot.wayland_surface_logical_size,
                wayland_surface_buffer_size: surface_snapshot.wayland_surface_buffer_size,
                selected_physical_device_name: present_selection.physical_device_name,
                selected_physical_device_type: present_selection.physical_device_type,
                present_queue_family_index,
                present_queue_flags: native_vulkan_queue_flag_labels(present_queue_flags),
                video_decode_queue_family_index: video_queue_family.queue_family_index,
                video_decode_queue_flags: native_vulkan_queue_flag_labels(
                    video_queue_family.queue_flags,
                ),
                video_decode_queue_codec_operations: native_vulkan_video_codec_operation_labels(
                    video_queue_family.video_codec_operations,
                ),
                cross_queue_sync_strategy: "decode-queue-wait-idle-then-present-queue-sampled-render",
                ...
                pacing_strategy,
                video_resource_memory_bytes: image.snapshot.memory_size,
                session_memory_bytes,
    ...
    unsafe {
        ...
        if let Some(image) = resource_image.take() {
            native_vulkan_destroy_video_session_resource_image(&device, image);
        }
        ...
        if swapchain != vk::SwapchainKHR::null()
            && let Some(loader) = swapchain_loader.as_ref()
        {
...
pub fn run_h265_ready_prefix_video(
    options: NativeVulkanOptions,
    source: PathBuf,
    width: u32,
    height: u32,
    ...
    if !native_vulkan_video_session_extent_supported(source_extent, &capabilities) {
        return Err(NativeVulkanError::Video(format!(
            "direct H.265 ready-prefix source extent {width}x{height} is outside {:?}..{:?} or not aligned to {:?}",
            (
                capabilities.min_coded_extent.width,
                capabilities.min_coded_extent.height
            ),
    ...
    {
        return Err(NativeVulkanError::Video(format!(
            "direct H.265 ready-prefix video requires NV12 decode+sampled support{}",
            format_probe
                .query_error
                .as_ref()
                .map(|err| format!(": {err}"))
    ...
    .map_err(|result| NativeVulkanError::Vulkan {
        operation: "vkCreateDevice(direct h265 ready-prefix video)",
        result,
    })?;
    ...
    let result =
        (|| -> Result<NativeVulkanDirectH265ReadyPrefixRuntimeSnapshot, NativeVulkanError> {
            ...
            swapchain =
                unsafe { swapchain_loader_ref.create_swapchain(&swapchain_plan.create_info, None) }
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkCreateSwapchainKHR(direct h265 ready-prefix video)",
                        result,
                    })?;
            let swapchain_images = unsafe { swapchain_loader_ref.get_swapchain_images(swapchain) }
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkGetSwapchainImagesKHR(direct h265 ready-prefix video)",
                    result,
                })?;
            swapchain_image_views = create_native_vulkan_swapchain_image_views(
                &device,
            ...
            let present_command_pool_info = vk::CommandPoolCreateInfo::default()
                .flags(vk::CommandPoolCreateFlags::RESET_COMMAND_BUFFER)
                .queue_family_index(present_queue_family_index);
            present_command_pool =
                unsafe { device.create_command_pool(&present_command_pool_info, None) }.map_err(
                    |result| NativeVulkanError::Vulkan {
                        operation: "vkCreateCommandPool(direct h265 ready-prefix present)",
                        result,
                    },
                )?;
            ...
            present_command_buffers =
                unsafe { device.allocate_command_buffers(&present_command_buffer_info) }.map_err(
                    |result| NativeVulkanError::Vulkan {
                        operation: "vkAllocateCommandBuffers(direct h265 ready-prefix present)",
                        result,
                    },
                )?;
            let video_command_pool_info = vk::CommandPoolCreateInfo::default()
                .flags(
                ...
                .queue_family_index(video_queue_family.queue_family_index);
            video_command_pool =
                unsafe { device.create_command_pool(&video_command_pool_info, None) }.map_err(
                    |result| NativeVulkanError::Vulkan {
                        operation: "vkCreateCommandPool(direct h265 ready-prefix decode)",
                        result,
                    },
                )?;
            ...
            for requirement in memory_requirements.iter() {
                if requirement.memory_requirements.size == 0 {
                    return Err(NativeVulkanError::Video(format!(
                        "direct H.265 ready-prefix session memory bind {} reported zero size",
                        requirement.memory_bind_index
                    )));
                }
                let memory_type_index = native_vulkan_memory_type_index(
                ...
                .ok_or(NativeVulkanError::MissingMemoryType(
                    "direct H.265 ready-prefix video session memory",
                ))?;
                let allocation_info = vk::MemoryAllocateInfo::default()
                    .allocation_size(requirement.memory_requirements.size)
                    .memory_type_index(memory_type_index);
                let memory = unsafe { device.allocate_memory(&allocation_info, None) }.map_err(
                    |result| NativeVulkanError::Vulkan {
                        operation: "vkAllocateMemory(direct h265 ready-prefix video session)",
                        result,
                    },
                )?;
                session_memories.push(memory);
            ...
            bitstream_buffer = Some(
                native_vulkan_create_video_session_bitstream_buffer_with_mapping(
                    ...
                    &memory_properties,
                    &profile_info,
                    streaming_bitstream.ring_capacity_bytes,
                    capabilities.min_bitstream_buffer_size_alignment,
                    None,
                    true,
                )?,
            ...
            for playback_frame_index in 0..playback_frame_count {
                ...
                if let Some(slot) = active_dpb_pocs.get_mut(frame.dst_base_array_layer as usize) {
                    *slot = Some(frame.pic_order_cnt_val);
                }

                let present_started_at = Instant::now();
                let texture =
                    NativeVulkanVideoTexture::Decoded(NativeVulkanDecodedVideoTexture::new(
                        &device,
                        image.image,
                        width,
                        height,
            ...
            Ok(NativeVulkanDirectH265ReadyPrefixRuntimeSnapshot {
                ...
                bitstream_buffer_slot_count: streaming_bitstream.ring_slot_count,
                bitstream_buffer_slot_bytes: streaming_bitstream.slot_bytes,
                bitstream_ring_capacity_bytes: streaming_bitstream.ring_capacity_bytes,
...
struct NativeVulkanVideoResourceImage {
    ...
    separate_slots: Vec<NativeVulkanVideoResourceImageSlot>,
    snapshot: NativeVulkanVideoSessionResourceImageSnapshot,
}

struct NativeVulkanVideoResourceImageSlot {
    image: vk::Image,
    memory: vk::DeviceMemory,
    view: vk::ImageView,
}
...
impl NativeVulkanVideoResourceImage {
    fn uses_separate_slots(&self) -> bool {
        !self.separate_slots.is_empty()
    }

    fn slot_image(&self, slot: u32) -> Result<vk::Image, NativeVulkanError> {
    ...
    fn slot_base_array_layer(&self, slot: u32) -> u32 {
        if self.uses_separate_slots() { 0 } else { slot }
    }

    fn slot_view(&self, slot: u32) -> Result<vk::ImageView, NativeVulkanError> {
        if self.uses_separate_slots() {
            self.separate_slots
                .get(slot as usize)
                .map(|slot| slot.view)
...
#[cfg(feature = "native-vulkan-gst-video")]
struct NativeVulkanVideoBitstreamSpoolExtract {
    file: File,
    path: PathBuf,
    records: Vec<NativeVulkanH265AccessUnitSpoolRecord>,
    file_position: u64,
    snapshot: NativeVulkanVideoBitstreamExtractSnapshot,
}

#[cfg(feature = "native-vulkan-gst-video")]
impl NativeVulkanVideoBitstreamSpoolExtract {
    fn write_payload_to_bitstream_ring(
        ...
        let record = self.records.get(index).ok_or_else(|| {
            NativeVulkanError::Video(format!(
                "H.265 spooled bitstream has {} records but AU {index} was requested",
                self.records.len()
            ))
        })?;
        let span = ring.allocate(record.len, record.slice_segment_offset)?;
        native_vulkan_write_video_session_bitstream_buffer_from_spool(
            device,
            buffer,
            span.offset,
            span.range,
...
#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug, Clone)]
struct NativeVulkanH265AccessUnitSpoolRecord {
    offset: u64,
    len: u64,
...
#[cfg(feature = "native-vulkan-gst-video")]
impl NativeVulkanH265AccessUnitSpoolRecord {
    fn to_extract(&self, bytes: Vec<u8>) -> NativeVulkanH265AccessUnitExtract {
        NativeVulkanH265AccessUnitExtract {
            bytes,
            pts_ms: self.pts_ms,
...
fn native_vulkan_create_video_decode_status_query_pool(
    device: &ash::Device,
    profile_info: &vk::VideoProfileInfoKHR<'_>,
    query_count: u32,
) -> Result<vk::QueryPool, NativeVulkanError> {
    let mut query_profile_info = *profile_info;
    let query_pool_info = vk::QueryPoolCreateInfo::default()
        .query_type(vk::QueryType::RESULT_STATUS_ONLY_KHR)
        .query_count(query_count)
        .push_next(&mut query_profile_info);
    unsafe { device.create_query_pool(&query_pool_info, None) }.map_err(|result| {
        NativeVulkanError::Vulkan {
            operation: "vkCreateQueryPool(video decode status)",
            result,
        }
    })
}

fn native_vulkan_video_decode_status_query_results(
    device: &ash::Device,
...
fn native_vulkan_query_result_status_label(status: vk::QueryResultStatusKHR) -> &'static str {
    if status == vk::QueryResultStatusKHR::COMPLETE {
        "complete"
    } else if status == vk::QueryResultStatusKHR::NOT_READY {
        "not-ready"
    } else if status == vk::QueryResultStatusKHR::ERROR {
        "error"
    } else if status == vk::QueryResultStatusKHR::INSUFFICIENTSTREAM_BUFFER_RANGE {
        "insufficient-bitstream-buffer-range"
    } else if status.as_raw() < 0 {
        "error-other"
    } else {
        "unknown-positive"
    }
...
fn native_vulkan_apply_decode_query_status(
    frame: &mut NativeVulkanH265DecodedFrameSnapshot,
    status: vk::QueryResultStatusKHR,
) {
    frame.decode_query_status_raw = Some(status.as_raw());
    frame.decode_query_status = Some(native_vulkan_query_result_status_label(status));
}
    ...
    for (physical_device_index, physical_device) in physical_devices.iter().copied().enumerate() {
        ...
        for queue_family in queue_families {
            ...
            let candidate = NativeVulkanVideoDecodeQueueSelection {
                ...
                query_result_status_support: queue_family.query_result_status_support,
            };
...
fn native_vulkan_video_decode_queue_family_infos(
    ...
    let mut query_result_status_properties =
        vec![vk::QueueFamilyQueryResultStatusPropertiesKHR::default(); queue_family_count];
    for ((queue, video), query_result) in queue_properties
        .iter_mut()
        .zip(video_properties.iter_mut())
        .zip(query_result_status_properties.iter_mut())
    {
        query_result.p_next = (video as *mut vk::QueueFamilyVideoPropertiesKHR<'_>).cast();
        queue.p_next =
            (query_result as *mut vk::QueueFamilyQueryResultStatusPropertiesKHR<'_>).cast();
    }
    unsafe {
        instance
    ...
    queue_properties
        ...
        .zip(query_result_status_properties.iter())
        .enumerate()
        .map(|(queue_family_index, ((queue, video), query_result))| {
            NativeVulkanVideoDecodeQueueFamilyInfo {
                queue_family_index: queue_family_index as u32,
                ...
                query_result_status_support: query_result.query_result_status_support != 0,
            }
        })
        .collect()
}
...
fn native_vulkan_video_session_create_and_bind(
    ...
    let available_extensions =
        native_vulkan_device_extension_names(instance, selection.physical_device)?;
    let mut enabled_extensions =
        native_vulkan_video_session_required_device_extensions(options.codec);
    ...
    {
        enabled_extensions.push(vk::KHR_VIDEO_MAINTENANCE1_NAME);
    }
    let decode_query_status_enabled =
        video_maintenance1_enabled && selection.query_result_status_support;
    ...
    let result = (|| -> Result<NativeVulkanVideoSessionSmokeSnapshot, NativeVulkanError> {
        ...
        let session_max_dpb_slots = bitstream_extract
            .as_ref()
        ...
        if options.allocate_video_images {
            let separate_resource_images = options.codec
                == NativeVulkanVideoSessionCodec::H265Main8
                && options.decode_h265_ready_prefix_frames > 0
                && capabilities
                    .capability_flags
                    .contains(vk::VideoCapabilityFlagsKHR::SEPARATE_REFERENCE_IMAGES)
                && !matches!(
                    std::env::var("GILDER_VULKAN_H265_SEPARATE_REFERENCE_IMAGES")
                        .ok()
                        .as_deref(),
        ...
        let h265_ready_prefix_bitstream = if options.decode_h265_ready_prefix_frames > 0 {
            let extract = bitstream_extract.as_ref().ok_or_else(|| {
                NativeVulkanError::Video(
                    "--decode-h265-ready-prefix requires --extract-bitstream".to_owned(),
                )
            })?;
            Some(native_vulkan_h265_ready_prefix_bitstream_payload(
                extract,
        ...
        if options.allocate_bitstream_buffer {
            ...
            let keep_mapped = options.decode_h265_ready_prefix_frames > 0
                && options.sample_h265_ready_prefix_sequence
                && native_vulkan_h265_sequence_streaming_bitstream_enabled();
            bitstream_buffer = Some(
                native_vulkan_create_video_session_bitstream_buffer_with_mapping(
                    &device,
                    &memory_properties,
        ...
        let session_parameters = match options.codec {
            ...
            NativeVulkanVideoSessionCodec::Av1Main8 | NativeVulkanVideoSessionCodec::Av1Main10 => {
                match bitstream_extract
                    ...
                    .map(|sequence_header| {
                        native_vulkan_create_av1_video_session_parameters(
                            &video_queue_device,
                            session,
                            sequence_header,
                        )
        ...
        let first_frame_decode = if options.decode_first_frame {
            let parameters = session_parameters.as_ref().ok_or_else(|| {
                NativeVulkanError::Video(
                    "--decode-first-frame requires codec session parameters".to_owned(),
                )
            })?;
            ...
            Some(match options.codec {
                NativeVulkanVideoSessionCodec::H264High8 => {
                    let parameter_sets =
                        extract
                            ...
                            .ok_or_else(|| {
                                NativeVulkanError::Video(
                                    "--decode-first-frame requires parsed H.264 parameter sets"
                                        .to_owned(),
                                )
                            })?;
                    native_vulkan_decode_h264_first_frame_smoke(
                        &device,
                        &video_queue_device,
                        &video_decode_queue_device,
                        selection.queue_family_index,
                ...
                | NativeVulkanVideoSessionCodec::H265Main10 => {
                    let parameter_sets =
                        extract
                            ...
                            .ok_or_else(|| {
                                NativeVulkanError::Video(
                                    "--decode-first-frame requires parsed H.265 parameter sets"
                                        .to_owned(),
                                )
                            })?;
                    native_vulkan_decode_h265_first_frame_smoke(
                        &device,
                        &video_queue_device,
                        &video_decode_queue_device,
                        selection.queue_family_index,
                ...
                | NativeVulkanVideoSessionCodec::Av1Main10 => {
                    return Err(NativeVulkanError::Video(
                        "--decode-first-frame does not support AV1 direct decode yet".to_owned(),
                    ));
                }
            })
        } else {
        ...
        let h265_ready_prefix_decode = if options.decode_h265_ready_prefix_frames > 0 {
            let parameters = session_parameters.as_ref().ok_or_else(|| {
                NativeVulkanError::Video(
                    "--decode-h265-ready-prefix requires H.265 session parameters".to_owned(),
                )
            })?;
            let image = resource_images.first().ok_or_else(|| {
                NativeVulkanError::Video(
                    "--decode-h265-ready-prefix requires --allocate-video-images".to_owned(),
                )
            })?;
            let buffer = bitstream_buffer.as_ref().ok_or_else(|| {
                NativeVulkanError::Video(
                    "--decode-h265-ready-prefix requires --allocate-bitstream-buffer".to_owned(),
                )
            })?;
            let extract = bitstream_extract.as_ref().ok_or_else(|| {
                NativeVulkanError::Video(
            ...
            let parameter_sets =
                extract
                    ...
                    .ok_or_else(|| {
                        NativeVulkanError::Video(
                            "--decode-h265-ready-prefix requires parsed H.265 parameter sets"
                                .to_owned(),
                        )
                    })?;
            let payload = h265_ready_prefix_bitstream.as_ref().ok_or_else(|| {
                NativeVulkanError::Video(
                    "--decode-h265-ready-prefix requires ready-prefix bitstream payload".to_owned(),
                )
            })?;
            Some(native_vulkan_decode_h265_ready_prefix_smoke(
                &device,
                &video_queue_device,
                &video_decode_queue_device,
                selection.queue_family_index,
        ...
        Ok(NativeVulkanVideoSessionSmokeSnapshot {
            ...
            selected_queue_query_result_status_support: selection.query_result_status_support,
...
fn native_vulkan_destroy_video_session_resource_image(
    device: &ash::Device,
...
fn native_vulkan_create_video_session_bitstream_buffer_with_mapping(
    device: &ash::Device,
    memory_properties: &vk::PhysicalDeviceMemoryProperties,
    profile_info: &vk::VideoProfileInfoKHR<'_>,
    requested_size: u64,
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_write_video_session_bitstream_buffer(
    device: &ash::Device,
    buffer: &NativeVulkanVideoBitstreamBuffer,
    offset: u64,
    range: u64,
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_write_video_session_bitstream_buffer_from_spool(
    device: &ash::Device,
    buffer: &NativeVulkanVideoBitstreamBuffer,
    offset: u64,
    range: u64,
    file: &mut File,
    path: &Path,
    record: &NativeVulkanH265AccessUnitSpoolRecord,
    ...
    if record.len > range {
        return Err(NativeVulkanError::Video(format!(
            "spooled video bitstream payload has {} bytes but decode range is {range}",
            record.len
        )));
    }
    let end = offset.checked_add(range).ok_or_else(|| {
    ...
    let payload_len = usize::try_from(record.len).map_err(|_| {
        NativeVulkanError::Video(format!(
            "H.265 spooled AU length {} does not fit usize",
            record.len
        ))
    })?;
    ...
    unsafe {
        ...
        file.read_exact(dst_payload).map_err(|err| {
            NativeVulkanError::Video(format!(
                "read H.265 AU spool {} at {} for {} bytes into bitstream buffer: {err}",
                path.display(),
                record.offset,
                record.len
            ))
        ...
        *file_position = record.offset.saturating_add(record.len);
        if range > record.len {
            ...
            let clear_len = usize::try_from(range - record.len).map_err(|_| {
                NativeVulkanError::Video(
                    "video bitstream padding length does not fit usize".to_owned(),
                )
            })?;
...
fn native_vulkan_decode_h264_first_frame_smoke(
    ...
    if session_parameters == vk::VideoSessionParametersKHR::null() {
        return Err(NativeVulkanError::Video(
            "H.264 first-frame decode requires VkVideoSessionParametersKHR".to_owned(),
        ));
    }
    if image.snapshot.array_layers == 0 {
        return Err(NativeVulkanError::Video(
            "H.264 first-frame decode requires at least one DPB/output image layer".to_owned(),
        ));
    }
    let first_frame =
        native_vulkan_h264_first_frame_decode_info(&extract.selected_access_unit, parameter_sets)
            .map_err(NativeVulkanError::Video)?;
    if !first_frame.idr {
        return Err(NativeVulkanError::Video(format!(
            "H.264 first-frame decode currently supports IDR only, got {}",
            first_frame.nal_type_label
        )));
    }
    ...
    if src_buffer_range > buffer.snapshot.size {
        return Err(NativeVulkanError::Video(format!(
            "H.264 first-frame decode needs {src_buffer_range} bytes but bitstream buffer has {} bytes",
            buffer.snapshot.size
        )));
    }
    ...
    let command_pool =
        unsafe { device.create_command_pool(&command_pool_info, None) }.map_err(|result| {
            NativeVulkanError::Vulkan {
                operation: "vkCreateCommandPool(h264 first-frame decode)",
                result,
            }
        })?;
    ...
    let result = (|| -> Result<NativeVulkanVideoFirstFrameDecodeSnapshot, NativeVulkanError> {
        ...
        let command_buffer = unsafe { device.allocate_command_buffers(&command_buffer_info) }
            .map_err(|result| NativeVulkanError::Vulkan {
                operation: "vkAllocateCommandBuffers(h264 first-frame decode)",
                result,
            })?[0];
        ...
        let mut setup_h264_slot_info =
            vk::VideoDecodeH264DpbSlotInfoKHR::default().std_reference_info(&std_reference_info);
        ...
        let slice_segment_offsets = first_frame.slice_offsets.clone();
        ...
        unsafe {
            ...
            device
                ...
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkBeginCommandBuffer(h264 first-frame decode)",
                    result,
                })?;
            ...
            device
                .end_command_buffer(command_buffer)
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkEndCommandBuffer(h264 first-frame decode)",
                    result,
                })?;
            let command_buffers = [command_buffer];
            ...
            device
                .queue_submit(queue, &[submit_info], vk::Fence::null())
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkQueueSubmit(h264 first-frame decode)",
                    result,
                })?;
            device
                .queue_wait_idle(queue)
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkQueueWaitIdle(h264 first-frame decode)",
                    result,
                })?;
        }
        ...
        Ok(NativeVulkanVideoFirstFrameDecodeSnapshot {
            ...
            sps_video_parameter_set_id: std_picture_info.seq_parameter_set_id,
            pps_seq_parameter_set_id: native_vulkan_h264_u8(
                parameter_sets.pps.sps_id,
                "pps_seq_parameter_set_id",
            )
            .map_err(NativeVulkanError::Video)?,
            pps_pic_parameter_set_id: std_picture_info.pic_parameter_set_id,
...
#[cfg(feature = "native-vulkan-gst-video")]
#[allow(clippy::too_many_arguments)]
fn native_vulkan_decode_h265_first_frame_to_image(
    device: &ash::Device,
    video_queue_device: &ash::khr::video_queue::Device,
    video_decode_queue_device: &ash::khr::video_decode_queue::Device,
    queue_family_index: u32,
...
fn native_vulkan_h265_num_bits_for_st_ref_pic_set_in_slice(
    ...
    std::env::var("GILDER_VULKAN_H265_NUM_BITS_FOR_ST_RPS")
        .ok()
        .and_then(|value| value.parse::<u16>().ok())
        .unwrap_or(first_slice.num_bits_for_st_ref_pic_set_in_slice)
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_h265_num_delta_pocs_of_ref_rps_idx(
    first_slice: &NativeVulkanH265AccessUnitSliceSnapshot,
) -> u8 {
    std::env::var("GILDER_VULKAN_H265_NUM_DELTA_POCS_OF_REF_RPS_IDX")
        .ok()
        .and_then(|value| value.parse::<u8>().ok())
        .unwrap_or(first_slice.num_delta_pocs_of_ref_rps_idx)
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_h265_slice_segment_offsets(
    span: &NativeVulkanH265ReadyPrefixBitstreamSpan,
) -> Result<Vec<u32>, NativeVulkanError> {
    let base_offset = match std::env::var("GILDER_VULKAN_H265_SLICE_OFFSET_BASE")
        .ok()
        .as_deref()
    {
        Some("src-buffer") | Some("buffer") | Some("absolute") => span
            .offset
            .checked_add(u64::from(span.slice_segment_offset))
            .ok_or_else(|| {
                NativeVulkanError::Video("H.265 absolute slice segment offset overflow".to_owned())
            })?,
        _ => u64::from(span.slice_segment_offset),
    };
    let adjust = std::env::var("GILDER_VULKAN_H265_SLICE_OFFSET_ADJUST")
        .ok()
        .and_then(|value| value.parse::<i64>().ok())
        .unwrap_or(0);
    let offset = if adjust >= 0 {
        base_offset.checked_add(adjust as u64)
    } else {
        base_offset.checked_sub(adjust.unsigned_abs())
    }
    .ok_or_else(|| {
        NativeVulkanError::Video(format!(
            "H.265 slice segment offset adjustment {adjust} overflows base offset {base_offset}"
        ))
    })?;
    Ok(vec![u32::try_from(offset).map_err(|_| {
        NativeVulkanError::Video(format!(
            "H.265 slice segment offset {offset} exceeds u32 range"
        ))
    })?])
}

fn native_vulkan_h265_sequence_streaming_bitstream_enabled() -> bool {
    matches!(
        std::env::var("GILDER_VULKAN_H265_SEQUENCE_BITSTREAM")
            .ok()
            .as_deref(),
...
fn native_vulkan_h265_begin_includes_setup_slot() -> bool {
    matches!(
...
#[cfg(feature = "native-vulkan-gst-video")]
#[allow(clippy::too_many_arguments)]
fn native_vulkan_decode_h265_ready_prefix_frame_to_image(
    device: &ash::Device,
    video_queue_device: &ash::khr::video_queue::Device,
    video_decode_queue_device: &ash::khr::video_decode_queue::Device,
    ...
    let decode_info = vk::VideoDecodeInfoKHR::default()
        ...
        .push_next(&mut h265_picture_info);
    let begin_slot_pocs = active_dpb_pocs
        .iter()
        .copied()
        .enumerate()
        .filter(|(_, poc)| !native_vulkan_h265_begin_reference_slots_active_only() || poc.is_some())
        .map(|(slot, poc)| (slot as u32, if reset_before_decode { None } else { poc }))
        .collect::<Vec<_>>();
    let begin_reference_resources = begin_slot_pocs
        .iter()
        .map(|(slot, _)| native_vulkan_video_picture_resource_info_for_image(image, extent, *slot))
        .collect::<Result<Vec<_>, _>>()?;
    let begin_reference_std_infos = begin_slot_pocs
        .iter()
        .map(|(_, poc)| vk::native::StdVideoDecodeH265ReferenceInfo {
            flags: vk::native::StdVideoDecodeH265ReferenceInfoFlags {
                _bitfield_align_1: [],
                _bitfield_1: vk::native::StdVideoDecodeH265ReferenceInfoFlags::new_bitfield_1(0, 0),
                __bindgen_padding_0: [0; 3],
            },
            PicOrderCntVal: poc.unwrap_or(0),
        })
        .collect::<Vec<_>>();
    let mut begin_reference_h265_slot_infos = begin_reference_std_infos
        ...
        .collect::<Vec<_>>();
    let mut begin_reference_slots = Vec::with_capacity(begin_slot_pocs.len());
    for (index, (slot, poc)) in begin_slot_pocs.iter().enumerate() {
        let mut reference_slot = vk::VideoReferenceSlotInfoKHR::default()
            .picture_resource(&begin_reference_resources[index])
            .slot_index(if poc.is_some() { *slot as i32 } else { -1 });
        if poc.is_some() {
    ...
    let control_info =
        vk::VideoCodingControlInfoKHR::default().flags(vk::VideoCodingControlFlagsKHR::RESET);
    if native_vulkan_h265_begin_includes_setup_slot() {
...
#[cfg(feature = "native-vulkan-gst-video")]
#[allow(clippy::too_many_arguments)]
fn native_vulkan_decode_h265_ready_prefix_smoke(
    device: &ash::Device,
    video_queue_device: &ash::khr::video_queue::Device,
    video_decode_queue_device: &ash::khr::video_decode_queue::Device,
    queue_family_index: u32,
    ...
    frame_count: u32,
    sample_decoded_output: bool,
    sample_decoded_sequence: bool,
    decode_query_status_enabled: bool,
    profile_info: &vk::VideoProfileInfoKHR<'_>,
) -> Result<NativeVulkanH265ReadyPrefixDecodeSnapshot, NativeVulkanError> {
    if session_parameters == vk::VideoSessionParametersKHR::null() {
        return Err(NativeVulkanError::Video(
            "H.265 ready-prefix decode requires VkVideoSessionParametersKHR".to_owned(),
        ));
    }
    if frame_count == 0 {
        return Err(NativeVulkanError::Video(
            "H.265 ready-prefix decode requires at least one frame".to_owned(),
        ));
    }
    native_vulkan_validate_h265_ready_prefix(&extract.snapshot, frame_count)?;
    if payload.spans.len() < frame_count as usize {
        return Err(NativeVulkanError::Video(format!(
            "H.265 ready-prefix payload has {} spans but {frame_count} frames were requested",
            payload.spans.len()
        )));
    }
    if payload.bytes.len() as u64 > buffer.snapshot.size {
        return Err(NativeVulkanError::Video(format!(
            "H.265 ready-prefix payload needs {} bytes but bitstream buffer has {} bytes",
            payload.bytes.len(),
            buffer.snapshot.size
        )));
    }
    ...
    for entry in plan {
        if entry.planned_output_slot >= image.snapshot.array_layers {
            return Err(NativeVulkanError::Video(format!(
                "H.265 AU {} planned DPB slot {} exceeds image array layers {}",
                entry.access_unit_index, entry.planned_output_slot, image.snapshot.array_layers
            )));
        }
    }
    if sample_decoded_sequence {
        return native_vulkan_decode_h265_ready_prefix_sequence_smoke(
            device,
            video_queue_device,
            video_decode_queue_device,
            queue_family_index,
    ...
    let command_pool =
        unsafe { device.create_command_pool(&command_pool_info, None) }.map_err(|result| {
            NativeVulkanError::Vulkan {
                operation: "vkCreateCommandPool(h265 ready-prefix decode)",
                result,
            }
        })?;
    let mut readback_buffer = None::<NativeVulkanVideoDecodeReadbackBuffer>;
    ...
    let result = (|| -> Result<NativeVulkanH265ReadyPrefixDecodeSnapshot, NativeVulkanError> {
        ...
        let command_buffer = unsafe { device.allocate_command_buffers(&command_buffer_info) }
            .map_err(|result| NativeVulkanError::Vulkan {
                operation: "vkAllocateCommandBuffers(h265 ready-prefix decode)",
                result,
            })?[0];
        ...
        unsafe {
            ...
            device
                .begin_command_buffer(command_buffer, &command_begin_info)
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkBeginCommandBuffer(h265 ready-prefix decode)",
                    result,
                })?;
            native_vulkan_video_first_frame_decode_barriers(
                device,
            ...
            if decode_query_pool != vk::QueryPool::null() {
                device.cmd_reset_query_pool(command_buffer, decode_query_pool, 0, frame_count);
            }
            (video_queue_device.fp().cmd_begin_video_coding_khr)(command_buffer, &begin_info);
            (video_queue_device.fp().cmd_control_video_coding_khr)(command_buffer, &control_info);
                ...
                .zip(payload.spans.iter())
            {
                ...
                if available_references.len() != entry.references.len() {
                    return Err(NativeVulkanError::Video(format!(
                        "H.265 AU {} is not fully reference-ready",
                        access_unit.index
                    )));
                }
                if available_references.len() > 8 {
                    return Err(NativeVulkanError::Video(format!(
                        "H.265 AU {} has {} references; first smoke supports at most 8",
                        access_unit.index,
                        available_references.len()
                    )));
                }
                let reference_resources = available_references
                    ...
                    .map(|reference| {
                        let dpb_slot = reference.dpb_slot.ok_or_else(|| {
                            NativeVulkanError::Video(format!(
                                "H.265 AU {} reference POC {} has no DPB slot",
                                access_unit.index, reference.poc
                            ))
                        })?;
                        native_vulkan_video_picture_resource_info_for_image(image, extent, dpb_slot)
                ...
                let decode_info = vk::VideoDecodeInfoKHR::default()
                    ...
                    .reference_slots(&reference_slots)
                    .push_next(&mut h265_picture_info);
                let decode_query_index = frame_snapshots.len() as u32;
                if decode_query_pool != vk::QueryPool::null() {
                ...
                let reference_snapshots = available_references
                    ...
                    .enumerate()
                    .map(|(reference_slot_index, reference)| {
                        NativeVulkanH265DecodedFrameReferenceSnapshot {
                            reference_slot_index: reference_slot_index as u32,
                            dpb_slot: reference
                                .dpb_slot
                                .expect("available references were checked for DPB slots"),
                            source_access_unit_index: reference
                    ...
                    .collect::<Vec<_>>();
                frame_snapshots.push(NativeVulkanH265DecodedFrameSnapshot {
                    access_unit_index: access_unit.index,
                    pts_ms: access_unit.pts_ms,
                    nal_type: first_slice.nal_type,
                    nal_type_label: first_slice.nal_type_label,
            ...
            let last_frame = frame_snapshots.last().ok_or_else(|| {
                NativeVulkanError::Video("H.265 ready-prefix decoded no frames".to_owned())
            })?;
            native_vulkan_video_first_frame_readback_commands(
                device,
                command_buffer,
            ...
            device
                .end_command_buffer(command_buffer)
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkEndCommandBuffer(h265 ready-prefix decode)",
                    result,
                })?;
            let command_buffers = [command_buffer];
            let submit_info = vk::SubmitInfo::default().command_buffers(&command_buffers);
            device
                .queue_submit(queue, &[submit_info], vk::Fence::null())
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkQueueSubmit(h265 ready-prefix decode)",
                    result,
                })?;
            device
                .queue_wait_idle(queue)
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkQueueWaitIdle(h265 ready-prefix decode)",
                    result,
                })?;
        }
        if decode_query_pool != vk::QueryPool::null() {
            let query_results = native_vulkan_video_decode_status_query_results(
                device,
                decode_query_pool,
                frame_count,
            )?;
            for (frame, status) in frame_snapshots.iter_mut().zip(query_results.into_iter()) {
                native_vulkan_apply_decode_query_status(frame, status);
            }
        }
        ...
        let output_sampling = if sample_decoded_output {
            ...
            Some(native_vulkan_sample_decoded_video_output(
                ...
                sampling_command_pool,
                extent,
                image.slot_image(last_frame.dst_base_array_layer)?,
                image.slot_base_array_layer(last_frame.dst_base_array_layer),
                None,
            )?)
        } else {
            None
        ...
        Ok(NativeVulkanH265ReadyPrefixDecodeSnapshot {
            ...
            reset_control_count,
            decode_query_status_enabled,
            decode_query_count: if decode_query_status_enabled {
                frame_count
            } else {
                0
            },
...
#[cfg(feature = "native-vulkan-gst-video")]
#[allow(clippy::too_many_arguments)]
fn native_vulkan_decode_h265_ready_prefix_sequence_smoke(
    device: &ash::Device,
    video_queue_device: &ash::khr::video_queue::Device,
    video_decode_queue_device: &ash::khr::video_decode_queue::Device,
    queue_family_index: u32,
    ...
    parameter_sets: &NativeVulkanH265ParameterSetSnapshot,
    frame_count: u32,
    plan: &[NativeVulkanH265DecodeReferencePlanEntrySnapshot],
    access_units: &[NativeVulkanH265AccessUnitSnapshot],
    decode_query_status_enabled: bool,
    profile_info: &vk::VideoProfileInfoKHR<'_>,
) -> Result<NativeVulkanH265ReadyPrefixDecodeSnapshot, NativeVulkanError> {
    let sampling_queue_family_index = graphics_queue_family_index.ok_or_else(|| {
        NativeVulkanError::Video(
            "H.265 ready-prefix sequence sampling requires a graphics queue family".to_owned(),
        )
    })?;
    let queue = unsafe { device.get_device_queue(queue_family_index, 0) };
    let sampling_queue = unsafe { device.get_device_queue(sampling_queue_family_index, 0) };
    ...
    let command_pool =
        unsafe { device.create_command_pool(&command_pool_info, None) }.map_err(|result| {
            NativeVulkanError::Vulkan {
                operation: "vkCreateCommandPool(h265 ready-prefix sequence decode)",
                result,
            }
        })?;
    let sampling_pool_info = vk::CommandPoolCreateInfo::default()
        .flags(
            vk::CommandPoolCreateFlags::TRANSIENT
                | vk::CommandPoolCreateFlags::RESET_COMMAND_BUFFER,
        )
        .queue_family_index(sampling_queue_family_index);
    let sampling_command_pool =
        match unsafe { device.create_command_pool(&sampling_pool_info, None) } {
            Ok(pool) => pool,
            Err(result) => {
                unsafe {
                    device.destroy_command_pool(command_pool, None);
                ...
                return Err(NativeVulkanError::Vulkan {
                    operation: "vkCreateCommandPool(decoded ready-prefix sequence sampling)",
                    result,
                });
            }
        };
    ...
    let result = (|| -> Result<NativeVulkanH265ReadyPrefixDecodeSnapshot, NativeVulkanError> {
        ...
        let command_buffer = unsafe { device.allocate_command_buffers(&command_buffer_info) }
            .map_err(|result| NativeVulkanError::Vulkan {
                operation: "vkAllocateCommandBuffers(h265 ready-prefix sequence decode)",
                result,
            })?[0];
        if decode_query_status_enabled {
            decode_query_pool =
                native_vulkan_create_video_decode_status_query_pool(device, profile_info, 1)?;
        }
        ...
        let streaming_bitstream_slot = native_vulkan_h265_sequence_streaming_bitstream_enabled();
        let bitstream_window_mode = native_vulkan_h265_ready_prefix_bitstream_window_mode();

        for (ready_prefix_index, ((entry, access_unit), span)) in plan
            .iter()
            .zip(access_units.iter())
            .zip(payload.spans.iter())
            .enumerate()
        {
            ...
            let reset_before_decode = frame_snapshots.is_empty() || first_slice.idr;
            if reset_before_decode {
                active_dpb_pocs.fill(None);
            }
            let mut decode_span = span.clone();
            let bitstream_barrier_size = if streaming_bitstream_slot {
                let access_unit_payload = extract
                    .h265_access_unit_payloads
                    .get(ready_prefix_index)
                    .ok_or_else(|| {
                        NativeVulkanError::Video(format!(
                            "H.265 streaming sequence has no AU payload {ready_prefix_index}"
                        ))
                    })?;
                let (streaming_payload, _) = native_vulkan_h265_ready_prefix_bitstream_window(
                    access_unit_payload,
                    bitstream_window_mode,
                )?;
            ...
            let begin_slot_pocs = active_dpb_pocs
                ...
                .enumerate()
                .filter(|(_, poc)| {
                    !native_vulkan_h265_begin_reference_slots_active_only() || poc.is_some()
                })
                .map(|(slot, poc)| (slot as u32, poc))
                .collect::<Vec<_>>();
            let begin_reference_resources = begin_slot_pocs
                .iter()
                .map(|(slot, _)| {
            ...
            let decode_info = vk::VideoDecodeInfoKHR::default()
                .src_buffer(buffer.buffer)
                .src_buffer_offset(decode_span.offset)
                .src_buffer_range(decode_span.range)
                .dst_picture_resource(dst_picture_resource)
                .setup_reference_slot(&setup_reference_slot)
                .reference_slots(&reference_slots)
                .push_next(&mut h265_picture_info);
            ...
            unsafe {
                device
                    .reset_command_buffer(command_buffer, vk::CommandBufferResetFlags::empty())
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkResetCommandBuffer(h265 ready-prefix sequence decode)",
                        result,
                    })?;
                let command_begin_info = vk::CommandBufferBeginInfo::default()
                    .flags(vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT);
                device
                    .begin_command_buffer(command_buffer, &command_begin_info)
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkBeginCommandBuffer(h265 ready-prefix sequence decode)",
                        result,
                    })?;
                native_vulkan_video_decode_prepare_frame_barriers(
                    device,
                    ...
                    image,
                    buffer.buffer,
                    bitstream_barrier_size,
                    &mut image_layer_layouts,
                )?;
                if decode_query_pool != vk::QueryPool::null() {
                    device.cmd_reset_query_pool(command_buffer, decode_query_pool, 0, 1);
                }
                (video_queue_device.fp().cmd_begin_video_coding_khr)(command_buffer, &begin_info);
                if reset_before_decode {
                    (video_queue_device.fp().cmd_control_video_coding_khr)(
                ...
                (video_decode_queue_device.fp().cmd_decode_video_khr)(command_buffer, &decode_info);
                if decode_query_pool != vk::QueryPool::null() {
                    device.cmd_end_query(command_buffer, decode_query_pool, 0);
                }
                (video_queue_device.fp().cmd_end_video_coding_khr)(
                    command_buffer,
                    &vk::VideoEndCodingInfoKHR::default(),
                ...
                device
                    .end_command_buffer(command_buffer)
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkEndCommandBuffer(h265 ready-prefix sequence decode)",
                        result,
                    })?;
                let command_buffers = [command_buffer];
                let submit_info = vk::SubmitInfo::default().command_buffers(&command_buffers);
                device
                    .queue_submit(queue, &[submit_info], vk::Fence::null())
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkQueueSubmit(h265 ready-prefix sequence decode)",
                        result,
                    })?;
                device
                    .queue_wait_idle(queue)
                    .map_err(|result| NativeVulkanError::Vulkan {
                        operation: "vkQueueWaitIdle(h265 ready-prefix sequence decode)",
                        result,
                    })?;
            }
            let decode_submit_wait_elapsed_us =
                native_vulkan_elapsed_us(decode_started_at.elapsed());
            let decode_query_status = if decode_query_pool != vk::QueryPool::null() {
                native_vulkan_video_decode_status_query_results(device, decode_query_pool, 1)?
                    .into_iter()
                    .next()
            } else {
                None
            ...
            let output_sampling = native_vulkan_sample_decoded_video_output(
                ...
                sampling_command_pool,
                extent,
                image.slot_image(entry.planned_output_slot)?,
                image.slot_base_array_layer(entry.planned_output_slot),
                None,
            )?;
            ...
            if let Some(slot) = active_dpb_pocs.get_mut(entry.planned_output_slot as usize) {
            ...
            let mut rendered_frame = native_vulkan_render_decoded_video_output(
                device,
                sampling_queue,
            ...
            rendered_frame.source_base_array_layer = entry.planned_output_slot;
            ...
            let reference_snapshots = available_references
                ...
                .collect::<Vec<_>>();
            let frame_snapshot = NativeVulkanH265DecodedFrameSnapshot {
                access_unit_index: access_unit.index,
                pts_ms: access_unit.pts_ms,
                nal_type: first_slice.nal_type,
                nal_type_label: first_slice.nal_type_label,
                ...
                setup_slot_index,
                decode_reference_slot_count: reference_slots.len() as u32,
                decode_query_status_raw: decode_query_status.map(|status| status.as_raw()),
                decode_query_status: decode_query_status
                    .map(native_vulkan_query_result_status_label),
                references: reference_snapshots,
                slice_segment_count: slice_segment_offsets.len() as u32,
                slice_segment_offsets,
            };
            sampled_frames.push(native_vulkan_h265_ready_prefix_sampled_frame_snapshot(
                &frame_snapshot,
                &output_readback,
                &output_sampling,
                pts_delta_ms,
        ...
        Ok(NativeVulkanH265ReadyPrefixDecodeSnapshot {
            ...
            command_buffer_allocated: true,
            command_buffer_recorded: true,
            begin_reference_slot_count: active_dpb_pocs.iter().filter(|poc| poc.is_some()).count()
                as u32,
            reset_control_recorded: true,
            reset_control_count,
            decode_query_status_enabled,
            decode_query_count: if decode_query_status_enabled { 1 } else { 0 },
            src_buffer_total_bytes: payload.bytes.len() as u64,
            readback_access_unit_index: last_frame.access_unit_index,
            readback_base_array_layer: last_frame.dst_base_array_layer,
            frames: frame_snapshots,
    ...
    unsafe {
        ...
        device.destroy_command_pool(sampling_command_pool, None);
        device.destroy_command_pool(command_pool, None);
    }
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_h265_ready_prefix_sampled_frame_snapshot(
    ...
    NativeVulkanH265ReadyPrefixSampledFrameSnapshot {
        ...
        pts_ms: frame.pts_ms,
        pts_delta_ms,
        source_base_array_layer: frame.dst_base_array_layer,
        readback_y_hash: readback.y_plane_hash,
        readback_uv_hash: readback.uv_plane_hash,
        readback_y_unique_values: readback.y_plane_unique_values,
        readback_uv_unique_values: readback.uv_plane_unique_values,
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_video_picture_resource_info_for_image(
    image: &NativeVulkanVideoResourceImage,
    extent: vk::Extent2D,
    base_array_layer: u32,
    ...
    Ok(native_vulkan_video_picture_resource_info(
        ...
        extent,
        image.slot_base_array_layer(base_array_layer),
    ))
}
...
#[cfg(feature = "native-vulkan-gst-video")]
unsafe fn native_vulkan_video_decode_prepare_frame_barriers(
    device: &ash::Device,
    command_buffer: vk::CommandBuffer,
    image: &NativeVulkanVideoResourceImage,
    buffer: vk::Buffer,
    ...
    for (layer, layout) in layer_layouts.iter_mut().enumerate() {
        ...
        let base_array_layer = image.slot_base_array_layer(layer as u32);
        let (src_stage, src_access) = native_vulkan_video_decode_layout_src_scope(*layout);
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_video_decode_layout_src_scope(
...
unsafe fn native_vulkan_video_first_frame_decode_barriers(
    ...
    let mut image_barriers = Vec::new();
    if image.uses_separate_slots() {
        for slot in 0..image.snapshot.array_layers {
            image_barriers.push(
                vk::ImageMemoryBarrier2::default()
                    ...
                    .src_queue_family_index(vk::QUEUE_FAMILY_IGNORED)
                    .dst_queue_family_index(vk::QUEUE_FAMILY_IGNORED)
                    .image(image.slot_image(slot)?)
                    .subresource_range(native_vulkan_video_decode_image_subresource_range(0, 1)),
            );
        }
    } else {
        image_barriers.push(
    ...
    let regions = [
        vk::BufferImageCopy::default()
            ...
            .image_subresource(vk::ImageSubresourceLayers {
                ...
                mip_level: 0,
                base_array_layer: resource_base_array_layer,
                layer_count: 1,
            })
            .image_offset(vk::Offset3D { x: 0, y: 0, z: 0 })
            .image_extent(vk::Extent3D {
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_extract_h265_bitstream_spooled(
    source: &Path,
    max_samples: u32,
) -> Result<NativeVulkanVideoBitstreamSpoolExtract, NativeVulkanError> {
    gst::init().map_err(|err| NativeVulkanError::Video(err.to_string()))?;
    let pipeline = native_vulkan_h265_bitstream_pipeline(source)?;
    let sink = pipeline
        .by_name("gilder-native-vulkan-h265-bitstream-appsink")
    ...
    let (spool_path, spool_file) = native_vulkan_create_h265_spool_file()?;

    let result = (|| -> Result<NativeVulkanVideoBitstreamSpoolExtract, NativeVulkanError> {
        pipeline
            .set_state(gst::State::Playing)
            .map_err(|err| NativeVulkanError::Video(err.to_string()))?;
        native_vulkan_collect_h265_bitstream_samples_spooled(
            source,
            &sink,
            &bus,
            max_samples,
...
fn native_vulkan_create_h265_spool_file() -> Result<(PathBuf, File), NativeVulkanError> {
    ...
    for attempt in 0..32u32 {
        let path = temp_dir.join(format!(
            "gilder-h265-au-spool-{pid}-{timestamp_ns}-{attempt}.bin"
        ));
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_read_h265_spooled_payload(
    file: &mut File,
    path: &Path,
    record: &NativeVulkanH265AccessUnitSpoolRecord,
) -> Result<Vec<u8>, NativeVulkanError> {
    let mut payload = Vec::new();
    native_vulkan_read_h265_spooled_payload_into(file, path, record, &mut payload)?;
    Ok(payload)
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_read_h265_spooled_payload_into(
    file: &mut File,
    path: &Path,
    record: &NativeVulkanH265AccessUnitSpoolRecord,
    payload: &mut Vec<u8>,
    ...
    file.read_exact(payload).map_err(|err| {
        NativeVulkanError::Video(format!(
            "read H.265 AU spool {} at {} for {} bytes: {err}",
            path.display(),
            record.offset,
            record.len
        ))
...
fn native_vulkan_h265_ready_prefix_bitstream_payload(
    ...
    min_offset_alignment: u64,
    min_size_alignment: u64,
) -> Result<NativeVulkanH265ReadyPrefixBitstreamPayload, NativeVulkanError> {
    native_vulkan_validate_h265_ready_prefix(&extract.snapshot, frame_count)?;
    let frame_count_usize = frame_count as usize;
    if extract.h265_access_unit_payloads.len() < frame_count_usize {
        return Err(NativeVulkanError::Video(format!(
    ...
    {
        let (payload, slice_segment_offset) =
        ...
        let padded_end = offset
            .checked_add(range)
            .ok_or_else(|| NativeVulkanError::Video("H.265 bitstream range overflow".to_owned()))?;
        bytes.resize(padded_end as usize, 0);
        spans.push(NativeVulkanH265ReadyPrefixBitstreamSpan {
            offset,
            range,
            payload_bytes: payload.len() as u64,
            slice_segment_offset,
            ring_allocation_index: 0,
            ring_wrap_count: 0,
        });
    }

    Ok(NativeVulkanH265ReadyPrefixBitstreamPayload { bytes, spans })
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum NativeVulkanH265ReadyPrefixBitstreamWindowMode {
    FullAccessUnit,
    NalUnit,
    SliceHeader,
}
...
#[cfg(feature = "native-vulkan-gst-video")]
#[allow(dead_code)]
fn native_vulkan_h265_ready_prefix_streaming_bitstream_plan(
    extract: &NativeVulkanVideoBitstreamExtract,
    frame_count: u32,
    min_size_alignment: u64,
        ...
        .take(frame_count_usize)
    {
        ...
        window_payload_bytes = window_payload_bytes.saturating_add(payload.len() as u64);
    }
    let ring_slot_count = native_vulkan_bitstream_ring_slot_count(frame_count);
    let ring_capacity_bytes = slot_bytes.saturating_mul(u64::from(ring_slot_count));
    ...
    Ok(NativeVulkanH265ReadyPrefixStreamingBitstreamPlan {
        ...
        min_size_alignment: min_size_alignment.max(1),
        window_payload_bytes,
    })
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_h265_ready_prefix_streaming_bitstream_plan_spooled(
    extract: &NativeVulkanVideoBitstreamSpoolExtract,
    frame_count: u32,
    min_offset_alignment: u64,
    min_size_alignment: u64,
) -> Result<NativeVulkanH265ReadyPrefixStreamingBitstreamPlan, NativeVulkanError> {
    ...
    if extract.records.len() < frame_count_usize {
        return Err(NativeVulkanError::Video(format!(
            "H.265 spooled bitstream has {} records but {frame_count} ready-prefix frames were requested",
            extract.records.len()
        )));
    }
    ...
    for record in extract.records.iter().take(frame_count_usize) {
        let range = native_vulkan_align_up(record.len, min_size_alignment.max(1));
        slot_bytes = slot_bytes.max(range);
    ...
    let ring_capacity_bytes = slot_stride
        ...
        .ok_or_else(|| NativeVulkanError::Video("bitstream ring capacity overflow".to_owned()))?;
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_bitstream_ring_slot_count(frame_count: u32) -> u32 {
    std::env::var("GILDER_VULKAN_BITSTREAM_RING_SLOTS")
        .ok()
        .and_then(|value| value.parse::<u32>().ok())
        .filter(|value| *value > 0)
        .unwrap_or_else(|| frame_count.clamp(1, 2))
}
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_av1_bitstream_pipeline(source: &Path) -> Result<gst::Pipeline, NativeVulkanError> {
    let pipeline = gst::Pipeline::new();
    let filesrc = native_vulkan_gst_element("filesrc")?;
    filesrc.set_property("location", source.to_string_lossy().as_ref());
    let demux = native_vulkan_gst_element(native_vulkan_av1_demux_element_name(source))?;
    let queue = native_vulkan_gst_element("queue")?;
    native_vulkan_configure_queue(&queue);
    let parser = native_vulkan_gst_element("av1parse")?;
    if parser.find_property("disable-passthrough").is_some() {
        parser.set_property("disable-passthrough", true);
    }
    let capsfilter = native_vulkan_gst_element("capsfilter")?;
    let caps = "video/x-av1,stream-format=obu-stream,alignment=tu"
        .parse::<gst::Caps>()
        .map_err(|err| NativeVulkanError::Video(err.to_string()))?;
    capsfilter.set_property("caps", &caps);
    let sink = native_vulkan_gst_element("appsink")?;
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_collect_av1_bitstream_samples(
    source: &Path,
    sink: &gst::Element,
    bus: &gst::Bus,
    max_samples: u32,
    ...
    let mut sample_count = 0u32;
    let mut total_bytes = 0u64;
    let mut temporal_units = Vec::<NativeVulkanAv1TemporalUnitExtract>::new();
    let mut selected_index = None::<usize>;
    let mut last_error = None::<String>;
    let mut reached_eos = false;
    ...
    while sample_count < max_samples && Instant::now() < deadline && !reached_eos {
        while let Some(message) = bus.pop() {
            match message.view() {
                ...
                gst::MessageView::Eos(_) => {
                    last_error = Some("AV1 bitstream pipeline reached EOS".to_owned());
                    reached_eos = true;
                    break;
                }
                _ => {}
        ...
        sample_count = sample_count.saturating_add(1);
        let temporal_unit = native_vulkan_av1_temporal_unit_from_sample(&sample)?;
        total_bytes = total_bytes.saturating_add(temporal_unit.bytes.len() as u64);
        if selected_index
            .map(|index| {
                let current = &temporal_units[index];
                !current.stats.sequence_header_present()
                    && temporal_unit.stats.sequence_header_present()
            })
            .unwrap_or(true)
        {
            selected_index = Some(temporal_units.len());
        }
        temporal_units.push(temporal_unit);
    }

    if temporal_units.is_empty() {
        return Err(NativeVulkanError::Video(last_error.unwrap_or_else(|| {
            "AV1 bitstream probe produced no samples".to_owned()
        })));
    }
    let selected_index = selected_index.ok_or_else(|| {
        NativeVulkanError::Video(
            last_error
                .unwrap_or_else(|| "AV1 bitstream probe produced no selectable samples".to_owned()),
        )
    })?;
    let selected = &temporal_units[selected_index];
    if !selected.stats.sequence_header_present() {
        return Err(NativeVulkanError::Video(format!(
            "AV1 bitstream probe did not find sequence header OBU in {sample_count} samples"
        )));
    }
    let av1_temporal_units = temporal_units
        .iter()
        .enumerate()
    ...
    Ok(NativeVulkanVideoBitstreamExtract {
        ...
        snapshot: NativeVulkanVideoBitstreamExtractSnapshot {
            ...
            requested_max_samples: max_samples,
            samples: temporal_units.len() as u32,
            total_bytes,
            selected_access_unit_index: selected_index as u32,
            selected_access_unit_bytes: selected.stats.bytes,
            selected_access_unit_pts_ms: selected.pts_ms,
            ...
            h265_decode_first_unready_missing_reference_pocs: Vec::new(),
            h265_decode_reference_plan: Vec::new(),
            av1_obu_count: selected.stats.obu_count,
            av1_sequence_header_count: selected.stats.sequence_header_count,
            av1_temporal_delimiter_count: selected.stats.temporal_delimiter_count,
            av1_frame_header_count: selected.stats.frame_header_count,
            av1_tile_group_count: selected.stats.tile_group_count,
            av1_frame_count: selected.stats.frame_count,
            av1_decode_candidate: selected.stats.decode_candidate(),
            av1_tile_payload_bytes: selected.stats.tile_payload_bytes,
            av1_frame_payload_bytes: selected.stats.frame_payload_bytes,
            av1_first_frame_header_obu_offset: selected.stats.first_frame_header_obu_offset,
            av1_first_tile_group_obu_offset: selected.stats.first_tile_group_obu_offset,
            av1_sequence_header_present: selected.stats.sequence_header_present(),
            av1_sequence_header: selected.stats.sequence_header.clone(),
            av1_obus: selected.stats.obus.clone(),
            av1_temporal_units,
        },
    })
}
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_collect_h265_bitstream_samples_spooled(
    source: &Path,
    sink: &gst::Element,
    bus: &gst::Bus,
    max_samples: u32,
    ...
    let mut sample_count = 0u32;
    let mut total_bytes = 0u64;
    let mut records = Vec::<NativeVulkanH265AccessUnitSpoolRecord>::new();
    let mut selected_index = None::<usize>;
    ...
    while sample_count < max_samples && Instant::now() < deadline && !reached_eos {
        ...
        spool_file.write_all(bytes).map_err(|err| {
            NativeVulkanError::Video(format!(
                "write H.265 AU spool {} at {offset}: {err}",
                spool_path.display()
            ))
        })?;
        total_bytes = total_bytes.saturating_add(bytes.len() as u64);
        records.push(NativeVulkanH265AccessUnitSpoolRecord {
            offset,
            len: bytes.len() as u64,
            slice_segment_offset: native_vulkan_h265_first_vcl_slice_segment_offset(bytes)
                .unwrap_or(0),
    ...
    let selected_access_unit = selected_access_unit.ok_or_else(|| {
        NativeVulkanError::Video("H.265 bitstream probe lost selected AU payload".to_owned())
    })?;
    let parameter_sets = native_vulkan_parse_h265_parameter_sets(&selected_access_unit)
        .map_err(NativeVulkanError::Video)?;
    drop(selected_access_unit);

    spool_file.flush().map_err(|err| {
        NativeVulkanError::Video(format!(
            "flush H.265 AU spool {}: {err}",
            spool_path.display()
        ))
    })?;
    let mut h265_access_units = Vec::with_capacity(records.len());
    for (index, record) in records.iter().enumerate() {
        let bytes = native_vulkan_read_h265_spooled_payload(&mut spool_file, &spool_path, record)?;
        let access_unit = record.to_extract(bytes);
        h265_access_units.push(native_vulkan_h265_access_unit_snapshot(
            index as u32,
            &access_unit,
            &parameter_sets,
        ));
    ...
    let snapshot = NativeVulkanVideoBitstreamExtractSnapshot {
        source: source.display().to_string(),
        frontend: "gstreamer-qtdemux-h265parse-appsink-spooled",
        requested_max_samples: max_samples,
    ...
    Ok(NativeVulkanVideoBitstreamSpoolExtract {
        file: spool_file,
        path: spool_path,
        records,
...
#[cfg(feature = "native-vulkan-gst-video")]
type NativeVulkanAv1TemporalUnitSampleMetadata = NativeVulkanH265AccessUnitSampleMetadata;
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_av1_temporal_unit_from_sample(
    sample: &gst::Sample,
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_av1_temporal_unit_metadata_from_sample(
    sample: &gst::Sample,
    buffer: &gst::BufferRef,
) -> NativeVulkanAv1TemporalUnitSampleMetadata {
    native_vulkan_h265_access_unit_metadata_from_sample(sample, buffer)
}
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_h265_access_unit_metadata_from_sample(
    sample: &gst::Sample,
    buffer: &gst::BufferRef,
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_h265_first_vcl_slice_segment_offset(bytes: &[u8]) -> Option<u32> {
    native_vulkan_h265_nal_payloads(bytes)
        .into_iter()
        .find(|nal| nal.nal_type <= 31)
        .and_then(|nal| u32::try_from(nal.slice_segment_offset).ok())
}
...
fn native_vulkan_h264_first_frame_decode_info(
    ...
    for slice in slices {
        ...
        {
            return Err(
                "H.264 access unit slice headers do not describe one IDR picture".to_owned(),
            );
        }
        slice_offsets.push(
...
fn native_vulkan_h264_slice_decode_info(
    ...
    let pic_order_cnt = match parameter_sets.sps.pic_order_cnt_type {
        ...
        1 if !parameter_sets.sps.delta_pic_order_always_zero_flag => {
            let _delta_pic_order_cnt_bottom = bits.read_se("delta_pic_order_cnt[0]")?;
            if parameter_sets
                .pps
                .bottom_field_pic_order_in_frame_present_flag
                && !field_pic_flag
            ...
            if idr {
                ...
                return Err(
                    "H.264 first-frame decode does not support non-IDR POC type 1 yet".to_owned(),
                );
            }
        }
        1 | 2 => {
            if idr {
                ...
                return Err(format!(
                    "H.264 first-frame decode does not support non-IDR POC type {} yet",
    ...
    Ok(NativeVulkanH264FirstFrameDecodeInfo {
        ...
        is_intra: normalized_slice_type
...
fn native_vulkan_video_session_smoke_result(
    ...
    if options.sample_h265_ready_prefix_sequence {
        return "h265-ready-prefix-decode-output-sequence-sampled-and-readback-completed";
    }
    ...
    if options.decode_first_frame {
        return "first-frame-decode-and-output-readback-completed";
    }
    match (
...
fn native_vulkan_h265_session_dpb_slots_for_count(
    ...
    if driver_slots == 0 {
        ...
        stream_slots.min(driver_slots).max(1)
    }
}
...
fn native_vulkan_create_av1_video_session_parameters(
    video_queue_device: &ash::khr::video_queue::Device,
    session: vk::VideoSessionKHR,
    sequence_header: &NativeVulkanAv1SequenceHeaderSnapshot,
) -> Result<NativeVulkanVideoSessionParameters, NativeVulkanError> {
...
fn native_vulkan_av1_std_color_config(
...
fn native_vulkan_av1_std_timing_info(
...
fn native_vulkan_av1_std_profile(
    profile: u8,
...
fn native_vulkan_av1_std_color_primaries(
    value: u8,
...
fn native_vulkan_av1_std_transfer_characteristics(
    value: u8,
...
fn native_vulkan_av1_std_matrix_coefficients(
    value: u8,
...
fn native_vulkan_av1_std_chroma_sample_position(
    value: u8,
