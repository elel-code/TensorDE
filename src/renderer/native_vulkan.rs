...
#[cfg(feature = "native-vulkan-gst-video")]
#[path = "native_vulkan/direct_av1_policy.rs"]
mod direct_av1_policy;
...
#[path = "native_vulkan/audio_telemetry.rs"]
mod audio_telemetry;
...
#[path = "native_vulkan/video_probe_snapshots.rs"]
mod video_probe_snapshots;
...
#[path = "native_vulkan/video_codec_ash.rs"]
mod video_codec_ash;
...
#[path = "native_vulkan/video_session_snapshots.rs"]
mod video_session_snapshots;

#[path = "native_vulkan/video_session_resources.rs"]
mod video_session_resources;

#[path = "native_vulkan/video_session_parameters.rs"]
...
#[cfg(feature = "native-vulkan-gst-video")]
use direct_runtime::{
    NativeVulkanDirectOptionalPresentTiming, NativeVulkanDirectPresentPendingContext,
...
fn native_vulkan_pace_video_after_frame(
    ...
    match pacing_master {
        pacing::NativeVulkanVideoPacingMaster::AudioClock => frame_pacer
            .pace_after_frame_with_master_clock(
                is_last_frame,
                next_video_clock_ns,
                audio_clock_probe.and_then(|probe| probe.audio_master_clock_estimate_ns()),
            ),
...
#[cfg(feature = "native-vulkan-gst-video")]
struct NativeVulkanDecodedVideoDisplayCopyBeforeRender<'a> {
    source_image: &'a NativeVulkanVideoResourceImage,
...
struct NativeVulkanDecodedVideoDisplayCopyBeforeRenderCommand {
...
#[cfg(feature = "native-vulkan-gst-video")]
impl NativeVulkanDecodedVideoDisplayCopyBeforeRender<'_> {
    fn to_command(
        &self,
    ) -> Result<NativeVulkanDecodedVideoDisplayCopyBeforeRenderCommand, NativeVulkanError> {
        Ok(NativeVulkanDecodedVideoDisplayCopyBeforeRenderCommand {
            source_image: self.source_image.slot_image(self.source_base_array_layer)?,
            source_base_array_layer: self
                .source_image
                .slot_base_array_layer(self.source_base_array_layer),
            display_image: self.display_image,
            display_base_array_layer: self.display_base_array_layer,
            display_old_layout: self.display_old_layout,
            extent: self.extent,
        })
    }
}
...
impl NativeVulkanVideoRenderer {
    ...
    fn record_clear_frame(
        ...
        unsafe {
            device
                ...
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkResetCommandBuffer(video preroll)",
                    result,
                })?;
            ...
            device
                ...
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkBeginCommandBuffer(video preroll)",
                    result,
                })?;
            ...
            device
                ...
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkEndCommandBuffer(video preroll)",
                    result,
                })?;
        }
        Ok(())
    ...
    fn record_frame_with_descriptor_set_index(
        ...
        fit: FitMode,
        descriptor_set_index: usize,
        display_copy_before_render: Option<NativeVulkanDecodedVideoDisplayCopyBeforeRender<'_>>,
    ) -> Result<(), NativeVulkanError> {
        let display_copy_before_render = display_copy_before_render
            .map(|copy| copy.to_command())
            .transpose()?;
        self.record_frame_with_descriptor_set_index_with_copy_command(
            device,
            command_buffer,
            image_index,
            swapchain_image,
    ...
    #[allow(clippy::too_many_arguments)]
    fn record_frame_with_descriptor_set_index_with_copy_command(
        &self,
        device: &ash::Device,
        command_buffer: vk::CommandBuffer,
        image_index: usize,
        ...
        fit: FitMode,
        descriptor_set_index: usize,
        display_copy_before_render: Option<NativeVulkanDecodedVideoDisplayCopyBeforeRenderCommand>,
    ) -> Result<(), NativeVulkanError> {
        let descriptor_set = self
            .descriptor_sets
            .get(descriptor_set_index)
        ...
        unsafe {
            ...
            if let Some(copy) = display_copy_before_render {
                native_vulkan_cmd_decoded_video_display_copy_for_graphics_image(
                    device,
                    command_buffer,
                    copy.source_image,
                    copy.source_base_array_layer,
                    copy.display_image,
                    copy.display_base_array_layer,
                    copy.display_old_layout,
                    copy.extent,
                )?;
            }
            ...
            if texture_barriers.is_empty() {
                let barriers = [swapchain_to_attachment];
                device.cmd_pipeline_barrier(
                    command_buffer,
                    vk::PipelineStageFlags::TOP_OF_PIPE | vk::PipelineStageFlags::ALL_COMMANDS,
                    vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT
                ...
                let mut barriers = Vec::with_capacity(1 + texture_barriers.len());
                barriers.push(swapchain_to_attachment);
                barriers.extend(texture_barriers);
                device.cmd_pipeline_barrier(
                    command_buffer,
                    vk::PipelineStageFlags::TOP_OF_PIPE | vk::PipelineStageFlags::ALL_COMMANDS,
                    vk::PipelineStageFlags::COLOR_ATTACHMENT_OUTPUT
...
impl NativeVulkanVideoTexture {
    ...
    fn shader_read_barriers(&self) -> Vec<vk::ImageMemoryBarrier<'static>> {
        match self {
            Self::Decoded(texture) => texture.shader_read_barriers(),
        }
    }

    fn clone_decoded_for_present_frame_queue(
        &self,
    ) -> Result<NativeVulkanDecodedVideoTexture, NativeVulkanError> {
        match self {
            Self::Decoded(texture) => texture.clone_cached_views(),
        }
    }
...
impl NativeVulkanDecodedVideoTexture {
    fn new(
        ...
        if width == 0 || height == 0 || !width.is_multiple_of(2) || !height.is_multiple_of(2) {
            return Err(NativeVulkanError::Video(format!(
                "decoded {} texture requires non-zero even dimensions, got {width}x{height}",
                native_vulkan_format_label(format)
            )));
        }
        let plane_formats = native_vulkan_decoded_video_plane_formats(format)?;
    ...
    fn from_cached_views(
        image: vk::Image,
        format: vk::Format,
        width: u32,
        height: u32,
        base_array_layer: u32,
        views: NativeVulkanDecodedVideoPlaneViews,
        source_layout: vk::ImageLayout,
        source_access: vk::AccessFlags,
    ) -> Result<Self, NativeVulkanError> {
        Self::from_cached_views_with_shader_layout(
            image,
            format,
            width,
            height,
    ...
    #[allow(clippy::too_many_arguments)]
    fn from_cached_views_with_shader_layout(
        image: vk::Image,
        format: vk::Format,
        width: u32,
        height: u32,
        ...
        native_vulkan_decoded_video_plane_formats(format)?;
        Ok(Self {
            image,
            width,
            height,
    ...
    fn shader_read_barriers(&self) -> Vec<vk::ImageMemoryBarrier<'static>> {
        if self.source_layout == self.shader_layout
            && self.source_access == vk::AccessFlags::SHADER_READ
        {
            return Vec::new();
        }
        vec![
    ...
    fn clone_cached_views(&self) -> Result<Self, NativeVulkanError> {
        if self.owns_views {
            return Err(NativeVulkanError::Video(
                "present-frame queue cannot clone an owning decoded texture".to_owned(),
            ));
        }
        Ok(Self {
            image: self.image,
...
#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug, Clone, Copy)]
struct NativeVulkanDecodedVideoPlaneViews {
    y_view: vk::ImageView,
    uv_view: vk::ImageView,
}
...
#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct NativeVulkanDecodedVideoPlaneFormats {
...
fn native_vulkan_decoded_video_plane_formats(
    format: vk::Format,
) -> Result<NativeVulkanDecodedVideoPlaneFormats, NativeVulkanError> {
    match format {
        vk::Format::G8_B8R8_2PLANE_420_UNORM => Ok(NativeVulkanDecodedVideoPlaneFormats {
            y_view_format: vk::Format::R8_UNORM,
            uv_view_format: vk::Format::R8G8_UNORM,
        }),
        vk::Format::G10X6_B10X6R10X6_2PLANE_420_UNORM_3PACK16 => {
            Ok(NativeVulkanDecodedVideoPlaneFormats {
                y_view_format: vk::Format::R16_UNORM,
                uv_view_format: vk::Format::R16G16_UNORM,
            })
        }
        _ => Err(NativeVulkanError::Video(format!(
            "{} decoded video plane sampling is not implemented",
            native_vulkan_format_label(format)
        ))),
    }
}
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_create_decoded_video_plane_view_cache(
    device: &ash::Device,
    image: vk::Image,
    format: vk::Format,
    array_layers: u32,
) -> Result<Vec<NativeVulkanDecodedVideoPlaneViews>, NativeVulkanError> {
    let plane_formats = native_vulkan_decoded_video_plane_formats(format)?;
    let mut views = Vec::with_capacity(array_layers as usize);
    for base_array_layer in 0..array_layers {
        let y_view = native_vulkan_create_decoded_video_plane_view(
            device,
            image,
            vk::ImageAspectFlags::PLANE_0,
        ...
        let uv_view = match native_vulkan_create_decoded_video_plane_view(
            ...
            Err(err) => {
                ...
                native_vulkan_destroy_decoded_video_plane_view_cache(device, views);
                return Err(err);
            }
        };
        views.push(NativeVulkanDecodedVideoPlaneViews { y_view, uv_view });
    }
    Ok(views)
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_destroy_decoded_video_plane_view_cache(
    device: &ash::Device,
    views: Vec<NativeVulkanDecodedVideoPlaneViews>,
) {
    unsafe {
        for view in views {
            device.destroy_image_view(view.uv_view, None);
            device.destroy_image_view(view.y_view, None);
        }
    }
}

#[cfg(feature = "native-vulkan-gst-video")]
struct NativeVulkanDecodedVideoDisplayImage {
    image: vk::Image,
    memory: vk::DeviceMemory,
    plane_views: Vec<NativeVulkanDecodedVideoPlaneViews>,
    layouts: Vec<vk::ImageLayout>,
    memory_size: u64,
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_create_decoded_video_display_image(
    device: &ash::Device,
...
) -> Result<NativeVulkanDecodedVideoDisplayImage, NativeVulkanError> {
    if extent.width == 0 || extent.height == 0 || array_layers == 0 {
        return Err(NativeVulkanError::Video(format!(
            "decoded video display image requires non-zero extent/layers, got {}x{}x{}",
            extent.width, extent.height, array_layers
        )));
    }
    ...
    let image = unsafe { device.create_image(&image_info, None) }.map_err(|result| {
        NativeVulkanError::Vulkan {
            operation: "vkCreateImage(decoded video display ring)",
            result,
        }
    })?;

    let result = (|| -> Result<NativeVulkanDecodedVideoDisplayImage, NativeVulkanError> {
        ...
        let memory =
            unsafe { device.allocate_memory(&allocation_info, None) }.map_err(|result| {
                NativeVulkanError::Vulkan {
                    operation: "vkAllocateMemory(decoded video display ring)",
                    result,
                }
            })?;
        if let Err(result) = unsafe { device.bind_image_memory(image, memory, 0) } {
            ...
            return Err(NativeVulkanError::Vulkan {
                operation: "vkBindImageMemory(decoded video display ring)",
                result,
            });
        }
        let plane_views = match native_vulkan_create_decoded_video_plane_view_cache(
            device,
            image,
            format,
            array_layers,
        ...
        Ok(NativeVulkanDecodedVideoDisplayImage {
            image,
            memory,
            plane_views,
            layouts: vec![vk::ImageLayout::UNDEFINED; array_layers as usize],
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_destroy_decoded_video_display_image(
    device: &ash::Device,
    image: NativeVulkanDecodedVideoDisplayImage,
) {
    native_vulkan_destroy_decoded_video_plane_view_cache(device, image.plane_views);
    unsafe {
...
fn native_vulkan_av1_dpb_reference_sign_bias(
    ...
    {
        Some("zero") => 0,
        Some("legacy") | Some("all") | Some("all-frames") => {
            native_vulkan_av1_ref_frame_sign_bias_from_order_hints(
                sequence_header,
                current_order_hint,
                order_hints,
...
fn native_vulkan_av1_reference_info_from_active(
    ...
    vk::native::StdVideoDecodeAV1ReferenceInfo {
        flags: vk::native::StdVideoDecodeAV1ReferenceInfoFlags {
            _bitfield_align_1: [],
            _bitfield_1: vk::native::StdVideoDecodeAV1ReferenceInfoFlags::new_bitfield_1(
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_video_display_layout_src_scope(
...
#[cfg(feature = "native-vulkan-gst-video")]
#[allow(clippy::too_many_arguments)]
unsafe fn native_vulkan_cmd_decoded_video_display_copy_for_graphics_image(
    device: &ash::Device,
    command_buffer: vk::CommandBuffer,
    resource_image: vk::Image,
    resource_base_array_layer: u32,
    display_image: vk::Image,
    display_base_array_layer: u32,
    display_old_layout: vk::ImageLayout,
    extent: vk::Extent2D,
) -> Result<(), NativeVulkanError> {
    let (display_src_stage, display_src_access) =
        native_vulkan_video_display_layout_src_scope(display_old_layout);
    unsafe {
        let display_before_copy = [vk::ImageMemoryBarrier2::default()
            .src_stage_mask(display_src_stage)
            .src_access_mask(display_src_access)
            .dst_stage_mask(vk::PipelineStageFlags2::TRANSFER)
            .dst_access_mask(vk::AccessFlags2::TRANSFER_WRITE)
            .old_layout(display_old_layout)
            .new_layout(vk::ImageLayout::TRANSFER_DST_OPTIMAL)
            .src_queue_family_index(vk::QUEUE_FAMILY_IGNORED)
            .dst_queue_family_index(vk::QUEUE_FAMILY_IGNORED)
            .image(display_image)
            .subresource_range(native_vulkan_video_decode_image_subresource_range(
                display_base_array_layer,
                1,
            ))];
        let display_before_copy_dependency =
            vk::DependencyInfo::default().image_memory_barriers(&display_before_copy);
        device.cmd_pipeline_barrier2(command_buffer, &display_before_copy_dependency);
        ...
        let regions = [
            vk::ImageCopy::default()
                ...
                .dst_subresource(vk::ImageSubresourceLayers {
                    aspect_mask: vk::ImageAspectFlags::PLANE_0,
                    mip_level: 0,
                    base_array_layer: display_base_array_layer,
                    layer_count: 1,
                })
        ...
        let display_after_copy = [vk::ImageMemoryBarrier2::default()
        ...
        let display_after_copy_dependency =
            vk::DependencyInfo::default().image_memory_barriers(&display_after_copy);
        device.cmd_pipeline_barrier2(command_buffer, &display_after_copy_dependency);
    }
    Ok(())
}
...
fn native_vulkan_validate_h264_ready_prefix(
    snapshot: &NativeVulkanVideoBitstreamExtractSnapshot,
    required_ready_prefix_access_units: u32,
) -> Result<(), NativeVulkanError> {
    if required_ready_prefix_access_units == 0
...
#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug)]
struct NativeVulkanAv1DecodeCommandSlot {
    command_buffer: vk::CommandBuffer,
    fence: vk::Fence,
    in_flight: bool,
}
...
#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug, Clone)]
struct NativeVulkanAv1PendingDecodeSubmission {
...
#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug, Clone, Copy)]
struct NativeVulkanAv1SampledResource {
    displayed_slot: u32,
    dedicated_output: bool,
    shared_general_layout: bool,
}

#[cfg(feature = "native-vulkan-gst-video")]
#[derive(Debug)]
struct NativeVulkanAv1FrameContext {
    ...
    acquired_image_queue_slot_index: Option<usize>,
...
#[cfg(feature = "native-vulkan-gst-video")]
struct NativeVulkanAv1PresentFrameJob {
    counted: bool,
    frame_index: usize,
    frame_context_index: usize,
...
#[cfg(feature = "native-vulkan-gst-video")]
struct NativeVulkanAv1QueuedPresentFrameJob {
    counted: bool,
    frame_index: usize,
    frame_context_index: usize,
    image_index: u32,
    ...
    acquire_start_since_start_us: u64,
    acquire_end_since_start_us: u64,
    record_elapsed_us: u64,
    queue_submit_elapsed_us: u64,
    present_submit_start_since_start_us: u64,
    run_started_at: Instant,
    present_started_at: Instant,
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_apply_av1_present_worker_result(
    frames: &mut [NativeVulkanDirectAv1ReadyPrefixFrameSnapshot],
    result: Result<NativeVulkanAv1PresentWorkerResult, NativeVulkanError>,
) -> Result<(), NativeVulkanError> {
    let result = result?;
    if !result.counted {
        return Ok(());
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_recv_av1_present_worker_result(
    ...
    let result = result_rx
        .recv()
        .map_err(|_| NativeVulkanError::Video(operation.to_owned()))?;
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_wait_av1_acquire_worker_results_for_grace(
    result_rx: &mpsc::Receiver<Result<NativeVulkanAv1AcquireWorkerResult, NativeVulkanError>>,
    frame_contexts: &mut [NativeVulkanAv1FrameContext],
    timeout: Duration,
) -> Result<Vec<NativeVulkanAv1AcquireWorkerResult>, NativeVulkanError> {
    ...
    loop {
        if frame_contexts
            .iter()
            .any(|context| context.preacquired_image_index.is_some())
            || !frame_contexts
                .iter()
                .any(|context| context.preacquire_pending)
        {
...
#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_av1_decode_entry_touches_sampled_resource(
    entry: &NativeVulkanAv1DecodeReferencePlanEntrySnapshot,
    begin_reference_slot_strategy: NativeVulkanAv1BeginReferenceSlotStrategy,
    has_dedicated_output_image: bool,
    resource: NativeVulkanAv1SampledResource,
) -> bool {
    if resource.dedicated_output {
        return has_dedicated_output_image;
    }
    if resource.shared_general_layout {
        return entry.output_slot == Some(resource.displayed_slot);
    }
    begin_reference_slot_strategy == NativeVulkanAv1BeginReferenceSlotStrategy::FullDpbGeneric
        || entry.output_slot == Some(resource.displayed_slot)
        || entry
            .decode_reference_slots
            .iter()
            .any(|slot| u32::try_from(*slot).ok() == Some(resource.displayed_slot))
}

#[cfg(feature = "native-vulkan-gst-video")]
fn native_vulkan_wait_av1_frame_context_resource_hazards(
    device: &ash::Device,
    frame_contexts: &mut [NativeVulkanAv1FrameContext],
    entry: &NativeVulkanAv1DecodeReferencePlanEntrySnapshot,
    begin_reference_slot_strategy: NativeVulkanAv1BeginReferenceSlotStrategy,
    ...
    for context in frame_contexts.iter_mut() {
        let Some(sampled_resource) = context.sampled_resource else {
            continue;
        };
        if !native_vulkan_av1_decode_entry_touches_sampled_resource(
            entry,
            begin_reference_slot_strategy,
            has_dedicated_output_image,
            sampled_resource,
        ) {
            continue;
        }
        ...
        unsafe {
            device
                .wait_for_fences(&[context.in_flight], true, u64::MAX)
                .map_err(|result| NativeVulkanError::Vulkan {
                    operation: "vkWaitForFences(direct av1 sampled resource frame context)",
                    result,
                })?;
        }
        stats.add_wait(native_vulkan_elapsed_us(started_at.elapsed()), false);
        context.sampled_resource = None;
    }
    Ok(stats)
}
...
fn native_vulkan_h264_slice_decode_info(
    ...
    let is_intra =
        normalized_slice_type == vk::native::StdVideoH264SliceType_STD_VIDEO_H264_SLICE_TYPE_I;
    let is_p =
        normalized_slice_type == vk::native::StdVideoH264SliceType_STD_VIDEO_H264_SLICE_TYPE_P;
    let is_b =
        normalized_slice_type == vk::native::StdVideoH264SliceType_STD_VIDEO_H264_SLICE_TYPE_B;
...
struct NativeVulkanAv1ParsedFrameHeader {
    ...
    interpolation_filter: vk::native::StdVideoAV1InterpolationFilter,
    ...
    NativeVulkanAv1ParsedFrameHeader {
        ...
        loop_restoration: NativeVulkanAv1ParsedLoopRestoration {
            frame_restoration_type: [
                vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_NONE,
                vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_NONE,
                vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_NONE,
            ],
    ...
    let (
        ...
        let inter_tail_parse = (|| -> Result<
            (
                ...
                u8,
                bool,
                vk::native::StdVideoAV1InterpolationFilter,
                bool,
                bool,
                bool,
                bool,
...
fn native_vulkan_parse_av1_interpolation_filter(
    bits: &mut NativeVulkanAv1BitReader<'_>,
) -> Result<(vk::native::StdVideoAV1InterpolationFilter, bool), String> {
    let is_filter_switchable = bits.read_bool("is_filter_switchable")?;
    if is_filter_switchable {
        return Ok((
            vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_SWITCHABLE,
            true,
        ));
    }
    let filter = match bits.read_bits(2, "interpolation_filter")? {
        0 => vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_EIGHTTAP,
        1 => {
            vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_EIGHTTAP_SMOOTH
        }
        2 => {
            vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_EIGHTTAP_SHARP
        }
        3 => vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_BILINEAR,
...
#[cfg(any(feature = "native-vulkan-gst-video", test))]
fn native_vulkan_av1_std_frame_restoration_type(
    value: u32,
) -> Result<vk::native::StdVideoAV1FrameRestorationType, String> {
    match value {
        0 => Ok(
            vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_NONE,
        ),
        1 => Ok(
            vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_WIENER,
        ),
        2 => Ok(
            vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_SGRPROJ,
        ),
        3 => Ok(
            vk::native::StdVideoAV1FrameRestorationType_STD_VIDEO_AV1_FRAME_RESTORATION_TYPE_SWITCHABLE,
        ),
...
fn native_vulkan_av1_interpolation_filter_label(
    filter: vk::native::StdVideoAV1InterpolationFilter,
) -> &'static str {
    match filter {
        vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_EIGHTTAP => {
            "eighttap"
        ...
        vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_BILINEAR => {
            "bilinear"
        }
        vk::native::StdVideoAV1InterpolationFilter_STD_VIDEO_AV1_INTERPOLATION_FILTER_SWITCHABLE => {
            "switchable"
...
fn native_vulkan_h264_session_dpb_slots(
    driver_max_dpb_slots: u32,
    parameter_sets: &NativeVulkanH264ParameterSetSnapshot,
) -> u32 {
    let stream_slots = native_vulkan_h264_sps_dpb_slot_count(&parameter_sets.sps);
...
fn native_vulkan_h264_reference_info_flags(
    ...
    vk::native::StdVideoDecodeH264ReferenceInfoFlags {
        ...
        _bitfield_1: vk::native::StdVideoDecodeH264ReferenceInfoFlags::new_bitfield_1(
            native_vulkan_bool_u32(field_pic_flag && !bottom_field_flag),
            native_vulkan_bool_u32(field_pic_flag && bottom_field_flag),
...
fn native_vulkan_h264_reference_info_frame_num(
    frame_num: u16,
    used_for_long_term_reference: bool,
...
pub fn wallpaper_type_support_matrix() -> Vec<NativeVulkanWallpaperTypeSupport> {
    vec![
        NativeVulkanWallpaperTypeSupport {
            ...
            current_renderer_status: "--run-static uses Vulkanalia sampled-image dynamic rendering; legacy ash static session and staging-copy runtime are removed",
...
mod tests {
    ...
    fn plans_h264_default_b_slice_short_term_references() {
        ...
        b_slice.slice_type_normalized =
            vk::native::StdVideoH264SliceType_STD_VIDEO_H264_SLICE_TYPE_B;
