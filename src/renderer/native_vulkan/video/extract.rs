...
//! plans, and Vulkanalia decode inputs.

use std::path::Path;

use super::super::*;

pub(in crate::renderer::native_vulkan) struct NativeVulkanVideoBitstreamExtract {
    pub(in crate::renderer::native_vulkan) snapshot: NativeVulkanVideoBitstreamExtractSnapshot,
}

pub(in crate::renderer::native_vulkan) fn native_vulkan_extract_video_bitstream(
...
pub(in crate::renderer::native_vulkan) fn native_vulkan_start_h264_streaming_packet_queue(
    source: &Path,
    capacity: usize,
...
pub(in crate::renderer::native_vulkan) fn native_vulkan_start_h265_streaming_packet_queue(
    source: &Path,
    capacity: usize,
...
pub(in crate::renderer::native_vulkan) fn native_vulkan_start_av1_streaming_packet_queue(
    source: &Path,
    capacity: usize,
...
fn native_vulkan_h264_bitstream_extract_from_queue(
    ...
    let snapshot = NativeVulkanVideoBitstreamExtractSnapshot {
        ...
        caps: None,
        stream_format: Some("byte-stream".to_owned()),
        alignment: Some("au".to_owned()),
        width: Some(parameter_sets.sps.width),
        height: Some(parameter_sets.sps.height),
        framerate: None,
    ...
    Ok(NativeVulkanVideoBitstreamExtract { snapshot })
}
...
fn native_vulkan_av1_bitstream_extract_from_queue(
    ...
    let snapshot = NativeVulkanVideoBitstreamExtractSnapshot {
        ...
        alignment: Some("frame".to_owned()),
        width: Some(parameter_sets.max_frame_width),
        height: Some(parameter_sets.max_frame_height),
        framerate: None,
...
pub(in crate::renderer::native_vulkan) fn native_vulkan_validate_h265_ready_prefix(
...
pub(in crate::renderer::native_vulkan) fn native_vulkan_validate_h264_ready_prefix(
    snapshot: &NativeVulkanVideoBitstreamExtractSnapshot,
    required_ready_prefix_access_units: u32,
) -> Result<(), NativeVulkanError> {
    if required_ready_prefix_access_units == 0
