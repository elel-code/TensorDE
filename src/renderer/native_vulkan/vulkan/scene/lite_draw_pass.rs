...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct NativeVulkanVulkanaliaSceneLiteSampledImagePipelineSnapshot {
    pub binding: &'static str,
    pub route: &'static str,
    pub target_format: String,
    pub extent: (u32, u32),
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct NativeVulkanVulkanaliaSceneLiteSampledImageCommandSnapshot {
    pub binding: &'static str,
    pub route: &'static str,
    pub extent: (u32, u32),
    pub index_count: u32,
...
pub(in crate::renderer::native_vulkan::vulkan) struct VulkanaliaSceneLiteSolidQuadPipelineResources
{
...
pub(in crate::renderer::native_vulkan::vulkan) struct VulkanaliaSceneLiteSampledImagePipelineResources
{
    pub(in crate::renderer::native_vulkan::vulkan) pipeline_layout: vk::PipelineLayout,
    pub(in crate::renderer::native_vulkan::vulkan) pipeline: vk::Pipeline,
    pub(in crate::renderer::native_vulkan::vulkan) snapshot:
...
pub(in crate::renderer::native_vulkan::vulkan) enum VulkanaliaSceneLiteSampledImageDescriptorBinding
{
    DescriptorHeap { resource_index: u32 },
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::native_vulkan::vulkan) struct VulkanaliaSceneLiteSampledImageDrawCommand {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(in crate::renderer::native_vulkan::vulkan) struct VulkanaliaSceneLiteSolidQuadDrawCommand {
    pub(in crate::renderer::native_vulkan::vulkan) layer_index: usize,
    pub(in crate::renderer::native_vulkan::vulkan) first_index: u32,
    pub(in crate::renderer::native_vulkan::vulkan) index_count: u32,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum VulkanaliaSceneLiteOrderedDrawPipeline {
    SolidQuad,
    SampledImage,
}

impl VulkanaliaSceneLiteOrderedDrawPipeline {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
struct VulkanaliaSceneLiteOrderedDrawStep {
    layer_index: usize,
    pipeline: VulkanaliaSceneLiteOrderedDrawPipeline,
    command_index: usize,
}

#[derive(Clone, Copy)]
pub(in crate::renderer::native_vulkan::vulkan) struct VulkanaliaSceneLiteSolidQuadDrawResources<'a>
{
    pub(in crate::renderer::native_vulkan::vulkan) pipeline_resources:
        &'a VulkanaliaSceneLiteSolidQuadPipelineResources,
    ...
    pub(in crate::renderer::native_vulkan::vulkan) draw_commands:
        &'a [VulkanaliaSceneLiteSolidQuadDrawCommand],
}

#[derive(Clone, Copy)]
pub(in crate::renderer::native_vulkan::vulkan) struct VulkanaliaSceneLiteDescriptorHeapDrawResources<
    'a,
> {
...
fn native_vulkan_vulkanalia_scene_lite_ordered_draw_steps(
    solid_commands: &[VulkanaliaSceneLiteSolidQuadDrawCommand],
    sampled_commands: &[VulkanaliaSceneLiteSampledImageDrawCommand],
) -> Vec<VulkanaliaSceneLiteOrderedDrawStep> {
    let mut ordered =
    ...
    for (command_index, command) in solid_commands.iter().enumerate() {
        ordered.push(VulkanaliaSceneLiteOrderedDrawStep {
            layer_index: command.layer_index,
            pipeline: VulkanaliaSceneLiteOrderedDrawPipeline::SolidQuad,
            command_index,
        });
    }
    for (command_index, command) in sampled_commands.iter().enumerate() {
        ordered.push(VulkanaliaSceneLiteOrderedDrawStep {
            layer_index: command.layer_index,
            pipeline: VulkanaliaSceneLiteOrderedDrawPipeline::SampledImage,
            command_index,
        });
    }
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_create_scene_lite_solid_quad_pipeline_resources(
    device: &Device,
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_destroy_scene_lite_solid_quad_pipeline_resources(
    device: &Device,
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_scene_lite_solid_quad_pipeline_snapshot(
    target_format: vk::Format,
    extent: vk::Extent2D,
) -> NativeVulkanVulkanaliaSceneLiteSolidQuadPipelineSnapshot {
    NativeVulkanVulkanaliaSceneLiteSolidQuadPipelineSnapshot {
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_create_scene_lite_sampled_image_pipeline_resources(
    device: &Device,
    target_format: vk::Format,
    extent: vk::Extent2D,
    ...
    let result = (|| -> Result<VulkanaliaSceneLiteSampledImagePipelineResources, String> {
        ...
        let result = (|| -> Result<VulkanaliaSceneLiteSampledImagePipelineResources, String> {
            let vertex_module = native_vulkan_vulkanalia_scene_lite_create_shader_module(
                ...
                "scene-lite sampled image vertex",
            )?;
            let result =
                (|| -> Result<VulkanaliaSceneLiteSampledImagePipelineResources, String> {
                    let fragment_module = native_vulkan_vulkanalia_scene_lite_create_shader_module(
                        ...
                        "scene-lite sampled image fragment",
                    )?;
                    let result =
                        (|| -> Result<VulkanaliaSceneLiteSampledImagePipelineResources, String> {
                            let shader_entry = b"main\0";
                        ...
                        .map_err(|err| {
                            format!(
                                "vkCreateGraphicsPipelines(vulkanalia scene-lite sampled image): {err:?}"
                            )
                        })?;
                            let pipeline = pipelines[0];
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_destroy_scene_lite_sampled_image_pipeline_resources(
    device: &Device,
    resources: VulkanaliaSceneLiteSampledImagePipelineResources,
) {
    unsafe {
        device.destroy_pipeline(resources.pipeline, None);
        device.destroy_pipeline_layout(resources.pipeline_layout, None);
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_scene_lite_sampled_image_pipeline_snapshot(
    target_format: vk::Format,
    extent: vk::Extent2D,
) -> NativeVulkanVulkanaliaSceneLiteSampledImagePipelineSnapshot {
    NativeVulkanVulkanaliaSceneLiteSampledImagePipelineSnapshot {
        binding: "vulkanalia",
        route: "scene-lite-sampled-image-dynamic-rendering-pipeline",
        target_format: format!("{target_format:?}"),
        extent: (extent.width, extent.height),
        shader_modules_created: true,
...
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_record_scene_lite_solid_quad_command_buffer(
    device: &Device,
...
#[allow(clippy::too_many_arguments)]
pub(in crate::renderer::native_vulkan::vulkan) fn native_vulkan_vulkanalia_record_scene_lite_sampled_image_command_buffer(
    device: &Device,
    command_buffer: vk::CommandBuffer,
    swapchain_image: vk::Image,
    swapchain_view: vk::ImageView,
    ...
    descriptor_heap_draw: Option<VulkanaliaSceneLiteDescriptorHeapDrawResources<'_>>,
    ...
    if let Some(draw) = solid_quad_draw {
        if draw.draw_commands.is_empty() {
            return Err("scene-lite mixed command requires non-empty solid draw steps".to_owned());
        }
        for solid_draw in draw.draw_commands {
            if solid_draw.index_count == 0 {
                return Err(
                    "scene-lite mixed command requires non-empty solid draw indices".to_owned(),
                );
            }
        }
    }
        ...
        match draw.descriptor_binding {
            VulkanaliaSceneLiteSampledImageDescriptorBinding::DescriptorHeap { resource_index } => {
                let Some(descriptor_heap_draw) = descriptor_heap_draw else {
                    return Err(
                        "scene-lite sampled-image descriptor heap draw requires heap resources"
                            .to_owned(),
                    );
                };
                if resource_index as usize >= descriptor_heap_draw.resources.plan.image_count {
                    return Err(format!(
                        "scene-lite sampled-image descriptor heap resource index {resource_index} exceeds heap image count {}",
    ...
    let solid_draw_commands: &[VulkanaliaSceneLiteSolidQuadDrawCommand] =
        solid_quad_draw.map_or(&[], |draw| draw.draw_commands);
    let ordered_draws =
        native_vulkan_vulkanalia_scene_lite_ordered_draw_steps(solid_draw_commands, draw_commands);

    unsafe {
        device
            .reset_command_buffer(command_buffer, vk::CommandBufferResetFlags::empty())
            .map_err(|err| {
                format!("vkResetCommandBuffer(vulkanalia scene-lite sampled image): {err:?}")
            })?;
        let begin_info = vk::CommandBufferBeginInfo::builder()
            .flags(vk::CommandBufferUsageFlags::ONE_TIME_SUBMIT)
            .build();
        device
            .begin_command_buffer(command_buffer, &begin_info)
            .map_err(|err| {
                format!("vkBeginCommandBuffer(vulkanalia scene-lite sampled image): {err:?}")
            })?;
        ...
        for draw in &ordered_draws {
            match draw.pipeline {
                VulkanaliaSceneLiteOrderedDrawPipeline::SolidQuad => {
                ...
                VulkanaliaSceneLiteOrderedDrawPipeline::SampledImage => {
        ...
        device.end_command_buffer(command_buffer).map_err(|err| {
            format!("vkEndCommandBuffer(vulkanalia scene-lite sampled image): {err:?}")
        })?;
    }
...
mod tests {
    ...
    #[test]
    fn sampled_image_scene_lite_path_is_dynamic_rendering_recordable() {
        let mut input = input();
    ...
    fn mixed_ordered_draw_steps_follow_scene_layer_order() {
        let solid_commands = [VulkanaliaSceneLiteSolidQuadDrawCommand {
            layer_index: 2,
            first_index: 0,
            index_count: 6,
        }];
        ...
        let ordered = native_vulkan_vulkanalia_scene_lite_ordered_draw_steps(
            &solid_commands,
            &sampled_commands,
        );
        let order = ordered
