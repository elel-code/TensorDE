...
// reverse-engineered reference: Wallpaper Engine colorBlendMode > 0 is
// materials/util/effectpassthrough. It samples the already-rendered screen
// and the layer/effect result, runs ApplyBlending in shader code, and writes
// screen alpha back.
...
layout(push_constant) uniform ScenePush {
    ...
    layout(offset = 96) uint blend_mode;
...
void main() {
    vec4 albedo = apply_vertex_color(texture(g_Texture0, v_uv));
    if (pc.blend_mode == 0u) {
        out_color = albedo;
        return;
    }
    ...
    vec4 screen = texture(g_Texture1, screen_uv);

    out_color.rgb = apply_blending(pc.blend_mode, screen.rgb, albedo.rgb, albedo.a);
