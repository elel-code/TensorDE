...
/// Builds an input-attachment mapping relative to the first descriptor in a
/// mixed per-draw resource slice.  The slice anchor is the transform uniform,
/// not an input attachment; no sampler-heap offset is ever emitted.
pub(in crate::renderer::native_vulkan) fn native_vulkan_vulkanalia_descriptor_heap_resource_relative_mixed_input_attachment_binding_mapping(
    plan: &NativeVulkanVulkanaliaDescriptorHeapResourcePlanSnapshot,
    binding: u32,
    base_resource_descriptor_index: usize,
    resource_descriptor_index: usize,
    ...
    let base_heap_offset = *plan
        ...
        .ok_or_else(|| {
            format!(
                "descriptor heap mixed input-attachment base resource descriptor index {base_resource_descriptor_index} has no resource offset"
            )
        })?;
    let base_heap_offset = align_down(base_heap_offset, plan.resource_heap_alignment);
    let heap_offset = plan
        ...
        .ok_or_else(|| {
            format!(
                "descriptor heap mixed input-attachment resource descriptor index {resource_descriptor_index} has no resource offset"
            )
        })?
        .checked_sub(base_heap_offset)
        .ok_or_else(|| {
            format!(
                "descriptor heap mixed input-attachment descriptor index {resource_descriptor_index} precedes heap-slice base {base_resource_descriptor_index}"
            )
        })?;
    let heap_offset = u32::try_from(heap_offset)
        .map_err(|_| "descriptor heap mixed input-attachment relative image offset exceeds u32".to_owned())?;
    let heap_array_stride = u32::try_from(plan.image_descriptor_stride)
        .map_err(|_| "descriptor heap mixed input-attachment stride exceeds u32".to_owned())?;
    let source = vk::DescriptorMappingSourceConstantOffsetEXT::builder()
        .heap_offset(heap_offset)
        .heap_array_stride(heap_array_stride)
        .sampler_heap_offset(0)
