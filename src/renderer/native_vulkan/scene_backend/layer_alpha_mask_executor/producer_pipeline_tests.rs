use super::super::NativeVulkanSceneLayerAlphaMaskDescriptorSource;
...
use crate::engine::scene_engine::{
    SceneGraphPipelineClass, SceneGraphTarget, SceneLayerCompositorCondition, SceneObjectId,
    ScenePuppetId, SceneResourceId,
};
...
#[test]
fn producer_pipeline_pairs_draws_with_clippingmaskimage4_heap_binds() {
    let producer_draws = producer_draws(vec![draw(0, vec![0])]);
    let resource_binds = resource_binds(vec![bind(0, 0, vec![0, 1])]);

    let plan = native_vulkan_plan_scene_layer_alpha_mask_producer_pipelines(
        &producer_draws,
        &resource_binds,
    )
    ...
    assert_eq!(plan.texture_slot_mask, 0b11);
    assert_eq!(plan.draw_requires_subdraw_selection_count, 0);
    assert_eq!(plan.cache_keys().len(), 1);
    assert_eq!(plan.cache_keys()[0].shader, "we/clippingmaskimage4");
    assert_eq!(plan.cache_keys()[0].target_format, vk::Format::R8_UNORM);

    let binding = &plan.bindings[0];
    assert_eq!(binding.command_index, 1);
    assert_eq!(binding.heap_bind_index, 0);
    assert_eq!(binding.heap_slice_index, 0);
    assert_eq!(binding.base_resource_descriptor_index, 4);
    assert_eq!(binding.base_sampler_descriptor_index, 8);
    assert_eq!(binding.texture_count, 2);
    assert_eq!(binding.clipping_record_index, 0);
    assert!(!binding.requires_subdraw_index_selection);
    assert_eq!(
...
#[test]
fn producer_pipeline_preserves_multi_record_subdraw_selection_requirement() {
    ...
    assert_eq!(plan.pipeline_binding_count, 2);
    assert_eq!(plan.cache_key_count, 1);
    assert_eq!(plan.draw_requires_subdraw_selection_count, 1);
    assert!(
        plan.bindings.iter().all(|binding| {
            binding.command_index == 1 && binding.requires_subdraw_index_selection
        })
    );
    assert_eq!(plan.bindings[0].clipping_record_index, 0);
    assert_eq!(plan.bindings[1].clipping_record_index, 1);
}

#[test]
fn producer_pipeline_rejects_missing_slot1_mask_texture() {
    let producer_draws = producer_draws(vec![draw(0, vec![0])]);
    let resource_binds = resource_binds(vec![bind(0, 0, vec![0])]);

    let err = native_vulkan_plan_scene_layer_alpha_mask_producer_pipelines(
        &producer_draws,
        &resource_binds,
    )
    .expect_err("slot1 mask texture must be required");

    assert!(err.contains("slot0/slot1 texture-only heap bind"));
}
