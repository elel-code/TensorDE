...
use super::frame_plan::{
    NativeVulkanSceneResourceHeapDescriptorBinding, NativeVulkanSceneResourceHeapFramePlan,
};

pub(super) fn write_scene_resource_heap_descriptors(
    device: &Device,
    ...
    for binding in &frame_plan.bindings {
        match *binding {
            ...
            NativeVulkanSceneResourceHeapDescriptorBinding::SampledImage {
                ...
                let view_info =
                    scene_resource_heap_image_view_create_info(image, format, mip_count);
                let sampler_info = scene_resource_heap_sampler_create_info(mip_count);
                native_vulkan_vulkanalia_write_descriptor_heap_resource_image_sampler(
                    device,
                    resources,
                    descriptor_index,
...
fn scene_resource_heap_image_view_create_info(
    image: vk::Image,
    format: vk::Format,
    mip_count: u32,
...
fn scene_resource_heap_sampler_create_info(mip_count: u32) -> vk::SamplerCreateInfo {
...
mod tests {
    ...
    #[test]
    fn resource_heap_view_create_info_uses_full_mip_range() {
        let view_info = scene_resource_heap_image_view_create_info(
            vk::Image::from_raw(77),
            vk::Format::R8G8B8A8_UNORM,
            5,
        );
        let sampler_info = scene_resource_heap_sampler_create_info(5);
        ...
        assert_eq!(view_info.subresource_range.level_count, 5);
