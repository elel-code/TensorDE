...
use crate::core::scene::{
    ...
    SceneSnapshotLayer, SceneSnapshotSampledImageLayer,
};
use crate::core::{
...
use self::blend::{native_vulkan_scene_blend_state, native_vulkan_scene_render_state};
...
pub(super) use self::color::native_vulkan_scene_tint_from_color;
use self::color::{
...
pub(super) use self::types::{
    ...
    NativeVulkanSceneWeImageGraphTargetBounds, NativeVulkanSceneWeImageGraphTextureBinding,
...
pub(super) fn native_vulkan_scene_append_sampled_image_vertices_from_render_layer(
    layer_index: usize,
    layer: &SceneRenderLayer,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Result<Option<u32>, &'static str> {
...
pub(super) fn native_vulkan_scene_append_sampled_image_vertices_from_sampled_layer_with_effect_chain(
    ...
    let Some(chain) = native_vulkan_scene_we_image_pass_chain(&quad).filter(|chain| {
        matches!(
            chain.execution,
    ...
    if !chain.local_target_required {
        return native_vulkan_scene_append_sampled_image_vertices(&quad, vertices)
            .ok_or("image-layer-missing-recordable-geometry")
            .map(Some);
    }
    let first_vertex = vertices.len();
    let mut discarded_indices = Vec::new();
    let plan = native_vulkan_scene_we_image_graph_plan(std::slice::from_ref(&quad));
    for step in native_vulkan_scene_we_image_graph_steps_for_layer(&plan, quad.layer_index) {
        native_vulkan_scene_append_sampled_image_we_graph_step_geometry(
            &quad,
            step,
            &plan,
            None,
...
fn native_vulkan_scene_sampled_image_recording_payload(
    ...
    for quad in &visible_quads {
        ...
        if native_vulkan_scene_sampled_image_needs_we_effect_chain(quad) {
            native_vulkan_scene_collect_we_graph_step_sources(&mut sources, we_graph_plan, quad);
        }
    }
    ...
    for quad in visible_quads {
        ...
        if native_vulkan_scene_sampled_image_needs_we_effect_chain(quad) {
            native_vulkan_scene_allocate_we_graph_effect_targets(
                &mut effect_targets,
                we_graph_plan,
                quad,
            );
            native_vulkan_scene_record_we_graph_sampled_image_steps(
                quad,
                scene_size,
                we_graph_plan,
                file_source_count,
...
enum NativeVulkanSceneDynamicSampledImageVertexSource<'a> {
    DirectRenderLayer {
        layer_index: usize,
        layer: &'a SceneRenderLayer,
    },
    WeGraphQuad {
        quad_index: usize,
    },
...
pub(super) fn native_vulkan_scene_append_draw_pass_sampled_image_vertices_from_render_layers(
    layers: &[SceneRenderLayer],
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Result<(), String> {
    vertices.clear();
    let mut sources = Vec::with_capacity(layers.len());
    let mut we_graph_quads = Vec::new();
    for (layer_index, layer) in layers.iter().enumerate() {
        if !native_vulkan_scene_render_layer_is_sampled_image_draw_op(layer)
            || native_vulkan_scene_full_extent_sampled_image_layer_ready(layer)
        {
            continue;
        }
        if native_vulkan_scene_render_layer_uses_direct_retained_sampled_vertices(layer) {
            sources.push(
                NativeVulkanSceneDynamicSampledImageVertexSource::DirectRenderLayer {
                    layer_index,
                    layer,
                },
            );
        ...
        if let Some(quad) =
            native_vulkan_scene_sampled_image_quad_from_render_layer(layer_index, layer)
        {
            let quad_index = we_graph_quads.len();
            we_graph_quads.push(quad);
            sources
                .push(NativeVulkanSceneDynamicSampledImageVertexSource::WeGraphQuad { quad_index });
        }
    }
    let we_graph_plan = native_vulkan_scene_we_image_graph_plan(&we_graph_quads);
    for source in sources {
        match source {
            NativeVulkanSceneDynamicSampledImageVertexSource::DirectRenderLayer {
                ...
                let _ = native_vulkan_scene_append_direct_render_layer_sampled_image_vertices(
                    layer_index,
                    layer,
                    vertices,
                );
            }
            NativeVulkanSceneDynamicSampledImageVertexSource::WeGraphQuad { quad_index } => {
                let Some(quad) = we_graph_quads.get(quad_index).filter(|quad| {
                    native_vulkan_scene_sampled_image_quad_has_recordable_geometry(quad)
                        && !native_vulkan_scene_sampled_image_suppresses_unimplemented_we_effect_chain(quad)
                }) else {
                    continue;
                };
                if native_vulkan_scene_sampled_image_needs_we_effect_chain(quad) {
                    for we_graph_step in native_vulkan_scene_we_image_graph_steps_for_layer(
                        &we_graph_plan,
                        quad.layer_index,
                    ) {
                        let _ = native_vulkan_scene_append_sampled_image_we_graph_step_vertices(
                            quad,
                            we_graph_step,
                            &we_graph_plan,
                            None,
                    ...
                    let _ = native_vulkan_scene_append_sampled_image_vertices_in_scene(
                        quad, None, vertices,
                    );
                }
            }
...
fn native_vulkan_scene_render_layer_is_sampled_image_draw_op(layer: &SceneRenderLayer) -> bool {
    layer.opacity > 0.0 && layer.kind == SceneNodeKind::Image && layer.source.is_some()
}

fn native_vulkan_scene_full_extent_sampled_image_layer_ready(layer: &SceneRenderLayer) -> bool {
...
fn native_vulkan_scene_render_layer_uses_direct_retained_sampled_vertices(
    layer: &SceneRenderLayer,
) -> bool {
    layer.image_effect_passes.is_empty() && layer.blend_mode != SceneBlendMode::HslColor
}

fn native_vulkan_scene_sampled_image_quad_from_render_layer(
    layer_index: usize,
    layer: &SceneRenderLayer,
    ...
    Some(NativeVulkanSceneSampledImageQuad {
        ...
        mesh: layer.mesh.clone(),
        effect_uv_space: native_vulkan_scene_opacity_effect_uv_space_from_render_layer(layer),
        effect_motion: layer.effect_motion,
        texture_region: layer.texture_region,
        transform: layer.transform,
    })
...
fn native_vulkan_scene_append_direct_render_layer_sampled_image_vertices(
    layer_index: usize,
    layer: &SceneRenderLayer,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<u32> {
    ...
    native_vulkan_scene_append_sampled_image_vertices_in_scene(&quad, None, vertices)
}

fn native_vulkan_scene_sampled_image_needs_we_effect_chain(
    quad: &NativeVulkanSceneSampledImageQuad,
) -> bool {
    native_vulkan_scene_we_image_pass_chain(quad).is_some_and(|chain| {
        matches!(
            chain.execution,
            NativeVulkanSceneWeImagePassExecution::FirstClassTarget
        )
...
fn native_vulkan_scene_collect_we_graph_step_sources(
    ...
    for step in native_vulkan_scene_we_image_graph_steps_for_layer(plan, quad.layer_index) {
        for binding in &step.texture_bindings {
            if let Some(source_path) = binding.source_path.as_ref() {
                let _ =
...
fn native_vulkan_scene_allocate_we_graph_effect_targets(
    effect_targets: &mut Vec<NativeVulkanSceneSampledImageEffectTarget>,
    plan: &NativeVulkanSceneWeImageGraphPlan,
    quad: &NativeVulkanSceneSampledImageQuad,
) {
    for target in plan.targets.iter().filter(|target| {
        target.layer_index == quad.layer_index
            && matches!(
                target.execution,
                NativeVulkanSceneWeImagePassExecution::FirstClassTarget
            )
    }) {
        if effect_targets.iter().any(|effect_target| {
        ...
        let effect_target_index = effect_targets.len().min(u32::MAX as usize) as u32;
        effect_targets.push(NativeVulkanSceneSampledImageEffectTarget {
            effect_target_index,
            layer_index: quad.layer_index,
            width: target.width,
            height: target.height,
            we_graph_chain_index: Some(target.chain_index),
            we_graph_target_index: Some(target.target_index),
            we_graph_endpoint: Some(target.endpoint),
        });
    }
}

#[allow(clippy::too_many_arguments)]
fn native_vulkan_scene_record_we_graph_sampled_image_steps(
    quad: &NativeVulkanSceneSampledImageQuad,
    scene_size: Option<SceneSize>,
    plan: &NativeVulkanSceneWeImageGraphPlan,
    file_source_count: usize,
    effect_targets: &[NativeVulkanSceneSampledImageEffectTarget],
    sources: &mut Vec<PathBuf>,
    steps: &mut Vec<NativeVulkanSceneSampledImageRecordingStep>,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
    indices: &mut Vec<u32>,
) {
    for we_graph_step in native_vulkan_scene_we_image_graph_steps_for_layer(plan, quad.layer_index)
    {
        let Some(range) = native_vulkan_scene_append_sampled_image_we_graph_step_geometry(
            quad,
            we_graph_step,
            plan,
            scene_size,
        ...
        let Some(render_target) =
            native_vulkan_scene_we_graph_step_render_target(plan, we_graph_step, effect_targets)
        else {
            continue;
        };
        let Some(texture_slot_bindings) = native_vulkan_scene_we_graph_step_texture_slot_bindings(
            sources,
            we_graph_step,
            file_source_count,
            effect_targets,
        ...
        let (alpha_texture_slot, alpha_texture_mode) =
            native_vulkan_scene_we_graph_step_alpha_texture(we_graph_step);
        let blend_mode = we_graph_step.pass.render_state.blend.mode;
        let step = native_vulkan_scene_sampled_image_recording_step(
            quad,
            texture_slot_bindings,
            alpha_texture_slot,
...
fn native_vulkan_scene_we_image_graph_steps_for_layer<'a>(
    plan: &'a NativeVulkanSceneWeImageGraphPlan,
    layer_index: usize,
) -> impl Iterator<Item = &'a NativeVulkanSceneWeImageGraphStep> {
    plan.steps.iter().filter(move |step| {
...
fn native_vulkan_scene_append_sampled_image_we_graph_step_geometry(
    quad: &NativeVulkanSceneSampledImageQuad,
...
fn native_vulkan_scene_we_graph_step_output_target_bounds(
    step: &NativeVulkanSceneWeImageGraphStep,
    plan: &NativeVulkanSceneWeImageGraphPlan,
) -> Option<NativeVulkanSceneWeImageGraphTargetBounds> {
    native_vulkan_scene_we_graph_target_bounds(plan, step.output_target_index)
}

fn native_vulkan_scene_we_graph_step_input_target_bounds(
    step: &NativeVulkanSceneWeImageGraphStep,
    plan: &NativeVulkanSceneWeImageGraphPlan,
) -> Option<NativeVulkanSceneWeImageGraphTargetBounds> {
    native_vulkan_scene_we_graph_target_bounds(plan, step.input_target_index)
}

fn native_vulkan_scene_we_graph_target_bounds(
    plan: &NativeVulkanSceneWeImageGraphPlan,
    ...
    let target = plan
        .targets
        .iter()
        .find(|target| target.target_index == target_index)?;
    Some(NativeVulkanSceneWeImageGraphTargetBounds {
        left: target.local_left,
        top: target.local_top,
        width: target.width as f64,
        height: target.height as f64,
    })
}

fn native_vulkan_scene_we_graph_step_colored_quad(
    quad: &NativeVulkanSceneSampledImageQuad,
    step: &NativeVulkanSceneWeImageGraphStep,
) -> NativeVulkanSceneSampledImageQuad {
    let mut local_quad = quad.clone();
    if !native_vulkan_scene_we_graph_step_samples_source_texture(step) {
        // WE applies layer color/alpha when the source image is drawn into the
        // local target. Later material-graph passes sample that target and must
        // not multiply opacity/tint again, or water/caustic chains fade as
        // alpha^pass_count.
        local_quad.opacity = 1.0;
        local_quad.tint = SCENE_SAMPLED_IMAGE_DEFAULT_TINT;
    }
    local_quad
}

fn native_vulkan_scene_we_graph_step_is_direct_terminal_source_effect(
    step: &NativeVulkanSceneWeImageGraphStep,
) -> bool {
    step.pass.final_scene_pass
        && step.pass.role == NativeVulkanSceneWeImagePassRole::EffectMaterial
        && step.pass.input == NativeVulkanSceneWeImagePassEndpoint::SourceTexture
}

fn native_vulkan_scene_we_graph_step_samples_source_texture(
    step: &NativeVulkanSceneWeImageGraphStep,
) -> bool {
    step.pass.input == NativeVulkanSceneWeImagePassEndpoint::SourceTexture
}

fn native_vulkan_scene_we_graph_step_effect_uv_space(
    quad: &NativeVulkanSceneSampledImageQuad,
    step: &NativeVulkanSceneWeImageGraphStep,
) -> Option<NativeVulkanSceneEffectUvSpace> {
    match step.pass.effect_kind {
        Some(NativeVulkanSceneEffectKind::Iris | NativeVulkanSceneEffectKind::OpacityMask) => {
            native_vulkan_scene_sampled_image_effect_final_uv_space(quad)
        }
        _ => None,
    }
}
...
fn native_vulkan_scene_we_graph_step_render_target(
    ...
    step: &NativeVulkanSceneWeImageGraphStep,
    effect_targets: &[NativeVulkanSceneSampledImageEffectTarget],
) -> Option<NativeVulkanSceneSampledImageRenderTarget> {
    let Some(output_target_index) = step.output_target_index else {
        return Some(NativeVulkanSceneSampledImageRenderTarget::Swapchain);
    };
    let target = native_vulkan_scene_we_image_graph_target_for_step_output(
        plan,
        step.chain_index,
        output_target_index,
    )?;
    let effect_target = native_vulkan_scene_effect_target_for_graph_target(
        effect_targets,
        step.chain_index,
        output_target_index,
        target.endpoint,
    )?;
    Some(NativeVulkanSceneSampledImageRenderTarget::EffectTarget {
        target_index: effect_target.effect_target_index,
        clear: target.clear_before_first_write && target.first_write_step_index == step.step_index,
    })
}

fn native_vulkan_scene_we_image_graph_target_for_step_output(
    plan: &NativeVulkanSceneWeImageGraphPlan,
    chain_index: usize,
    target_index: u32,
...
fn native_vulkan_scene_effect_target_for_graph_target(
    effect_targets: &[NativeVulkanSceneSampledImageEffectTarget],
    chain_index: usize,
    graph_target_index: u32,
    endpoint: NativeVulkanSceneWeImagePassEndpoint,
) -> Option<&NativeVulkanSceneSampledImageEffectTarget> {
    effect_targets.iter().find(|target| {
        target.we_graph_chain_index == Some(chain_index)
            && target.we_graph_target_index == Some(graph_target_index)
            && target.we_graph_endpoint == Some(endpoint)
    })
}

fn native_vulkan_scene_we_graph_step_texture_slot_bindings(
    sources: &mut Vec<PathBuf>,
    step: &NativeVulkanSceneWeImageGraphStep,
    file_source_count: usize,
    effect_targets: &[NativeVulkanSceneSampledImageEffectTarget],
) -> Option<Vec<NativeVulkanSceneTextureSlotResourceBinding>> {
    ...
    for binding in &step.texture_bindings {
            ...
            (binding.target_index, binding.endpoint)
        {
            ...
            file_source_count
                .saturating_add(effect_target.effect_target_index as usize)
                .min(u32::MAX as usize) as u32
        } else {
            return None;
        };
        bindings.push(NativeVulkanSceneTextureSlotResourceBinding {
            slot: binding.slot,
            resource_index,
        });
    }
...
fn native_vulkan_scene_we_graph_step_alpha_texture(
    step: &NativeVulkanSceneWeImageGraphStep,
) -> (Option<u32>, SceneRenderAlphaTextureMode) {
    match step.pass.effect_kind {
        Some(NativeVulkanSceneEffectKind::Iris) => (
            native_vulkan_scene_we_graph_step_first_nonzero_texture_slot(step),
        ...
        _ => (None, SceneRenderAlphaTextureMode::Multiply),
    }
}

fn native_vulkan_scene_we_graph_step_first_nonzero_texture_slot(
    step: &NativeVulkanSceneWeImageGraphStep,
) -> Option<u32> {
    step.texture_bindings
        .iter()
        .filter(|binding| binding.slot != 0)
...
fn native_vulkan_scene_sampled_image_recording_step(
    ...
    let material_pass = we_graph_step.map_or_else(
        || {
            native_vulkan_scene_sampled_image_material_pass_with_effect_blend(
                material_kind,
                blend_mode,
        ...
        |step| {
            native_vulkan_scene_sampled_image_material_pass_for_we_graph_step(
                material_kind,
                step,
                blend_mode,
                alpha_texture_slot,
    ...
    NativeVulkanSceneSampledImageRecordingStep {
        ...
        fit: quad.fit,
        texture_region: match we_graph_step.map(|step| step.pass.role) {
            Some(
                NativeVulkanSceneWeImagePassRole::EffectMaterial
                | NativeVulkanSceneWeImagePassRole::ColorBlendPassthrough,
            ) => None,
            _ => quad.texture_region,
        },
        texture_slot_bindings,
        material_pass,
...
fn native_vulkan_scene_sampled_image_material_pass_for_we_graph_step(
    ...
    _quad_effect_passes: &[NativeVulkanSceneEffectRecord],
) -> NativeVulkanSceneMaterialPass {
    NativeVulkanSceneMaterialPass {
        kind,
        shader: step.pass.shader.clone(),
        blending: step.pass.blending.clone(),
        render_state: native_vulkan_scene_render_state(
            blend_mode,
            step.pass.depth_test,
            step.pass.depth_write,
            step.pass.cull_mode.clone(),
        ),
        alpha_texture_slot,
        alpha_texture_mode,
        texture_slot_count,
...
fn native_vulkan_scene_we_graph_step_system_shader_uniforms(
    ...
    step.texture_bindings
        .iter()
        .filter_map(|binding| {
            binding.resolution.map(|resolution| {
                NativeVulkanSceneShaderUniform::texture_resolution(&binding.uniform, resolution)
            })
        })
        .collect()
}

fn native_vulkan_scene_we_graph_step_effect_kinds(
    step: &NativeVulkanSceneWeImageGraphStep,
) -> Vec<NativeVulkanSceneEffectKind> {
    if let Some(effect_kind) = step.pass.effect_kind {
        return vec![effect_kind];
    }
    Vec::new()
}
    ...
    if let Some(effect_pass) = quad.effect_target_pass.as_ref() {
        if matches!(
            ...
            return match step.render_target {
                NativeVulkanSceneSampledImageRenderTarget::EffectTarget { .. } => {
                    if step.we_graph_step_index.is_some_and(|index| index > 0) {
                        "we-opacity-effect-pass-space-target-quad"
        ...
        if matches!(
            ...
            return match step.render_target {
                NativeVulkanSceneSampledImageRenderTarget::EffectTarget { .. } => {
                    if step.we_graph_step_index.is_some_and(|index| index > 0) {
                        "we-iris-effect-pass-space-target-quad"
                    } else if quad.mesh.is_some() {
...
fn native_vulkan_scene_append_sampled_image_vertices_in_scene(
    quad: &NativeVulkanSceneSampledImageQuad,
    scene_size: Option<SceneSize>,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<u32> {
    let before_vertices = vertices.len();
    let vertex_count = match native_vulkan_scene_append_sampled_image_vertices(quad, vertices) {
        Some(vertex_count) => vertex_count,
        None => {
            vertices.truncate(before_vertices);
            return None;
        }
...
fn native_vulkan_scene_append_sampled_image_effect_base_vertices(
    quad: &NativeVulkanSceneSampledImageQuad,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<u32> {
    let before_vertices = vertices.len();
    let result = if let Some(mesh) = &quad.mesh {
        native_vulkan_scene_append_sampled_image_effect_base_mesh_vertices(quad, mesh, vertices)
    } else {
        native_vulkan_scene_append_sampled_image_effect_base_quad_vertices(quad, vertices)
    };
    if result.is_none() {
        vertices.truncate(before_vertices);
        return None;
...
fn native_vulkan_scene_append_sampled_image_effect_final_vertices(
    quad: &NativeVulkanSceneSampledImageQuad,
    scene_size: Option<SceneSize>,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<u32> {
...
fn native_vulkan_scene_append_sampled_image_effect_final_geometry_with_target_bounds(
    ...
    let before_vertices = vertices.len();
    for (layer_position, uv, effect_position) in target_vertices {
        let local_x = layer_position[0] - quad.transform.anchor_x * quad.width;
        let local_y = layer_position[1] - quad.transform.anchor_y * quad.height;
        let position = native_vulkan_scene_transform_point_with_rotation(
            local_x,
            local_y,
            quad.transform,
            cos,
...
fn native_vulkan_scene_append_sampled_image_effect_final_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    effect_uv_transform: Option<SceneEffectUvTransform>,
    mask_uv_uses_target_uv: bool,
...
fn native_vulkan_scene_target_bounds_valid(
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
) -> bool {
...
fn native_vulkan_scene_append_full_sampled_image_quad_indices(
    first_vertex: u32,
    indices: &mut Vec<u32>,
) {
    indices.extend_from_slice(&[
...
fn native_vulkan_scene_graph_target_effect_uv(
    layer_x: f64,
    layer_y: f64,
    quad: &NativeVulkanSceneSampledImageQuad,
    ...
    let u = layer_x / quad.width;
    let v = 1.0 - layer_y / quad.height;
...
fn native_vulkan_scene_append_sampled_image_effect_base_geometry_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
    indices: &mut Vec<u32>,
    ...
    let first_index = indices.len().min(u32::MAX as usize) as u32;
    let index_count = if let Some(mesh) = &quad.mesh {
        native_vulkan_scene_append_sampled_image_effect_base_mesh_geometry_with_target_bounds(
            quad,
            mesh,
            bounds,
            first_vertex,
        ...
        native_vulkan_scene_append_sampled_image_effect_base_quad_geometry_with_target_bounds(
            quad,
            bounds,
            first_vertex,
            vertices,
...
fn native_vulkan_scene_append_sampled_image_effect_base_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<u32> {
    ...
    let before_vertices = vertices.len();
    let result = if let Some(mesh) = &quad.mesh {
        native_vulkan_scene_append_sampled_image_effect_base_mesh_vertices_with_target_bounds(
            quad, mesh, bounds, vertices,
        )
    } else {
        native_vulkan_scene_append_sampled_image_effect_base_quad_vertices_with_target_bounds(
            quad, bounds, vertices,
        )
    };
    if result.is_none() {
...
fn native_vulkan_scene_append_sampled_image_vertex_skew_effect_pass_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    pass: &NativeVulkanSceneWeImagePass,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
...
fn native_vulkan_scene_append_sampled_image_effect_pass_geometry_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    effect_uv_transform: Option<SceneEffectUvTransform>,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
    ...
    let opacity = quad.opacity.clamp(0.0, 1.0) as f32;
    let target_vertices = [
        ([0.0, 0.0], [0.0, 1.0], [bounds.left, bounds.top]),
        (
            [bounds.width, 0.0],
            [1.0, 1.0],
            [bounds.left + bounds.width, bounds.top],
    ...
    for (position, uv, effect_position) in target_vertices {
        vertices.push(NativeVulkanSceneSampledImageVertex {
            position: [position[0] as f32, position[1] as f32],
            uv,
            // Source sampling stays in expanded target pass-space. Mask and
            // time-offset textures are authored in the original layer/source
            // UV basis, so undo Gilder's extra padding when deriving
            // v_effect_uv. Otherwise body masks are compressed into the middle
            // of expanded puppet targets.
...
fn native_vulkan_scene_append_sampled_image_effect_pass_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    effect_uv_transform: Option<SceneEffectUvTransform>,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
...
fn native_vulkan_scene_append_sampled_image_effect_base_quad_geometry(
    ...
    for (position, uv, default_effect_uv) in target_vertices {
        let effect_uv = if quad.effect_uv_space.is_some() {
            native_vulkan_scene_sampled_image_vertex_effect_uv(
                position,
                f64::from(position[0]),
                f64::from(position[1]),
                uv,
...
fn native_vulkan_scene_append_sampled_image_effect_base_quad_vertices(
    quad: &NativeVulkanSceneSampledImageQuad,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<()> {
    if !quad.width.is_finite()
...
fn native_vulkan_scene_append_sampled_image_effect_base_quad_geometry_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    first_vertex: u32,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
    ...
    let target_vertices = [
        (
            [0.0 - bounds.left, 0.0 - bounds.top],
            [region.u_min as f32, region.v_max as f32],
            [0.0, 0.0],
        ),
        (
            [quad.width - bounds.left, 0.0 - bounds.top],
            [region.u_max as f32, region.v_max as f32],
            [quad.width, 0.0],
        ),
        (
            [0.0 - bounds.left, quad.height - bounds.top],
            [region.u_min as f32, region.v_min as f32],
            [0.0, quad.height],
        ),
        (
            [quad.width - bounds.left, quad.height - bounds.top],
            [region.u_max as f32, region.v_min as f32],
...
fn native_vulkan_scene_append_sampled_image_effect_base_quad_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<()> {
...
fn native_vulkan_scene_append_sampled_image_waterwaves_uv_domain_base_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<u32> {
...
fn native_vulkan_scene_append_sampled_image_effect_base_mesh_geometry(
    ...
    for vertex in &mesh.vertices {
        ...
        let effect_uv = if quad.effect_uv_space.is_some() {
            native_vulkan_scene_sampled_image_vertex_effect_uv(
                position, vertex.x, vertex.y, uv, quad,
            )
        } else {
...
fn native_vulkan_scene_append_sampled_image_effect_base_mesh_vertices(
    quad: &NativeVulkanSceneSampledImageQuad,
    mesh: &SceneMesh,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
) -> Option<()> {
...
fn native_vulkan_scene_append_sampled_image_effect_base_mesh_geometry_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    mesh: &SceneMesh,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    first_vertex: u32,
    ...
    for vertex in &mesh.vertices {
        ...
        let layer_x = vertex.x + quad.width * 0.5;
        let layer_y = vertex.y + quad.height * 0.5;
        let uv = native_vulkan_scene_sampled_image_effect_base_mesh_uv(vertex, region);
        vertices.push(NativeVulkanSceneSampledImageVertex {
            position: [
                (layer_x - bounds.left) as f32,
...
fn native_vulkan_scene_append_sampled_image_effect_base_mesh_vertices_with_target_bounds(
    quad: &NativeVulkanSceneSampledImageQuad,
    mesh: &SceneMesh,
    bounds: NativeVulkanSceneWeImageGraphTargetBounds,
    vertices: &mut Vec<NativeVulkanSceneSampledImageVertex>,
...
mod tests {
    ...
    #[test]
    fn iris_graph_effect_uv_space_does_not_require_folded_alpha_slot() {
        let quad = NativeVulkanSceneSampledImageQuad {
            layer_index: 0,
            layer_id: "eye-iris".to_owned(),
            ...
            material_pass: native_vulkan_scene_sampled_image_material_pass(
                NativeVulkanSceneMaterialKind::SampledImage,
        ...
        let space = native_vulkan_scene_sampled_image_effect_final_uv_space(&quad).expect(
            "iris graph effect UV should come from transform even without folded alpha slot",
        );
    ...
    #[test]
    fn composelayer_final_scene_pass_uses_pass_space_full_frame_geometry() {
        let mut image = draw_op(0, NativeVulkanSceneDrawOpKind::Image);
        image.layer_id = "node-7-models-util-composelayer-json".to_owned();
        image.source = Some(PathBuf::from("/tmp/full-frame.gtex"));
        image.texture_slots = vec![SceneRenderTextureSlot {
            slot: 0,
            source: PathBuf::from("/tmp/full-frame.gtex"),
            width: Some(3840),
            height: Some(2160),
        }];
        image.width = Some(3840.0);
        image.height = Some(2160.0);
        ...
        let max_y = vertices
            ...
            .map(|vertex| vertex.position[1])
            .fold(f32::NEG_INFINITY, f32::max);
        assert_eq!((min_x, max_x, min_y, max_y), (0.0, 3840.0, 0.0, 2160.0));
    }
    ...
    fn draw_pass_plan_routes_opacity_effect_through_first_class_target() {
        ...
        assert_eq!(base_step.material_pass.system_shader_uniforms.len(), 1);
        assert_eq!(
            base_step.material_pass.system_shader_uniforms[0].name,
            "g_Texture0Resolution"
        );
        assert_eq!(
            base_step.material_pass.system_shader_uniforms[0].float_values(),
            vec![663.0_f32, 230.0_f32]
        );
        assert_eq!(
        ...
        assert_eq!(final_step.material_pass.system_shader_uniforms.len(), 2);
        assert_eq!(
            final_step
                .material_pass
                .system_shader_uniforms
                .iter()
                .map(|uniform| (uniform.name.as_str(), uniform.float_values()))
                .collect::<Vec<_>>(),
            vec![
                ("g_Texture0Resolution", vec![663.0_f32, 230.0_f32]),
                ("g_Texture1Resolution", vec![331.0_f32, 115.0_f32])
            ]
        );
        assert_eq!(
    ...
    fn draw_pass_plan_preserves_we_effect_bind_overrides_as_graph_bindings() {
        ...
        image.image_effect_passes = vec![crate::renderer::SceneRenderImageEffectPass {
            ...
            combos: Default::default(),
            constant_shader_values: BTreeMap::from([
                ("strength".to_owned(), serde_json::json!(0.25)),
        ...
        assert_eq!(
            effect_step.pass.parameter_keys,
            vec!["direction".to_owned(), "strength".to_owned()]
        );
        assert_eq!(
            effect_step.pass.constant_shader_values.get("strength"),
            Some(&serde_json::json!(0.25))
        );
        assert_eq!(
            effect_step.pass.constant_shader_values.get("direction"),
    ...
    #[test]
    fn draw_pass_plan_executes_water_ripple_material_graph_source_quad() {
        ...
        water.opacity = 0.24;
        water.color = Some("#102030".to_owned());
        ...
        assert_eq!(
            pass_plan.sampled_image_sources,
            vec![
                PathBuf::from("/tmp/water-source.gtex"),
                PathBuf::from("/tmp/waterripplenormal.gtex"),
    ...
    #[test]
    fn draw_pass_plan_executes_plain_water_ripple_material_graph_source_quad() {
        let mut water = draw_op(0, NativeVulkanSceneDrawOpKind::Image);
        ...
        assert_eq!(
            pass_plan.sampled_image_recording_steps[0].texture_slot_bindings,
            vec![
                NativeVulkanSceneTextureSlotResourceBinding {
                    slot: 0,
                    resource_index: 0
    ...
    #[test]
    fn draw_pass_plan_routes_waterwaves_character_quad_through_first_class_graph() {
    ...
    fn draw_pass_plan_runs_waterwaves_puppet_mesh_in_layer_uv_domain() {
        ...
        let waterwaves_pass = crate::renderer::SceneRenderImageEffectPass {
            effect_file: "effects/waterwaves/effect.json".to_owned(),
            runtime: Some("native-effect-motion".to_owned()),
            pass_index: 0,
            command: None,
        ...
        let mut second_waterwaves_pass = waterwaves_pass.clone();
        second_waterwaves_pass.pass_index = 1;
        layer.image_effect_passes = vec![waterwaves_pass, second_waterwaves_pass];
        layer.opacity = 0.3;
        ...
        assert!(base_step.material_pass.combo_values.is_empty());
    ...
    #[test]
    fn draw_pass_plan_executes_iris_then_water_ripple_first_class_chain() {
        let mut image = draw_op(0, NativeVulkanSceneDrawOpKind::Image);
        image.source = Some(PathBuf::from("/tmp/eye.gtex"));
        image.texture_slots = vec![SceneRenderTextureSlot {
            slot: 0,
        ...
        image.image_effect_passes = vec![
            ...
            crate::renderer::SceneRenderImageEffectPass {
                ...
                texture_slots: vec![
                    SceneRenderTextureSlot {
                        slot: 1,
                        source: PathBuf::from("/tmp/waterripple-mask.gtex"),
                        width: Some(331),
                        height: Some(115),
                    },
                    SceneRenderTextureSlot {
        ...
        assert_eq!(
            pass_plan.sampled_image_effect_targets[1].effect_target_index,
            1
        );
        assert_eq!(
            pass_plan.sampled_image_effect_targets[1].we_graph_endpoint,
        ...
        assert_eq!(pass_plan.sampled_image_sources.len(), 4);
        assert_eq!(pass_plan.sampled_image_recording_steps.len(), 3);
        assert_eq!(
        ...
        assert_eq!(
            pass_plan.sampled_image_recording_steps[1].we_graph_input_target_index,
            Some(0)
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[1].we_graph_output_target_index,
            Some(1)
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[1].texture_slot_bindings,
            texture_slot_bindings(&[4, 1])
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[1]
                .material_pass
                .alpha_texture_mode,
            SceneRenderAlphaTextureMode::Iris
        );
        ...
        assert_eq!(pass_plan.sampled_image_vertices[4].effect_uv, [0.0, 2.0]);
        assert_eq!(pass_plan.sampled_image_vertices[7].effect_uv, [2.0, 0.0]);
        assert_eq!(
            pass_plan.sampled_image_recording_steps[2].render_target,
            NativeVulkanSceneSampledImageRenderTarget::Swapchain
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[2].we_graph_step_index,
            Some(2)
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[2].we_graph_input_target_index,
            Some(1)
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[2].we_graph_output_target_index,
            None
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[2].texture_slot_bindings,
            texture_slot_bindings(&[5, 3, 2])
        );
        assert_eq!(
            pass_plan.sampled_image_recording_steps[2]
                .material_pass
                .alpha_texture_slot,
            None
        );
    ...
    #[test]
    fn draw_pass_plan_uses_pass_space_quad_for_iris_graph_effect_after_puppet_base() {
        let mut image = draw_op(0, NativeVulkanSceneDrawOpKind::Image);
        image.source = Some(PathBuf::from("/tmp/eye.gtex"));
        image.texture_slots = vec![SceneRenderTextureSlot {
            slot: 0,
        ...
        let pass_plan = native_vulkan_scene_draw_pass_plan(&draw_plan);
        let base_step = &pass_plan.sampled_image_recording_steps[0];
        let base_first = base_step.first_vertex as usize;
        let iris_step = &pass_plan.sampled_image_recording_steps[1];
        let first = iris_step.first_vertex as usize;

        assert_eq!(base_step.vertex_count, 3);
        assert_eq!(
            pass_plan.sampled_image_vertices[base_first].uv,
            [0.25, 0.75]
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            vec![NativeVulkanSceneEffectKind::Iris]
        );
        assert_eq!(iris_step.vertex_count, 4);
        assert_eq!(iris_step.index_count, 6);
        assert_eq!(pass_plan.sampled_image_vertices[first].position, [0.0, 0.0]);
        assert_eq!(pass_plan.sampled_image_vertices[first].uv, [0.0, 1.0]);
        assert_eq!(
            pass_plan.sampled_image_vertices[first].effect_uv,
            [0.0, 1.0]
        );
        assert_eq!(
            pass_plan.sampled_image_vertices[first + 3].position,
            [663.0, 230.0]
        );
        assert_eq!(pass_plan.sampled_image_vertices[first + 3].uv, [1.0, 0.0]);
        assert_eq!(
            pass_plan.sampled_image_vertices[first + 3].effect_uv,
            [1.0, 0.0]
        );
    }
    ...
    fn draw_pass_plan_keeps_legacy_cpu_effect_motion_disabled_for_quads() {
        ...
        assert!(pass_plan.backend_ready);
        assert_eq!(pass_plan.sampled_image_vertices.len(), 4);
        assert_eq!(pass_plan.sampled_image_indices.len(), 6);
        assert_eq!(pass_plan.sampled_image_vertex_buffer_bytes, 176);
        assert_eq!(pass_plan.sampled_image_index_buffer_bytes, 24);
        let step = &pass_plan.sampled_image_recording_steps[0];
        assert_eq!(step.vertex_count, 4);
        assert_eq!(step.index_count, 6);
        assert_eq!(pass_plan.sampled_image_vertices[0].position, [-60.0, -30.0]);
    }
    ...
    fn draw_pass_plan_keeps_legacy_cpu_effect_motion_disabled_for_quad_meshes() {
        ...
        assert_eq!(pass_plan.sampled_image_vertices.len(), 4);
        assert_eq!(pass_plan.sampled_image_indices.len(), 6);
        assert_eq!(pass_plan.sampled_image_recording_steps[0].vertex_count, 4);
        assert_eq!(pass_plan.sampled_image_recording_steps[0].index_count, 6);
        assert_eq!(
            pass_plan.sampled_image_vertices[0].position,
            [-364.0, -378.5]
        );
    ...
    fn draw_pass_plan_keeps_legacy_cpu_effect_motion_disabled_for_complex_meshes() {
        ...
        assert_eq!(pass_plan.sampled_image_vertices[0].position, [-50.0, -50.0]);
