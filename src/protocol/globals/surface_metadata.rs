...
impl SurfaceMetadataProtocol {
    ...
    pub(crate) fn committed_background_has_area(&self, surface: &WlSurface) -> bool {
        self.surfaces
            .borrow()
            .get(&surface.id())
            .is_some_and(|state| state.current.background_has_area)
    }
    ...
    pub(super) fn set_pending_background(&self, surface: &WlSurface, has_area: bool) {
        self.surfaces
            .borrow_mut()
            .entry(surface.id())
            .or_default()
            .pending
            .background_has_area = Some(has_area);
    }
    ...
    fn detach(&self, surface: &WlSurface, kind: ResourceKind, resource: &ObjectId) {
        ...
        *kind.resource_mut(state) = None;
        match kind {
            ...
            ResourceKind::Background => state.pending.background_has_area = Some(false),
        }
    }
...
impl SurfaceMetadataState {
    fn commit(&mut self) -> Option<SurfaceAlpha> {
        ...
        if let Some(has_area) = self.pending.background_has_area.take() {
            self.current.background_has_area = has_area;
        }
...
struct PendingMetadata {
    ...
    background_has_area: Option<bool>,
}
